...
class TestLDS(unittest.TestCase):
  ...
  def test_lds_unroll(self):
    ...
    for sz in [2,4,8]:
      helper_lds_allclose(opts=[Opt(OptOps.UNROLL, 0, sz)], expected_bufs=[(0,1),(1,1),(2,1)])
  ...
  def test_lds_local(self):
    ...
    helper_lds_allclose(opts=basic_local_opts, expected_bufs=[(0,2),(1,2),(2,2)])

    multi_local_opts = [Opt(OptOps.LOCAL, 0, 2),
                        Opt(OptOps.LOCAL, 0, 8)]
    helper_lds_allclose(opts=multi_local_opts, expected_bufs=[(0,16),(1,16),(2,16)])

    multi_axis_local_opts = [Opt(OptOps.LOCAL, 1, 4),
                             Opt(OptOps.LOCAL, 0, 2)]
    helper_lds_allclose(opts=multi_axis_local_opts, expected_bufs=[(0,8),(1,8),(2,8)])

    full_local_opts = [Opt(OptOps.LOCAL, 0, 16),
                       Opt(OptOps.LOCAL, 0, 16)]
    helper_lds_allclose(opts=full_local_opts, expected_bufs=[(0,256),(1,256),(2,256)])
  ...
  def test_lds_upcast(self):
    ...
    helper_lds_allclose(opts=basic_upcast_opts, expected_bufs=[(0,2),(1,2),(2,2)])

    multi_upcast_opts = [Opt(OptOps.UPCAST, 0, 2),
                         Opt(OptOps.UPCAST, 0, 8)]
    helper_lds_allclose(opts=multi_upcast_opts, expected_bufs=[(0,16),(1,16),(2,16)])

    multi_axis_upcast_opts = [Opt(OptOps.UPCAST, 1, 4),
                              Opt(OptOps.UPCAST, 0, 2)]
    helper_lds_allclose(opts=multi_axis_upcast_opts, expected_bufs=[(0,8),(1,8),(2,8)])

    full_upcast_opts = [Opt(OptOps.UPCAST, 0, 16),
                        Opt(OptOps.UPCAST, 0, 16)]
    helper_lds_allclose(opts=full_upcast_opts, expected_bufs=[(0,256),(1,256),(2,256)])
  ...
  def test_lds_tc(self):
    for tc in Device[Device.DEFAULT].renderer.tensor_cores:
      ...
      helper_lds_allclose(opts=opts, expected_bufs=[(0,M*N),(1,M*N),(2,M*N)], N=N, M=M, K=K, dtype_in=tc.dtype_in, acc_dtype=tc.dtype_out)

      opts = [Opt(OptOps.TC, 0, (-1, 0)),
              Opt(OptOps.LOCAL, 0, 2),
              Opt(OptOps.UPCAST, 1, 2)]
      helper_lds_allclose(opts=opts, expected_bufs=[(0,M*N*4),(1,M*N*4),(2,M*N*4)], N=sz, M=sz, K=sz, dtype_in=tc.dtype_in, acc_dtype=tc.dtype_out)

      opts = [Opt(OptOps.TC, 0, (-1, 0)),
              Opt(OptOps.UNROLL, 0, 2)]
      helper_lds_allclose(opts=opts, expected_bufs=[(0,M*N),(1,M*N),(2,M*N)], N=sz, M=sz, K=sz, dtype_in=tc.dtype_in, acc_dtype=tc.dtype_out)

      opts = [Opt(OptOps.TC, 0, (-1, 0)),
              Opt(OptOps.UNROLL, 0, 2),
              Opt(OptOps.UPCAST, 1, 2)]
      helper_lds_allclose(opts=opts, expected_bufs=[(0,M*N*2),(1,M*N*2),(2,M*N*2)], N=sz, M=sz, K=sz, dtype_in=tc.dtype_in, acc_dtype=tc.dtype_out)
  ...
  def test_lds_tc_padded(self):
    for tc in Device[Device.DEFAULT].renderer.tensor_cores:
      ...
      helper_lds_allclose(opts=opts, expected_bufs=[(0,M*N),(1,M*N),(2,M*N)], N=N+3, M=M+3, K=K+3, dtype_in=tc.dtype_in, acc_dtype=tc.dtype_out)
  ...
  def test_lds_full(self):
    opts = [Opt(OptOps.LOCAL, 0, 2),
            Opt(OptOps.UPCAST, 1, 2)]
    helper_lds_allclose(opts=opts, expected_bufs=[(0,4),(1,4),(2,4)])

    opts = [Opt(OptOps.LOCAL, 0, 2),
            Opt(OptOps.UPCAST, 0, 4),
            Opt(OptOps.LOCAL, 1, 8)]
    helper_lds_allclose(opts=opts, expected_bufs=[(0,64),(1,64),(2,64)])

    opts = [Opt(OptOps.LOCAL, 0, 16),
            Opt(OptOps.UPCAST, 1, 2)]
    helper_lds_allclose(opts=opts, expected_bufs=[(0,16),(1,16),(2,16)])

    opts = [Opt(OptOps.LOCAL, 0, 16),
            Opt(OptOps.UPCAST, 0, 16)]
    helper_lds_allclose(opts=opts, expected_bufs=[(0,256),(1,256),(2,256)])
    ...
    opts = [Opt(OptOps.LOCAL, 1, 4),
            Opt(OptOps.UNROLL, 0, 2),
            Opt(OptOps.UPCAST, 0, 2)]
    helper_lds_allclose(opts=opts, expected_bufs=[(0,8),(1,8),(2,8)])
