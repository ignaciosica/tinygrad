...
class TestLinearizer(unittest.TestCase):
  ...
  @unittest.skipUnless(Device[Device.DEFAULT].renderer.tensor_cores, "test requires tensor cores")
  def test_tensor_cores_emulation_padded(self):
    for tc in Device[Device.DEFAULT].renderer.tensor_cores:
      if not is_dtype_supported(tc.dtype_in) or not is_dtype_supported(tc.dtype_out): continue
      pad = 1
      helper_tc_allclose(tc.dims[0]+pad, tc.dims[1]+pad, tc.dims[2]+pad, tc.dtype_in, tc.dtype_out, tc_opt=2, use_tensor_cores=3)
      helper_tc_allclose(tc.dims[0]-pad, tc.dims[1]-pad, tc.dims[2]+pad, tc.dtype_in, tc.dtype_out, tc_opt=2, use_tensor_cores=3)
      helper_tc_allclose(tc.dims[0]+pad, tc.dims[1]-pad, tc.dims[2]+pad, tc.dtype_in, tc.dtype_out, tc_opt=2, use_tensor_cores=3)
      helper_tc_allclose(tc.dims[0]-pad, tc.dims[1]+pad, tc.dims[2]+pad, tc.dtype_in, tc.dtype_out, tc_opt=2, use_tensor_cores=3)
