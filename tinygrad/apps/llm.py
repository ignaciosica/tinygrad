...
class SimpleTokenizer:
  def __init__(self, normal_tokens: dict[str, int], special_tokens: dict[str, int]):
    ...
    self._byte_decoder = {chr(b): b for b in bs} | {
      chr(256 + i): b for i, b in enumerate(b for b in range(256) if b not in bs)
    }
    ...
    def ucat_range(pre: str):
      return "".join(
        ...
        if unicodedata.category(chr(cp)).startswith(pre)
      )
    ...
    self._split_to_sentence = re.compile(
      "|".join(re.escape(tok) for tok in special_tokens.keys())
      if special_tokens
      else r"(?!)"
    )

    self._normal_tokens = {
      bytes(self._byte_decoder[c] for c in tok): tid
      for tok, tid in normal_tokens.items()
    }
    self._special_tokens = special_tokens
    self._tok2bytes = {tid: tok for tok, tid in self._normal_tokens.items()} | {
      tid: tok.encode() for tok, tid in self._special_tokens.items()
    }
  ...
  def from_gguf_kv(kv: dict):
    ...
    vocab: typing.Iterable[tuple[str, int]] = (
      (tok, idx) for idx, tok in enumerate(kv["tokenizer.ggml.tokens"])
    )
    normal_tokens, special_tokens = helpers.partition(
      vocab, lambda e: kv["tokenizer.ggml.token_type"][e[1]] == 1
    )
  ...
  def _encode_word(self, word: bytes) -> list[int]:
    ...
    while True:
      i = min(
        [(sys.maxsize, -1)]
        + [
          (self._normal_tokens.get(parts[j] + parts[j + 1], sys.maxsize), j)
  ...
  def _encode_sentence(self, chunk: str) -> list[int]:
    return [
      tok
      for word in self._split_to_word.findall(chunk)
      for tok in self._encode_word(word.encode())
    ]
  ...
  def encode(self, text: str) -> list[int]:
    ...
    for match in self._split_to_sentence.finditer(text):
      tokens.extend(
        self._encode_sentence(text[pos : match.start(0)])
        + [self._special_tokens[text[match.start(0) : match.end(0)]]]
      )
      pos = match.end(0)
...
def apply_rope(x: Tensor, start_pos: int | UOp, base: float = 10000.0) -> Tensor:
  B, H, T, Hd = x.shape
  assert isinstance(Hd, int) and (Hd & 1) == 0, (
    "RoPE requires an even head dimension"
  )
  half = Hd // 2
  t_start_pos = start_pos if isinstance(start_pos, int) else Tensor(start_pos)
  angles = (Tensor.arange(T, dtype="float32") + t_start_pos)[:, None] * (
    base ** (-(Tensor.arange(half, dtype="float32") / half))
  )[None, :]
  # contiguous here allows RoPE to be pruned in the JIT
  cos, sin = (
    angles.cos().reshape(1, 1, T, half).cast(x.dtype).contiguous(),
    angles.sin().reshape(1, 1, T, half).cast(x.dtype).contiguous(),
  )
...
class TransformerBlock:
  ...
  def _attention(self, x: Tensor, start_pos: int | UOp) -> Tensor:
    ...
    k = k.reshape(B, T, self.n_kv_heads, self.head_dim).transpose(
      1, 2
    )  # (B,KvH,T,Hd)
    v = v.reshape(B, T, self.n_kv_heads, self.head_dim).transpose(
      1, 2
    )  # (B,KvH,T,Hd)
    ...
    mask = (
      Tensor.full(
        (1, 1, T, start_pos + T), float("-inf"), dtype=x.dtype, device=x.device
      ).triu(start_pos + 1)
      if T > 1
      else None
    )
...
class Transformer:
  def __init__(
    ...
    *,
    ...
    self.blk = [
      TransformerBlock(dim, hidden_dim, n_heads, n_kv_heads, norm_eps, max_context)
  ...
  def forward(self, tokens: Tensor, start_pos: int | UOp) -> Tensor:
    ...
    return (
      self.output(self.output_norm(x))[:, -1, :]
      .softmax(-1, dtype="float")
  ...
  @staticmethod
  def from_gguf(
    gguf: Tensor, max_context: int | None = None
    ...
    state_dict = {
      k: v.cast("float16") if getenv("HALF", 1) else v for k, v in state_dict.items()
    }
    ...
    nn.state.load_state_dict(
      model, state_dict, verbose=False, consume=True, realize=False
    )  # NOTE: rope_freqs.weight (32,) is unused
    # NOTE: without this contiguous, it unpacks the weights from the model every time. we shouldn't need this, but for now it's faster
    for s in nn.state.get_parameters(model):
  ...
  def generate(self, tokens: list[int], start_pos=0):
    ...
    while len(tokens) < self.max_context:
      t = self(
        t,
        v_start_pos.bind(start_pos)
        if getenv("SYM", 1) and start_pos != 0 and t.shape[-1] == 1
        else start_pos,
      )
      next_id = int(t.item())
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "--max_context", type=int, default=4096, help="Max Context Length"
  )
  args = parser.parse_args()

  # load the model
  model, kv = Transformer.from_gguf(
    Tensor.from_url(models[args.size]), args.max_context
  )
