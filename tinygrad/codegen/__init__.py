...
from tinygrad.codegen.late.expander import (
  expander,
  pm_pre_expander,
  pm_group_for_reduce,
)
from tinygrad.codegen.late.devectorizer import (
  load_store_folding,
  load_store_indexing,
  devectorize,
  pm_reduce,
...
pm_syntactic_sugar = PatternMatcher([
  ...
  (
    ...
    lambda i1, i2: i2.replace(src=i1.src + i2.src[1:])
    if isinstance(i1.dtype, PtrDType) and not isinstance(i2.dtype, PtrDType)
    else None,
  ),
])


def full_rewrite_to_sink(
  sink: UOp, ren: Renderer | None = None, optimize: bool = True
) -> UOp:
  if ren is None:
    ren = Renderer()

  if SPEC:
    type_verify(sink, kernel_spec)

  # preprocess
  sink = graph_rewrite(
    sink, pm_mops + pm_syntactic_sugar, name="early movement ops", bottom_up=True
  )
  ...
  if optimize:
    ...
    # split ranges
    sink = graph_rewrite(
      sink, pm_split_ranges + pm_flatten_range, ctx={}, name="split ranges"
    )
    ...
    sink = graph_rewrite(
      sink, pm_split_store, ctx=ren.device, name="cut store ranges"
    )
  ...
  # expand
  sink = graph_rewrite(
    sink, sym + pm_pre_expander + pm_group_for_reduce + expander, name="expander"
  )

  # add locals
  sink = graph_rewrite(
    sink,
    pm_add_buffers_local + rangeify_codegen,
    ctx=itertools.count(0),
    name="add local buffers",
  )

  # ** devectorizer (full_graph_rewrite) **
  # remove reduce
  sink = graph_rewrite(
    sink, pm_reduce + gep_pushing, ctx=ReduceContext(), name="remove_reduce"
  )
  ...
  if DEVECTORIZE >= 2:
    pm_devectorize = sym + load_store_folding + load_store_indexing
  elif DEVECTORIZE:
    pm_devectorize = (
      sym
      + devectorize
    ...
    pm_devectorize = (
      sym + load_store_folding + correct_load_store + load_store_indexing
    )
  ...
  pm_decomp = symbolic_simple + get_late_rewrite_patterns(
    supported_ops, TRANSCENDENTAL >= 2
  )
  sink = graph_rewrite(sink, pm_decomp, ctx=ren.device, name="decompositions")

  # final rules for the renderer (without sym)
  extra_matcher = (
    ren.extra_matcher if ren.extra_matcher is not None else PatternMatcher([])
  )
...
pm_linearize_cleanups = PatternMatcher([
  ...
  (
    UPat((Ops.IF, Ops.ENDIF)),
    lambda: panic(RuntimeError("if not allowed in graph")),
  ),
  ...
  (
    UPat(
      ...
      src=(
        UPat(
          Ops.INDEX, src=(UPat(), UPat(), UPat(name="gate", dtype=dtypes.bool))
        ).or_casted(),
        UPat(),
      ),
      allow_any_len=True,
    ...
    lambda u, gate: (
      u,
      [mif := UOp(Ops.IF, src=(gate, u.src[0])), u, UOp(Ops.ENDIF, src=(mif,))],
    ),
  ),
])
...
def line_rewrite(lst: list[UOp], pm: PatternMatcher) -> list[UOp]:
  ...
  for u in lst:
    ...
    ret: tuple[UOp, list[UOp]] = cast(
      tuple[UOp, list[UOp]] | None, pm.rewrite(nu)
    ) or (nu, [nu])
    replaced[u] = ret[0]
    newlst.extend(ret[1])
  return newlst
