...
def add_gpudims(ctx:Renderer, s:UOp):
  ...
  ki: KernelInfo = s.arg
  if ki.global_dims == 0 and ki.local_dims == 0 and ki.group_dims == 0: return None
  s_topo = list(s.toposort())
  if any(x.op is Ops.SPECIAL for x in s_topo): return None
  ranges = sorted([x for x in s_topo if x.op is Ops.RANGE and x.arg < (ki.global_dims+ki.local_dims+ki.group_dims)], key=lambda x: x.arg)
  ...
  if ki.dont_use_locals:
    assert ki.local_dims == 0 and ki.group_dims == 0, "can't use locals if there's no local dims"
