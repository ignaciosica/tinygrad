    ...
    def fixup_ast(op:UOp, apply_to_st=None) -> UOp:
      ...
      if op.op is UOps.REDUCE_AXIS:
        ...
        if op in self.bufs_for_tensor_core and (tc := self.tensor_core):
          ...
          src0_fix_st1 = (rsrc.src[0], fix_st1)
          src1_fix_st2 = (rsrc.src[1], fix_st2)
          ...
          if self.use_tensor_cores >= 2:
            if self.use_tensor_cores == 3:
              ...
              srcs = []
              for i,(src,fix_st_fxn) in enumerate((src0_fix_st1, src1_fix_st2)):
            ...
            wmma = UOp(UOps.WMMA, dtype=tc.dtype_out.vec(wmma_sz[2]), src=(
              UOp(UOps.CONTRACT, dtype=rsrc.src[0].dtype.vec(wmma_sz[0]), src=(fixup_ast(*src0_fix_st1),), arg=wmma_upcast_axes[0]),
              UOp(UOps.CONTRACT, dtype=rsrc.src[1].dtype.vec(wmma_sz[1]), src=(fixup_ast(*src1_fix_st2),), arg=wmma_upcast_axes[1]),
