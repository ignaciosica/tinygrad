...
class Kernel:
  ...
  def viz_tile(self, uop: UOp):
    ...
    shape = tuple(1 if i < self.global_dims or (self.first_reduce <= i < self.first_upcast) or (self.first_upcast <= i and s == 0)
                  # else (st.shape[i] if st.shape[i] != 1 else 2) for i, s in enumerate(st.real_strides(True)))
                  else st.shape[i] for i, s in enumerate(st.real_strides(True)))
    print(st.real_strides(True))
    print(shape)
    print(st.real_size())
    ...
    upcast_strides = tuple(ShapeTracker.from_shape(shape[self.first_upcast :][::-1]).real_strides(True)[::-1])

    for c in grid(shape): layout.setdefault(dot(c, st.real_strides(True)), []).append(c)
