...
def expand_index(buf: UOp, vec: UOp):
  ...
  midx = graph_rewrite(
    UOp.sink(*[buf.index(vec.gep(i), ptr=True) for i in range(vec.dtype.count)]), symbolic_flat + load_store_indexing, name=f"index_buf_{buf.arg}"
  )
  ...
  for offsets in offsets_rootsrc.values():
    ...
    for grp in grouped_offsets:
      ...
      if len(grp) > 1:
        lidx = lidx.cast(buf.ptrdtype.base.vec(len(grp)).ptr(size=buf.ptrdtype.size, addrspace=buf.ptrdtype.addrspace))
...
def split_load_store(ctx: Renderer | None, ls: UOp, idx: UOp):
  ...
  while global_offset < sz:
    ...
    for fold_length in lengths:
      ...
      if fold_length > 1:
        lidx = lidx.cast(buf.ptrdtype.base.vec(fold_length).ptr(size=buf.ptrdtype.size, addrspace=buf.ptrdtype.addrspace))
      if ls.op is Ops.STORE:
        ...
        ret.append(ls.replace(src=(lidx,) + ls.src[1:], dtype=ls.dtype.scalar().vec(fold_length)))
...
pm_render = PatternMatcher([
  ...
  (
    UPat.var("c").where(
      UPat(Ops.LOAD, src=(UPat().index(UPat.var("idx"), UPat.var("c")).or_casted(),), allow_any_len=True, name="l").or_casted(), UPat.var("a")
    ),
    lambda c, idx, l, a: l.replace(src=(l.src[0], a.cast(l.dtype)) + l.src[2:]).cast(a.dtype),
  ),
  (
    UPat.var("c").where(
      UPat.var("a"),
      UPat(Ops.LOAD, src=(UPat().index(UPat.var("idx"), UPat.var("c").logical_not()).or_casted(),), allow_any_len=True, name="l").or_casted(),
    ),
    lambda c, idx, l, a: l.replace(src=(l.src[0], a.cast(l.dtype)) + l.src[2:]).cast(a.dtype),
  ),
])
...
pm_add_loads = PatternMatcher([
  ...
  (
    ...
    lambda idx: None if isinstance(idx.dtype, (PtrDType, ImageDType)) else idx.replace(dtype=idx.src[0].dtype).load(dtype=idx.dtype.base),
  ),
