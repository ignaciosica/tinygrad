    ...
    if src.op is Ops.UNROLL:
      if expand_args == src.arg:
        ...
        if src.dtype.count > 1:
          lst = flatten([[i * src.dtype.count + j for j in range(src.dtype.count)] for i in lst])
...
expander = PatternMatcher([
  ...
  (UPat(Ops.BARRIER, src=(UPat(Ops.UNROLL, name="ex"),)), lambda ex: UOp(Ops.UNROLL, src=(UOp(Ops.BARRIER, src=ex.src),) * len(ex.src), arg=ex.arg)),
