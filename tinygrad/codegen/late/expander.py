...
def _expand_arg_to_idx(
  args: tuple[tuple[int, int], ...], rpk: dict[int, int]
) -> int:
  idx, mul = 0, 1
...
def _choices_from_args(args: tuple[tuple[int, int], ...]) -> list[dict[int, int]]:
  return [
    ...
    for x in itertools.product(*[
      zip(itertools.repeat(axis), range(m)) for axis, m in args
    ])
  ]
...
@functools.cache
def _swizzle_args(
  cargs: tuple[tuple[int, int], ...],
  eargs: tuple[tuple[int, int], ...],
  exclude_args: tuple[int, ...],
) -> list[int]:
  return [
    _expand_arg_to_idx(
      eargs, {**rpk, **{x: 0 for x in exclude_args}} if exclude_args else rpk
    )
    for rpk in _choices_from_args(cargs)
  ]
...
def do_expand(root: UOp):
  ...
  exclude_args = (
    tuple(dedup(root.arg[-1] + tuple(y[0] for y in flatten(root.arg[-2]))))
    if root.op is Ops.WMMA
    else ()
  )
  if all_same(expands_args := [x.arg for x in expands]) and len(exclude_args) == 0:
    ...
    expand_args = tuple(
      x for x in sorted(dedup(flatten(expands_args))) if x[0] not in exclude_args
    )
    ...
    if src.op is Ops.UNROLL:
      if expand_args == src.arg:
        ...
        if src.dtype.count > 1:
          lst = flatten([
            [i * src.dtype.count + j for j in range(src.dtype.count)] for i in lst
          ])
      ...
      if root.op in range_start and i >= range_start[root.op]:
        ...
        new_srcs.append(
          UOp(
            Ops.CAT,
            src.dtype.scalar().vec(expand_sz * src.dtype.count),
            (src,) * expand_sz,
          )
        )
      else:
  ...
  if root.op is Ops.GEP:
    ...
    new_arg = tuple(
      range(
        root.arg[0], new_srcs[0].dtype.count, new_srcs[0].dtype.count // expand_sz
      )
    )
  nsrc = UOp(
    root.op,
    root.dtype.scalar().vec(root.dtype.count * expand_sz),
    tuple(new_srcs),
    new_arg,
  )
...
def do_contract(con: UOp):
  ...
  if ex.op is not Ops.UNROLL:
    return UOp(Ops.VECTORIZE, con.dtype, con.src * con.dtype.count)
  # CONTRACT may remove several axes from UNROLL
  assert con.dtype == dtypes.void or con.dtype.count == prod([
    x[1] for x in con.arg
  ]), "dtype is wrong"
  idxs = []
  for rpk in _choices_from_args(
    new_ex_args := tuple(x for x in ex.arg if x not in con.arg)
  ):
    idxs += [
      _expand_arg_to_idx(ex.arg, {**rpk, **lrpk})
      for lrpk in _choices_from_args(con.arg)
    ]
...
def end_unrolls(u: UOp):
  ...
  if not len(unrolls):
    return None
  ret = UOp(
    Ops.CONTRACT, dtypes.void, (u.src[0],), sum([x.arg for x in unrolls], start=())
  )
...
expander = PatternMatcher([
  ...
  (
    ...
    lambda x: x.replace(
      src=tuple(
        UOp(
          Ops.CONTRACT,
          dtype=s.dtype.vec(x.src[1].src[0].dtype.count),
          src=(s,),
          arg=x.src[1].arg,
        )
        for s in x.src
  ...
  (
    ...
    lambda outer, inner: UOp(
      Ops.UNROLL, outer.dtype, (inner.src[0],), inner.arg + outer.arg
    ),
  ),
  # do expansion
  (
    UPat(
      ...
      name="root",
      custom_early_reject=set([Ops.UNROLL]),
    ),
    do_expand,
  ),
  ...
  (
    ...
    lambda ex: UOp(
      Ops.UNROLL, src=(UOp(Ops.BARRIER, src=ex.src),) * len(ex.src), arg=ex.arg
    ),
  ),
  ...
  (
    UPat(
      ...
      name="ex",
      src=tuple(
        UPat.var("x").gep(i) + UPat.var("y").gep(i) for i in range(256 if AMX else 8)
      ),
    ),
    lambda ex, x, y: UOp(
      Ops.UNROLL,
      ex.dtype,
      tuple((x + y).gep(i) for i in range(256 if AMX else 8)),
      ex.arg,
    ),
  ),
])
...
def fix_reduce_unroll(x: UOp):
  ...
  if len(reduce_expand) == 0:
    return None
  reduce_expand = [x for x in reduce_expand if x.op is not Ops.CONST]
  assert all(x.op is Ops.UNROLL for x in reduce_expand), (
    f"not all UNROLLS in {reduce_expand}"
  )
  ret = x.src[0]
...
def fix_store_unroll(x: UOp):
  ...
  if len(store_expand) == 0:
    return None
  return UOp(
    Ops.CONTRACT,
    dtypes.void,
    (x.replace(src=x.src[:2] + tuple(store_range)),),
    tuple(flatten(x.arg for x in store_expand)),
    tag=1,
  )
...
def fix_group_for_reduce(x: UOp):
  reduce_gfr, reduce_r = partition(
    x.src[1:], lambda u: u.op is Ops.RANGE and u.arg[1] == AxisType.GROUP_REDUCE
  )
  if len(reduce_gfr) == 0:
    return None
  ...
  upstream_locals = [
    u for u in x.toposort() if u.op is Ops.RANGE and u.arg[1] == AxisType.LOCAL
  ]

  # do only the non grouped reduces early
  ret = x.replace(src=(x.src[0],) + tuple(reduce_r))
  reduce_loop = [
    x.replace(arg=(x.arg[0] + 100, AxisType.REDUCE)) for x in reduce_gfr
  ]
  buf = ret.bufferize(
    *upstream_locals,
    *reduce_gfr,
    arg=BufferizeOpts(reduce_gfr[0].arg[0], AddrSpace.LOCAL),
...
pm_pre_expander = PatternMatcher([
  ...
  (
    ...
    lambda r: UOp(
      Ops.UNROLL,
      r.dtype,
      (UOp.const(r.dtype.vec(s := r.vmax + 1), tuple(range(s))),),
      ((r.arg[0], s),),
    )
    if r.arg[1] in {AxisType.UNROLL, AxisType.UPCAST}
    else None,
  ),
