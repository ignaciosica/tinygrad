...
def linearize(u: UOp) -> list[UOp]:
  ...
  for u in reversed(lst):
    ...
    if u.op in {
      ...
      priority.append(-2000)
    priorities[u] = min(priority)

  # number the uops in "ideal" order
  nkey = {
    u: i for i, u in enumerate(sorted(lst, key=lambda x: (priorities[x],) + x.tuplize))
  }
...
class CFGContext:
  def __init__(self, sink: UOp):
    ...
    for u in sink.toposort():
      ...
      if u.op in (Ops.END, Ops.SINK):
        nesting |= {
          ...
          for x in deps[u]
          if x.op is Ops.END
          and (u.op is Ops.SINK or u.src[1] in deps[x])
          and x not in nesting
        }
      if u.op in (Ops.RANGE, Ops.END):
        deps[u][u] = None

    self.edges: dict[UOp, UOp] = {}
    siblings: dict[UOp, list[UOp]] = {}
    for k, vv in nesting.items():
      siblings.setdefault(vv, []).append(k)
    for k, v in siblings.items():
      ...
      zipped = (
        zip(order, order[1:]) if k.op is Ops.SINK else zip([k.src[1]] + order, order)
      )
      for x, y in zipped:
...
pm_add_control_flow = PatternMatcher([
  (
    ...
    lambda ctx, x: x.replace(src=x.src + (y,))
    if (y := ctx.edges.get(x)) is not None
    else None,
  ),
])
...
def do_split_ends(e: UOp):
  ret = e.src[0]
  for r in sorted(UOp.sink(*e.src[1:]).ranges, key=lambda x: x.arg, reverse=True):
    ret = ret.end(r)
  return ret
