...
def hand_coded_optimizations(k: Scheduler) -> Scheduler:
  ...
  for buf_index, buf in enumerate(k.bufs):
    if isinstance(buf.src[0].dtype, ImageDType):
      ...
      unit_stride_axes_mul_4 = [
        k.rngs.index(c) for c in k.bufs[buf_index].src[1].get_idx().split_uop(Ops.ADD) if c.op is Ops.RANGE and (c.vmax + 1) % 4 == 0
      ]
  ...
  while resolve(prod(k.output_shape[i] for i in k.upcastable_dims) >= 1024):
    ...
    for axis, upcast_amount in itertools.product(k.upcastable_dims, ([128] if not len(upcasted_axis) else []) if is_dsp else [3, 4]):
      ...
      if any(
        ...
        and all(r2 in b.src[1].get_idx().backward_slice for r2 in k.ranges_of(AxisType.UPCAST, AxisType.UNROLL))
        for b in k.bufs
      ):
  ...
  if k.ren.has_local:
    if NOLOCALS:
      ...
      local_axis_ranking = [
        (any(k.rngs[axis] not in b.src[1].get_idx().backward_slice for b in k.bufs), axis)
      ...
      for _, axis in sorted(local_axis_ranking, key=lambda x: (-x[0], -x[1])):
        ...
        local_sz: int | None = next(
          (x for x in ([32] * (axis == 0) + [16, 8, 4, 3, 2]) if k.full_shape[axis] % x == 0 and local_size * x <= 128), None
        )
  ...
  if k.ren.has_threads and k.ren.global_max is not None:
    for threads in [32, 16, 12, 8, 6, 5, 4, 3, 2]:
      ...
      if threads > k.ren.global_max[0] or resolve(prod(k.full_shape) // (128 << 10) < threads):
        continue
