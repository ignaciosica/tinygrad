...
class Scheduler:
  ...
  def colored_shape(self) -> str:
    return " ".join([colored(f"{x.src[0].render():>4s}", color) for x, color in zip(self.rngs, self.colors())])
  ...
  def upcastable_dims(self) -> list[int]:
    return [i for i in self.axes_of(AxisType.GLOBAL, AxisType.LOCAL, AxisType.LOOP) if isinstance(s := self.full_shape[i], int) and s > 1]

  @property
  def unrollable_dims(self) -> list[int]:
    return [i for i in self.axes_of(AxisType.GROUP_REDUCE, AxisType.REDUCE) if isinstance(s := self.full_shape[i], int) and s > 1]
  ...
  def apply_opt(self, opt: Opt, append_opt: bool = True):
    ...
    if opt.op in opt_to_at:
      ...
      if self.reduceop is not None and (
        opt.op in {OptOps.GROUP, OptOps.GROUPTOP} or (self.group_for_reduces and opt.op not in {OptOps.NOLOCALS, OptOps.PADTO})
      ):
      ...
      if self.reduceop is not None and (opt.op in {OptOps.GROUP, OptOps.GROUPTOP}):
        ...
        check(
          not any(u.arg[-1] in (AxisType.REDUCE, AxisType.UNROLL, AxisType.GROUP_REDUCE) for u in reduce.ranges),
      ...
      self.ast = self.ast.substitute(
        {rng: rng.replace(arg=(*altrng.arg[0:-1], rng.arg[-1]), tag=1), altrng: altrng.replace(arg=(*rng.arg[0:-1], altrng.arg[-1]), tag=1)},
