...
actions += [
  Opt(op=OptOps.GROUP, axis=axis, arg=amt) for amt in [0, 4, 8, 16] for axis in range(3)
]
if getenv("BEAM_PADTO", 0):
  actions += [
...
actions += [
  Opt(op=OptOps.TC, axis=axis, arg=(-1, getenv("TC_OPT", 2), getenv("TC", 1)))
  for axis in range(9)
]
actions += [
...
actions += [
  Opt(op=OptOps.THREAD, axis=axis, arg=amt)
...
def _time_program(
  ...
  allow_test_size: int = True,
  max_global_size: int | None = 65536,
  clear_l2=False,
  cnt=3,
  name="test",
) -> list[float]:
...
def timeout_handler(signum, frame):
  if DEBUG >= 2:
    print("*** BEAM COMPILE TIMEOUT")
  raise TimeoutException()


def _try_compile(
  x: tuple[int, Scheduler], compiler: Compiler
) -> tuple[int, tuple[ProgramSpec, bytes, float] | None]:
  ...
  try:
    ...
    if len(p.uops) >= (uops_max := getenv("BEAM_UOPS_MAX", 3000)) > 0:
      if getenv("BEAM_LOG_SURPASS_MAX"):
        print(f"too many uops. {len(p.uops)=}, {uops_max=}")
...
def _ensure_buffer_alloc(bufs: list[Buffer]) -> list[Buffer]:
  return [buf.ensure_allocated() if buf is not None else buf for buf in bufs]
...
def get_kernel_actions(
  s: Scheduler, include_0=True, candidates: list[Opt] | None = None
) -> dict[int, Scheduler]:
  acted, max_up, max_lcl = (
    {0: s} if include_0 else {},
  ...
  for i, a in enumerate(kernel_actions):
    if a.axis is not None and a.op is not OptOps.TC:
      try:
        ax = s.real_axis(a.op, a.axis)
      except KernelOptError:
        continue
      if (ax >= s.shape_len) or (
        s.full_shape[ax] == a.arg and Opt(a.op, a.axis, 0) in kernel_actions
      ):
        continue
    s2 = s.copy()
    try:
      ...
      up, lcl, tc_up = (
        1,
        1,
        prod(tc.dims) // tc.threads
        if hasattr(s2, "tensor_core") and (tc := s2.tensor_core)
        else 1,
      )
      for x, t in zip(s2.full_shape, s2.axis_types):
        if t in (AxisType.UPCAST, AxisType.UNROLL):
          up *= x
        elif t in (AxisType.WARP, AxisType.LOCAL, AxisType.GROUP_REDUCE):
          lcl *= x
      if up // tc_up > max_up or lcl > max_lcl:
        if getenv("BEAM_LOG_SURPASS_MAX"):
          print(f"too many upcast/local. {up//tc_up=}, {max_up=}, {lcl=}, {max_lcl=}")
        continue
      acted[i + 1] = s2
    except KernelOptError:
      pass
...
def beam_search(
  ...
  default_parallel = (
    ...
    if s.ren.device in {"CUDA", "AMD", "NV", "METAL", "HIP"}
    else 0
  )
  ...
  if DEBUG >= 2:
    print(f"   0.00s:                from   1 ->   1 actions {s.colored_shape()}")
  ...
  try:
    ...
    while not exiting:
      candidates: list[Scheduler] = flatten([
        get_kernel_actions(si, include_0=False).values() for si, _ in beam
      ])
      timed: list[tuple[Scheduler, float]] = []
      _compile_fn = functools.partial(_try_compile, compiler=dev.compiler)
      least_compute_ops = math.inf
      for i, proc in (
        map(_compile_fn, enumerate(candidates))
        if beam_pool is None
        else beam_pool.imap_unordered(_compile_fn, enumerate(candidates))
      ):
        if proc is None:
          continue
        ...
        least_compute_ops = min(
          this_compute_ops := sym_infer(p.estimates.ops, var_vals), least_compute_ops
        )
        ...
        try:
          ...
          if BEAM_DEBUG:
            print(f"BEAM failed for opts: {candidates[i].applied_opts}\n{e}")
        ...
        if BEAM_DEBUG > 1:
          print(
            f"{time.perf_counter() - st:7.2f}s: {i:5d} {len(unwrap(p.uops)):5d} uops",
      ...
      if DEBUG >= 2:
        print(
          ...
          colored(time_to_str(beam[0][1], w=12), "green" if exiting else None),
