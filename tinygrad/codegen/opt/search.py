...
if getenv("BEAM_PADTO", 0):
  actions += [Opt(op=OptOps.PADTO, axis=axis, arg=amt) for amt in [32] for axis in range(7)]
  ...
  for i, a in enumerate(kernel_actions):
    if a.axis is not None and a.op is not OptOps.TC:
      try:
        ...
        continue
      if (ax >= s.shape_len) or (s.full_shape[ax] == a.arg and Opt(a.op, a.axis, 0) in kernel_actions):
        continue
    s2 = s.copy()
    try:
      s2.apply_opt(a)
...
def beam_search(s: Scheduler, rawbufs: list[Buffer], amt: int, allow_test_size=True, disable_cache=IGNORE_BEAM_CACHE.value):
    ...
    while not exiting:
      ...
      for i, proc in map(_compile_fn, enumerate(candidates)) if beam_pool is None else beam_pool.imap_unordered(_compile_fn, enumerate(candidates)):
        ...
        if least_compute_ops * 1000 < this_compute_ops:
          if getenv("BEAM_LOG_SURPASS_MAX"):
            print(f"too much compute. {this_compute_ops} when least is {least_compute_ops}")
          continue
        seen_libs.add(lib)
        try:
          tms = _time_program(
  ...
  if BEAM_DEBUG:
    print(f"BEAM_SEARCH: final tm={time_to_str(beam[0][1], w=0)}, applied_opts={beam[0][0].applied_opts}")
  return beam[0][0]
