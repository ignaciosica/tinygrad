...
class TensorCore:  # D = A * B + C, A is (M x K), B is (K x N), C and D are (M x N)
  ...
  elements_per_thread: tuple[
    int, int, int
  ]  # elements per-thread to load/store from A/B/C
  dtype_in: DType  # dtype for A and B
  dtype_out: DType  # dtype for C and D
  opts: tuple[
    str, ...
  ]  # ordered tuple of "ux" or "lx" specifying kernel opts to perform. "ux" upcasts dim x and "lx" localizes dim x
  ...
  def _remaps(self) -> list[dict[str, str]]:
    local_axes, upcast_axes, reduce_axes = (
      len(self.get_local_axes()),
      len(self.get_upcast_axes()),
      len(self.get_reduce_axes()),
    )
    fwd_st = (
      [f"l{i}" for i in range(local_axes)]
      + [f"u{i}" for i in range(upcast_axes)]
      + [f"r{i}" for i in range(reduce_axes)]
    )
    return [dict(zip(fwd_st, sum(s, ()))) for s in self.swizzle]

  def permutes_for_shape_str(
    self, shape_str: list[str]
  ) -> tuple[tuple[int, ...], tuple[int, ...]]:
    ret = [
      [
        shape_str.index(remap[ss]) if ss in remap else i
        for i, ss in enumerate(shape_str)
      ]
  ...
  def get_reduce_axes(self):
    return [(i, 2) for i in range(int(math.log2(self.dims[2])))]

  def get_upcast_axes(self):
    return [opt for opt in self.opts if opt[0] == "u"]

  def get_local_axes(self):
    return [opt for opt in self.opts if opt[0] == "l"]

  def base_upcast_axes(self):
    # this is defined in the swizzle. first we use the upcast axes, then the reduce
    return (
      [f"r{i}" for i in range(len(self.get_reduce_axes()))]
      + [f"u{i}" for i in range(len(self.get_upcast_axes()))]
    )[::-1]

  def __str__(self):
    return "_".join(
      ["WMMA"] + list(map(str, self.dims)) + [self.dtype_in.name, self.dtype_out.name]
    )
  ...
  def __post_init__(self):
    ...
    assert 2**local_axes == self.threads, (
      f"{self.threads} threads construct the warp but found {2**local_axes} in {self.opts}"
    )
    ...
    assert self.dims[0] == 2 ** len(gd := [x for x in self.opts if x[1] == "0"]), (
      f"opts wrong on dims[0], {self.dims[0]} vs {gd}"
    )
    assert self.dims[1] == 2 ** len(gd := [x for x in self.opts if x[1] == "1"]), (
      f"opts wrong on dims[1], {self.dims[1]} vs {gd}"
    )
    # NOTE: the K opts is implictly set by the dim
    # check swizzle
    assert len(self.swizzle[0]) == 3 and len(self.swizzle[1]) == 3, (
      "swizzle has wrong part count"
    )
    assert len(self.swizzle[0][0]) == len(self.swizzle[1][0]) == local_axes, (
      "local swizzle size is wrong"
    )
    assert len(self.swizzle[0][1]) == len(self.swizzle[1][1]) == upcast_axes, (
    ...
    assert all(
      len(s) == local_axes + upcast_axes + reduce_axes for s in self._remaps()
    ...
    for o in self.opts:
      if o[1] == "0":
        zero_stride_0.append(o[0] + str(un if o[0] == "u" else ln))
      if o[1] == "1":
        zero_stride_1.append(o[0] + str(un if o[0] == "u" else ln))
      if o[0] == "u":
        un += 1
      if o[0] == "l":
        ln += 1
    ...
    upcasted_0 = [
      x
      for x in (self.swizzle[0][1] + self.swizzle[0][2])
      if x not in zero_stride_0 and x[0] != "l"
    ]
    upcasted_1 = [
      x
      for x in (self.swizzle[1][1] + self.swizzle[1][2])
      if x not in zero_stride_1 and x[0] != "l"
    ]
...
cuda_tc_opts = (
  ...
  "l1",
  "u1",
)  # shared by all shapes with M=16 N=8
...
if getenv("ALLOW_TF32", 0):
  cuda_sm80 += cuda_8168_tf32
...
amd_cdna_161632 = [
  ...
  for di, do in [
    (dtypes.fp8e5m2, dtypes.float),
    (dtypes.fp8e4m3, dtypes.float),
    (dtypes.half, dtypes.float),
    (dtypes.bfloat16, dtypes.float),
  ]
]
