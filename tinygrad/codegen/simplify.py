...
pm_reduce_collapse = (
  ...
  + PatternMatcher([
    ...
    (
      ...
      lambda x, y, c: (x < (c.cast(y.dtype) - y)) if no_range(y) and no_range(c) else None,
    ),
    ...
    (
      ((UPat.var("r") < UPat.var("lower")).logical_not() & (UPat(Ops.RANGE, name="r") < UPat.var("upper")))
...
def reduce_collapse(red: UOp, u: UOp, pm=pm_reduce_collapse):
  for r in red.src[1:]:
    ...
    for u in included:
      for s in u.src:
        if s in included or s in replaces or s.op in {Ops.CONST, Ops.VCONST, Ops.DEFINE_GLOBAL, Ops.DEFINE_LOCAL, Ops.DEFINE_VAR}:
          continue
