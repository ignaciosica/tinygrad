...
def mark_range_mod(ctx, r: UOp, c: UOp):
  if r not in ctx and r.src[0].op is Ops.CONST and r.src[0].divides(c.arg) is not None:
    ctx[r] = c
...
pm_reduce_collapse = (
  ...
  + PatternMatcher([
    ...
    (
      ...
      lambda r, cut, val: (r.src[0] - cut).maximum(0).minimum(r.src[0]).cast(val.dtype)
      * val
      if no_range(val)
      else None,
    ),
...
pm_reduce_simplify = pm_reduce_unparented + PatternMatcher([
  (
    UPat(Ops.REDUCE, src=(UPat.var("u"),), allow_any_len=True, arg=Ops.ADD, name="red"),
    reduce_collapse,
  ),
])
...
pm_load_collapse = PatternMatcher([
  ...
  (
    ...
    lambda x, y, c: x < c - y if no_load(y) and no_load(c) and not no_load(x) else None,
  ),
])
