...
class _Device:
  ...
  def __get_canonicalized_item(self, ix: str) -> Compiled:
    assert ALLOW_DEVICE_USAGE or ix.split(":")[0] in [
      "DISK",
      "TINYFS",
      "NPY",
      "PYTHON",
    ], f"usage of device {ix} disallowed"
  ...
  def DEFAULT(self) -> str:
    ...
    from_env = dedup(
      ...
      + [
        ...
        if d not in ["DISK", "TINYFS", "NPY"] and getenv(d) == 1
      ]
    )
...
class Buffer:
  ...
  def __init__(
    ...
    (
      ...
      self.options,
      self.offset,
      self.allocated_views,
    ) = device, size, dtype, options, offset, 0
    if base is None:
  ...
  def is_allocated(self) -> bool:
    return (
      self.base.is_allocated() if self._base is not None else hasattr(self, "_buf")
    )
  ...
  def allocate(self, opaque=None, external_ptr=None) -> Buffer:
    ...
    if not self.device.startswith("NULL") and self.size > MAX_BUFFER_SIZE > 0:
    ...
    if self._base is not None:
      ...
      if PROFILE:
        Buffer.profile_events.append(
          ProfilePointEvent(
            self.device,
            "alloc",
            self.trace_num,
            {"dtype": self.dtype, "sz": self.size},
          )
        )
    return self
  ...
  def deallocate(self):
    ...
    if self._base is None and (
      ...
      if PROFILE:
        Buffer.profile_events.append(
          ProfilePointEvent(self.device, "free", self.trace_num)
        )
  ...
  def trace_num(self) -> int:
    if not hasattr(self, "_trace_num"):
      self._trace_num = len(Buffer.profile_events)
  ...
  def as_dmaref(self) -> DMARef:
    assert hasattr(self.allocator, "_as_dmaref"), (
      f"Device {self.device} doesn't support DMA"
    )
  ...
  def as_typed_buffer(
    self, shape=None, allow_zero_copy=False, force_zero_copy=False
  ) -> memoryview:
    assert self.dtype.base.fmt is not None, f"no fmt dtype for {self.dtype.base}"
    assert self.dtype.base.fmt != "e" or sys.version_info >= (3, 12)
    return self.as_buffer(allow_zero_copy, force_zero_copy).cast(
      self.dtype.base.fmt, shape if shape is not None else (self.size,)
    )

  def numpy(self) -> "np.ndarray":  # type: ignore [name-defined] # noqa: F821
    import numpy as np

    assert _to_np_dtype(self.dtype.base) is not None, (
      f"no np dtype for {self.dtype.base}"
    )
...
class Allocator(Generic[DeviceType]):
  ...
  def alloc(self, size: int, options: BufferSpec | None = None):
    ...
    return self._alloc(
      size, options if options is not None else self.default_buffer_spec
    )
...
class Compiler:
  ...
  def compile_cached(self, src: str) -> bytes:
    if self.cachekey is None or (lib := diskcache_get(self.cachekey, src)) is None:
      assert not getenv("ASSERT_COMPILE"), (
...
CompilerPairT = tuple[
  functools.partial | type[Renderer], functools.partial | type[Compiler]
]
...
class Compiled:
  ...
  def __init__(
    ...
    self.device, self.allocator, self.runtime, self.graph, self.group_id = (
      device,
      allocator,
      runtime,
      graph,
    ...
    enable_comps = set(
      (en, comp_pair)
      for en, comp_pair in zip(envnames, self.compilers)
      if en is not None and getenv(en, -1) == 1
    )
    disable_comps = set(
      (en, comp_pair)
      for en, comp_pair in zip(envnames, self.compilers)
      if en is not None and getenv(en, -1) == 0
    )

    if len(enable_comps) > 1:
      raise RuntimeError(
        f"{self.device}: multiple compilers set in env {enable_comps}"
      )
    for _, comp_pair in disable_comps:
      self.compilers.remove(comp_pair)

    try:
      self.renderer, self.compiler = next(
        self._get_available_compilers(
          [list(enable_comps)[0][1]] if len(enable_comps) == 1 else self.compilers
        )
      )
    except StopIteration as exc:
      raise RuntimeError(f"no usable compilers for {self.device}") from exc

    if DEBUG >= 1:
      print(f"{self.device}: using {self.compiler.__class__.__name__}")
  ...
  def _get_available_compilers(
    self, compilers
  ) -> Iterator[tuple[Renderer, Compiler]]:
    for renderer, compiler in compilers:
      with contextlib.suppress(Exception):
        yield renderer(), compiler()
...
def is_dtype_supported(dtype: DType, device: str | None = None) -> bool:
  ...
  if dtype == dtypes.bfloat16:
    ...
    if device in {"CUDA", "NV"}:
      return not CI and not getenv(f"{device}_PTX") and not getenv("NV_NAK")
    if device in {"CPU"}:
      return (
        not CI
        and platform.machine() in {"arm", "arm64", "aarch64", "x86_64", "amd64"}
        and not getenv("CPU_LVP")
      )
  ...
  if dtype in dtypes.fp8s:
    ...
    if device == "AMD":
      return (
        not CI
        and not AMD_LLVM
        and getattr(Device["AMD"], "target") in {(9, 4, 2), (9, 5, 0)}
      )
  ...
  if dtype == dtypes.half:
    ...
    if device in ["CUDA", "NV"]:
      return not CI
    if device == "CPU" and CPU_LLVM:
      return OSX
    if device == "PYTHON":
      return sys.version_info >= (3, 12)
  if dtype == dtypes.float64:
    return device != "METAL" and not (OSX and device == "CL")
  return True
    ...
    try:
      ...
      for i, (r, c) in enumerate(d.compilers):
        try:
          ...
          default_text = (
            "(default)"
            if type(default_compiler) is type(d.compiler)
            else f"({d._get_compiler_envvar(c)}=1 to make default)"
          )
          compilers_results.append(
            f"{colored('+', 'green')} {unwrap_class_type(c).__name__} {default_text}"
          )
          any_works = True
        except Exception as e:
          compilers_results.append(
            f"{colored('-', 'yellow')} {unwrap_class_type(c).__name__}: {e}"
          )
      result = (
        colored("PASS", "green") if any_works else f"{colored('FAIL', 'yellow')}"
...
if __name__ == "__main__":
  for s in enumerate_devices_str():
    print(s)
