...
class dtypes:
  ...
  def min(dtype: DType):
    if dtypes.is_int(dtype):
      return 0 if dtypes.is_unsigned(dtype) else -(2 ** (dtype.scalar().itemsize * 8 - 1))
  ...
  @staticmethod
  def imageh(shp):
    return ImageDType(100, 2, "imageh", "e", 1, None, dtypes.float32, AddrSpace.GLOBAL, 1, prod(shp), shp)

  @staticmethod
  def imagef(shp):
    return ImageDType(100, 4, "imagef", "f", 1, None, dtypes.float32, AddrSpace.GLOBAL, 1, prod(shp), shp)
...
def can_safe_cast(dt0: DType, dt1: DType) -> bool:
  ...
  match dt1:
    ...
    case dtypes.float:
      return dt0 in (dtypes.half, dtypes.bfloat16, *dtypes.fp8s, dtypes.uint16, dtypes.uint8, dtypes.int16, dtypes.int8)
    case dtypes.uint64:
      return dt0 in (dtypes.uint32, dtypes.uint16, dtypes.uint8)
    case dtypes.uint32:
      return dt0 in (dtypes.uint16, dtypes.uint8)
    case dtypes.int64:
      return dt0 in (dtypes.uint32, dtypes.uint16, dtypes.uint8, dtypes.int32, dtypes.int16, dtypes.int8)
    case dtypes.int32:
