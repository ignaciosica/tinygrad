...
class InvalidTypeMetaClass(type):
  ...
  def __call__(cls):
    if (ret := InvalidTypeMetaClass.instance) is not None:
      return ret
...
class InvalidType(metaclass=InvalidTypeMetaClass):
  ...
  def __reduce__(self):
    return (InvalidType, ())  # Return the global Invalid instance
...
class DTypeMetaClass(type):
  ...
  def __call__(cls, *args, **kwargs):
    if (ret := DTypeMetaClass.dcache.get(args, None)) is not None:
      return ret
...
class DType(metaclass=DTypeMetaClass):
  ...
  def __repr__(self):
    return f"dtypes.{INVERSE_DTYPES_DICT[self.scalar().name]}" + (
      f".vec({self.count})" if self.count != 1 else ""
    )

  def __lt__(self, o: DType):
    return (self.priority, self.itemsize, self.name, self.fmt, self.count) < (
      o.priority,
      o.itemsize,
      o.name,
      o.fmt,
  ...
  def vec(self, sz: int) -> DType:
    ...
    if sz == 1 or self == dtypes.void:
      return self  # void doesn't vectorize, and sz=1 is scalar
    return DType(
      self.priority,
      self.itemsize * sz,
      f"{INVERSE_DTYPES_DICT[self.name]}{sz}",
      None,
      sz,
      self,
    )
  ...
  def scalar(self) -> DType:
    return self._scalar if self._scalar is not None else self

  def nbytes(self) -> int:
    raise RuntimeError("only ptr types have nbytes")

  @property
  def min(self):
    return dtypes.min(self)

  @property
  def max(self):
    return dtypes.max(self)
...
class PtrDType(DType):
  ...
  def nbytes(self) -> int:
    if self.size == -1:
      raise RuntimeError("can't get nbytes of a pointer with unlimited size")
  ...
  def __repr__(self):
    return (
      f"{self.base.__repr__()}.ptr({self.size}{', ' + str(self.addrspace) if self.addrspace != AddrSpace.GLOBAL else ''})"
...
class dtypes:
  ...
  def from_py(x) -> DType:
    ...
    if x.__class__ is list or x.__class__ is tuple:
      return max(dtypes.from_py(xi) for xi in x) if x else dtypes.default_float
  ...
  def min(dtype: DType):
    if dtypes.is_int(dtype):
      return (
        0 if dtypes.is_unsigned(dtype) else -(2 ** (dtype.scalar().itemsize * 8 - 1))
      )
  ...
  def max(dtype: DType):
    if dtypes.is_int(dtype):
      return 2 ** (dtype.scalar().itemsize * 8) - 1 + dtypes.min(dtype)
  ...
  half = float16
  float = float32
  double = float64  # noqa: E702
  uchar = uint8
  ushort = uint16
  uint = uint32
  ulong = uint64  # noqa: E702
  char = int8
  short = int16
  int = int32
  long = int64  # noqa: E702
...
promo_lattice = {
  ...
  dtypes.int64: [dtypes.fp8e4m3, dtypes.fp8e5m2],
...
def least_upper_dtype(*ds: DType) -> DType:
  return (
    min(set.intersection(*[_get_recursive_parents(d.scalar()) for d in ds]))
...
DTYPES_DICT = {
  ...
  if isinstance(v, DType) and not k.startswith(("default", "void", "index"))
}
INVERSE_DTYPES_DICT = {
  **{v.name: k for k, v in DTYPES_DICT.items()},
  "void": "void",
  "index": "index",
}
...
def can_safe_cast(dt0: DType, dt1: DType) -> bool:
  ...
  if dt0 == dt1 or dt0 == dtypes.bool:
    return True
  match dt1:
    case dtypes.index:
      return dt0 in dtypes.ints
    case dtypes.double:
      return dt0 in (
        dtypes.float,
        dtypes.half,
    ...
    case dtypes.uint64:
      return dt0 in (dtypes.uint32, dtypes.uint16, dtypes.uint8)
    case dtypes.uint32:
      return dt0 in (dtypes.uint16, dtypes.uint8)
    case dtypes.int64:
      return dt0 in (
        dtypes.uint32,
        dtypes.uint16,
    ...
    case dtypes.int32:
      return dt0 in (dtypes.uint16, dtypes.uint8, dtypes.int16, dtypes.int8)
    case dtypes.int16:
      return dt0 in (dtypes.uint8, dtypes.int8)
    case _:
      return False
...
def sum_acc_dtype(dt: DType):
  ...
  if dtypes.is_unsigned(dt):
    return least_upper_dtype(dt, dtypes.uint)
  if dtypes.is_int(dt) or dt == dtypes.bool:
    return least_upper_dtype(dt, dtypes.int)
...
def float_to_fp8(x: float, dtype: DType) -> int:
  ...
  config = {
    dtypes.fp8e4m3: {
      ...
      "SIGNIFICAND_BITS": 4,
      "MANTISSA_MASK": 0x7,
      "MINDENORM_O2": 0x3F50000000000000,
      "OVERFLOW_THRESHOLD": 0x407D000000000000,
      "MAXNORM": 0x7E,
      "MINNORM": 0x3F90000000000000,
      "INF_VALUE": 0x7F,
    },
    dtypes.fp8e5m2: {
      "EXP_BIAS": 15,
      "SIGNIFICAND_BITS": 3,
      "MANTISSA_MASK": 0x3,
      "MINDENORM_O2": 0x3EE0000000000000,
      "OVERFLOW_THRESHOLD": 0x40EE000000000000 - 1,
      "MAXNORM": 0x7B,
      "MINNORM": 0x3F10000000000000,
      "INF_VALUE": 0x7E,
    },
  }[dtype]
  ...
  if absx <= config["MINDENORM_O2"]:
    res = 0
  elif absx > 0x7FF0000000000000:
    res = 0x7F if dtype == dtypes.fp8e4m3 else 0x7E | mantissa
    ...
    if (round_bits > FP8_DP_HALF_ULP) or (
      round_bits == FP8_DP_HALF_ULP and (mantissa & 1)
    ):
      res = res + 1
  else:
    shift = 1 - exp
    ...
    if (round_bits > (FP8_DP_HALF_ULP << shift)) or (
      round_bits == (FP8_DP_HALF_ULP << shift) and (res & 1)
    ):
      res = res + 1
...
truncate: dict[DType, Callable] = {
  ...
  **{
  ...
  dtypes.float32: lambda x: ctypes.c_float(x).value,
  dtypes.float64: lambda x: ctypes.c_double(x).value,
  dtypes.uint8: lambda x: ctypes.c_uint8(x).value,
  dtypes.uint16: lambda x: ctypes.c_uint16(x).value,
  dtypes.uint32: lambda x: ctypes.c_uint32(x).value,
  dtypes.uint64: lambda x: ctypes.c_uint64(x).value,
  dtypes.int8: lambda x: ctypes.c_int8(x).value,
  dtypes.int16: lambda x: ctypes.c_int16(x).value,
  dtypes.int32: lambda x: ctypes.c_int32(x).value,
...
def _from_torch_dtype(torchdtype: "torch.dtype") -> DType:  # type: ignore [name-defined] # noqa: F821
  return {v: k for k in DTYPES_DICT.values() if (v := _to_torch_dtype(k)) is not None}[
    torchdtype
  ]
