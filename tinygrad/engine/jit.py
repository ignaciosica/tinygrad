...
def apply_graph_to_jit(jit_cache: list[ExecItem], input_rawbuffers: list[Buffer], var_vals: dict[str, int], max_batch_size=0) -> list[ExecItem]:
  ...
  for ji in jit_cache:
    match ji.prg:
      ...
      case BufferCopy():
        ji_graph_dev = next((Device[unwrap(b).device] for b in ji.bufs if unwrap(b).device != "CPU"), None)
      case ViewOp():
  ...
  def __init__(self, jit_cache: list[ExecItem], input_rawbuffers: list[Buffer], var_vals: dict[str, int]):
    ...
    for j, ji in enumerate(jit_cache):
      ...
      if isinstance(ji.prg, CompiledRunner):
        if ji.prg.p.vars:
          self.var_vals_replace[j] = [(i, self.vars.index(v.expr)) for i, v in enumerate(ji.prg.p.vars) if v.expr not in ji.fixedvars]
