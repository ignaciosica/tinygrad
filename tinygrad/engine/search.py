...
def get_kernel_actions(lin:Kernel, include_0=True) -> dict[int, Kernel]:
  acted_lins, max_up, max_lcl, tc_actions = {0:lin} if include_0 else {}, getenv("BEAM_UPCAST_MAX", 256), getenv("BEAM_LOCAL_MAX", 1024), []

  for action in [action for action in actions if action.op == OptOps.TC]:
    axis, tc_opt = action.axis, cast(tuple, action.arg)[1]
    tc_actions += [Opt(op=OptOps.TC, axis=axis, arg=(select, tc_opt)) for select, _ in enumerate(lin.opts.tensor_cores)]

  for i,a in enumerate(actions + tc_actions):
