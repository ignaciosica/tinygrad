...
def reduce_gradient(ctx: UOp, ret: UOp):
  def broadcast_to_input(x):
    return x.reshape(x.shape + (1,) * (len(ret.src[0].shape) - len(x.shape))).expand(
      ret.src[0].shape
    )
...
pm_gradient = PatternMatcher([
  ...
  (
    ...
    lambda ctx, ret, b, e: (
      ...
      ctx
      * b.eq(0).where(
        (e < 0).where(ret.const_like(-math.inf), 0), ret * b.log2() * math.log(2.0)
      ),
    ),
  ),
  (
    UPat(Ops.MAX, src=(UPat.var("x"), UPat.var("y"))),
    lambda ctx, x, y: (
  ...
  (
    ...
    lambda ctx, ret: (
      ctx.r(
        Ops.ADD,
        tuple(i for i, (s, n) in enumerate(zip(ret.src[0].shape, ret.shape)) if s != n),
      ),
      None,
    ),
  ),
  (
    ...
    lambda ctx, ret: (
      ctx.shrink(tuple([(p[0], s + p[0]) for s, p in zip(ret.src[0].shape, ret.marg)])),
      None,
      None,
    ),
  ),
  (
    ...
    lambda ctx, ret: (
      ctx.pad(tuple([(p[0], s - p[1]) for s, p in zip(ret.src[0].shape, ret.marg)])),
      None,
      None,
    ),
  ),
...
def _deepwalk(root: UOp, targets: set[UOp]) -> list[UOp]:
  ...
  return list(
    root.toposort(
      lambda node: node.op not in {Ops.DETACH, Ops.ASSIGN} and in_target_path[node]
    )
  )
  ...
  for t0 in reversed(_deepwalk(root, targets)):
    ...
    for k, v in zip(t0.src, lgrads):
      ...
      if len(forward_metadata := all_metadata.get(t0, ())):
        backward_metadata = tuple(
          dataclasses.replace(x, backward=True) for x in forward_metadata
        )
