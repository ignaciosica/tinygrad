...
def time_to_str(t: float, w=8) -> str:
  return next(
    (f"{t * d:{w}.2f}{pr}" for d, pr in [(1, "s "), (1e3, "ms")] if t > 10 / d),
    f"{t * 1e6:{w}.2f}us",
  )
  ...
  return (
    acc := list(
      itertools.accumulate([
        (1 if ch == "(" else -1 if ch == ")" else 0) for ch in s
      ])
    )
  )[-1] == 0 and all(x >= 0 for x in acc)


def strip_parens(fst: str) -> str:
  return (
    fst[1:-1]
    if fst and fst[0] == "(" and fst[-1] == ")" and _is_balanced(fst[1:-1])
    else fst
  )
...
def merge_dicts(ds: Iterable[dict[T, U]]) -> dict[T, U]:
  ...
  if len(kvs) != len(set(kv[0] for kv in kvs)):
    raise RuntimeError(f"{kvs} contains different values for the same key")
...
def pad_bytes(b: bytes, align: int) -> bytes:
  return b + b"\x00" * ((align - (len(b) % align)) % align)
...
def strides_for_shape(shape: tuple[T, ...]) -> tuple[T, ...]:
  if not shape:
    return ()
  strides = tuple(
    itertools.accumulate(reversed(shape[1:]), operator.mul, initial=1)
  )[::-1]
...
def get_contraction(
  old_shape: tuple[T, ...], new_shape: tuple[T, ...]
) -> list[list[int]] | None:  # T is sint
  acc_old, acc_new = (
    list(itertools.accumulate(old_shape, operator.mul)),
    list(itertools.accumulate(new_shape, operator.mul)),
  )
  try:
...
def unwrap_class_type(cls_t):
  return cls_t.func if isinstance(cls_t, functools.partial) else cls_t
...
class LazySeq(
  Generic[T]
):  # NOTE: Mapping requires __iter__ and __len__, Sequence requires supporting __len__ and slicing in __getitem__
...
JIT, JIT_BATCH_SIZE = (
  ContextVar("JIT", 2 if OSX and ARCH_X86 else 1),
  ContextVar("JIT_BATCH_SIZE", 32),
)
...
VALIDATE_WITH_CPU, DISABLE_FAST_IDIV = (
  ContextVar("VALIDATE_WITH_CPU", 0),
  ContextVar("DISABLE_FAST_IDIV", 0),
)
CORRECT_DIVMOD_FOLDING, FUSE_OPTIM = (
  ContextVar("CORRECT_DIVMOD_FOLDING", 0),
  ContextVar("FUSE_OPTIM", 0),
)
ALLOW_DEVICE_USAGE, MAX_BUFFER_SIZE = (
  ContextVar("ALLOW_DEVICE_USAGE", 1),
  ContextVar("MAX_BUFFER_SIZE", 0),
)
EMULATE = ContextVar("EMULATE", "")
CPU_COUNT = ContextVar(
  "CPU_COUNT",
  max(
    1,
...
CPU_LLVM, CPU_LVP, AMD_LLVM = (
  ContextVar("CPU_LLVM", 0),
  ContextVar("CPU_LVP", 0),
  ContextVar("AMD_LLVM", 1),
)
...
def cpu_profile(
  name: str | TracingKey, device="CPU", is_copy=False, display=True
) -> Generator[ProfileRangeEvent, None, None]:
...
def fetch(
  ...
  if not fp.is_file() or not allow_caching:
    ...
    with urllib.request.urlopen(
      urllib.request.Request(url, headers={"User-Agent": "tinygrad 0.11.0"}),
      timeout=10,
    ) as r:
...
def wait_cond(cb, *args, value=True, timeout_ms=10000, msg="") -> bool:
  ...
  while int(time.perf_counter() * 1000) - start_time < timeout_ms:
    if (val := cb(*args)) == value:
      return val
  raise TimeoutError(
    f"{msg}. Timed out after {timeout_ms} ms, condition not met: {val} != {value}"
  )
...
def from_mv(
  mv: memoryview, to_type: type[ctypes._SimpleCData] = ctypes.c_char
) -> ctypes.Array:
  return ctypes.cast(
...
class tqdm(Generic[T]):
  ...
  def update(self, n: int = 0, close: bool = False):
    ...
    def SI(x):
      return (
        (
          f"{x / 1000 ** int(g := round(math.log(x, 1000), 6)):.{int(3 - 3 * math.fmod(g, 1))}f}"[
            :4
          ].rstrip(".")
