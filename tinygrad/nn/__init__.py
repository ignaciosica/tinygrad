...
class BatchNorm:
  ...
  def __init__(
    ...
    self.num_batches_tracked = Tensor.zeros(
      dtype="long" if is_dtype_supported(dtypes.long) else "int", requires_grad=False
    )
  ...
  def calc_stats(self, x: Tensor) -> tuple[Tensor, Tensor]:
    ...
    if self.track_running_stats and not Tensor.training:
      return self.running_mean, self.running_var.reshape(shape=shape_mask).expand(
        x.shape
      )
...
class Conv2d:
  ...
  def __init__(
    ...
    if isinstance(padding, str):
      ...
      pad = [
        ...
        for d, k in zip(
          make_tuple(dilation, len(self.kernel_size)), self.kernel_size[::-1]
        )
      ]
    ...
    self.weight = Tensor.uniform(
      out_channels, in_channels // groups, *self.kernel_size, low=-scale, high=scale
    )
  ...
  def __call__(self, x: Tensor) -> Tensor:
    return x.conv2d(
      self.weight, self.bias, self.groups, self.stride, self.dilation, self.padding
    )
...
class ConvTranspose2d(Conv2d):
  ...
  def __init__(
    ...
    scale = 1 / math.sqrt(in_channels * prod(self.kernel_size))
    self.weight = Tensor.uniform(
      in_channels, out_channels // groups, *self.kernel_size, low=-scale, high=scale
    )
...
class GroupNorm:
  ...
  def __call__(self, x: Tensor) -> Tensor:
    ...
    return x * self.weight.reshape(1, -1, *[1] * (x.ndim - 2)) + self.bias.reshape(
      1, -1, *[1] * (x.ndim - 2)
    )
...
class LayerNorm:
  ...
  def __init__(
    ...
    self.weight: Tensor | None = (
      Tensor.ones(*self.normalized_shape) if elementwise_affine else None
    )
    self.bias: Tensor | None = (
      Tensor.zeros(*self.normalized_shape) if elementwise_affine else None
    )
...
class Embedding:
  ...
  def __init__(self, vocab_size: int, embed_size: int):
    self.vocab_sz, self.embed_sz, self.weight = (
      vocab_size,
      embed_size,
  ...
  def __call__(self, idx: Tensor) -> Tensor:
    ...
    if not dtypes.is_int(idx.dtype):
      raise TypeError(
        f"Expected integer dtype for index in embedding, got {idx.dtype}"
      )
    big_shp = idx.shape + (self.vocab_sz, self.embed_sz)
    arange, idx, vals = (
      self.arange.expand(big_shp),
      idx.reshape(idx.shape + (1, 1)).expand(big_shp),
      self.weight.expand(big_shp),
    )
...
class LSTMCell:
  ...
  def __init__(self, input_size: int, hidden_size: int, bias: bool = True):
    ...
    self.weight_ih = Tensor.uniform(
      hidden_size * 4, input_size, low=-stdv, high=stdv
    )
    self.weight_hh = Tensor.uniform(
      hidden_size * 4, hidden_size, low=-stdv, high=stdv
    )
  ...
  def __call__(
    self, x: Tensor, hc: tuple[Tensor, Tensor] | None = None
  ) -> tuple[Tensor, Tensor]:
    if hc is None:
      hc = (
        Tensor.zeros(
          x.size(0), self.weight_hh.size(1), dtype=x.dtype, device=x.device
        ),
      ) * 2
    gates = x.linear(self.weight_ih.T, self.bias_ih) + hc[0].linear(
