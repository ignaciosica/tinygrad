...
def to_python_const(t: Any, op: str, idx: int) -> list[ConstType] | ConstType | bytes:
  if idx not in required_input_python_consts.get(op, ()) or not isinstance(t, Tensor):
    return t
  global cache_misses
  ...
  def _parse_input(self, name: str, value: Any, spec: OnnxValue):
    ...
    if spec.is_sequence:
      if not isinstance(value, Sequence):
        raise RuntimeError(f"input {name} received {value}, expected a sequence type")
      sequence = [
...
def get_onnx_ops() -> dict[str, types.FunctionType | dict[OpSetId, types.FunctionType]]:
  ...
  def _resolve_pool_pads(x: Tensor, p_, k_, d_, s_, auto_pad: AUTO_PAD_OPTIONS):
    ...
    return _onnx_pads_to_tiny_pads(
      _auto_pad([(o - 1) * s + k - i for o, i, k, s in zip(o_, i_, k_, s_)], auto_pad)
    )
  ...
  def Constant(
    ...
    if (
      value_string is not None or value_strings is not None or sparse_value is not None
    ):
      raise NotImplementedError(
  ...
  def ImageDecoder(encoded_stream: bytes, pixel_format="RGB"):
    try:
      ...
      raise ImportError("Pillow must be installed for the ImageDecoder operator") from e
  ...
  def EyeLike(x: Tensor, dtype: int | None = None, k: int = 0):
    ...
    return (
      ...
      if x.size(0) == x.size(1)
        ...
        tuple(None if d == ret.size(0) else (k, d - ret.shape[0] - k) for d in x.shape)
      )
    )
  ...
  def ArgMax(x: Tensor, axis: int = 0, keepdims: int = 1, select_last_index: int = 0):
    if select_last_index:
      return ((x.shape[axis] - 1) - x.flip(axis).argmax(axis, keepdim=keepdims)).cast(
        dtypes.int64
      )
  ...
  def Reshape(data: Tensor, shape: list[int], allowzero: int = 0):
    return data.reshape([
      x if x != 0 else (0 if allowzero else data.shape[i]) for i, x in enumerate(shape)
    ])
  ...
  def MaxPool(
    ...
    return ret, idx.transpose(-2, -1).cast(dtypes.int64) if storage_order else idx.cast(
      dtypes.int64
    )
  ...
  def Gemm(
    ...
    if C is not None:
      ret = ret + beta * (
        C
        if broadcast == 0
        else C.reshape([-1 if i < len(C.shape) else 1 for i in range(ret.ndim)][::-1])
      )
    return ret
  ...
  def EmbedLayerNormalization(
    ...
    if position_ids is None:
      position_ids = (
        Tensor.arange(seq_length, requires_grad=False).unsqueeze(0).expand(*input_shape)
      )
  ...
  def attention_onnx(
    ...
    if softmax_precision:
      scores = scores.cast(
        {1: dtypes.float32, 10: dtypes.float16, 16: dtypes.bfloat16}[softmax_precision]
      )
  ...
  def RMSNormalization(
    X: Tensor, scale: Tensor, axis: int = -1, epsilon: float = 1e-5, stash_type: int = 1
  ):
    assert stash_type == 1, "only float32 is supported"
    norm = (
      X.cast(dtypes.float)
  ...
  def Adam(
    ...
    *inputs: Tensor,
    ...
    if T == 0:
      ...
      opt.b1_t = Tensor(
        [alpha ** (T - 1)], dtype=dtypes.float32, device=X.device, requires_grad=False
      )
      opt.b2_t = Tensor(
