...
class AttributeType(enum.IntEnum):
  ...
  def to_field_name(self) -> str:
    return {1: "f", 2: "i", 3: "s", 4: "t", 5: "g", 6: "floats", 7: "ints", 8: "strings"}[self.value]
  ...
  def _parse_TensorProto(self) -> dict:
    ...
    for fid, wire_type in self._parse_message(self._decode_end_pos()):
      match fid:
        ...
        case 13:
          obj.setdefault("external_data", []).append(self._parse_StringStringEntryProto())
        case 14:
    ...
    if self.load_external_data and obj.get("data_location", 0) == 1:
      ...
      if self.file_path is None:
        if isinstance(self.tensor.device, str) and self.tensor.device.startswith("DISK:"):
          ...
          raise ValueError("onnx external_data needs the origin file path, try passing onnx file path to onnx_load")
  ...
  def _parse_AttributeProto(self) -> dict:
    ...
    for fid, wire_type in self._parse_message(self._decode_end_pos()):
      match fid:
        ...
        case 9:
          obj["strings"].append(self.reader.read_bytes().data().tobytes().decode("utf8"))
        case 20:
...
class OnnxRunner:
  ...
  def _parse_input(self, name: str, value: Any, spec: OnnxValue):
    ...
    if tensor.dtype is not spec.dtype:
      warnings.warn(f"input {name} has mismatch on dtype. Expected {spec.dtype}, received {tensor.dtype}.")
    for dim, (onnx_dim, user_dim_input) in enumerate(zip(spec.shape, tensor.shape, strict=True)):
      ...
      if user_dim_input != onnx_dim:
        raise RuntimeError(f"input {name} has mismatch on {dim=}. Expected {onnx_dim}, received {user_dim_input}.")
    return tensor
  ...
  def _select_op(self, op: str, required_opset: OpSetId) -> types.FunctionType:
    ...
    if not eligible_ops:
      raise NotImplementedError(f"{op=} is not supported for domain {required_opset.domain} and version {required_opset.version}")
  ...
  def __call__(self, inputs: dict[str, Any], debug=debug):
    ...
    for num, node in enumerate(self.graph_nodes):
      ...
      if debug >= 1:
        print((f"[{self.graph_name}] " if self.graph_name else "") + f"{num}: op '{node.op}' opt {opts}")
...
def get_onnx_ops() -> dict[str, types.FunctionType | dict[OpSetId, types.FunctionType]]:
  ...
  def _auto_pad(pads, auto_pad: AUTO_PAD_OPTIONS):
    if auto_pad == "SAME_UPPER":
      return [pads[i] // 2 for i in range(len(pads))] + [pads[i] - pads[i] // 2 for i in range(len(pads))]
  ...
  def _prepare_quantize(x: Tensor, scale: Tensor, zero_point: Tensor | int, axis=1, block_size=0):
    ...
    def reshape(val: Tensor):
      if val.numel() == 1:
        return val
      if block_size == 0:
        return val.reshape([val.shape[0] if dim == axis else 1 for dim in range(x.ndim)])
  ...
  def _qlinearop_quantized(
    op, inputs: list[Tensor], zero_points: list[Tensor], scales: list[Tensor], out_scale: Tensor, out_zero_point: Tensor, **opts
  ):
  ...
  def ImageDecoder(encoded_stream: bytes, pixel_format="RGB"):
    ...
    if pixel_format == "Grayscale":
      return Tensor(img.convert("L").tobytes(), dtype=dtypes.uint8).reshape(*img.size, 1)
  ...
  def Gelu(x: Tensor, approximate: str | None = None):
    return x.gelu() if approximate == "tanh" else 0.5 * x * (1 + (x / math.sqrt(2)).erf())
  ...
  def FastGelu(x: Tensor, bias: Tensor | None = None):
    return (x + bias).gelu() if bias is not None else x.gelu()  # this is tanh approximated
  ...
  def ArgMax(x: Tensor, axis: int = 0, keepdims: int = 1, select_last_index: int = 0):
    if select_last_index:
      return ((x.shape[axis] - 1) - x.flip(axis).argmax(axis, keepdim=keepdims)).cast(dtypes.int64)
  ...
  def Split(data: Tensor, split: list[int] | None = None, num_outputs: int = 0, axis: int = 0):
    sz = data.shape[axis]
    if split is None:
      split = [sz // num_outputs + (1 if i < sz % num_outputs else 0) for i in range(num_outputs)]
  ...
  def Pad(
    ...
    for i, axis in enumerate(axes):
      real_pads[axis % x.ndim], real_pads[axis % x.ndim + x.ndim] = pads[i], pads[i + len(axes)]
  ...
  def ConvTranspose(
    ...
    if output_shape is not None:  # we pad according to output_shape
      pads = _auto_pad(
        [
          ...
          for s_, i, op_, k_, d_, os in zip(strides_, input_shape_, output_padding_, kernel_shape_, dilations_, output_shape)
        ],
        auto_pad,
      )
    if pads is None:  # we generate pads
      ...
      pads = [
        strides_[i] * (input_shape_[i] - 1) + output_padding_[i] + ((kernel_shape_[i] - 1) * dilations_[i] + 1) - output_shape[i]
  ...
  def Gemm(A: Tensor, B: Tensor, C: Tensor | None = None, alpha: float = 1.0, beta: float = 1.0, transA: int = 0, transB: int = 0, broadcast=0):
    ...
    if C is not None:
      ret = ret + beta * (C if broadcast == 0 else C.reshape([-1 if i < len(C.shape) else 1 for i in range(ret.ndim)][::-1]))
    return ret
  ...
  def Resize(
    ...
    def _apply_transformation(input_sz, output_sz, scale_dim, mode):
      ...
      if mode == "align_corners":
        return index * (input_sz - 1) / (output_sz - 1) if output_sz != 1 else Tensor.zeros_like(index)
      if mode == "asymmetric":
        return index / scale_dim
      if mode == "pytorch_half_pixel":
        return ((index + 0.5) / scale_dim - 0.5) if output_sz != 1 else Tensor.zeros_like(index)
    ...
    if scales is not None:
      assert all(sc == 1 for sc in scales[: -len(input_shape)]), "resizing batch_size dim or channel dim not supported"
    if sizes is not None:
      assert tuple(sizes[:-2]) == tuple(X.shape[X.ndim - len(sizes) : -2]), "resizing batch_size dim or channel dim not supported"
  ...
  def DepthToSpace(X: Tensor, blocksize: int, mode: str = "DCR"):
    return X.rearrange(
      "b (c h1 w1) h w -> b c (h h1) (w w1)" if mode == "CRD" else "b (h1 w1 c) h w -> b c (h h1) (w w1)", h1=blocksize, w1=blocksize
    )
  ...
  def NegativeLogLikelihoodLoss(
    x: Tensor, target: Tensor, weight: Tensor | None = None, ignore_index: int | None = None, reduction: ReductionStr = "mean"
  ):
    return x.nll_loss(target, weight, ignore_index, reduction)

  def SoftmaxCrossEntropyLoss(
    scores: Tensor, labels: Tensor, weights: Tensor | None = None, ignore_index: int | None = None, reduction: ReductionStr = "mean"
  ):
    ...
    if mask_index is not None:
      ...
      if mask_index.ndim != 1:
        ...
        if mask_index.shape[0] == batch_size:
          ...
          raise NotImplementedError("mask_index with shape (3 * batch_size + 2) is not implemented")
      while mask.ndim < 4:
  ...
  def attention_onnx(
    ...
    if softmax_precision:
      scores = scores.cast({1: dtypes.float32, 10: dtypes.float16, 16: dtypes.bfloat16}[softmax_precision])
  ...
  def QuantizeLinear(
    ...
    if isinstance(y_zero_point, Tensor):
      ...
      out_dtype = dtype_fallback(OnnxDataType(output_dtype).to_dtype(), "QuantizeLinear op")
    else:
  ...
  def QLinearMatMul(
    a: Tensor, a_scale: Tensor, a_zero_point: Tensor, b: Tensor, b_scale: Tensor, b_zero_point: Tensor, y_scale: Tensor, y_zero_point: Tensor
  ) -> Tensor:
    return _qlinearop_quantized(Tensor.matmul, [a, b], [a_zero_point, b_zero_point], [a_scale, b_scale], y_scale, y_zero_point)

  def QLinearAdd(
    a: Tensor, a_scale: Tensor, a_zero_point: Tensor, b: Tensor, b_scale: Tensor, b_zero_point: Tensor, c_scale: Tensor, c_zero_point: Tensor
  ):
  ...
  def ConvInteger(
    x: Tensor, w: Tensor, x_zero_point: Tensor = Tensor(0), w_zero_point: Tensor = Tensor(0), B: Tensor | None = None, **opts
  ) -> Tensor:
