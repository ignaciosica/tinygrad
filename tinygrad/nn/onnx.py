...
class OnnxDataType(enum.IntEnum):
  ...
  UINT32 = 12  # noqa: E702
  UINT64 = 13
  BFLOAT16 = 16  # noqa: E702

  def to_dtype(self) -> DType:
    return dtypes.fields()[self.name.lower()]
...
def dtype_fallback(dtype: DType, fallback_context: str) -> DType:
  if is_dtype_supported(dtype):
    return dtype
  default_dtype = (
    dtypes.default_int if dtypes.is_int(dtype) else dtypes.default_float
  )
  warnings.warn(
    f"dtype {dtype} on {Device.DEFAULT} from {fallback_context} is not supported, falling back to {default_dtype}"
  )
  assert is_dtype_supported(default_dtype), (
    f"dtype {default_dtype} must be supported on {Device.DEFAULT}"
  )
...
class Domain(enum.Enum):
  ...
  @classmethod
  def from_onnx(cls, domain: str | None) -> "Domain":
    return cls.ONNX if domain is None or domain == "" else cls(domain)
...
class PBBufferedReader(BufferedReader):
  ...
  def decode_varint(self) -> int:
    ...
    while True:
      data = self.read(1)
      if data == b"":
        raise EOFError("decode_varint EOF")
  ...
  def read_string(self) -> str:
    return self.read_delimited().decode("utf-8")

  def read_bytes(self) -> Tensor:
    return self.read_delimited(use_tensor=True)

  def read_float(self) -> float:
    return struct.unpack("<f", self.read(4))[0]

  def read_packed_floats(self) -> Tensor:
    return self.read_delimited(use_tensor=True)

  def read_int64(self) -> int:
    return truncate[dtypes.int64](self.decode_varint())
  ...
  def skip_field(self, wire_type: WireType) -> None:
    ...
    match wire_type:
      ...
      case WireType.LENGTH_DELIMITED:
        self.seek(self.decode_varint(), os.SEEK_CUR)
      case _:
...
class OnnxPBParser:
  ...
  def __init__(
    self, inp: Tensor | str | pathlib.Path, load_external_data: bool = True
  ):
  ...
  def _parse_message(
    self, end_pos: int
  ) -> Generator[tuple[int, WireType], None, None]:
  ...
  def _parse_ModelProto(self) -> dict:
    ...
    for fid, wire_type in self._parse_message(self.reader.len):
      match fid:
        case 4:
          obj["domain"] = self.reader.read_string()
        case 5:
          obj["model_version"] = self.reader.read_int64()
        case 7:
          obj["graph"] = self._parse_GraphProto()
        case 8:
          obj["opset_import"].append(self._parse_OperatorSetIdProto())
        case _:
          self.reader.skip_field(wire_type)

    # update opset version
    opset_imports = {
      Domain.from_onnx(x.get("domain")): x.get("version", 1)
      for x in obj["opset_import"]
    }
    for n in obj["graph"]["node"]:
      n_ = n["parsed_node"]
      n["parsed_node"] = OnnxNode(
        n_.op,
        OpSetId(n_.opset_id.domain, opset_imports.get(n_.opset_id.domain, 1)),
        n_.inputs,
        n_.outputs,
        n_.opts,
      )
  ...
  def _parse_GraphProto(self) -> dict:
    ...
    for fid, wire_type in self._parse_message(self._decode_end_pos()):
      match fid:
        case 1:
          obj["node"].append(self._parse_NodeProto())
        case 2:
          obj["name"] = self.reader.read_string()
        case 5:
          obj["initializer"].append(self._parse_TensorProto())
        case 11:
          obj["input"].append(self._parse_ValueInfoProto())
        case 12:
          obj["output"].append(self._parse_ValueInfoProto())
        case _:
          self.reader.skip_field(wire_type)
    return obj
  ...
  def _parse_NodeProto(self) -> dict:
    ...
    for fid, wire_type in self._parse_message(self._decode_end_pos()):
      match fid:
        case 1:
          obj["input"].append(self.reader.read_string())
        case 2:
          obj["output"].append(self.reader.read_string())
        case 3:
          obj["name"] = self.reader.read_string()
        case 4:
          obj["op_type"] = self.reader.read_string()
        case 5:
          obj["attribute"].append(self._parse_AttributeProto())
        case 6:
          obj["doc_string"] = self.reader.read_string()
        case 7:
          obj["domain"] = self.reader.read_string()
        case _:
          self.reader.skip_field(wire_type)
    ...
    # parse node
    attributes = {
      attr_dict["name"]: attr_dict[AttributeType(attr_dict["type"]).to_field_name()]
      for attr_dict in obj["attribute"]
    }
    opset_id = OpSetId(
      Domain.from_onnx(obj.get("domain")), 1
    )  # default version, to be updated later in _parse_ModelProto
    obj["parsed_node"] = OnnxNode(
      obj["op_type"], opset_id, tuple(obj["input"]), tuple(obj["output"]), attributes
    )
    return obj
  ...
  def _parse_TensorProto(self) -> dict:
    ...
    for fid, wire_type in self._parse_message(self._decode_end_pos()):
      match fid:
        case 1:
          obj["dims"].append(self.reader.read_int64())
        case 2:
          obj["data_type"] = self.reader.read_int64()
        case 4:
          obj["float_data"] = self.reader.read_packed_floats()
        case 5:
          obj["int32_data"] = self.reader.read_packed_int64s()
        case 7:
          obj["int64_data"] = self.reader.read_packed_int64s()
        case 8:
          obj["name"] = self.reader.read_string()
        case 9:
          obj["raw_data"] = self.reader.read_bytes()
        case 10:
          obj["double_data"] = self.reader.read_packed_floats()
        case 11:
          obj["uint64_data"] = self.reader.read_packed_int64s()
        case 13:
          obj.setdefault("external_data", []).append(
            self._parse_StringStringEntryProto()
          )
        case 14:
          obj["data_location"] = self.reader.read_int64()
        case _:
          self.reader.skip_field(wire_type)

    # load external data
    if self.load_external_data and obj.get("data_location", 0) == 1:
      if "external_data" not in obj:
        raise ValueError("no external_data")
      ...
      for kv in obj["external_data"]:
        if kv["key"] == "location":
          ...
          length = int(kv["value"])
      if location is None:
        raise ValueError("no location in external_data")

      if self.file_path is None:
        if isinstance(self.tensor.device, str) and self.tensor.device.startswith(
          "DISK:"
        ):
          self.file_path = pathlib.Path(self.tensor.device[5:])
        else:
          raise ValueError(
            "onnx external_data needs the origin file path, try passing onnx file path to onnx_load"
          )
      ext_path = self.file_path.parent.joinpath(location)
      if not ext_path.exists():
        raise FileNotFoundError(f"external location not exists: {ext_path}")

      ext_tensor = Tensor(ext_path)
      obj["raw_data"] = (
        ext_tensor[offset : offset + length]
        if length is not None
        else ext_tensor[offset:]
      )
      obj["data_location"] = 0

    # parse tensor
    to_dtype = dtype_fallback(
      true_dtype := OnnxDataType(obj["data_type"]).to_dtype(), "buffer parse"
    )
    ...
    data = (
      data.to(Device.DEFAULT)
      if true_dtype is to_dtype
      else data.to("cpu").cast(to_dtype).to(Device.DEFAULT)
    )
    # const folding
    if shape == ():
      if data.dtype == dtypes.float16 and sys.version_info < (3, 12):
    ...
    for fid, wire_type in self._parse_message(self._decode_end_pos()):
      match fid:
        case 1:
          obj["name"] = self.reader.read_string()
        case 2:
          obj["f"] = self.reader.read_float()
        case 3:
          obj["i"] = self.reader.read_int64()
        case 4:
          obj["s"] = self.reader.read_bytes().data().tobytes().decode("utf8")
        case 5:
          obj["t"] = self._parse_TensorProto()["parsed_tensor"]
        case 6:
          obj["g"] = OnnxRunner._from_subgraph(self._parse_GraphProto())
        case 7:
          obj["floats"].append(self.reader.read_float())
        case 8:
          obj["ints"].append(self.reader.read_int64())
        case 9:
          obj["strings"].append(
            self.reader.read_bytes().data().tobytes().decode("utf8")
          )
        case 20:
          obj["type"] = self.reader.read_int64()
        case _:
          self.reader.skip_field(wire_type)
    obj["floats"], obj["ints"], obj["strings"] = (
      tuple(obj["floats"]),
      tuple(obj["ints"]),
      tuple(obj["strings"]),
    )
    ...
    for fid, wire_type in self._parse_message(self._decode_end_pos()):
      match fid:
        case 1:
          obj["name"] = self.reader.read_string()
        case 2:
          obj["type"] = self._parse_TypeProto()
        case _:
          self.reader.skip_field(wire_type)

    # parse type
    if "type" not in obj:
      return {**obj, "parsed_type": None}
    type_obj = obj["type"]
    if is_optional := "optional_type" in type_obj:
      type_obj = type_obj["optional_type"]["elem_type"]
    if is_sequence := "sequence_type" in type_obj:
      type_obj = type_obj["sequence_type"]["elem_type"]
    assert "tensor_type" in type_obj, type_obj
    shape_dims = type_obj["tensor_type"].get("shape", {}).get("dim", [])
    obj["parsed_type"] = OnnxValue(
      tuple(d.get("dim_param") or d.get("dim_value") for d in shape_dims),
      OnnxDataType(type_obj["tensor_type"]["elem_type"]).to_dtype(),
      is_optional,
      is_sequence,
    )
    return obj
  ...
  def _parse_TypeProto(self) -> dict:
    ...
    for fid, wire_type in self._parse_message(self._decode_end_pos()):
      match fid:
        case 1:
          obj["tensor_type"] = self._parse_TypeProtoTensor()
        case 4:
          obj["sequence_type"] = self._parse_TypeProtoSequence()
        case 9:
          obj["optional_type"] = self._parse_TypeProtoOptional()
        case _:
          self.reader.skip_field(wire_type)
    return obj
  ...
  def _parse_TypeProtoTensor(self) -> dict:
    ...
    for fid, wire_type in self._parse_message(self._decode_end_pos()):
      match fid:
        case 1:
          obj["elem_type"] = self.reader.read_int64()
        case 2:
          obj["shape"] = self._parse_TensorShapeProto()
        case _:
          self.reader.skip_field(wire_type)
    return obj
  ...
  def _parse_TypeProtoSequence(self) -> dict:
    ...
    for fid, wire_type in self._parse_message(self._decode_end_pos()):
      match fid:
        case 1:
          obj["elem_type"] = self._parse_TypeProto()
        case _:
          self.reader.skip_field(wire_type)
    return obj
  ...
  def _parse_TensorShapeProto(self) -> dict:
    ...
    for fid, wire_type in self._parse_message(self._decode_end_pos()):
      match fid:
        case 1:
          obj["dim"].append(self._parse_TensorShapeProtoDimension())
        case _:
          self.reader.skip_field(wire_type)
    return obj
  ...
  def _parse_TensorShapeProtoDimension(self) -> dict:
    ...
    for fid, wire_type in self._parse_message(self._decode_end_pos()):
      match fid:
        case 1:
          obj["dim_value"] = self.reader.read_int64()
        case 2:
          obj["dim_param"] = self.reader.read_string()
        case _:
          self.reader.skip_field(wire_type)
    return obj
  ...
  def _parse_StringStringEntryProto(self) -> dict:
    ...
    for fid, wire_type in self._parse_message(self._decode_end_pos()):
      match fid:
        case 1:
          obj["key"] = self.reader.read_string()
        case 2:
          obj["value"] = self.reader.read_string()
        case _:
          self.reader.skip_field(wire_type)
    return obj
    ...
    for fid, wire_type in self._parse_message(self._decode_end_pos()):
      match fid:
        case 1:
          obj["domain"] = self.reader.read_string()
        case 2:
          obj["version"] = self.reader.read_int64()
        case _:
          self.reader.skip_field(wire_type)
    return obj
...
def to_python_const(
  t: Any, op: str, idx: int
) -> list[ConstType] | ConstType | bytes:
  if idx not in required_input_python_consts.get(op, ()) or not isinstance(
    t, Tensor
  ):
    return t
  global cache_misses
  ...
  if (
    info := _cached_to_python_const.cache_info()
  ).misses > cache_misses and DEBUG >= 3:
...
class OnnxRunner:
  ...
  def _init_from_graph(self, graph: dict, is_subgraph: bool = False):
    self.is_training = any(
      n["parsed_node"].opset_id.domain
      in {Domain.AI_ONNX_TRAINING, Domain.AI_ONNX_PREVIEW_TRAINING}
      for n in graph["node"]
    )
    self.graph_name = graph["name"] if is_subgraph else ""
    self.graph_values = {
      "": None,
      **{i["name"]: i["parsed_tensor"] for i in graph["initializer"]},
    }
    self.graph_inputs = {
      i["name"]: i["parsed_type"]
      for i in graph["input"]
      if i["name"] not in self.graph_values
    }
  ...
  def _parse_input(self, name: str, value: Any, spec: OnnxValue):
    if spec.is_optional and value is None:
      return None
    if spec.is_sequence:
      if not isinstance(value, Sequence):
        raise RuntimeError(
          f"input {name} received {value}, expected a sequence type"
        )
      sequence = [
        Tensor(v, dtype=spec.dtype, requires_grad=self.is_training)
        if not isinstance(v, Tensor)
        else v
        for v in value
      ]
      if not all_same(tuple(t.shape for t in sequence)):
        raise RuntimeError(f"Shapes for input {name} sequence must be homogeneous")
      if not all(t.dtype is spec.dtype for t in sequence):
        warnings.warn(f"Dtypes for input {name} sequence aren't all {spec.dtype}")
      return sequence
    dtype = _from_np_dtype(value.dtype) if is_numpy_ndarray(value) else spec.dtype
    tensor = (
      Tensor(value, dtype=dtype, requires_grad=self.is_training)
      if not isinstance(value, Tensor)
      else value
    )
    if tensor.dtype is not spec.dtype:
      warnings.warn(
        f"input {name} has mismatch on dtype. Expected {spec.dtype}, received {tensor.dtype}."
      )
    for dim, (onnx_dim, user_dim_input) in enumerate(
      zip(spec.shape, tensor.shape, strict=True)
    ):
      if isinstance(onnx_dim, str):
        onnx_dim = (
          self.variable_dims[onnx_dim]
          if onnx_dim in self.variable_dims
          else self.variable_dims.setdefault(onnx_dim, int(user_dim_input))
        )
      if user_dim_input != onnx_dim:
        raise RuntimeError(
          f"input {name} has mismatch on {dim=}. Expected {onnx_dim}, received {user_dim_input}."
        )
    return tensor

  def _select_op(self, op: str, required_opset: OpSetId) -> types.FunctionType:
    if op not in self.onnx_ops:
      raise NotImplementedError(f"{op=} is not supported")
    # return default implementation if no opset_id is specified
    if isinstance(impl := self.onnx_ops[op], types.FunctionType):
      return impl
    # match domain and select implementation with latest compatible version
    eligible_ops = {
      impl_opset.version: impl_fxn
      for impl_opset, impl_fxn in impl.items()
      if impl_opset.domain == required_opset.domain
      and impl_opset.version <= required_opset.version
    }
    if not eligible_ops:
      raise NotImplementedError(
        f"{op=} is not supported for domain {required_opset.domain} and version {required_opset.version}"
      )
    return eligible_ops[max(eligible_ops.keys())]

  def get_empty_input_data(
    self, device: str | None = None, dtype: DType | None = None
  ) -> dict[str, Tensor]:
    return {
      name: Tensor.empty(*spec.shape, device=device, dtype=dtype or spec.dtype)
      for name, spec in self.graph_inputs.items()
    }

  def to(self, device: str | None):
    self.graph_values = {
      k: (v.to(device) if isinstance(v, Tensor) else v)
      for k, v in self.graph_values.items()
    }
    self.graph_nodes = tuple(
      OnnxNode(
        ...
        tuple(n.outputs),
        {
          k: (v.to(device) if isinstance(v, (Tensor, OnnxRunner)) else v)
          for k, v in n.opts.items()
        },
      )
  ...
  def __call__(self, inputs: dict[str, Any], debug=debug):
    ...
    for num, node in enumerate(self.graph_nodes):
      inps = [
        to_python_const(self.graph_values[name], node.op, i)
      ...
      self.graph_values.update(
        dict(zip(node.outputs, ret[: len(node.outputs)], strict=True))
      )
...
##### ONNX OPS #####
####################
def get_onnx_ops() -> dict[
  str, types.FunctionType | dict[OpSetId, types.FunctionType]
]:
  # ***** helper functions *****
  def _resolve_const(x: Sequence[ConstType] | ConstType):
    return get_single_element(x) if isinstance(x, Sequence) else x

  def _axes(axes, noop_with_empty_axes):
    return axes or ([] if noop_with_empty_axes else None)
  ...
  def _auto_pad(pads, auto_pad: AUTO_PAD_OPTIONS):
    if auto_pad == "SAME_UPPER":
      return [pads[i] // 2 for i in range(len(pads))] + [
        pads[i] - pads[i] // 2 for i in range(len(pads))
      ]
    return [pads[i] - pads[i] // 2 for i in range(len(pads))] + [
      pads[i] // 2 for i in range(len(pads))
    ]
  ...
  def _resolve_pool_pads(x: Tensor, p_, k_, d_, s_, auto_pad: AUTO_PAD_OPTIONS):
    ...
    i_, (s_, d_, p_) = (
      x.shape[-len(k_) :],
      (make_tuple(x, len(k_) * 2) for x in (s_, d_, p_)),
    )
    if auto_pad == "NOTSET":
      return _onnx_pads_to_tiny_pads(p_ if len(p_) == len(k_) * 2 else p_ * 2)
    o_ = [
      ((i - (1 if auto_pad in ("SAME_UPPER", "SAME_LOWER") else k)) // s + 1)
      for i, k, s in zip(i_, k_, s_)
    ]
    return _onnx_pads_to_tiny_pads(
      _auto_pad(
        [(o - 1) * s + k - i for o, i, k, s in zip(o_, i_, k_, s_)], auto_pad
      )
    )

  def _clamp_cast(x: Tensor, dtype: DType):
    return x.clamp(dtypes.min(dtype), dtypes.max(dtype)).cast(dtype)

  def _prepare_quantize(
    ...
    def reshape(val: Tensor):
      ...
      if block_size == 0:
        return val.reshape([
          val.shape[0] if dim == axis else 1 for dim in range(x.ndim)
        ])
      return val.repeat_interleave(block_size, axis)

    return (
      reshape(scale),
      reshape(zero_point) if isinstance(zero_point, Tensor) else zero_point,
    )
  ...
  def _qlinearop_float(
    ...
    **opts,
    ...
    dequantized_inputs = [
      (inp.int() - zp) * scale for inp, zp, scale in zip(inputs, zero_points, scales)
    ]
  ...
  def _onnx_training(input_group_size):
    def __decorator(func):
      def ___wrapper(R: Tensor, T: int, *inputs: Tensor, **kwargs):
        ...
        ret = [
          func(R, T, *inps, **kwargs)
          for inps in (inputs[i::groups] for i in range(groups))
        ]
  ...
  def If(
    condition: Tensor,
    else_branch: OnnxRunner,
    then_branch: OnnxRunner,
    ...
    assert len(else_out) == len(then_out), (
      f"else_out and then_out must have the same number of outputs: {len(else_out)} != {len(then_out)}"
    )
    # can use where op when output shape is the same
    if all(t.shape == e.shape for t, e in zip(then_out.values(), else_out.values())):
      return tuple(
        condition.where(t, e) for t, e in zip(then_out.values(), else_out.values())
      )
    # otherwise, use condition to select the output in python
    cond = _resolve_const(_cached_to_python_const(condition))
    return tuple(
      t if cond else e for t, e in zip(then_out.values(), else_out.values())
    )
  ...
  def Constant(
    ...
    if (
      ...
      or value_strings is not None
      or sparse_value is not None
    ):
      raise NotImplementedError(
        "Constant OP not implemented for value_string, value_strings and sparse_value"
      )

  def Range(
    start: float | int | list[float | int],
    limit: float | int | list[float | int],
    delta: float | int | list[float | int],
  ):
    return Tensor.arange(
      start=_resolve_const(start),
      stop=_resolve_const(limit),
      step=_resolve_const(delta),
    )
  ...
  def ImageDecoder(encoded_stream: bytes, pixel_format="RGB"):
    try:
      ...
      raise ImportError(
        "Pillow must be installed for the ImageDecoder operator"
      ) from e
    img = PIL.Image.open(io.BytesIO(encoded_stream))
    if pixel_format == "BGR":
      return Tensor(img.tobytes(), dtype=dtypes.uint8).reshape(*img.size, 3).flip(-1)
    if pixel_format == "RGB":
      return Tensor(img.tobytes(), dtype=dtypes.uint8).reshape(*img.size, 3)
    if pixel_format == "Grayscale":
      return Tensor(img.convert("L").tobytes(), dtype=dtypes.uint8).reshape(
        *img.size, 1
      )
  ...
  def EyeLike(x: Tensor, dtype: int | None = None, k: int = 0):
    ret = Tensor.eye(
      cast(int, min(x.shape)),
      dtype=dtype_fallback(OnnxDataType(dtype).to_dtype(), "EyeLike op")
    ...
    return (
      ...
      if x.size(0) == x.size(1)
        ...
        tuple(
          None if d == ret.size(0) else (k, d - ret.shape[0] - k) for d in x.shape
        )
      )
    )

  def OptionalHasElement(x: Tensor | None = None):
    return Tensor(x is not None and x.numel() > 0)

  def OptionalGetElement(x: Tensor | None = None):
    return x if x is not None else Tensor([])

  def ConstantOfShape(shape: list[int], value: Tensor | None = None):
    if value is None:
      value = Tensor(0, dtype=dtypes.float32)
    if shape == [0]:
  ...
  def Shape(data: Tensor, end: int | None = None, start: int = 0):
    return Tensor(data.shape[start:end], dtype=dtypes.int64)
  ...
  def Clip(x: Tensor, min: Tensor | None = None, max: Tensor | None = None):
    return x if min is None and max is None else x.clip(min, max)  # noqa: A002 # pylint: disable=redefined-builtin

  def IsInf(x: Tensor, detect_negative: int = 1, detect_positive: int = 1):
    return x.isinf(bool(detect_positive), bool(detect_negative))

  # ***** Unary Ops (activation) *****
  def softmax_1(x: Tensor, axis: int = 1):
    return x.softmax(axis)

  def softmax_13(x: Tensor, axis: int = -1):
    return x.softmax(axis)

  Softmax = {
    OpSetId(Domain.ONNX, 1): softmax_1,
    OpSetId(Domain.ONNX, 13): softmax_13,
  }

  def HardSigmoid(x: Tensor, alpha: float = 0.2, beta: float = 0.5):
    return (alpha * x + beta).clip(0, 1)

  def Gelu(x: Tensor, approximate: str | None = None):
    return (
      x.gelu() if approximate == "tanh" else 0.5 * x * (1 + (x / math.sqrt(2)).erf())
    )

  def BiasGelu(x: Tensor, bias: Tensor, approximate: str | None = None) -> Tensor:
    return Gelu(x + bias, approximate)

  def FastGelu(x: Tensor, bias: Tensor | None = None):
    return (
      (x + bias).gelu() if bias is not None else x.gelu()
    )  # this is tanh approximated

  def PRelu(X: Tensor, slope: Tensor):
    return (X > 0).where(X, X * slope)

  def LeakyRelu(X: Tensor, alpha: float = 0.01):
    return X.leaky_relu(alpha)

  def ThresholdedRelu(X: Tensor, alpha: float = 1.0):
    return (X > alpha).where(X, 0)

  def LogSoftmax(x: Tensor, axis: int = -1):
    return x.log_softmax(axis)

  def Binarizer(x: Tensor, threshold: float = 0.0):
  ...
  def Sub(x: Tensor | int, y: Tensor):
    return x - y  # some test has input as int

  def Div(x: Tensor, y: Tensor):
    return x.div(y, rounding_mode="trunc" if dtypes.is_int(x.dtype) else None)
  ...
  def And(x: Tensor, y: Tensor):
    return (x == y).where(x, False)

  def Or(x: Tensor, y: Tensor):
    return (x == y).where(x, True)

  def Xor(x: Tensor, y: Tensor):
    return x.bool().bitwise_xor(y.bool())

  def BitwiseAnd(x: Tensor, y: Tensor):
    return x & y

  def BitwiseOr(x: Tensor, y: Tensor):
    return x | y

  def BitwiseXor(x: Tensor, y: Tensor):
    return x ^ y

  def BitwiseNot(x: Tensor):
    return ~x

  def Mod(x: Tensor, y: Tensor, fmod=0):
    return x - x.div(y, rounding_mode="trunc") * y if fmod else x % y

  # ***** Casting Ops *****
  # TODO: saturate
  def Cast(x: Tensor, to: int, saturate: int = 1):
    return x.cast(dtype_fallback(OnnxDataType(to).to_dtype(), "Cast op"))

  def CastLike(x: Tensor, target_type: Tensor, saturate: int = 1):
    return x.cast(target_type.dtype)

  # ***** Reduce Ops *****
  def Max(*data_0: Tensor):
    return functools.reduce(Tensor.maximum, data_0)

  def Min(*data_0: Tensor):
    return functools.reduce(Tensor.minimum, data_0)

  def Sum(*data_0: Tensor):
    return functools.reduce(Tensor.add, data_0)

  def Mean(*data_0: Tensor):
    return Sum(*data_0) / len(data_0)
  ...
  def ReduceL2(
    ...
    dtype = (
      dtypes.float if data.dtype in (dtypes.float16, dtypes.bfloat16) else data.dtype
    )
    return (
      ReduceSum(data.cast(dtype).square(), axes, keepdims, noop_with_empty_axes)
      .sqrt()
      .cast(data.dtype)
    )
  ...
  def ArgMax(
    x: Tensor, axis: int = 0, keepdims: int = 1, select_last_index: int = 0
  ):
    if select_last_index:
      return (
        (x.shape[axis] - 1) - x.flip(axis).argmax(axis, keepdim=keepdims)
      ).cast(dtypes.int64)
    return x.argmax(axis, keepdim=keepdims).cast(dtypes.int64)

  def ArgMin(x, axis: int = 0, keepdims: int = 1, select_last_index: int = 0):
    return ArgMax(
      -x, axis=axis, keepdims=keepdims, select_last_index=select_last_index
    )

  # ***** Movement Ops *****
  def Reshape(data: Tensor, shape: list[int], allowzero: int = 0):
    return data.reshape([
      x if x != 0 else (0 if allowzero else data.shape[i])
      for i, x in enumerate(shape)
    ])

  def Flatten(x: Tensor, axis: int = 1):
    return x.reshape(prod(x.shape[0:axis]), -1)

  def Expand(x: Tensor, shape: list[int]):
    return x.expand(_broadcast_shape(x.shape, tuple(shape)))

  def Shrink(x: Tensor, bias: float = 0.0, lambd: float = 0.5):
    return (x < -lambd) * (x + bias) + (x > lambd) * (x - bias)

  def Transpose(x: Tensor, perm: list[int] | None = None):
    return x.permute(order=perm or list(range(x.ndim)[::-1]))
  ...
  def Squeeze(data: Tensor, axes: list[int] | None = None):
    return (
      ...
      if axes is None
        ...
        lambda d, dim: d.squeeze(dim), sorted(axes, reverse=True), data
      )
    )

  def Unsqueeze(data: Tensor, axes: list[int]):
    return functools.reduce(lambda d, dim: d.unsqueeze(dim), sorted(axes), data)

  def Tile(x: Tensor, repeats: list[int]):
    return x.repeat(repeats)

  def Concat(*xs: Tensor, axis: int):
  ...
  def Split(
    data: Tensor, split: list[int] | None = None, num_outputs: int = 0, axis: int = 0
  ):
    sz = data.shape[axis]
    if split is None:
      split = [
        sz // num_outputs + (1 if i < sz % num_outputs else 0)
  ...
  def Pad(
    x: Tensor,
    pads: list[int],
    constant_value: ConstType | None = None,
    axes: list[int] | None = None,
    mode: Literal["constant", "reflect", "edge", "wrap"] = "constant",
    value=0,
  ):
    ...
    for i, axis in enumerate(axes):
      real_pads[axis % x.ndim], real_pads[axis % x.ndim + x.ndim] = (
        pads[i],
        pads[i + len(axes)],
      )
    return x.pad(
      padding=_onnx_pads_to_tiny_pads(real_pads),
      mode={"edge": "replicate", "wrap": "circular"}.get(mode, mode),
      value=value,
    )
  ...
  def CenterCropPad(t: Tensor, shape: list[int], axes: list[int] | None = None):
    ...
    for s, x in zip(shape, axes or range(t.ndim)):
      tx = t.shape[x]
      if s < tx:
        shrink_arg[x] = (tx // 2 - (s + 1) // 2, tx // 2 + s // 2)
      elif s > tx:
        pad_arg[x] = ((s - tx) // 2, (s - tx + 1) // 2)
  ...
  def AveragePool(
    ...
    auto_pad: AUTO_PAD_OPTIONS = "NOTSET",
    ceil_mode: int = 0,
    ...
    pool_pads = _resolve_pool_pads(
      X, pads, kernel_shape, dilations, strides, auto_pad
    )
    return X.avg_pool2d(
      tuple(kernel_shape),
      strides,
  ...
  def MaxPool(
    ...
    return ret, idx.transpose(-2, -1).cast(
      dtypes.int64
    ) if storage_order else idx.cast(dtypes.int64)
  ...
  def Conv(
    ...
    return X.conv2d(
      ...
      groups=group,
      dilation=dilations,
      padding=_resolve_pool_pads(
        X, pads, kernel_shape or W.shape[2:], dilations, strides, auto_pad
      ),
    )
  ...
  def ConvTranspose(
    ...
    output_padding: list[int] | int = 0,
    strides: list[int] | int = 1,
  ):
    input_shape_, kernel_shape_ = X.shape[2:], (kernel_shape or W.shape[2:])
    strides_, dilations_, output_padding_ = (
      make_tuple(x, len(input_shape_)) for x in (strides, dilations, output_padding)
    )
    if output_shape is not None:  # we pad according to output_shape
      pads = _auto_pad(
        [
          s_ * (i - 1) + op_ + ((k_ - 1) * d_ + 1) - os
          for s_, i, op_, k_, d_, os in zip(
            strides_,
            input_shape_,
            output_padding_,
            kernel_shape_,
    ...
    if pads is None:  # we generate pads
      output_shape = output_shape or [
        X.shape[i + 2] * strides_[i] for i in range(len(strides_))
      ]
      pads = [
        strides_[i] * (input_shape_[i] - 1)
        + output_padding_[i]
        + ((kernel_shape_[i] - 1) * dilations_[i] + 1)
        - output_shape[i]
        for i in range(len(input_shape_))
      ]
      pads = (
        _auto_pad(pads, auto_pad)
        if auto_pad != "NOTSET"
        else [0] * len(input_shape_) * 2
      )
    pads = _onnx_pads_to_tiny_pads(pads)
    return X.conv_transpose2d(
      W, B, group, strides_, dilations_, pads, output_padding_
    )
  ...
  def MaxUnpool(
    ...
    return Tensor.max_unpool2d(
      ...
      1,
      pads_,
      outshape if outshape is None else tuple(outshape),
    )

  def GlobalAveragePool(X: Tensor):
    return X.mean(axis=tuple(range(2, X.ndim)), keepdim=True)

  def GlobalMaxPool(X: Tensor):
    return X.max(axis=tuple(range(2, X.ndim)), keepdim=True)
    ...
    if C is not None:
      ret = ret + beta * (
        ...
        if broadcast == 0
          ...
          [-1 if i < len(C.shape) else 1 for i in range(ret.ndim)][::-1]
        )
      )
    return ret

  def Einsum(*Inputs: list[Tensor], equation: str):
    return Tensor.einsum(equation, *Inputs)
  ...
  def CumSum(X: Tensor, axis: int | list[int], exclusive: int = 0, reverse: int = 0):
    ...
    if reverse:
      X = X.flip(axis)
    if exclusive:
      X = X.pad(tuple((1, 0) if i == axis else None for i in range(X.ndim))).shrink(
        tuple((0, X.shape[axis]) if i == axis else None for i in range(X.ndim))
      )
  ...
  def Resize(
    ...
    def _apply_transformation(input_sz, output_sz, scale_dim, mode):
      ...
      if mode == "half_pixel":
        return (index + 0.5) / scale_dim - 0.5
      if mode == "align_corners":
        return (
          index * (input_sz - 1) / (output_sz - 1)
          if output_sz != 1
          else Tensor.zeros_like(index)
        )
      if mode == "asymmetric":
        return index / scale_dim
      if mode == "pytorch_half_pixel":
        return (
          ((index + 0.5) / scale_dim - 0.5)
          if output_sz != 1
          else Tensor.zeros_like(index)
        )
      if mode == "half_pixel_symmetric":
        ...
        return (
          (input_sz / 2) * (1 - (output_sz / output_dim_scaled))
          + (index + 0.5) / scale_dim
          - 0.5
        )
    ...
    if antialias:
      raise NotImplementedError("antialias is not implemented")
    axes = axes or list(range(X.ndim))
    ...
    if scales is not None:
      assert all(sc == 1 for sc in scales[: -len(input_shape)]), (
        "resizing batch_size dim or channel dim not supported"
      )
    if sizes is not None:
      assert tuple(sizes[:-2]) == tuple(X.shape[X.ndim - len(sizes) : -2]), (
        "resizing batch_size dim or channel dim not supported"
      )

    scales, sizes = (
      (None if scales is None else scales[-len(input_shape) :]),
      (None if sizes is None else sizes[-len(input_shape) :]),
    )
    if sizes is not None:
      if keep_aspect_ratio_policy in ["not_larger", "not_smaller"]:
        ...
        sizes, scales = (
          [int(scale * sh + 0.5) for sh in input_shape],
          [scale] * len(input_shape),
        )
      else:
        scales = [sz / sh for sz, sh in zip(sizes, input_shape)]
    else:
      assert scales is not None, "either sizes or scales must be provided"
      sizes = [int(sc * sh) for sc, sh in zip(scales, input_shape)]

    if all(sz == sh for sz, sh in zip(sizes, input_shape)):
    ...
    for input_sz, output_sz, scale in zip(input_shape, sizes, scales):
      indexes.append(
        _apply_transformation(
          input_sz, output_sz, scale, coordinate_transformation_mode
        )
      )
    ...
    if mode == "nearest":
      ...
      if nearest_mode not in mode_operations:
        raise ValueError(f"invalid {nearest_mode=}")
    ...
    if mode == "cubic":
      ...
      def W0_1(x: Tensor):
        return polyN(x, [A + 2, -(A + 3), 0, 1])

      def W1_2(x: Tensor):
        return polyN(x, [A, -5 * A, 8 * A, -4 * A])
      ...
      for i in range(-len(sizes), 0):
        ...
        # Reshape and expand
        expanded_indices = [
          y.clip(0, input_sz - 1).reshape(reshape).expand(expand)
          for y in [idx0, idx1, idx2, idx3]
        ]
        expanded_coeffs = [
          y.reshape(reshape).expand(expand) for y in [c0, c1, c2, c3]
        ]
  ...
  def Upsample(X, scales, mode):
    return Resize(X=X, scales=scales, mode=mode)  # deprecated

  def TopK(
    X: Tensor, K: int | list[int], axis: int = -1, largest: int = 1, sorted: int = 1
  ...
  def BatchNormalization(
    ...
    if training_mode:
      ...
      return (
        X.batchnorm(scale, B, current_mean, current_invstd).cast(X.dtype),
        running_mean.cast(input_mean.dtype),
        running_var.cast(input_var.dtype),
      )
  ...
  def GroupNormalization(
    ...
    x = (
      ...
      .cast(dtypes.float)
      .layernorm(eps=epsilon)
      .cast(x.dtype)
      .reshape(x.shape)
    )
    return x * scale.reshape(1, -1, *[1] * (x.ndim - 2)) + bias.reshape(
      1, -1, *[1] * (x.ndim - 2)
    )

  def InstanceNormalization(
    x: Tensor, scale: Tensor, bias: Tensor, epsilon: float = 1e-05
  ):
    return GroupNormalization(
      x, scale, bias, num_groups=cast(int, x.shape[1]), epsilon=epsilon
    )
  ...
  def LayerNormalization(
    ...
    inv_std_dev = (
      (x32.sub(mean)).square().mean(axis=axes, keepdim=True).add(epsilon).rsqrt()
    )
    return (
      (x32.sub(mean) * inv_std_dev).cast(x.dtype).mul(scale).add(bias),
      mean,
      inv_std_dev,
    )
  ...
  def EmbedLayerNormalization(
    ...
    assert mask is None and not mask_index_type, (
    ...
    def embedding(x: Tensor, vocab_size, weight: Tensor) -> Tensor:
      return (
        x.unsqueeze(-1).expand(*x.shape, vocab_size)._one_hot_along_dim(vocab_size)
        @ weight
      )
    ...
    if seg_embedding_res is not None:
      embedding_sum = embedding_sum + seg_embedding_res
  ...
  def MeanVarianceNormalization(x: Tensor, axis: list[int] | None = None):
    if axis is None:
      axis = [0, 2, 3]
    return (x - x.mean(axis, keepdim=True)) / (
      x.std(axis, keepdim=True, correction=0) + 1e-9
    )

  def OneHot(
    indices: Tensor,
    depth: float | int | list[int | float],
    values: Tensor,
    axis: int = -1,
  ):
    ...
    return (
      indices.unsqueeze(axis)
      ._one_hot_along_dim(depth, dim=axis)
      .where(values[1], values[0])
    )
  ...
  def DepthToSpace(X: Tensor, blocksize: int, mode: str = "DCR"):
    return X.rearrange(
      ...
      if mode == "CRD"
      else "b (h1 w1 c) h w -> b c (h h1) (w w1)",
      h1=blocksize,
      w1=blocksize,
    )
  ...
  def SpaceToDepth(X: Tensor, blocksize: int):
    return X.rearrange(
      "b c (h h1) (w w1) -> b (h1 w1 c) h w", h1=blocksize, w1=blocksize
    )
  ...
  def dropout_7(
    ...
    if not training_mode:
      return data, data.full_like(True, dtype=dtypes.bool)
    if seed is not None:
      rand = Tensor(
        np.random.RandomState(seed).random(cast(tuple[int, ...], data.shape)),
        requires_grad=False,
        dtype=data.dtype,
        device=data.device,
      )
  ...
  def dropout_6(data: Tensor, ratio: float = 0.5, is_test=0):
    return dropout_7(data, ratio, training_mode=not is_test)

  Dropout = {OpSetId(Domain.ONNX, 6): dropout_6, OpSetId(Domain.ONNX, 7): dropout_7}

  def LRN(
    x: Tensor, size: int, alpha: float = 1e-4, beta: float = 0.75, bias: float = 1.0
  ):
    pooled_x = (
      (x**2)
      .rearrange("b c h w -> b 1 c (h w)")
      .pad((0, 0, (size - 1) // 2, size // 2))
      .avg_pool2d((size, 1), 1)
    )
  ...
  def AffineGrid(theta: Tensor, size: list[int], align_corners: int = 0):
    ...
    def generate_grid(steps):
      if align_corners:
        return Tensor.linspace(-1, 1, steps, device=theta.device)
      return Tensor.linspace(
        -1 + 1 / steps, 1 - 1 / steps, steps, device=theta.device
      )

    grids = Tensor.meshgrid(*(generate_grid(d) for d in spatial_dims))
    base_grid = Tensor.stack(
      *reversed(grids), Tensor.ones_like(grids[0], device=theta.device), dim=-1
    )
    base_grid = base_grid.reshape(1, prod(spatial_dims), len(grids) + 1).expand(
      N, -1, -1
    )
    return (base_grid @ theta.transpose(1, 2)).reshape(N, *spatial_dims, -1)

  def attention_contrib(
    x: Tensor,
    weights: Tensor,
    bias: Tensor | None = None,
    mask_index: Tensor | None = None,
    past: Tensor | None = None,
    attention_bias: Tensor | None = None,
    past_sequence_length: Tensor | None = None,
    do_rotary: int = 0,
    mask_filter_value: float = -10000.0,
    ...
    if qkv_hidden_sizes is None:
      qkv_hidden_sizes = [weights.shape[1] // 3] * 3
    ...
    q_head_size, k_head_size, v_head_size = (
      sz // num_heads for sz in qkv_hidden_sizes
    )
    q, k, v = (
      x.reshape(batch_size, seq_len, num_heads, hsz).transpose(1, 2)
      for x, hsz in zip((q, k, v), (q_head_size, k_head_size, v_head_size))
    )
    ...
    if scale is None:
      scale = 1.0 / math.sqrt(q_head_size)
    ...
    if mask_index is not None:
      ...
      if mask_index.ndim != 1:
        mask = mask_index.bool()
      else:
        if mask_index.shape[0] == batch_size:
          mask = Tensor.arange(
            attn_scores.shape[-1], requires_grad=False, device=mask_index.device
          ).unsqueeze(0) < mask_index.unsqueeze(1)
          ...
          mask = (arange < end_positions.unsqueeze(1)) & (
            arange >= start_positions.unsqueeze(1)
          )
        else:
          raise NotImplementedError(
            "mask_index with shape (3 * batch_size + 2) is not implemented"
          )
      while mask.ndim < 4:
  ...
  def attention_onnx(
    ...
    past_key: Tensor | None = None,
    past_value: Tensor | None = None,
    is_causal: int = 0,
    ...
    effective_scale = (
      scale if scale is not None else 1.0 / (cast(int, Q.shape[-1]) ** 0.5)
    )
    scores = (Q @ K.transpose(-1, -2)) * effective_scale
    qk_matmul_return_val = scores

    if is_causal:
      causal_mask = Tensor.ones(
        Q.shape[-2],
        K.shape[-2],
        device=Q.device,
        dtype=dtypes.bool,
    ...
    if softmax_precision:
      scores = scores.cast(
        {1: dtypes.float32, 10: dtypes.float16, 16: dtypes.bfloat16}[
          softmax_precision
        ]
      )
    qk_softmax = scores.softmax(-1).cast(Q.dtype)
    if qk_matmul_output_mode == 3:
      qk_matmul_return_val = qk_softmax

    output = (qk_softmax @ V).cast(Q.dtype)
    if input_shape_len == 3:
      output = output.permute(0, 2, 1, 3).reshape(Q.shape[0], Q.shape[2], -1)
    return output, present_key, present_value, qk_matmul_return_val

  Attention = {
    OpSetId(Domain.ONNX, 1): attention_onnx,
    OpSetId(Domain.MICROSOFT_CONTRIB_OPS, 1): attention_contrib,
  }
  ...
  def RMSNormalization(
    ...
    norm = (
      ...
      .square()
      .mean(
        axis=tuple(range(axis + X.ndim if axis < 0 else axis, X.ndim)), keepdim=True
      )
      .add(epsilon)
      .rsqrt()
    )
  ...
  def RotaryEmbedding(
    ...
    cos = (
      cos_cache[position_ids] if position_ids is not None else cos_cache[:head_size]
    )
    sin = (
      sin_cache[position_ids] if position_ids is not None else sin_cache[:head_size]
    )
    cos = cos[..., : rot_dim // 2].unsqueeze(2)
    sin = sin[..., : rot_dim // 2].unsqueeze(2)

    x1, x2 = (
      (x_rotate[..., ::2], x_rotate[..., 1::2])
      if interleaved
      else x_rotate.chunk(2, dim=-1)
    )
    real = x1 * cos - x2 * sin
    imag = x1 * sin + x2 * cos
    x_rotated = (
      real.stack(imag, dim=-1).flatten(start_dim=-2)
      if interleaved
      else real.cat(imag, dim=-1)
    )

    output = x_rotated.cat(x_pass, dim=-1)
    return (
      output.flatten(start_dim=2)
      if len(original_input_shape) == 3
      else output.permute(0, 2, 1, 3)
    )
  ...
  def Gather(x: Tensor, indices: Tensor, axis: int = 0):
    if (
      ...
      index_consts = (
        [_cached_to_python_const(indices)]
        if indices.shape == ()
        else _cached_to_python_const(indices)
      )
      index_consts = [x.shape[axis] + i if i < 0 else i for i in index_consts]
      args = [
        [(0, x) if j != axis else (i, i + 1) for j, x in enumerate(x.shape)]
        for i in index_consts
      ]
      return (
        x.shrink(arg=tuple(args[0]))
        .cat(*[x.shrink(arg=tuple(arg)) for arg in args[1:]], dim=axis)
        .reshape(ret_shape)
      )
  ...
  def Scatter(*args, **kwargs):
    return ScatterElements(*args, **kwargs)  # deprecated

  def GatherND(x: Tensor, indices: Tensor, batch_dims: int = 0):
    if batch_dims == 0:
      return x[tuple(i.squeeze(-1) for i in indices.split(1, -1))]
    ...
    b_idx = (
      Tensor.arange(b, device=x.device)
      .reshape(b, *(1,) * (indices.ndim - 2))
      .expand(*indices.shape[:-1])
    )
    ret = x[(b_idx,) + tuple(i.squeeze(-1) for i in indices.split(1, -1))]
    return ret.reshape(
      *x_shape[:batch_dims], *i_shape[batch_dims:-1], *ret.shape[indices.ndim - 1 :]
    )
  ...
  def ScatterND(
    ...
    indices: Tensor,
    updates: Tensor,
    reduction: Literal["none", "add", "mul"] = "none",
  ):
    assert (
      updates.shape == indices.shape[:-1] + x.shape[cast(int, indices.shape[-1]) :]
    )
    x = x.contiguous()
    for index, u in zip(indices.split(1, 0), updates.split(1, 0)):
      ...
      if reduction == "none":
        ...
        raise NotImplementedError("reduction doesn't support max or min")
    return x
  ...
  def ScatterElements(
    ...
    updates: Tensor,
    axis=0,
    reduction: Literal["none", "add", "mul", "min", "max"] = "none",
  ):
    indices = (indices < 0).where(x.shape[axis], 0) + indices
    if reduction == "none":
      return x.scatter(axis, indices, updates)
    reduction_ = cast(
      Literal["sum", "prod", "amin", "amax"],
      {"add": "sum", "mul": "prod", "min": "amin", "max": "amax"}[reduction],
    )
  ...
  def Compress(inp: Tensor, condition: list[bool], axis: int | None = None):
    ...
    con = Tensor([
      i for i, cond in enumerate(condition) if cond
    ])  # compress in python
  ...
  def QuantizeLinear(
    x: Tensor,
    y_scale: Tensor,
    y_zero_point: Tensor | int = 0,
    axis: int = 1,
    block_size: int = 0,
    output_dtype: int = 0,
    saturate=1,
  ):
    if isinstance(y_zero_point, Tensor):
      out_dtype = y_zero_point.dtype
    elif output_dtype != 0:
      out_dtype = dtype_fallback(
        OnnxDataType(output_dtype).to_dtype(), "QuantizeLinear op"
      )
    else:
      out_dtype = dtypes.uint8
    y_scale, y_zero_point = _prepare_quantize(
      x, y_scale, y_zero_point, axis, block_size
    )
  ...
  def DequantizeLinear(
    x: Tensor,
    x_scale: Tensor,
    x_zero_point: Tensor | int = 0,
    axis: int = 1,
    block_size: int = 0,
  ):
    x_scale, x_zero_point = _prepare_quantize(
      x, x_scale, x_zero_point, axis, block_size
    )
  ...
  def QLinearGlobalAveragePool(
    ...
    return _qlinearop_float(
      GlobalAveragePool, [X], [x_zero_point], [x_scale], y_scale, y_zero_point
    )

  def ConvInteger(
    x: Tensor,
    w: Tensor,
    x_zero_point: Tensor = Tensor(0),
    w_zero_point: Tensor = Tensor(0),
    B: Tensor | None = None,
    **opts,
  ) -> Tensor:
    return _op_integer(
      Conv, [x, w], [x_zero_point, w_zero_point], **{"B": B, **opts}
    )

  def MatMulInteger(
    A: Tensor,
    B: Tensor,
    a_zero_point: Tensor = Tensor(0),
    b_zero_point: Tensor = Tensor(0),
  ) -> Tensor:
  ...
  def Adam(
    ...
    *inputs: Tensor,
    ...
    opt.m, opt.v, opt.lr = (
      [V.reshape(opt.m[0].shape)],
      [H.reshape(opt.v[0].shape)],
      R,
    )
    # need no-op for m_hat and v_hat if T == 0
    if T == 0:
      opt.b1_t, opt.b2_t = opt.b1_t.zeros_like(), opt.b2_t.zeros_like()
    else:
      ...
      opt.b2_t = Tensor(
        [beta ** (T - 1)], dtype=dtypes.float32, device=X.device, requires_grad=False
      )
    opt.step()
  ...
  def Gradient(
    *inputs: Tensor, y: str, intermediate_tensors: dict[str, Tensor], **_
  ):
