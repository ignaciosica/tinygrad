...
class Optimizer:
  ...
  def __init__(self, params: list[Tensor], lr: float, fused=FUSE_OPTIM):
    ...
    if self.fused:
      self.pos_params = list(itertools.accumulate(self.params, lambda x, y: x + y.numel(), initial=0))

  def _new_optim_param(self) -> list[Tensor]:
    param_dtype = getenv("OPTIM_DTYPE", "float32")
    if self.fused:
      return [Tensor.zeros(self.pos_params[-1], dtype=param_dtype, device=self.device, requires_grad=False).contiguous()]
  ...
  def schedule_step(self) -> list[Tensor]:
    ...
    if self.fused:
      ...
      out, extra = self._step(
        [Tensor.cat(*[t.flatten() for t in self.params], dim=0)], [Tensor.cat(*[unwrap(t.grad).contiguous().flatten() for t in self.params], dim=0)]
      )
