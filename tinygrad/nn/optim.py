...
class Optimizer:
  ...
  def __init__(self, params: list[Tensor], lr: float, fused=FUSE_OPTIM):
    ...
    if self.fused:
      self.pos_params = list(
        itertools.accumulate(self.params, lambda x, y: x + y.numel(), initial=0)
      )
  ...
  def _new_optim_param(self) -> list[Tensor]:
    ...
    return [
      Tensor.zeros(
        *t.shape, dtype=param_dtype, device=t.device, requires_grad=False
      ).contiguous()
  ...
  def schedule_step(self) -> list[Tensor]:
    ...
    if self.fused:
      ...
      out, extra = self._step(
        [Tensor.cat(*[t.flatten() for t in self.params], dim=0)],
        [
          Tensor.cat(
            *[unwrap(t.grad).contiguous().flatten() for t in self.params], dim=0
          )
        ],
      )
      updated_params = [
        out[0][self.pos_params[i] : self.pos_params[i + 1]].reshape(tt.shape)
        for i, tt in enumerate(self.params)
      ]
    else:
      updated_params, extra = self._step(
        self.params, [unwrap(t.grad) for t in self.params]
      )
    for i, tt in enumerate(self.params):
      tt.assign(updated_params[i])
...
class OptimizerGroup(Optimizer):
  ...
  def __init__(self, *optimizers: Optimizer):  # pylint: disable=super-init-not-called
    ...
    self.params, self.buffers = (
      flatten([o.params for o in self.optimizers]),
      flatten([o.buffers for o in self.optimizers]),
    )
  ...
  def schedule_step(self) -> list[Tensor]:
    return [x for o in self.optimizers for x in o.schedule_step()]
...
class LARS(Optimizer):
  ...
  def __init__(
    ...
    self.momentum, self.wd, self.ns_steps, self.ns_coefficients = (
      momentum,
      weight_decay,
      ns_steps,
      ns_coefficients,
    )
    self.nesterov, self.classic, self.pre_wd, self.tcoef = (
      nesterov,
      classic,
      pre_wd,
      tcoef,
  ...
  def _step(
    ...
    for i, (t, g) in enumerate(zip(params, grads)):
      if self.tcoef != 0:
        ...
        r: Tensor | float = (r1 > 0).where(
          (r2 > 0).where(self.tcoef * r1 / (r2 + self.wd * r1), 1.0), 1.0
        )
      else:
        r = 1.0
      ...
      if self.ns_coefficients:
        g = (
          ...
          .newton_schulz(self.ns_steps, self.ns_coefficients)
          .reshape(g.shape)
        )
      # muon does post momentum weight decay
      if not self.pre_wd and self.wd > 0:
        t = t.detach() * (1.0 - self.wd * self.lr)
...
def Adam(
  params: list[Tensor], lr=0.001, b1=0.9, b2=0.999, eps=1e-8, fused=FUSE_OPTIM
):
  """
  Adam optimizer.
...
class LAMB(Optimizer):
  ...
  def _step(
    ...
    for i, (t, g) in enumerate(zip(params, grads)):
      self.m[i].assign(
        (self.b1 * self.m[i] + (1.0 - self.b1) * g).cast(self.m[i].dtype)
      )
      self.v[i].assign(
        (self.b2 * self.v[i] + (1.0 - self.b2) * (g * g)).cast(self.v[i].dtype)
      )
      ...
      if not self.adam:
        ...
        r: Tensor | float = Tensor.where(
          r1 > 0, Tensor.where(r2 > 0, r1 / r2, 1.0), 1.0
        )
      else:
        r = 1.0
