...
) -> Callable[[Tensor | str | pathlib.Path], T]:
...
def safe_save(
  tensors: dict[str, Tensor], fn: str, metadata: dict[str, Any] | None = None
):
  """
...
def torch_load(t: Tensor) -> dict[str, Tensor]:
  ...
  def _rebuild_tensor_v2(
    ...
    if tuple(permute_indexes) != tuple(range(len(permute_indexes))):
      ...
      if DEBUG >= 3:
        print(
          f"WARNING: this torch load is slow. to permute {intermediate_shape} with {permute_indexes}"
        )
  ...
  if passthrough_reset(
    ...
    for zi in myzip.filelist:
      if base_name is None:
        base_name = zi.filename.split("/", 1)[0]
      if zi.filename.startswith(f"{base_name}/data/"):
        header_offsets[zi.filename.split("/")[-1]] = zi.header_offset
    ...
    header_contents = [
      t[v + 26 : v + 30].bitcast(dtypes.uint16).to("CPU")
      for v in header_offsets.values()
    ]
    ...
    with tarfile.open(fileobj=fobj, mode="r") as tar:
      ...
      for _ in range(TorchPickle(f).load()):  # num_tensors
        ...
        deserialized_objects[str(key)] = _rebuild_tensor_v2(
          (None, storage_type, storage_id, None, -1), storage_offset, size, stride
        )
      return {
        k: v.tensor if isinstance(v, Parameter) else v
        for k, v in TorchPickle(unwrap(tar.extractfile("pickle"))).load().items()
      }
  else:
...
def ggml_data_to_tensor(t: Tensor, n: int, ggml_type: int) -> Tensor:
  ...
  def q_to_uint8(t: Tensor, b: int) -> Tensor:
    ...
    shift_tensor, bitmask = (
      Tensor.stack(*[
        Tensor(2 ** (i * b), device=t.device, dtype=t.dtype) for i in range(8 // b)
      ]),
      0xFF >> (8 - b),
    )
    return (
  ...
  if (
    ...
    if ggml_type == 14:
      xl, xh = (
        q_to_uint8(blocks[:, :128].reshape((-1, 2, 64)), 4),
      ...
      d = (
        blocks[:, -2:].bitcast(dtypes.float16).cast(dtypes.float32).expand((-1, 256))
      )
    ...
    if ggml_type == 39:
      ...
      exp, mant = (
        codes.rshift(1).bitwise_and(0x3).cast(dtypes.float32),
        codes.bitwise_and(0x1).cast(dtypes.float32),
      )
      fp4_val = sign * (
        (exp != 0).cast(dtypes.float32) * (1.0 + 0.5 * mant) * (exp - 1.0).exp2()
...
def gguf_load(tensor: Tensor) -> tuple[dict, dict[str, Tensor]]:
  ...
  """
  reader, kv_data, state_dict = (
    io.BufferedReader(TensorIO(tensor), 1_000_000),
    {},
    {},
  )
