...
def hand_coded_optimizations(k:Kernel) -> list[Opt]:
  ...
  for axis in range(k.get_offset("reduce")):
    # we might want to be able to split axes that are masked, or refuse to merge them in simplify_merge_adjacent
    # for now skip upcasting here if there is a symbolic axis
    print(k.full_shape, axis)
  ...
  while resolve(prod(k.sts[0].shape[:k.get_offset("reduce")]) >= 1024):
    xb_choices = []
    # consider all the non reduce axes, and a 3 or 4 reduce. (128 on the DSP)
    for axis, upcast_amount in itertools.product(range(k.get_offset("reduce")), ([128] if not len(upcasted_axis) else []) if is_dsp else [3,4]):
    ...
    if isinstance(s:=k.full_unupcasted_shape[-1], int) and s <= 32:  # NOTE: cannot loop unroll symbolic axis
      ...
      for splits in [4]:
        if k.full_unupcasted_shape[-1]%splits == 0:
          k.apply_opt(Opt(OptOps.UNROLL, len(k.full_unupcasted_shape)-1-k.get_offset("reduce"), splits))
          break
  ...
  if k.opts.has_local:
    if NOLOCALS and k.axes["local"] == 0 and not k.axes["group"]:
      ...
      local_axis_ranking = [(any(k.sts[buf_index].views[-1].strides[axis] == 0 for buf_index in range(len(k.sts))), axis) \
                            for axis in range(len(k.full_shape[:k.get_offset("reduce")]))]
