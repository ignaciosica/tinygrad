...
class Kernel:
  def __init__(self, ast:UOp, opts:Optional[Renderer]=None):
    ...
    self.update_info(global_dims=self.shape_len - len(reduces), reduce_dims=len(reduces))
    self.simplify_ones()
    ...
    self.update_info(global_dims=self.shape_len - len(final_reduces), reduce_dims=len(final_reduces))
  ...
  @property
  def first_reduce(self) -> int: return self.info.get_offset("reduce_dims")
  ...
  def simplify_ones(self) -> bool:
    ...
    local_offset = self.info.get_offset("local_dims")
    reduce_offset = self.info.get_offset("reduce_dims")
    upcast_offset = self.info.get_offset("upcasted")
    self.update_info(global_dims=self.info.global_dims - sum(all_ones[:local_offset]),
                     local_dims=self.info.local_dims - sum(all_ones[local_offset:reduce_offset]),
                     reduce_dims=self.info.reduce_dims - sum(all_ones[reduce_offset:upcast_offset]),
                     upcasted=self.info.upcasted - sum(all_ones[upcast_offset:]))
  ...
  def apply_opt(self, opt:Opt, append_opt:bool=True):
    ...
    if opt.op is OptOps.LOCAL:    # cyan
      ...
      self.update_info(reduce_dims=self.info.reduce_dims + 1)
