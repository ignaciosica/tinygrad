...
class Kernel:
  def __init__(self, ast:UOp, opts:Optional[Renderer]=None):
    ...
    reduce_dims, global_dims = len(reduces), self.shape_len - len(reduces)
    self.axes = OrderedDict((("global", global_dims), ("local", 0), ("reduce", reduce_dims), ("upcast", 0)))
    ...
    self.axes["reduce"], self.axes["global"] = len(final_reduces), self.shape_len - len(final_reduces)
  ...
  def colors(self) -> list[str]:
    ...
    assert len(colors) == self.shape_len, f"colors size mismatch {self.axes} {self.full_shape}"
    return colors
  ...
  def simplify_ones(self) -> bool:
    ...
    ones_local  = sum(all_ones[self.get_offset("local") : self.get_offset("reduce")])
  ...
  def apply_opt(self, opt:Opt, append_opt:bool=True):
    ...
    if self.reduceop is not None and (opt.op in {OptOps.GROUP, OptOps.GROUPTOP} or \
      ...
      local_sz = prod(self.full_shape[self.first_reduce-self.axes["local"]:self.first_reduce+self.group_for_reduces])
    ...
    if opt.op is OptOps.LOCAL:    # cyan
      ...
      # if self.full_shape[axis] == amt and axis == self.first_reduce: self.axes["local"] += 1 # first_reduce will ++, so offset loss in simplify_ones
    ...
    def fixup_ast(op:UOp) -> UOp:
      ...
      if op.op is Ops.SINK:
        ...
        return ret.replace(arg = KernelInfo(ret.arg.name if ret.arg is not None else self.name if name_override is None else name_override,
                                            self.global_dims if self.opts.has_local else 0, self.axes["local"]+self.group_for_reduces,
      ...
      if op.op is Ops.REDUCE_AXIS:
        ...
        if self.group_for_reduces and grouped_axes:
          local_shape = (1,) * self.global_dims + self.full_shape[self.global_dims:self.global_dims+self.axes["local"]] + \
            ...
            (1,) * (self.shape_len - self.axes["upcast"] - self.group_for_reduces - self.first_reduce) + tuple([x[0] for x in self.upcasted_axis(0)])
