...
class TensorCore: # D = A * B + C, A is (M x K), B is (K x N), C and D are (M x N)
  ...
  swizzle: tuple[Optional[tuple[int, ...]], Optional[tuple[int, ...]]] = (None, None)
  ...
  #   assert all(len(perm[0]) == local_axes + upcast_axes and len(perm[1]) == reduce_axes for perm in self.swizzle if perm), (
  #     f"swizzle perm should be of len (({local_axes + upcast_axes})({reduce_axes}))")
