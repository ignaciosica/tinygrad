...
class Estimates:
  ...
  def __add__(self, o: Estimates):
    return Estimates(self.ops + o.ops, self.lds + o.lds, self.mem + o.mem)

  def simplify(self):
    return Estimates(ssimplify(self.ops), ssimplify(self.lds), ssimplify(self.mem))
  ...
  def from_uops(uops: list[UOp], ignore_indexing=False) -> Estimates:
    ...
    if ignore_indexing:
      ...
      def range_gate(x):
        return x.op is not Ops.RANGE

      for u in uops:
        if u.op in {Ops.LOAD, Ops.STORE}:
          # if u.src[0] is INDEX, we have to include the buffer since it might be an AFTER
          dont_count = dont_count.union(
            (
              UOp.sink(*u.src[0].src[1:]) if u.src[0].op is Ops.INDEX else u.src[0]
            ).toposort(range_gate)
          )
          # TODO: is this correct? this all needs to be cleaned up
          if len(u.src) > 2:
            dont_count = dont_count.union(u.src[2].toposort())
        elif u.op is Ops.IF:
    ...
    for u in uops:
      if u.op in {Ops.LOAD, Ops.STORE}:
        ...
        while len(buf.src):
          buf = buf.src[0]
        if (
          buf.op is Ops.DEFINE_GLOBAL
        ):  # assume all DEFINE_GLOBAL memory is accessed
      ...
      if u.op is Ops.RANGE:
        ...
        mults = (
          mults.substitute({
            x: x.const_like(0) for x in mults.toposort() if x.op is Ops.SPECIAL
          })
          if isinstance(mults, UOp)
          else mults
        )
        ...
        mults *= cast(
          sint, u.src[0].ssimplify()
        )  # NOTE: we don't push to the mult_stack here, you can't end these
      elif u.op is Ops.LOAD and (
        not isinstance(u.src[0].dtype, PtrDType)
        or u.src[0].dtype.addrspace != AddrSpace.REG
      ):
        lds += u.dtype.itemsize * mults
      elif u.op is Ops.STORE and (
        not isinstance(u.src[0].dtype, PtrDType)
        or u.src[0].dtype.addrspace != AddrSpace.REG
      ):
...
class ProgramSpec:
  ...
  _ran_post_init: bool = (
    False  # NOTE: this is needed if you call replace on the Program
  )
    ...
    if not self._ran_post_init and self.uops is not None:
      ...
      for u in self.uops:
        ...
        if u.op is Ops.DEFINE_GLOBAL:
          self.globals.append(u.arg)
        if u.op in (Ops.STORE, Ops.LOAD):
          if (idx := u.src[0]).op is Ops.INDEX or (
            u.src[0].op is Ops.CAST and (idx := u.src[0].src[0]).op is Ops.INDEX
          ):
            if (buf := idx.src[0]).op is Ops.DEFINE_GLOBAL:
              (self.outs if u.op is Ops.STORE else self.ins).append(buf.arg)
        ...
        if u.op is Ops.SPECIAL:
          ...
          if special_size is not None:
            special_size[int(u.arg[-1])] = cast(int, u.src[0].ssimplify())
  ...
  def estimates(self) -> Estimates:
    return (
      ...
      else Estimates.from_uops(self.uops, ignore_indexing=True)
    )
...
class Renderer:
  ...
  global_max: tuple[int, ...] | None = (0x8FFFFFFF,) * (
    3
  )  # TODO: Ops.SPECIAL int32 indexes right now
  local_max: tuple[int, ...] | None = (0x8FFFFFFF,) * (
    3
  )  # TODO: Ops.SPECIAL int32 indexes right now
