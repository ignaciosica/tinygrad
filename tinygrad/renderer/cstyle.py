...
base_rewrite = PatternMatcher([
  ...
  (
    ...
    lambda ctx, x: f"__builtin_convertvector({ctx[x.src[0]]}, {ctx.render_dtype(x.dtype)})"
  ...
  (
    UPat(Ops.GEP, name="x"),
    lambda ctx, x: ctx[x.src[0]] + (f"[{x.arg[0]}]" if x.src[0].dtype.count > ctx.gep_arr_threshold else f".{'xyzwabcd'[x.arg[0]]}"),
  ),
...
def create_non_native_float_pats(dts: tuple[DType, ...], casting: bool = True):
  patterns = PatternMatcher([
    ...
    (UPat(GroupOp.ALU, dtype=dts, name="x"), lambda x: UOp(x.op, dtypes.float, tuple(vv.cast(dtypes.float) for vv in x.src), x.arg).cast(x.dtype)),
    (
...
class CStyleLanguage(Renderer):
  ...
  def render_dtype(self, dt: DType, mutable=True) -> str:
    ...
    if dt.count > 1:
      return self.type_map.get(scalar := dt.scalar(), scalar.name).replace(" ", "_") + str(dt.count)
  ...
  def _render(self, uops: list[UOp]) -> tuple[str, list[str], list[tuple[str, tuple[DType, bool]]]]:
    ...
    for u in uops:
      ...
      if (u.op is not Ops.CAST or u.dtype.vcount == 1) and (
        ...
        or (u.op in {Ops.VECTORIZE, *(GroupOp.ALU - {Ops.WHERE}), Ops.CAST, Ops.BITCAST} and child_count[u] == 1 and not getenv("EXPAND_SSA"))
      ):
        r[u] = l
      else:
...
class ClangRenderer(CStyleLanguage):
  ...
  code_for_op = {
    **({k: v for k, v in CStyleLanguage.code_for_op.items() if k not in [Ops.EXP2, Ops.SIN, Ops.LOG2, Ops.TRUNC, Ops.RECIPROCAL]}),
  ...
  string_rewrite = (
    PatternMatcher([
      ...
      (
        UPat(
          Ops.STORE,
          src=(UPat.var("buf").index(UPat.var("idx", dtypes.int.vec(2)), allow_any_len=True), UPat.var("var", dtypes.float.vec(4))),
          allow_any_len=True,
        ),
...
class CUDARenderer(CStyleLanguage):
  ...
  def render_kernel(self, function_name, kernel, bufs, uops, prefix=None):
    ...
    prefix += [
      ...
      if (dt.count in (4, 8) and dt.scalar() in {dtypes.half, dtypes.bfloat16}) or (dt.count in (2, 4, 8, 16) and dt.scalar() in dtypes.fp8s)
    ]
...
class AMDRenderer(CStyleLanguage):
  ...
  kernel_typedef = "\n".join(
    f'extern "C" __attribute__((device{f", {atr}" if atr else ""})) {dto} {meth}({dti});' for meth, dti, dto, atr in ockl + ocml
  )
  ...
  extra_matcher = (
    ...
    + PatternMatcher([
      (
        ...
        lambda x: UOp(Ops.WMMA, x.dtype, (x.src[0].bitcast(dtypes.uint64), x.src[1].bitcast(dtypes.uint64), x.src[2]), (*x.arg,))
