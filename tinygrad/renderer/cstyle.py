...
base_rewrite = PatternMatcher([
  ...
  (
    ...
    lambda ctx, x: f"__{x.arg[0]}({ctx[x.src[0]]}, {ctx[x.src[1]]}, {ctx[x.src[2]]})",
  ),
  # r method accesses
  (
  ...
  def _render(
    ...
    for u in uops:
      ...
      if (u.op is not Ops.CAST or u.dtype.vcount == 1) and (
        ...
        or (
          u.op in {Ops.VECTORIZE, *(GroupOp.ALU - {Ops.WHERE}), Ops.CAST, Ops.BITCAST}
          and child_count[u] == 1
    ...
    PatternMatcher([
      ...
      (
        UPat(
          ...
          src=(
            UPat.var("buf").index(UPat.var("idx", dtypes.int.vec(2)), UPat.var("gate")),
            UPat.var("var"),
          ),
        ),
        lambda ctx,
    ...
    + PatternMatcher([
      (
        ...
        lambda x: UOp(
          Ops.WMMA,
          x.dtype,
          (x.src[0].bitcast(dtypes.uint64), x.src[1].bitcast(dtypes.uint64), x.src[2]),
          (*x.arg,),
        )
