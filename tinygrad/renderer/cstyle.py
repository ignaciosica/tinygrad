...
base_rewrite = PatternMatcher([
  (
    ...
    lambda ctx, x: f"{ctx.render_dtype(x.dtype.base)} {ctx[x]}[{x.dtype.size}];",
  ),
  ...
  (
    ...
    lambda ctx,
    x: f"__{x.arg[0]}({ctx[x.src[0]]}, {ctx[x.src[1]]}, {ctx[x.src[2]]})",
  ),
  # r method accesses
  (
    UPat(Ops.RANGE, name="x"),
    lambda ctx,
    x: f"for ({ctx.render_dtype(x.dtype)} {ctx[x]} = 0; {ctx[x]} < {ctx[x.src[0]]}; {ctx[x]}++) {{",
  ),
  (
  ...
  (
    ...
    lambda ctx,
    x: f"__builtin_convertvector({ctx[x.src[0]]}, {ctx.render_dtype(x.dtype)})"
    if x.dtype.count > 1 and not isinstance(x.dtype, PtrDType)
    else None,
  ),
  (
    UPat(Ops.CAST, name="x"),
    lambda ctx, x: f"({ctx.render_cast(x.dtype, ctx[x.src[0]])})",
  ),
  (
    UPat(Ops.BITCAST, name="x"),
    lambda ctx,
    x: f"(*(({ctx.buffer_prefix}{ctx.render_dtype(x.dtype)}*)&{ctx[x.src[0]]}))",
  ),
  (
    UPat(Ops.DEFINE_LOCAL, name="x"),
    lambda ctx,
    x: f"{ctx.smem_align}{ctx.smem_prefix}{ctx.render_dtype(x.dtype.base)} {ctx[x]}[{x.dtype.size}];",
  ),
  ...
  (
    ...
    lambda ctx,
    x: f"{ctx.code_for_workitem[x.arg[0]](x.arg[-1])}; /* {(x.src[0]).render()} */",
  ),
  # const
  (
    UPat(Ops.CONST, arg=math.inf, name="x"),
    lambda ctx, x: f"({ctx.render_cast(x.dtype, ctx.infinity)})",
  ),
  (
    UPat(Ops.CONST, arg=-math.inf, name="x"),
    lambda ctx, x: f"({ctx.render_cast(x.dtype, f'-{ctx.infinity}')})",
  ),
  (
    UPat(Ops.CONST, dtype=dtypes.floats, name="x"),
    lambda ctx, x: f"({ctx.render_cast(x.dtype, ctx.nan)})"
    if math.isnan(x.arg)
    else None,
  ),
  ...
  (
    ...
    lambda ctx, x: f"{truncate[x.dtype](x.arg)}ull",
  ),
  (
    UPat(Ops.CONST, dtype=dtypes.uint32, name="x"),
    lambda ctx, x: f"{truncate[x.dtype](x.arg)}u",
  ),
  (
    UPat(Ops.CONST, dtype=dtypes.bool, name="x"),
    lambda ctx, x: "1" if x.arg else "0",
  ),
  ...
  (
    UPat(Ops.CONST, (dtypes.uint8, dtypes.uint16), name="x"),
    lambda ctx, x: f"({ctx.render_cast(x.dtype, f'{x.arg}u')})",
  ),
  (
    UPat(Ops.CONST, (dtypes.int8, dtypes.int16), name="x"),
    lambda ctx, x: f"({ctx.render_cast(x.dtype, str(x.arg))})",
  ),
  ...
  (
    ...
    lambda ctx,
    buf,
    idx: f"({ctx[buf]}+{strip_parens(ctx[idx]) if idx.arg == Ops.ADD else ctx[idx]})",
  ),
  (
    UPat(
      Ops.LOAD,
  ...
  (
    ...
    lambda ctx, x: ctx.code_for_op[x.op](
      *([
        strip_parens(ctx[v])
        if v.op == x.op and x.op in {Ops.ADD, Ops.MUL, Ops.XOR, Ops.OR, Ops.AND}
        else ctx[v]
        for v in x.src
      ]),
      x.dtype,
  ...
  (
    UPat(Ops.GEP, name="x"),
    lambda ctx, x: ctx[x.src[0]]
    + (
      f"[{x.arg[0]}]"
      if x.src[0].dtype.count > ctx.gep_arr_threshold
      else f".{'xyzwabcd'[x.arg[0]]}"
    ),
  ),
  # custom passes through with format
  (
    UPat((Ops.CUSTOM, Ops.CUSTOMI), name="x"),
    lambda ctx, x: x.arg.format(*[ctx[y] for y in x.src]),
  ),
])
...
extra_pm = PatternMatcher([
  ...
  (
    UPat(Ops.BITCAST, name="x"),
    lambda x: UOp(Ops.BITCAST, x.dtype, (UOp(Ops.PRECAST, x.src[0].dtype, x.src),))
    if x.src[0].op not in {Ops.PRECAST, Ops.LOAD, Ops.CUSTOM}
    else None,
  ),
  # devectorize any bools
  (
    UPat(
      (*GroupOp.ALU, Ops.CAST, Ops.BITCAST, Ops.INDEX), dtype=dtypes.bool, name="alu"
    ),
    no_vectorized_alu,
  ),
...
def create_non_native_float_pats(dts: tuple[DType, ...], casting: bool = True):
  patterns = PatternMatcher([
    (
      UPat(
        Ops.WHERE,
        src=(UPat.var("b"), UPat.var("x", dtype=dts), UPat.var("y", dtype=dts)),
      ),
      lambda b, x, y: UOp(
        Ops.WHERE,
        dtype=dtypes.float,
        src=(b, x.cast(dtypes.float), y.cast(dtypes.float)),
      ).cast(x.dtype),
    ),
    (
      UPat(GroupOp.ALU, dtype=dts, name="x"),
      lambda x: UOp(
        x.op, dtypes.float, tuple(vv.cast(dtypes.float) for vv in x.src), x.arg
      ).cast(x.dtype),
    ),
    (
      UPat(
        ...
        dtypes.bool,
        name="alu",
        src=(UPat.var("x", dtype=dts), UPat.var("y", dtype=dts)),
      ),
      lambda alu, x, y: UOp(
        alu.op, dtypes.bool, (x.cast(dtypes.float), y.cast(dtypes.float)), alu.arg
      ),
    ),
  ])
  if casting:
    ...
    patterns += PatternMatcher([
      (
        ...
        if x.dtype != dtypes.float
        else None,
      ),
      (
        UPat(Ops.CAST, name="x", src=(UPat.var("y", dts),)),
        lambda x, y: y.cast(dtypes.float).cast(x.dtype)
        if x.dtype != dtypes.float
        else None,
      ),
    ])
...
def uops_to_dtypes(uops: list[UOp]) -> list[DType]:
  return dedup(
    u.dtype for u in uops if not isinstance(u.dtype, (ImageDType, PtrDType))
  )
...
def wmma_args(uops: list[UOp]):
  return dedup(
    (uop.arg[0], uop.arg[1], uop.arg[2], uop.dtype.scalar(), *(uop.arg[4:8]))
    for uop in uops
    if uop.op is Ops.WMMA
  )
...
class CStyleLanguage(Renderer):
  ...
  def render_kernel(
    ...
    buftypes = [
      (
        name,
        self.render_dtype(dtype, mutable) + self.buffer_suffix
        if isinstance(dtype, (ImageDType, PtrDType))
    ...
    prg = "".join(
      [
        f"{self.kernel_typedef.format(launch_bounds=launch_bounds)} {function_name}(",
      ]
  ...
  def render_dtype(self, dt: DType, mutable=True) -> str:
    ...
    if isinstance(dt, PtrDType):
      prefix = ""
      if dt.addrspace == AddrSpace.LOCAL and self.smem_prefix_for_cast:
        prefix = self.smem_prefix
      if dt.addrspace == AddrSpace.GLOBAL:
        prefix = self.buffer_prefix
      return prefix + self.render_dtype(dt.base) + "*"
    if dt.count > 1:
      return self.type_map.get(scalar := dt.scalar(), scalar.name).replace(
        " ", "_"
      ) + str(dt.count)
  ...
  def _render(
    self, uops: list[UOp]
  ) -> tuple[str, list[str], list[tuple[str, tuple[DType, bool]]]]:
    r: dict[UOp, str] = {}
    self.r = r
    ...
    for u in uops:
      ...
      if u.op in (Ops.DEFINE_GLOBAL, Ops.DEFINE_VAR):
        r[u] = (
          (f"data{u.arg}_{sz}" if (sz := u.ptrdtype.size) > 0 else f"data{u.arg}")
      ...
      if u.op is Ops.SPECIAL:
        ...
        r[u] = f"{axis_letters[u.arg[-1]]}idx" + range_str(u)
      else:
        prefix = {
          Ops.WMMA: "wmma",
      ...
      assert l is not None, (
        f"failed to render {u.op} {u.dtype} {[(x.op, x.dtype) for x in u.src]} {u.arg}"
      )

      if u.op in {Ops.ENDIF, Ops.END}:
        depth -= 1
      if (u.op is not Ops.CAST or u.dtype.vcount == 1) and (
        u.op in {Ops.CONST, Ops.GEP, Ops.INDEX, Ops.CUSTOMI}
        or (u.op is Ops.LOAD and u.src[0].ptrdtype.addrspace == AddrSpace.REG)
        or (u.op is Ops.CAST and isinstance(u.dtype, PtrDType))
        or (
          u.op
          in {Ops.VECTORIZE, *(GroupOp.ALU - {Ops.WHERE}), Ops.CAST, Ops.BITCAST}
          and child_count[u] == 1
        ...
        if (
          u.op in {Ops.RANGE, Ops.DEFINE_LOCAL, Ops.STORE, Ops.DEFINE_REG}
          or u.dtype == dtypes.void
        ):
          pass
        else:
          l = f"{self.render_dtype(u.dtype)} {r[u]} = {l}" + (
            ";" if u.op is not Ops.SPECIAL else ""
          )
...
class ClangRenderer(CStyleLanguage):
  ...
  code_for_op = {
    **({
      k: v
      for k, v in CStyleLanguage.code_for_op.items()
      if k not in [Ops.EXP2, Ops.SIN, Ops.LOG2, Ops.TRUNC, Ops.RECIPROCAL]
    }),
    Ops.SQRT: lambda x, dtype: f"__builtin_sqrt({x})"
    if dtype == dtypes.float64
    else f"__builtin_sqrtf({x})",
    Ops.TRUNC: lambda x, dtype: f"__builtin_trunc({x})"
    if dtype == dtypes.float64
    else f"__builtin_truncf({x})",
  ...
  def render_vector_prefix(self, dt: DType) -> str:
    ...
    alignment = (
      2 ** int(math.log2(dt.itemsize))
      if getenv("ALIGNED", 1) and not dtypes.is_bool(dt)
      else 1
    )
  ...
  def _render_defines(self, uops) -> list[str]:
    ...
    for name, (N, M, _), dtype_in, _, _, _, _, _ in wmma_args(uops):
      ...
      out, dt1, dt2 = (
        self.render_dtype(dtype_in.vec(N * N)),
        self.render_dtype(dtype_in.vec(N)),
        self.render_dtype(dtype_in.vec(M)),
      )
      prefix += [
        f"""static {out} __{name}({dt1} data1, {dt2} data2, {out} data0){{
  ...
  def _render_entry(
    self, function_name: str, bufs: list[tuple[str, tuple[DType, bool]]]
  ) -> str:
    return ""
...
class OpenCLRenderer(CStyleLanguage):
  ...
  string_rewrite = (
    PatternMatcher([
      (
        UPat(Ops.BITCAST, name="x"),
        lambda ctx, x: f"as_{ctx.render_dtype(x.dtype)}({ctx[x.src[0]]})",
      ),
      ...
      (
        UPat(
          ...
          src=(
            UPat.var("buf").index(
              UPat.var("idx", dtypes.int.vec(2)), UPat.var("gate")
            ),
            UPat.var("var"),
          ),
        ),
        ...
        idx,
        var,
        gate: f"({ctx[gate]}?read_imagef({ctx[buf]}, smp, {ctx[idx]}):{ctx[var]})",
      ),
      (
        UPat(
          Ops.LOAD,
          dtype=dtypes.float.vec(4),
          src=(UPat.var("buf").index(UPat.var("idx", dtypes.int.vec(2))),),
        ),
      ...
      (
        UPat(
          ...
          src=(
            UPat.var("buf").index(
              UPat.var("idx", dtypes.int.vec(2)), allow_any_len=True
            ),
        ...
        buf,
        idx,
        var: f"write_imagef({ctx[buf]}, {ctx[idx]}, {ctx[var]});",
      ),
    ])
    + base_rewrite
  )
...
class IntelRenderer(OpenCLRenderer):
  ...
  string_rewrite = (
    PatternMatcher([
      (
        UPat(
          Ops.CAST, dtype=dtypes.bfloat16, src=(UPat.var("x", dtype=dtypes.float),)
        ),
        lambda ctx, x: f"intel_convert_bfloat16_as_ushort({ctx[x]})",
      ),
      (
        UPat(
          Ops.CAST, dtype=dtypes.float, src=(UPat.var("x", dtype=dtypes.bfloat16),)
        ),
        lambda ctx, x: f"intel_convert_as_bfloat16_float({ctx[x]})",
      ),
    ])
  ...
  def render_kernel(self, function_name, kernel, bufs, uops, prefix=None) -> str:
    ...
    for name, _, dtype_in, dtype_out, _, _, _, _ in wmma_args(uops):
      dt_in = (
        ("ushort", "bf16") if dtype_in == dtypes.bfloat16 else (dtype_in.name, "f16")
      )
...
class MetalRenderer(CStyleLanguage):
  ...
  def __init__(self):
    self.tensor_cores = (
      tc.metal if hasattr(os, "uname") and os.uname().machine == "arm64" else []
    )
  ...
  float4 = "float4"
  code_for_workitem = {
    "g": lambda x: f"gid.{chr(120 + int(x))}",
    "l": lambda x: f"lid.{chr(120 + int(x))}",
  }
  ...
  code_for_op = {
    **CStyleLanguage.code_for_op,
    Ops.SIN: lambda x, dtype: f"precise::sin({x})",
  }
  ...
  extra_matcher = (
    PatternMatcher([
      ...
      (
        UPat(
          (Ops.SQRT, Ops.EXP2, Ops.LOG2, Ops.SIN), dtype=dtypes.bfloat16, name="x"
        ),
        lambda x: (
          UOp(
            x.op, dtypes.float, tuple(vv.cast(dtypes.float) for vv in x.src), x.arg
  ...
  string_rewrite = (
    PatternMatcher([
      (
        UPat(Ops.BITCAST, name="x"),
        lambda ctx, x: f"as_type<{ctx.render_dtype(x.dtype)}>({ctx[x.src[0]]})",
      ),
    ])
    + base_rewrite
  )
  ...
  def render_kernel(self, function_name, kernel, bufs, uops, prefix=None):
    ...
    deduped_wmma_args = dedup([
      (name, dtype_in, dtype_out)
      for name, _, dtype_in, dtype_out, _, _, _, _ in wmma_args(uops)
    ])
...
class CUDARenderer(CStyleLanguage):
  ...
  gep_arr_threshold = 8
  code_for_workitem = {
    "g": lambda x: f"blockIdx.{chr(120 + int(x))}",
    "l": lambda x: f"threadIdx.{chr(120 + int(x))}",
    "i": lambda x: f"(blockIdx.{chr(120 + int(x))}*blockDim.{chr(120 + int(x))}+threadIdx.{chr(120 + int(x))})",
  }
  code_for_op = {
    **CStyleLanguage.code_for_op,
    Ops.SIN: lambda x, dtype: f"hsin({x})"
    if dtype in (dtypes.half, dtypes.bfloat16)
    else f"sin({x})",
    Ops.LOG2: lambda x, dtype: f"hlog2({x})"
    if dtype in (dtypes.half, dtypes.bfloat16)
    else f"log2({x})",
    Ops.EXP2: lambda x, dtype: f"hexp2({x})"
    if dtype in (dtypes.half, dtypes.bfloat16)
    else f"exp2({x})",
    Ops.SQRT: lambda x, dtype: f"hsqrt({x})"
    if dtype in (dtypes.half, dtypes.bfloat16)
    else f"sqrt({x})",
    Ops.RECIPROCAL: lambda x, dtype: f"hrcp({x})"
    if dtype in (dtypes.half, dtypes.bfloat16)
    else f"(1/{x})",
  }
  type_map = {
    dtypes.bfloat16: "nv_bfloat16",
    dtypes.fp8e4m3: "__nv_fp8_e4m3",
    dtypes.fp8e5m2: "__nv_fp8_e5m2",
  }
  extra_matcher = (
    create_non_native_float_pats(dtypes.fp8s, casting=False)
    + PatternMatcher([
      (
  ...
  def render_vector_prefix(self, dt: DType) -> str:
    vec, scal = (
      self.render_dtype(dt),
      self.render_dtype(dt.scalar()),
    )
    elems, header = (
      ", ".join(_nms[: dt.count]),
      ", ".join([f"{scal} {x}" for x in _nms[: dt.count]]),
    )
  ...
  def render_kernel(self, function_name, kernel, bufs, uops, prefix=None):
    ...
    if any(dt.scalar() == dtypes.half for dt in used_dtypes):
      prefix.append("#include <cuda_fp16.h>")
    if any(dt.scalar() == dtypes.bfloat16 for dt in used_dtypes):
      prefix.append("#include <cuda_bf16.h>")
    prefix += [
      self.render_vector_prefix(dt)
      for dt in used_dtypes
      if (dt.count in (4, 8) and dt.scalar() in {dtypes.half, dtypes.bfloat16})
    ...
    for name, (N, M, K), dtype_in, dtype_out, _, _, upcast_axes, _ in wmma_args(
      uops
    ):
      upcast_sizes = [prod(size for _, size in upcast) for upcast in upcast_axes]
      wmma_dtypes = [
        self.render_dtype(dtype.vec(size))
        for dtype, size in zip([dtype_in, dtype_in, dtype_out], upcast_sizes)
      ]
      n_operands = [
...
def cast_float_to_bf16(x: UOp) -> UOp:
  ...
  x = (-x & 0x7F800000).where(
    x + ((x >> 16) & 1) + 0x7FFF, (x & 0xFFFF).where((x | 0x10000), x)
  )
...
class AMDRenderer(CStyleLanguage):
  ...
  def get_tensor_cores(arch):
    return {
      "gfx942": tc.amd_cdna3,
      "gfx950": tc.amd_cdna4,
      "gfx1200": tc.amd_rdna4,
      "gfx1201": tc.amd_rdna4,
    }.get(arch.split(":")[0], tc.amd_rdna3)

  @staticmethod
  def is_cdna(arch):
    return arch.split(":")[0] in {"gfx942", "gfx950"}
      ...
      self.string_rewrite = (
        PatternMatcher([
          (
            UPat(Ops.WMMA, name="x"),
            lambda ctx,
            x: f"__{x.arg[0]}({ctx[x.src[0]]}, {ctx[x.src[1]]}, {ctx[x.src[2]]}, 0, 0, 0)",
          ),
          (
            UPat(
              Ops.CAST,
              dtypes.fp8s,
              (UPat.var("y", dtypes.float),),
              name="x",
            ),
            lambda ctx,
            x,
            y: f"f32_to_fp8({ctx[x.src[0]]}, {'1' if x.dtype == dtypes.fp8e5m2 else '0'})",
          ),
          (
            UPat(
              Ops.CAST,
              dtypes.float,
              (UPat.var("y", dtypes.fp8s),),
              name="x",
            ),
            lambda ctx,
            x,
            y: f"__builtin_amdgcn_cvt_f32_{'bf8' if y.dtype == dtypes.fp8e5m2 else 'fp8'}((unsigned int){ctx[x.src[0]]}, 0)",
          ),
        ])
        + base_rewrite
      )
  ...
  # language options
  ockl = [
    (f"__ockl_get_{name}", "unsigned int", "size_t", "const")
    for name in ["local_id", "group_id", "local_size"]
  ]
  ocml = [
    (f"__ocml_{name}_f{n}", f"{dt}, {dt}" if "fmax" == name else dt, dt, atr)
    for dt, n in [
      (dtype.name, dtype.itemsize * 8)
      for dtype in [dtypes.float, dtypes.double, dtypes.half]
    ]
    for name, atr in [
      ("fmax", "const"),
      ("exp2", "pure"),
  ...
  kernel_typedef = "\n".join(
    ...
    for meth, dti, dto, atr in ockl + ocml
  )
  ...
  code_for_op = {
    **CStyleLanguage.code_for_op,
    Ops.TRUNC: lambda x,
    dtype: f"__ocml_trunc_f{ {dtypes.half: 16, dtypes.double: 64}.get(dtype, 32) }({x})",
    Ops.SIN: lambda x,
    dtype: f"__ocml_sin_f{ {dtypes.half: 16, dtypes.double: 64}.get(dtype, 32) }({x})",
    Ops.LOG2: lambda x,
    dtype: f"__ocml_log2_f{ {dtypes.half: 16, dtypes.double: 64}.get(dtype, 32) }({x})",
    Ops.EXP2: lambda x,
    dtype: f"__ocml_exp2_f{ {dtypes.half: 16, dtypes.double: 64}.get(dtype, 32) }({x})",
    Ops.SQRT: lambda x,
    dtype: f"__ocml_sqrt_f{ {dtypes.half: 16, dtypes.double: 64}.get(dtype, 32) }({x})",
  }
  ...
  float4 = "make_float4"
  type_map = {
    dtypes.bfloat16: "hip_bfloat16",
    dtypes.fp8e4m3: "hip_fp8",
    dtypes.fp8e5m2: "hip_bf8",
  }
  extra_matcher = (
    create_non_native_float_pats((dtypes.bfloat16, *dtypes.fp8s))
    + PatternMatcher([
      (
        ...
        lambda x: UOp(
          ...
          x.dtype,
          (
            x.src[0].bitcast(dtypes.uint64),
            x.src[1].bitcast(dtypes.uint64),
            x.src[2],
          ),
          (*x.arg,),
        )
        if x.src[0].dtype in (dtypes.fp8e4m3.vec(8), dtypes.fp8e5m2.vec(8))
        else None,
      ),
      # bfloat16 casting
      (
        UPat.cvar("x", dtypes.bfloat16),
        lambda x: cast_float_to_bf16(UOp.const(dtypes.float, x.arg)),
      ),
      (
        UPat(Ops.CAST, dtypes.float, (UPat.var("x", dtypes.bfloat16),)),
        lambda x: (x.bitcast(dtypes.ushort).cast(dtypes.uint) << 16).bitcast(
          dtypes.float
        ),
      ),
      (
  ...
  def render_vector_prefix(self, dtype: DType) -> str:
    ...
    return (
      f"typedef {scal} {vec} __attribute__((ext_vector_type({dtype.count})));\nstatic inline __attribute__((device)) "
  ...
  def render_kernel(self, function_name, kernel, bufs, uops, prefix=None) -> str:
    ...
    type_map = {
      ...
      dtypes.float: "f32",
      dtypes.half: "f16",
      dtypes.fp8e4m3: "_fp8_fp8",
      dtypes.fp8e5m2: "_bf8_bf8",
    }
    used_dtypes = uops_to_dtypes(uops)
    if any(dt.scalar() == dtypes.bfloat16 for dt in used_dtypes):
      prefix.append("typedef unsigned short hip_bfloat16;")
    if any(dt.scalar() in dtypes.fp8s for dt in used_dtypes):
  ...
  return (unsigned char)(is_bf8?__builtin_amdgcn_cvt_pk_bf8_f32(v,v,0,false):__builtin_amdgcn_cvt_pk_fp8_f32(v,v,0,false));\n}""")
    ...
    for name, (N, M, K), dtype_in, dtype_out, _, _, _, _ in wmma_args(
      uops
    ):  # TODO: handle TCs f32_bf16 and bf16_bf16 w/ wrapper
      if self.is_cdna(self.arch):
        if (N, M, K) == (16, 16, 16):
          type_map[dtypes.bfloat16] = "bf16_1k"
        elif (N, M, K) == (16, 16, 32):
          type_map = {**type_map, dtypes.bfloat16: "_bf16", dtypes.half: "_f16"}
        prefix.append(
          f"#define __{name} __builtin_amdgcn_mfma_f32_{N}x{M}x{K}{type_map[dtype_in]}"
        )
      # #define __WMMA_16_16_16_half_half __builtin_amdgcn_wmma_f16_16x16x16_f16_w32_gfx12
      elif self.tensor_cores == tc.amd_rdna4:
        prefix.append(
          f"#define __{name} __builtin_amdgcn_wmma_{type_map[dtype_out]}_16x16x16_{type_map[dtype_in]}_w32_gfx12"
        )
      elif dtype_out == dtypes.float:
        prefix.append(
          f"#define __{name} __builtin_amdgcn_wmma_f32_16x16x16_{'f16' if dtype_in == dtypes.half else 'bf16'}_w32"
        )
      else:
        prefix.append(
          f"static inline __attribute__((device)) half8 __{name}"
          + """(half16 a, half16 b, half8 c) {
...
class NVRenderer(CUDARenderer):
  device = "NV"


class HIPRenderer(AMDRenderer):
  device = "HIP"
