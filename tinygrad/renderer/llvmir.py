...
base_rewrite = PatternMatcher([
  ...
  (
    ...
    lambda ctx, x, y: f"  {ctx[x]}_z = insertelement <1 x {ldt(y.dtype)}> poison, {ldt(y.dtype)} {ctx[y]}, i32 0\n"
  ...
  (
    ...
    lambda ctx, x: f"  {ctx[x]} = {lop[x.src[0].dtype.scalar()][x.op]} {ldt(x.src[0].dtype)} {ctx[x.src[0]]}, {ctx[x.src[1]]}",
  ),
  (
    ...
    if self.is_cdna:
      self.extra_matcher += PatternMatcher([
        (
          ...
          lambda x: UOp(
            Ops.WMMA, dtypes.float.vec(4), (x.src[0].bitcast(dtypes.uint16.vec(4)), x.src[1].bitcast(dtypes.uint16.vec(4)), x.src[2]), (*x.arg,)
          )
    ...
    if self.arch.split(":")[0] == "gfx1100":
      self.extra_matcher += PatternMatcher([
        ...
        (
          ...
          lambda x: UOp(Ops.WMMA, x.dtype, (x.src[0].bitcast(dtypes.uint16.vec(16)), x.src[1].bitcast(dtypes.uint16.vec(16)), x.src[2]), x.arg)
      ...
      self.extra_matcher += PatternMatcher([
        (
          ...
          lambda x: UOp(
            Ops.WMMA,
            dtypes.uint16.vec(8),
            (x.src[0].bitcast(dtypes.uint16.vec(8)), x.src[1].bitcast(dtypes.uint16.vec(8)), x.src[2].bitcast(dtypes.uint16.vec(8))),
            (*x.arg,),
        ...
        (
          ...
          lambda x: UOp(
            Ops.WMMA, dtypes.float.vec(8), (x.src[0].bitcast(dtypes.uint16.vec(8)), x.src[1].bitcast(dtypes.uint16.vec(8)), x.src[2]), (*x.arg,)
          )
          if x.src[0].dtype == dtypes.bfloat16.vec(8)
          else None,
        ),
