...
glsl_base = {
  ...
  **{getattr(dtypes, d): g(f"GLSL_TYPE_{d.upper()}") for d in ["double", "float", "float16"]},
...
def c(t: DType, u: bool = True) -> str:
  return "u" if t in dtypes.uints and u else ("i" if t in dtypes.ints else ("f" if t in dtypes.floats else "b"))
...
def ncast(b: mesa.nir_builder, src: mesa.nir_def, it: DType, ot: DType) -> mesa.nir_def:
  if isinstance(it, PtrDType) and ot == dtypes.long:
    return src
  if ot == dtypes.bool:
    return nalu(b, c(it, False) + "ne" + ("u" if c(it) == "f" else ""), src, nimm(b, 0, it))
...
def nir_instr(nc=1, bs=lambda: None, intrins=None, srcs=None, has_def=True, df=None, also=lambda: None, **contents):
  def dec(f: Callable):
    ...
    def wrapper(*args, **kwargs) -> mesa.nir_def:
      ...
      def go(g):
        return g(**{nm: ba.arguments[nm] for nm in inspect.signature(g).parameters}) if callable(g) else g

      instr = f(*args, **kwargs)
      if has_def:
        mesa.nir_def_init(instr.contents.instr, getattr(instr.contents, "def"), go(nc), go(bs))
      ...
      for k, v in {k: vcomp for k, v in contents.items() if (vcomp := go(v)) is not None}.items():
...
def scope(space):
  return "global" if space == AddrSpace.GLOBAL else ("shared" if space == AddrSpace.LOCAL else "deref")


nstore = nir_instr(
  has_def=False,
  df=lambda addr: addr,
  intrins=lambda space, val: {"WRITE_MASK": (1 << val.num_components) - 1, **iointr(space)},
...
nload = nir_instr(
  ...
  intrins=lambda space: {**({"ACCESS": mesa.ACCESS_CAN_REORDER} if space == AddrSpace.GLOBAL else {}), **iointr(space)},
...
def nidx(b: mesa.nir_builder, buf, off, dtype, gate=None) -> mesa.nir_def:
  ...
  f = (
    ...
    else lambda: nalu(b, "iadd", buf, nalu(b, "imul", off, nimm(b, dtype.itemsize, dtypes.long)))
  )
...
class NIRRenderer(Renderer):
  ...
  extra_matcher = PatternMatcher([
    ...
    (
      ...
      lambda x: x.replace(dtype=dtypes.uint8, src=x.src[0:1] + ((x.src[1].cast(dtypes.uint8),) if len(x.src) >= 2 else ()) + x.src[2:]).cast(
        dtypes.bool
      ),
    ),
    (
  ...
  def_rewrite = PatternMatcher([
    ...
    (
      UPat(
        Ops.STORE, src=(UPat(Ops.INDEX, src=(UPat.var("buf"), UPat.var("off")), allow_any_len=True), UPat.var("val")), allow_any_len=True, name="x"
      ),
    ...
    (
      ...
      lambda ctx, x, buf, off, alt, gate: if_phi(
        ctx.b,
        ctx.r[gate],
        lambda: nload(ctx.b, buf.ptrdtype.addrspace, nidx(ctx.b, ctx.r[buf], ctx.r[off], buf.dtype, ctx.r[gate]), x.dtype),
        lambda: ctx.r[alt],
      ),
    ),
    (
    ...
    (
      ...
      lambda ctx, x: mesa.nir_local_variable_create(ctx.b.impl, glsl_type(x.dtype), f"acc{x.arg[0]}".encode()).contents,
    ),
  ...
  def render(self, uops: list[UOp]):
    ...
    if DEBUG >= 4:
      mesa.nir_print_shader(
        self.b.shader, ctypes.POINTER(mesa.struct__IO_FILE).in_dll(ctypes.CDLL(ctypes.util.find_library("c")), "__stdoutp" if OSX else "stdout")
      )
...
class NAKRenderer(NIRRenderer):
  ...
  param = nir_instr(
    ...
    srcs=lambda self, b: [nsrc(nimm(b, 0, dtypes.int)), nsrc(nimm(b, self.param_idx, dtypes.int))],
...
class LVPRenderer(NIRRenderer):
  ...
  param = nir_instr(
    ...
    srcs=lambda b, self: [nsrc(nimm(b, 0, dtypes.int)), nsrc(nimm(b, self.param_idx, dtypes.int))],
    also=lambda self, sz: setattr(self, "param_idx", self.param_idx + sz),
  )(lambda self, b, dtype, sz: mesa.nir_intrinsic_instr_create(b.shader, mesa.nir_intrinsic_load_ubo))
