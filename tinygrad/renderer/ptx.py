...
asm_for_op: dict[Ops, Callable] = {
  ...
  Ops.WHERE: lambda d, a, b, c, dt, name: [f"@{a} mov.{name} {d}, {b};", f"@!{a} mov.{name} {d}, {c};"]
  if dt == dtypes.bool
...
ptx_matcher = PatternMatcher([
  ...
  (
    UPat(Ops.INDEX, src=(UPat.var("buf"), UPat.var("idx")), name="op", allow_any_len=True),
...
def modifier(a: DType, b: DType):
  return (
    ...
    if dtypes.is_float(a) and (a.itemsize < b.itemsize or dtypes.is_int(b) or b == dtypes.bool)
    else ""
  )
...
string_rewrite = PatternMatcher([
  ...
  (UPat(Ops.RANGE, name="r"), lambda ctx, r: [f"mov.u32 {ctx.r[r]}, -1;", f"bra END_{ctx.r[r][1:]};", "LOOP_" + f"{ctx.r[r][1:]}:"]),
  (
  ...
  def render(self, uops: list[UOp]) -> str:
    ...
    for u in uops:
      ...
      if u.op is Ops.SPECIAL:
        ...
        self.wmma_r = [
          [ssa("wmma_in", dtype="b32") for _ in range(0, len(r[u.src[0]]), 4 // u.src[0].dtype.scalar().itemsize)],
