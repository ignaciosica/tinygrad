...
def sign_extend(val: UOp, sext_am: int):
  return (
    UOp.where((val >> (sext_am - 1)) > 0, UOp.const(dtypes.uint32, 0xFFFFFFFF) << sext_am, UOp.const(dtypes.uint32, 0)) | val.bitcast(dtypes.uint32)
  ).bitcast(dtypes.int)
...
def packed_load(root: UOp, bidx: UOp, dtype: DType, var: UOp | None = None):
  ...
  if var is not None:
    load = UOp.load(UOp(Ops.INDEX, bidx.dtype, (bidx.src[0], div_idx, bidx.src[2])), var, dtype=dtypes.uint32, arg=root.arg)
  else:
    load = UOp.load(UOp(Ops.INDEX, bidx.dtype, (bidx.src[0], div_idx)), *root.src[1:], dtype=dtypes.uint32, arg=root.arg)
...
wgsl_matcher = (
  PatternMatcher([
    ...
    (
      ...
      lambda a, b: (a.bitcast(dtypes.uint32) << b.cast(dtypes.uint32)).bitcast(a.dtype) if b.dtype != dtypes.uint32 else None,
    ),
