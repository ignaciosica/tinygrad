...
try:
  ...
  amd_comgr_set_data_name.argtypes = [
    amd_comgr_data_t,
    ctypes.POINTER(ctypes.c_char),
  ]
except AttributeError:
...
try:
  ...
  pass
try:
  amd_comgr_action_info_get_bundle_entry_id_count = _libraries[
    "libamd_comgr.so"
  ...
  amd_comgr_action_info_get_bundle_entry_id_count.argtypes = [
    amd_comgr_action_info_t,
    ctypes.POINTER(ctypes.c_uint64),
  ]
except AttributeError:
...
try:
  ...
  amd_comgr_map_elf_virtual_address_to_code_object_offset.restype = (
    amd_comgr_status_t
  )
