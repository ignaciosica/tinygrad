...
def _do_ioctl(__idir, __base, __nr, __user_struct, __fd, __payload=None, **kwargs):
  ret = __fd.ioctl(
    (__idir << 30)
    | (ctypes.sizeof(made := (__payload or __user_struct(**kwargs))) << 16)
    | (__base << 8)
    | __nr,
    made,
  )
...
KGSL_MEMFLAGS_USERMEM_PMEM = KGSL_USERMEM_FLAG(
  KGSL_USER_MEM_TYPE_PMEM
)  # macro (from list)
KGSL_MEMFLAGS_USERMEM_ASHMEM = KGSL_USERMEM_FLAG(
  KGSL_USER_MEM_TYPE_ASHMEM
)  # macro (from list)
KGSL_MEMFLAGS_USERMEM_ADDR = KGSL_USERMEM_FLAG(
  KGSL_USER_MEM_TYPE_ADDR
)  # macro (from list)
KGSL_MEMFLAGS_USERMEM_ION = KGSL_USERMEM_FLAG(
...
IOCTL_KGSL_DEVICE_GETPROPERTY = _IOWR(
  0x09, 0x2, struct_kgsl_device_getproperty
)  # macro (from list)
IOCTL_KGSL_SETPROPERTY = _IOW(
  0x09, 0x32, struct_kgsl_device_getproperty
)  # macro (from list)
...
IOCTL_KGSL_DEVICE_WAITTIMESTAMP = _IOW(
  0x09, 0x6, struct_kgsl_device_waittimestamp
)  # macro (from list)
...
IOCTL_KGSL_DEVICE_WAITTIMESTAMP_CTXTID = _IOW(
  0x09, 0x7, struct_kgsl_device_waittimestamp_ctxtid
)  # macro (from list)
...
IOCTL_KGSL_RINGBUFFER_ISSUEIBCMDS = _IOWR(
  0x09, 0x10, struct_kgsl_ringbuffer_issueibcmds
)  # macro (from list)
...
IOCTL_KGSL_CMDSTREAM_READTIMESTAMP_OLD = _IOR(
  0x09, 0x11, struct_kgsl_cmdstream_readtimestamp
)  # macro (from list)
IOCTL_KGSL_CMDSTREAM_READTIMESTAMP = _IOWR(
  0x09, 0x11, struct_kgsl_cmdstream_readtimestamp
)  # macro (from list)
...
IOCTL_KGSL_CMDSTREAM_FREEMEMONTIMESTAMP = _IOW(
  0x09, 0x12, struct_kgsl_cmdstream_freememontimestamp
)  # macro (from list)
IOCTL_KGSL_CMDSTREAM_FREEMEMONTIMESTAMP_OLD = _IOR(
  0x09, 0x12, struct_kgsl_cmdstream_freememontimestamp
)  # macro (from list)
...
IOCTL_KGSL_DRAWCTXT_CREATE = _IOWR(
  0x09, 0x13, struct_kgsl_drawctxt_create
)  # macro (from list)
...
IOCTL_KGSL_DRAWCTXT_DESTROY = _IOW(
  0x09, 0x14, struct_kgsl_drawctxt_destroy
)  # macro (from list)
...
IOCTL_KGSL_MAP_USER_MEM = _IOWR(
  0x09, 0x15, struct_kgsl_map_user_mem
)  # macro (from list)
...
IOCTL_KGSL_CMDSTREAM_READTIMESTAMP_CTXTID = _IOWR(
  0x09, 0x16, struct_kgsl_cmdstream_readtimestamp_ctxtid
)  # macro (from list)
...
IOCTL_KGSL_CMDSTREAM_FREEMEMONTIMESTAMP_CTXTID = _IOW(
  0x09, 0x17, struct_kgsl_cmdstream_freememontimestamp_ctxtid
)  # macro (from list)
...
IOCTL_KGSL_SHAREDMEM_FROM_PMEM = _IOWR(
  0x09, 0x20, struct_kgsl_sharedmem_from_pmem
)  # macro (from list)
...
IOCTL_KGSL_SHAREDMEM_FREE = _IOW(
  0x09, 0x21, struct_kgsl_sharedmem_free
)  # macro (from list)
IOCTL_KGSL_SHAREDMEM_FLUSH_CACHE = _IOW(
  0x09, 0x24, struct_kgsl_sharedmem_free
)  # macro (from list)
...
IOCTL_KGSL_CFF_USER_EVENT = _IOW(
  0x09, 0x31, struct_kgsl_cff_user_event
)  # macro (from list)
...
IOCTL_KGSL_DRAWCTXT_BIND_GMEM_SHADOW = _IOW(
  0x09, 0x22, struct_kgsl_bind_gmem_shadow
)  # macro (from list)
...
IOCTL_KGSL_SHAREDMEM_FROM_VMALLOC = _IOWR(
  0x09, 0x23, struct_kgsl_sharedmem_from_vmalloc
)  # macro (from list)
...
IOCTL_KGSL_DRAWCTXT_SET_BIN_BASE_OFFSET = _IOW(
  0x09, 0x25, struct_kgsl_drawctxt_set_bin_base_offset
)  # macro (from list)
...
IOCTL_KGSL_CMDWINDOW_WRITE = _IOW(
  0x09, 0x2E, struct_kgsl_cmdwindow_write
)  # macro (from list)
...
IOCTL_KGSL_GPUMEM_ALLOC = _IOWR(
  0x09, 0x2F, struct_kgsl_gpumem_alloc
)  # macro (from list)
...
IOCTL_KGSL_CFF_SYNCMEM = _IOW(
  0x09, 0x30, struct_kgsl_cff_syncmem
)  # macro (from list)
...
IOCTL_KGSL_TIMESTAMP_EVENT_OLD = _IOW(
  0x09, 0x31, struct_kgsl_timestamp_event
)  # macro (from list)
IOCTL_KGSL_TIMESTAMP_EVENT = _IOWR(
  0x09, 0x33, struct_kgsl_timestamp_event
)  # macro (from list)
...
IOCTL_KGSL_GPUMEM_ALLOC_ID = _IOWR(
  0x09, 0x34, struct_kgsl_gpumem_alloc_id
)  # macro (from list)
...
IOCTL_KGSL_GPUMEM_FREE_ID = _IOWR(
  0x09, 0x35, struct_kgsl_gpumem_free_id
)  # macro (from list)
...
IOCTL_KGSL_GPUMEM_GET_INFO = _IOWR(
  0x09, 0x36, struct_kgsl_gpumem_get_info
)  # macro (from list)
...
IOCTL_KGSL_GPUMEM_SYNC_CACHE = _IOW(
  0x09, 0x37, struct_kgsl_gpumem_sync_cache
)  # macro (from list)
...
IOCTL_KGSL_PERFCOUNTER_GET = _IOWR(
  0x09, 0x38, struct_kgsl_perfcounter_get
)  # macro (from list)
...
IOCTL_KGSL_PERFCOUNTER_PUT = _IOW(
  0x09, 0x39, struct_kgsl_perfcounter_put
)  # macro (from list)
...
IOCTL_KGSL_PERFCOUNTER_QUERY = _IOWR(
  0x09, 0x3A, struct_kgsl_perfcounter_query
)  # macro (from list)
...
IOCTL_KGSL_PERFCOUNTER_READ = _IOWR(
  0x09, 0x3B, struct_kgsl_perfcounter_read
)  # macro (from list)
...
IOCTL_KGSL_GPUMEM_SYNC_CACHE_BULK = _IOWR(
  0x09, 0x3C, struct_kgsl_gpumem_sync_cache_bulk
)  # macro (from list)
...
IOCTL_KGSL_SUBMIT_COMMANDS = _IOWR(
  0x09, 0x3D, struct_kgsl_submit_commands
)  # macro (from list)
...
IOCTL_KGSL_SYNCSOURCE_CREATE = _IOWR(
  0x09, 0x40, struct_kgsl_syncsource_create
)  # macro (from list)
...
IOCTL_KGSL_SYNCSOURCE_CREATE_FENCE = _IOWR(
  0x09, 0x42, struct_kgsl_syncsource_create_fence
)  # macro (from list)
...
IOCTL_KGSL_SYNCSOURCE_SIGNAL_FENCE = _IOWR(
  0x09, 0x43, struct_kgsl_syncsource_signal_fence
)  # macro (from list)
...
IOCTL_KGSL_CFF_SYNC_GPUOBJ = _IOW(
  0x09, 0x44, struct_kgsl_cff_sync_gpuobj
)  # macro (from list)
...
IOCTL_KGSL_GPUOBJ_ALLOC = _IOWR(
  0x09, 0x45, struct_kgsl_gpuobj_alloc
)  # macro (from list)
...
IOCTL_KGSL_GPUOBJ_FREE = _IOW(
  0x09, 0x46, struct_kgsl_gpuobj_free
)  # macro (from list)
...
IOCTL_KGSL_GPUOBJ_INFO = _IOWR(
  0x09, 0x47, struct_kgsl_gpuobj_info
)  # macro (from list)
...
IOCTL_KGSL_GPUOBJ_IMPORT = _IOWR(
  0x09, 0x48, struct_kgsl_gpuobj_import
)  # macro (from list)
...
IOCTL_KGSL_GPUOBJ_SYNC = _IOW(
  0x09, 0x49, struct_kgsl_gpuobj_sync
)  # macro (from list)
...
IOCTL_KGSL_GPU_COMMAND = _IOWR(
  0x09, 0x4A, struct_kgsl_gpu_command
)  # macro (from list)
...
IOCTL_KGSL_PREEMPTIONCOUNTER_QUERY = _IOWR(
  0x09, 0x4B, struct_kgsl_preemption_counters_query
)  # macro (from list)
...
IOCTL_KGSL_GPUOBJ_SET_INFO = _IOW(
  0x09, 0x4C, struct_kgsl_gpuobj_set_info
)  # macro (from list)
__all__ = [
  "KGSL_CACHEMODE_MASK",
  ...
  "KGSL_FLAGS_INITIALIZED0",
  "KGSL_FLAGS_NORMALMODE",
  "KGSL_FLAGS_PER_CONTEXT_TIMESTAMPS",
  "KGSL_FLAGS_RESERVED0",
  "KGSL_FLAGS_RESERVED1",
  "KGSL_FLAGS_RESERVED2",
  "KGSL_FLAGS_SAFEMODE",
  "KGSL_FLAGS_SOFT_RESET",
  "KGSL_FLAGS_STARTED",
  "KGSL_GPUMEM_CACHE_CLEAN",
  "KGSL_GPUMEM_CACHE_FLUSH",
  "KGSL_GPUMEM_CACHE_FROM_GPU",
  "KGSL_GPUMEM_CACHE_INV",
  "KGSL_GPUMEM_CACHE_RANGE",
  "KGSL_GPUMEM_CACHE_TO_GPU",
  "KGSL_GPUOBJ_ALLOC_METADATA_MAX",
  "KGSL_GPUOBJ_FREE_ON_EVENT",
  "KGSL_GPUOBJ_SET_INFO_METADATA",
  "KGSL_GPUOBJ_SET_INFO_TYPE",
  "KGSL_GPU_EVENT_FENCE",
  "KGSL_GPU_EVENT_TIMESTAMP",
  "KGSL_IBDESC_MEMLIST",
  "KGSL_IBDESC_PROFILING_BUFFER",
  "KGSL_IOC_TYPE",
  "KGSL_MEMALIGN_MASK",
  "KGSL_MEMALIGN_SHIFT",
  ...
  "KGSL_MEMFLAGS_NOT_USERMEM",
  "KGSL_MEMFLAGS_SECURE",
  "KGSL_MEMFLAGS_USERMEM_MASK",
  "KGSL_MEMFLAGS_USERMEM_SHIFT",
  "KGSL_MEMFLAGS_USE_CPU_MAP",
  "KGSL_MEMTYPE_2D",
  "KGSL_MEMTYPE_ARRAYBUFFER",
  "KGSL_MEMTYPE_CL",
  ...
  "KGSL_PERFCOUNTER_GROUP_RBBM",
  "KGSL_PERFCOUNTER_GROUP_RB_PWR",
  "KGSL_PERFCOUNTER_GROUP_SP",
  "KGSL_PERFCOUNTER_GROUP_SP_PWR",
  "KGSL_PERFCOUNTER_GROUP_SQ",
  "KGSL_PERFCOUNTER_GROUP_SX",
  "KGSL_PERFCOUNTER_GROUP_TCF",
  "KGSL_PERFCOUNTER_GROUP_TCM",
  "KGSL_PERFCOUNTER_GROUP_TCR",
  "KGSL_PERFCOUNTER_GROUP_TP",
  "KGSL_PERFCOUNTER_GROUP_TP_PWR",
  "KGSL_PERFCOUNTER_GROUP_TSE",
  "KGSL_PERFCOUNTER_GROUP_UCHE",
  "KGSL_PERFCOUNTER_GROUP_UCHE_PWR",
  "KGSL_PERFCOUNTER_GROUP_VBIF",
  "KGSL_PERFCOUNTER_GROUP_VBIF_PWR",
  "KGSL_PERFCOUNTER_GROUP_VFD",
  ...
  "KGSL_PROP_DEVICE_SHADOW",
  "KGSL_PROP_GPMU_VERSION",
  "KGSL_PROP_GPU_RESET_STAT",
  "KGSL_PROP_INTERRUPT_WAITS",
  "KGSL_PROP_MMU_ENABLE",
  "KGSL_PROP_PWRCTRL",
  "KGSL_PROP_PWR_CONSTRAINT",
  "KGSL_PROP_SHMEM",
  "KGSL_PROP_SHMEM_APERTURES",
  "KGSL_PROP_SP_GENERIC_MEM",
  "KGSL_PROP_UCHE_GMEM_VADDR",
  "KGSL_PROP_UCODE_VERSION",
  "KGSL_PROP_VERSION",
  "KGSL_SYNCOBJ_SERVER_TIMEOUT",
  "KGSL_TIMESTAMP_CONSUMED",
  "KGSL_TIMESTAMP_EVENT_FENCE",
  "KGSL_TIMESTAMP_EVENT_GENLOCK",
  ...
  "struct_kgsl_cff_sync_gpuobj",
  "struct_kgsl_cff_syncmem",
  "struct_kgsl_cff_user_event",
  "struct_kgsl_cmd_syncpoint",
  ...
  "struct_kgsl_command_syncpoint",
  "struct_kgsl_device_constraint",
  ...
  "struct_kgsl_gpmu_version",
  "struct_kgsl_gpu_command",
  "struct_kgsl_gpu_event_fence",
  ...
  "struct_kgsl_gpumem_sync_cache_bulk",
  "struct_kgsl_gpuobj_alloc",
  "struct_kgsl_gpuobj_free",
  "struct_kgsl_gpuobj_import",
  "struct_kgsl_gpuobj_import_dma_buf",
  "struct_kgsl_gpuobj_import_useraddr",
  "struct_kgsl_gpuobj_info",
  "struct_kgsl_gpuobj_set_info",
  "struct_kgsl_gpuobj_sync",
  "struct_kgsl_gpuobj_sync_obj",
  "struct_kgsl_ibdesc",
  ...
  "struct_kgsl_sp_generic_mem",
  "struct_kgsl_submit_commands",
  ...
  "struct_kgsl_ucode_version",
  "struct_kgsl_version",
  "uint64_t",
]


def KGSL_CONTEXT_PRIORITY(val):
  return (val << KGSL_CONTEXT_PRIORITY_SHIFT) & KGSL_CONTEXT_PRIORITY_MASK


def KGSL_CONTEXT_PREEMPT_STYLE(val):
  return (val << KGSL_CONTEXT_PREEMPT_STYLE_SHIFT) & KGSL_CONTEXT_PREEMPT_STYLE_MASK


def KGSL_CONTEXT_TYPE(val):
  return (val << KGSL_CONTEXT_TYPE_SHIFT) & KGSL_CONTEXT_TYPE_MASK


def KGSL_CACHEMODE(val):
  return (val << KGSL_CACHEMODE_SHIFT) & KGSL_CACHEMODE_MASK


def KGSL_MEMTYPE(val):
  return (val << KGSL_MEMTYPE_SHIFT) & KGSL_MEMTYPE_MASK


def KGSL_MEMALIGN(val):
  return (val << KGSL_MEMALIGN_SHIFT) & KGSL_MEMALIGN_MASK


def KGSL_MEMFLAGS_USERMEM(val):
  return (val << KGSL_MEMFLAGS_USERMEM_SHIFT) & KGSL_MEMFLAGS_USERMEM_MASK
