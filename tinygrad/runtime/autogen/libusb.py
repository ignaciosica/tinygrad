...
_libraries = {}
_libraries["libusb"] = (
  None if (lib_path := os.getenv("LIBUSB_PATH", ctypes.util.find_library("usb-1.0"))) is None else ctypes.CDLL(lib_path)
)  #  ctypes.CDLL('libusb')
...
libusb_hotplug_callback_fn = ctypes.CFUNCTYPE(
  ctypes.c_int32, ctypes.POINTER(struct_libusb_context), ctypes.POINTER(struct_libusb_device), c__EA_libusb_hotplug_event, ctypes.POINTER(None)
)
try:
