...
class Structure(ctypes.Structure, AsDictMixin):
  ...
  def bind(cls, bound_fields):
    ...
    if len(bound_fields) != 0:
      raise ValueError("Cannot bind the following unknown callback(s) {}.{}".format(cls.__name__, bound_fields.keys()))
    return cls(**fields)
...
try:
  nir_op_infos = (struct_nir_op_info * 489).in_dll(_libraries["libtinymesa_cpu.so"], "nir_op_infos")
...
try:
  nir_intrinsic_index_names = (ctypes.POINTER(ctypes.c_char) * 75).in_dll(_libraries["libtinymesa_cpu.so"], "nir_intrinsic_index_names")
except (AttributeError, ValueError):
  pass
...
try:
  nir_intrinsic_infos = (struct_nir_intrinsic_info * 732).in_dll(_libraries["libtinymesa_cpu.so"], "nir_intrinsic_infos")
except (AttributeError, ValueError):
  pass
try:
...
try:
  ...
  nir_sort_variables_with_modes.argtypes = [
    ctypes.POINTER(struct_nir_shader),
    ctypes.CFUNCTYPE(ctypes.c_int32, ctypes.POINTER(struct_nir_variable), ctypes.POINTER(struct_nir_variable)),
    nir_variable_mode,
  ]
except AttributeError:
  pass
...
nir_lower_instr_cb = ctypes.CFUNCTYPE(
  ctypes.POINTER(struct_nir_def), ctypes.POINTER(struct_nir_builder), ctypes.POINTER(struct_nir_instr), ctypes.POINTER(None)
)
try:
...
nir_lower_non_uniform_src_access_callback = ctypes.CFUNCTYPE(
  ctypes.c_bool, ctypes.POINTER(struct_nir_tex_instr), ctypes.c_uint32, ctypes.POINTER(None)
)
...
nir_combine_barrier_cb = ctypes.CFUNCTYPE(
  ctypes.c_bool, ctypes.POINTER(struct_nir_intrinsic_instr), ctypes.POINTER(struct_nir_intrinsic_instr), ctypes.POINTER(None)
)
try:
...
struct_nir_opt_preamble_options._fields_ = [
  ...
  (
    "def_size",
    ctypes.CFUNCTYPE(
      None, ctypes.POINTER(struct_nir_def), ctypes.POINTER(ctypes.c_uint32), ctypes.POINTER(ctypes.c_uint32), ctypes.POINTER(c__EA_nir_preamble_class)
    ),
  ),
...
nir_intrinsic_pass_cb = ctypes.CFUNCTYPE(
  ctypes.c_bool, ctypes.POINTER(struct_nir_builder), ctypes.POINTER(struct_nir_intrinsic_instr), ctypes.POINTER(None)
)
...
struct_lp_build_sampler_soa._fields_ = [
  (
    "emit_tex_sample",
    ctypes.CFUNCTYPE(
      None, ctypes.POINTER(struct_lp_build_sampler_soa), ctypes.POINTER(struct_gallivm_state), ctypes.POINTER(struct_lp_sampler_params)
    ),
  ),
  (
    "emit_size_query",
    ctypes.CFUNCTYPE(
      None, ctypes.POINTER(struct_lp_build_sampler_soa), ctypes.POINTER(struct_gallivm_state), ctypes.POINTER(struct_lp_sampler_size_query_params)
    ),
  ),
]
...
struct_lp_build_gs_iface._fields_ = [
  ...
  (
    "gs_epilogue",
    ctypes.CFUNCTYPE(
      None, ctypes.POINTER(struct_lp_build_gs_iface), ctypes.POINTER(struct_LLVMOpaqueValue), ctypes.POINTER(struct_LLVMOpaqueValue), ctypes.c_uint32
    ),
  ),
]
...
struct_lp_build_image_soa._fields_ = [
  (
    "emit_op",
    ctypes.CFUNCTYPE(None, ctypes.POINTER(struct_lp_build_image_soa), ctypes.POINTER(struct_gallivm_state), ctypes.POINTER(struct_lp_img_params)),
  ),
  (
    "emit_size_query",
    ctypes.CFUNCTYPE(
      None, ctypes.POINTER(struct_lp_build_image_soa), ctypes.POINTER(struct_gallivm_state), ctypes.POINTER(struct_lp_sampler_size_query_params)
    ),
  ),
]
...
lvp_nir_options = gzip.decompress(
  base64.b64decode("H4sIAAAAAAAAA2NgZGRkYGAAkYxgCsQFsxigwgwQBoxmhCqFq2WEKwIrAEGIkQxoAEMALwCqVsCiGUwLMHA0QPn29nBJkswHANb8YpH4AAAA")
)


def __getattr__(nm):
  raise (
    AttributeError("LLVMpipe requires tinymesa_cpu" if "tinymesa_cpu" not in dll._name else f"attribute {nm} not found")
    if dll
    else FileNotFoundError(f"libtinymesa not found (MESA_PATH={BASE}). See https://github.com/sirhcm/tinymesa (tinymesa-32dc66c, mesa-25.2.4)")
  )
