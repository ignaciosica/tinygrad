...
class HCQGraph(MultiGraphRunner):
  def __init__(self, jit_cache: list[ExecItem], input_rawbuffers: list[Buffer], var_vals: dict[str, int]):
    ...
    self.kernargs_bufs: dict[Compiled, HCQBuffer] = {
      d: d.allocator._alloc(max(sz, 1), BufferSpec(cpu_access=True)) for d, sz in kernargs_size.items()
    }
    ...
    self.signals: dict[Any, HCQSignal] = {
      ...
      **{dev: self.devices[0].new_signal(value=0) for dev in self.devices if dev._is_cpu()},
    }
    ...
    for j, ji in enumerate(jit_cache):
      if is_exec_prg := isinstance(ji.prg, CompiledRunner):
        ...
        for b in cast(list[Buffer], ji.bufs[::-1]):
          if (enqueue_dev := cast(HCQCompiled, Device[b.device])).hw_copy_queue_t is not None:
            break
    ...
    for dev in self.devices:
      self.comp_queues[dev].memory_barrier().wait(self.virt_timeline_signals[dev], self.virt_timeline_vals[dev]).wait(
        self.signals["KICK"], self.kickoff_var
      ).signal(self.signals[dev], self.kickoff_var)
