...
class MetalGraph(GraphRunner):
  def __init__(
    ...
    if not all(isinstance(ji.prg, CompiledRunner) for ji in jit_cache):
      raise GraphException

    # create metal batch exec
    icb_descriptor = msg("new", objc_instance)(
      libobjc.objc_getClass(b"MTLIndirectCommandBufferDescriptor")
    )
    msg("setCommandTypes:")(
      icb_descriptor, MTLIndirectCommandType.MTLIndirectCommandTypeConcurrentDispatch
    )
    ...
    self.icb = msg(
      "newIndirectCommandBufferWithDescriptor:maxCommandCount:options:", objc_instance
    )(
      self.dev.sysdevice,
      icb_descriptor,
      len(jit_cache),
    ...
    if self.icb.value is None:
      raise GraphException(
        "create indirect command buffer failed, does your system support this?"
      )
    icb_label = bytes(
      msg("UTF8String", ctypes.c_char_p)(msg("description", objc_instance)(self.icb))
    ).decode()
    self.needs_icb_fix = int(
      (m := re.search(r"AGXG(\d+)XFamily", icb_label)) is None or int(m.group(1)) < 15
    )  # not required on M3+

    self.fixedvars = merge_dicts([ji.fixedvars for ji in jit_cache])
    self.varlist = self.vars + list(self.fixedvars.keys())
    if len(self.varlist):
      self.int_buf = self.dev.allocator.alloc(len(self.varlist) * dtypes.int32.itemsize)
    ...
    for j, ji in enumerate(jit_cache):
      ...
      for i, b in enumerate(ji.bufs):
        if b is not None and b not in input_rawbuffers:
          msg("setKernelBuffer:offset:atIndex:")(
            icb_command, b._buf.buf, b._buf.offset, i
          )
      ...
      for i, v in enumerate(prg.p.vars):
        msg("setKernelBuffer:offset:atIndex:")(
          icb_command,
          self.int_buf.buf,
          self.varlist.index(v.expr) * 4,
          len(ji.bufs) + i,
        )

      global_size, local_size = prg.p.launch_dims(var_vals)
      msg("concurrentDispatchThreadgroups:threadsPerThreadgroup:")(
        icb_command, to_struct(*global_size), to_struct(*local_size)
      )
    ...
    if len(self.varlist):
      self.int_buf_view = self.dev.allocator._as_buffer(self.int_buf).cast("i")
    for var in self.fixedvars:
      self.int_buf_view[self.varlist.index(var)] = self.fixedvars[var]
  ...
  def __call__(
    ...
    if (
      ...
      and self.command_buffer in self.dev.mtl_buffers_in_flight
    ):
      wait_check(self.command_buffer)
    # NOTE: old command buffer may not be inflight anymore
    if self.command_buffer is not None and PROFILE:
      self.collect_timestamps()

    all_resources = dedup(
      self.all_resources
      + [
        input_rawbuffers[input_idx]._buf.buf
    ...
    for (j, i), input_idx in self.input_replace.items():
      ...
      msg("setKernelBuffer:offset:atIndex:")(
        computeCommand,
        input_rawbuffers[input_idx]._buf.buf,
        input_rawbuffers[input_idx]._buf.offset,
        i,
      )
    ...
    for j, global_dims, local_dims in self.updated_launch_dims(var_vals):
      computeCommand = msg("indirectComputeCommandAtIndex:", objc_id)(self.icb, j)
      msg("concurrentDispatchThreadgroups:threadsPerThreadgroup:")(
        computeCommand, to_struct(*global_dims), to_struct(*local_dims)
      )
    for var in self.vars:
      self.int_buf_view[self.varlist.index(var)] = var_vals[var]
    ...
    msg("useResources:count:usage:")(
      encoder,
      (objc_id * len(all_resources))(*all_resources),
      len(all_resources),
    ...
    if getenv("FIX_METAL_ICB", self.needs_icb_fix):
      for ps in self.all_pipelines:
        ...
        msg("dispatchThreadgroups:threadsPerThreadgroup:")(
          encoder, to_struct(0, 0, 0), to_struct(0, 0, 0)
        )
  ...
  def collect_timestamps(self):
    ...
    st, en = (
      decimal.Decimal(cmdbuf_st_time(self.command_buffer)) * 1000000,
      decimal.Decimal(cmdbuf_en_time(self.command_buffer)) * 1000000,
    )
    ents = [
      ProfileGraphEntry(
        self.device, cast(CompiledRunner, ji.prg)._prg.name, i, i + 1, is_copy=False
      )
      for i, ji in enumerate(self.jit_cache)
    ]
    step = (en - st) / len(ents)
    self.dev.profile_events += [
      ProfileGraphEvent(ents, [], [st + step * i for i in range(len(ents) + 1)])
    ]
