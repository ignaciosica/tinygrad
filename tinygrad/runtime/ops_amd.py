...
class AMDComputeQueue(HWQueue):
  ...
  def set_grbm(self, instance=None, se=None, sh=None, wgp=None):
    ...
    self.wreg(
      ...
      **{
        (f"{key}_broadcast_writes" if val is None else f"{key}_index"): (1 if val is None else val)
  ...
  def pmc_reset_counters(self, en=True):
    ...
    if en:
      self.wreg(self.gc.regCP_PERFMON_CNTL if self.dev.target[0] <= 11 else self.gc.regCP_PERFMON_CNTL_1, perfmon_state=1)
    return self

  def pmc_start(self, counters):
    self.pmc_reset_counters(en=False)
    self.wreg(
      self.gc.regSQ_PERFCOUNTER_CTRL, cs_en=1, ps_en=1, gs_en=1, hs_en=1, **({"vmid_mask": 0xFFFF} if (gfx9 := self.dev.target[0] == 9) else {})
    )
    ...
    for name, block, idx in counters:
      ...
      inst_cnt, se_cnt, sa_cnt, wgp_cnt = {
        ...
        "GL2C": (32, 1, 1, 1),
        "TCC": (16, 1, 1, 1),
        "SQ": (1, self.dev.se_cnt // self.dev.xccs) + ((1, 1) if gfx9 else (2, self.dev.iface.props["cu_per_simd_array"] // 2)),
      }[block]
      ...
      if (
        regsel := getattr(self.gc, (f"reg{block}_PERFCOUNTER{(pcid * 2) if self.dev.target[0] >= 11 and block == 'SQ' else pcid}_SELECT"), None)
      ) is None:
      ...
      for xcc in range(s.xcc):
        with self.pred_exec(xcc_mask=1 << xcc):
          for inst, se_idx, sa_idx, wgp_idx in itertools.product(range(s.inst), range(s.se), range(s.sa), range(s.wgp)):
            self.set_grbm(
              **({"instance": inst} if s.inst > 1 else ({"se": se_idx} | ({"sh": sa_idx, "wgp": wgp_idx} if self.dev.target[0] != 9 else {})))
            )
            ...
            if hi is not None:
              self.pkt3(self.pm4.PACKET3_COPY_DATA, (2 << 8) | 4, hi.addr[0], 0, *data64_le(buf.va_addr + loff + 4))
  ...
  def sqtt_start(self, buf0s: list[HCQBuffer], se_mask: int):
    ...
    if self.dev.target[0] == 9:
      ...
      for se in range(len(buf0s)):
        ...
        if (se_mask >> se) & 0b1:
          mask |= (__SQTTINST := 1 << 10) | (__SQTT_INST_PC := 1 << 11) | (__SQTT_ISSUE := 1 << 13)
...
class AMDProgram(HCQProgram):
  ...
  def __call__(
    ...
    if self.dev.pmc_enabled:
      cast(AMDComputeQueue, self.dev.hw_compute_queue_t()).pmc_read(self.dev.pmc_buffer, self.dev.pmc_sched).signal(
...
class PCIIface(PCIIfaceBase):
  ...
  def _setup_adev(self, pci_dev: PCIDevice, dma_regions: list[tuple[int, MMIOInterface]] | None = None):
    ...
    self.props = {
      "cu_per_simd_array": (cu_per_sa := 2 * (self.dev_impl.gc_info.gc_num_wgp0_per_sa + self.dev_impl.gc_info.gc_num_wgp1_per_sa)),
  ...
  def __init__(self, device: str = ""):
    ...
    if self.pmc_enabled:
      ...
      if not self.iface.is_in_profile_mode():
        raise RuntimeError("PMC requires stable power state: run `amd-smi set -l stable_std` for KFD iface")
      ...
      for k in (PMC_COUNTERS := getenv("PMC_COUNTERS", pmc_default).split(",")):
        if k not in self.pmc_counters:
          raise RuntimeError(f"PMC counter {k} is not supported. Available: {','.join(self.pmc_counters.keys())}")
  ...
  def _at_profile_finalize(self):
    if self.sqtt_enabled:
      ...
      for i, buf0 in enumerate(self.sqtt_buffers):
        ...
        if self.target[0] == 9:
          sqtt_buf = memoryview(struct.pack("<Q", 0x11 | (4 << 13) | (0xF << 16) | (i << 24)) + sqtt_buf)
