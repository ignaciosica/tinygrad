...
cl_errors = {
  attr: k
  for k in dir(cl)
  if k.startswith("CL_") and isinstance(attr := getattr(cl, k), int) and attr <= 0
}
...
def check(status):
  if status != 0:
    raise RuntimeError(
      f"OpenCL Error {status}: {cl_errors.get(status, 'Unknown error')}"
    )
  ...
  def compile(self, src: str) -> bytes:
    program = checked(
      cl.clCreateProgramWithSource(
        self.dev.context,
        1,
        to_char_p_p([src.encode()]),
        None,
        status := ctypes.c_int32(),
      ),
      status,
    )
    build_status: int = cl.clBuildProgram(
      program, 1, self.dev.device_id, None, cl.clBuildProgram.argtypes[4](), None
    )
    if build_status != 0:
    ...
    check(
      cl.clGetProgramInfo(
        ...
        (ctypes.c_void_p * 1)(
          ctypes.addressof(binary := ctypes.create_string_buffer(binary_sizes[0]))
        ),
        None,
      )
    )
...
class CLProgram:
  def __init__(self, device: CLDevice, name: str, lib: bytes):
    ...
    self.program = checked(
      cl.clCreateProgramWithBinary(
        ...
        binary_status := ctypes.c_int32(),
        errcode_ret := ctypes.c_int32(),
      ),
      errcode_ret,
    )
    ...
    check(
      cl.clBuildProgram(
        self.program, 1, device.device_id, None, cl.clBuildProgram.argtypes[4](), None
      )
    )  # NOTE: OSX requires this
    self.kernel = checked(
      cl.clCreateKernel(self.program, name.encode(), status := ctypes.c_int32()), status
    )
  ...
  def __call__(
    self,
    *bufs: tuple[ctypes._CData, BufferSpec],
    ...
    for i, v in enumerate(vals, start=len(bufs)):
      check(cl.clSetKernelArg(self.kernel, i, 4, ctypes.byref(ctypes.c_int32(v))))
    if local_size is not None:
      global_size = cast(
        tuple[int, int, int], tuple(int(g * l) for g, l in zip(global_size, local_size))
      )
    ...
    check(
      cl.clEnqueueNDRangeKernel(
        ...
        (ctypes.c_size_t * len(local_size))(*local_size) if local_size else None,
        0,
        None,
        event,
      )
    ...
    if wait:
      ...
      check(
        cl.clGetEventProfilingInfo(
          ...
          8,
          ctypes.byref(start := ctypes.c_uint64()),
          None,
        )
      )
      check(
        cl.clGetEventProfilingInfo(
          ...
          8,
          ctypes.byref(end := ctypes.c_uint64()),
          None,
        )
      )
...
class CLAllocator(LRUAllocator["CLDevice"]):
  def _alloc(self, size: int, options: BufferSpec) -> tuple[ctypes._CData, BufferSpec]:
      ...
      return (
        checked(
          cl.clCreateImage2D(
            ...
            cl.cl_image_format(
              cl.CL_RGBA, {2: cl.CL_HALF_FLOAT, 4: cl.CL_FLOAT}[options.image.itemsize]
            ),
    ...
    return (
      checked(
        cl.clCreateBuffer(
          self.dev.context, cl.CL_MEM_READ_WRITE, size, None, status := ctypes.c_int32()
        ),
        status,
      ),
      options,
  ...
  def _free(self, opaque: tuple[ctypes._CData, BufferSpec], options: BufferSpec):
    check(cl.clReleaseMemObject(opaque[0]))
  ...
  def _copyin(self, dest: tuple[ctypes._CData, BufferSpec], src: memoryview):
    if dest[1].image is not None:
      check(
        cl.clEnqueueWriteImage(
          ...
          dest[0],
          False,
          (ctypes.c_size_t * 3)(0, 0, 0),
          (ctypes.c_size_t * 3)(dest[1].image.shape[1], dest[1].image.shape[0], 1),
          0,
          0,
          from_mv(src),
          0,
      ...
      if mv_address(src) % 16:
        src = memoryview(bytearray(src))
      check(
    ...
    self.dev.pending_copyin.append(
      src
    ...
    if src[1].image is not None:
      check(
        cl.clEnqueueReadImage(
          ...
          False,
          (ctypes.c_size_t * 3)(0, 0, 0),
          (ctypes.c_size_t * 3)(src[1].image.shape[1], src[1].image.shape[0], 1),
          0,
          0,
          from_mv(dest),
          0,
...
class CLDevice(Compiled):
  ...
  def __init__(self, device: str = ""):
    if CLDevice.device_ids is None:
      ...
      for device_type in [cl.CL_DEVICE_TYPE_GPU, cl.CL_DEVICE_TYPE_DEFAULT]:
        err = cl.clGetDeviceIDs(
          platform_ids[0], device_type, 0, None, num_devices := ctypes.c_uint32()
        )
      ...
      CLDevice.device_ids = init_c_var(
        ...
        lambda x: check(
          cl.clGetDeviceIDs(platform_ids[0], device_type, num_devices, x, None)
        ),
      )
    ...
    if DEBUG >= 1:
      print(f"CLDevice: opening {self.device_name} with version {self.driver_version}")
    ...
    self.queue = checked(
      cl.clCreateCommandQueue(
        self.context, self.device_id, cl.CL_QUEUE_PROFILING_ENABLE, status
      ),
      status,
    )
    ...
    self.device_exts = (
      ...
      ctypes.string_at(buf, size=total.value).decode(),
    )[1]

    compilers = [
      (
        IntelRenderer
        if "cl_intel_subgroup_matrix_multiply_accumulate" in self.device_exts
        else OpenCLRenderer,
        functools.partial(
          CLCompiler,
          self,
          f"compile_cl_{hashlib.md5(self.device_name.encode() + self.driver_version.encode()).hexdigest()}",
        ),
      )
    ]
    super().__init__(
      device, CLAllocator(self), compilers, functools.partial(CLProgram, self)
    )
