...
class CLProgram:
  def __init__(self, device: CLDevice, name: str, lib: bytes):
    ...
    self.kernel = checked(
      cl.clCreateKernel(self.program, name.encode(), status := ctypes.c_int32()),
      status,
    )
  ...
  def __call__(
    ...
    *bufs: tuple[ctypes._CData, BufferSpec],
    ...
    if local_size is not None:
      global_size = cast(
        tuple[int, int, int],
        tuple(int(g * l) for g, l in zip(global_size, local_size)),
      )
...
class CLAllocator(LRUAllocator["CLDevice"]):
  def _alloc(
    self, size: int, options: BufferSpec
  ) -> tuple[ctypes._CData, BufferSpec]:
      ...
      return (
        checked(
          cl.clCreateImage2D(
            ...
            cl.cl_image_format(
              cl.CL_RGBA,
              {2: cl.CL_HALF_FLOAT, 4: cl.CL_FLOAT}[options.image.itemsize],
            ),
...
class CLDevice(Compiled):
  ...
  def __init__(self, device: str = ""):
    ...
    if DEBUG >= 1:
      print(
        f"CLDevice: opening {self.device_name} with version {self.driver_version}"
      )
