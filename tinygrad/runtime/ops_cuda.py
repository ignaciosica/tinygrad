...
def check(status):
  if status != 0:
    error = ctypes.string_at(
      init_c_var(
        ctypes.POINTER(ctypes.c_char)(),
        lambda x: cuda.cuGetErrorString(status, ctypes.byref(x)),
      )
    ).decode()
  ...
  def __init__(self, dev: CUDADevice, name: str, lib: bytes, smem: int = 0):
    ...
    if DEBUG >= 5:
      print(
        "\n".join([
          f"{i + 1:>3} {line}"
          for i, line in enumerate(pretty_ptx(lib.decode("utf-8")).split("\n"))
        ])
      )
    ...
    check(
      cuda.cuModuleGetFunction(
        ctypes.byref(prg := cuda.CUfunction()), self.module, name.encode("utf-8")
      )
    )
    self.prg = prg
    if self.smem > 0:
      check(
        cuda.cuFuncSetAttribute(
          self.prg, cuda.CU_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES, self.smem
        )
      )
  ...
  def __call__(
    ...
    *args,
    ...
    return cu_time_execution(
      lambda: check(
        cuda.cuLaunchKernel(
          self.prg, *global_size, *local_size, self.smem, None, None, self.vargs
        )
      ),
      enable=wait,
    )
...
class CUDAAllocator(LRUAllocator["CUDADevice"]):
  def _alloc(self, size, options: BufferSpec):
    ...
    return init_c_var(
      cuda.CUdeviceptr(), lambda x: check(cuda.cuMemAlloc_v2(ctypes.byref(x), size))
    )
...
class CUDADevice(Compiled):
  ...
  def __init__(self, device: str):
    ...
    self.cu_device = init_c_var(
      cuda.CUdevice(), lambda x: check(cuda.cuDeviceGet(ctypes.byref(x), device_id))
    )
    ...
    compilers: list[CompilerPairT] = [
      (
        functools.partial(CUDARenderer, self.arch),
        functools.partial(CUDACompiler, self.arch),
      ),
      (
        functools.partial(PTXRenderer, self.arch),
        functools.partial(PTXCompiler, self.arch),
      ),
      (
        functools.partial(CUDARenderer, self.arch),
        functools.partial(NVCCCompiler, self.arch),
      ),
    ]
    super().__init__(
      device,
