...
dsp_pm = PatternMatcher([
  ...
  (
    ...
    lambda x: UOp(
      ...
      x.dtype,
      x.src + x.src,
      "__builtin_shufflevector({0}, {1}, " + ",".join([str(y) for y in x.arg]) + ")",
    )
...
dsp_pm_late = PatternMatcher([
  (
    ...
    lambda x, y: x + UOp(Ops.CUSTOMI, x.dtype, (y,), arg="{0}")
    if x.op is not Ops.CUSTOMI
    else None,
  ),
  (
    UPat.var("x") * UPat(Ops.VECTORIZE, src=UPat.var("y")),
  ...
  (
    ...
    lambda d: d.replace(
      src=(UOp(Ops.CUSTOMI, d.dtype, arg="__builtin_HEXAGON_V6_vd0_128B()"),)
      + d.src[1:]
    ),
  ),
])
...
dsp_string = PatternMatcher([
  (
    UPat(Ops.CONST, (dtypes.int8, dtypes.uint8), name="x"),
    lambda ctx, x: str(x.arg),
  ),
])
...
class DSPRenderer(ClangRenderer):
  ...
  def _render_entry(
    ...
    msrc += [
      f"int sz_or_val_{i} = ((int*)pra[0].buf.pv)[{i}];" for i, b in enumerate(bufs)
    ]
    msrc += [
...
class DSPDevice(Compiled):
  def __init__(self, device: str = ""):
    ...
    if getenv("MOCKDSP"):
      ...
      super().__init__(
        device, DSPAllocator(self), compilers, functools.partial(DSPProgram, self)
      )
  ...
  def exec_lib(self, lib, sc, args, fds, attrs):
    def _exec_lib():
      ...
      qcom_dsp.FASTRPC_IOCTL_INVOKE_ATTRS(
        ...
        fds=fds,
        attrs=attrs,
        inv=qcom_dsp.struct_fastrpc_ioctl_invoke(handle=handle, sc=sc, pra=args),
      )
  ...
  def run(self):
    ...
    while True:
      ...
      if sc == 0x20200:
        ...
        if (fd := in_args[0].cast("I")[0]) == TINYFD:
          assert in_args[0].cast("I")[2] == qcom_dsp.APPS_STD_SEEK_SET, (
  ...
  def __call__(self, *bufs, vals: tuple[int, ...] = (), wait=False):
    with tempfile.NamedTemporaryFile(suffix=".out") as dsp_lib:
      ...
      proc = subprocess.run(
        ...
        input=b"".join(
          [bytes(to_mv(x.va_addr, x.size)) for x in bufs]
          + [struct.pack("I", x) for x in vals]
        ),
