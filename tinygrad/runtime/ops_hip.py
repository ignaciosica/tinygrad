...
class HIPDevice(Compiled):
  def __init__(self, device: str = ""):
    ...
    self.arch = init_c_var(
      hip.hipDeviceProp_t(),
      lambda x: check(hip.hipGetDeviceProperties(x, self.device_id)),
    ).gcnArchName.decode()
    self.time_event_st, self.time_event_en = [
      init_c_var(hip.hipEvent_t(), lambda x: hip.hipEventCreate(ctypes.byref(x), 0))
      for _ in range(2)
    ]

    compilers = [
      (
        functools.partial(HIPRenderer, self.arch),
        functools.partial(HIPCompiler, self.arch),
      )
    ]
    super().__init__(
      device, HIPAllocator(self), compilers, functools.partial(HIPProgram, self)
    )
...
class HIPProgram:
  def __init__(self, dev: HIPDevice, name: str, lib: bytes):
    ...
    self.module = init_c_var(
      hip.hipModule_t(), lambda x: check(hip.hipModuleLoadData(ctypes.byref(x), lib))
    )
    self.prg = init_c_var(
      hip.hipFunction_t(),
      lambda x: check(
        hip.hipModuleGetFunction(ctypes.byref(x), self.module, name.encode("utf-8"))
      ),
    )
    ...
    *args,
    ...
    if not hasattr(self, "vargs"):
      ...
      self.vargs = (ctypes.c_void_p * 5)(
        ...
        2,
        ctypes.cast(
          ctypes.pointer(ctypes.c_size_t(ctypes.sizeof(self.c_args))), ctypes.c_void_p
        ),
        3,
      )
    ...
    if wait:
      check(hip.hipEventRecord(self.dev.time_event_st, None))

    check(
      hip.hipModuleLaunchKernel(
        self.prg, *global_size, *local_size, 0, None, None, self.vargs
      )
    )
    ...
    if wait:
      ...
      check(
        hip.hipEventElapsedTime(
          ctypes.byref(ret := ctypes.c_float()),
          self.dev.time_event_st,
          self.dev.time_event_en,
        )
      )
