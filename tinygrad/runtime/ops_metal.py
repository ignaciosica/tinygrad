...
class objc_instance(
  objc_id
):  # method with name "new", "alloc" should be freed after use
  def __del__(self):
    ...
    if msg is not None and libobjc is not None:
      msg("release")(self)
...
def msg(selector: str, restype: type[T] = objc_id):  # type: ignore [assignment]
  ...
  def _msg(ptr: objc_id, *args: Any) -> T:
    return sender(ptr, resname, *args)
...
@functools.cache
def to_ns_str(s: str):
  return msg("stringWithUTF8String:", objc_instance)(
    libobjc.objc_getClass(b"NSString"), s.encode()
  )


def from_ns_str(s):
  return bytes(msg("UTF8String", ctypes.c_char_p)(s)).decode()
...
def cmdbuf_label(cbuf: objc_id) -> str | None:
  return (
    from_ns_str(label)
    if (label := msg("label", objc_id)(cbuf)).value is not None
    else None
  )


def cmdbuf_st_time(cbuf: objc_id) -> float:
  return cast(float, msg("GPUStartTime", ctypes.c_double)(cbuf))


def cmdbuf_en_time(cbuf: objc_id) -> float:
  return cast(float, msg("GPUEndTime", ctypes.c_double)(cbuf))


def error_check(
  error: objc_instance, error_constructor: type[Exception] = RuntimeError
):
  if error.value is None:
    return None
  raise error_constructor(
    from_ns_str(msg("localizedDescription", objc_instance)(error))
  )
...
class MetalDevice(Compiled):
  def __init__(self, device: str):
    ...
    self.mtl_queue = msg("newCommandQueueWithMaxCommandBufferCount:", objc_instance)(
      self.sysdevice, 1024
    )
    if self.mtl_queue is None:
      raise RuntimeError("Cannot allocate a new command queue")
    ...
    super().__init__(
      device,
      MetalAllocator(self),
      [(MetalRenderer, MetalCompiler), (MetalRenderer, Compiler)],
      functools.partial(MetalProgram, self),
      MetalGraph
      if "virtual" not in from_ns_str(msg("name")(self.sysdevice)).lower()
      else None,
    )
  ...
  def synchronize(self):
    for cbuf in self.mtl_buffers_in_flight:
      wait_check(cbuf)
      st, en = (
        decimal.Decimal(cmdbuf_st_time(cbuf)) * 1000000,
        decimal.Decimal(cmdbuf_en_time(cbuf)) * 1000000,
      )
      # NOTE: command buffers from MetalGraph are not profiled here
      if (
        PROFILE
        and (lb := cmdbuf_label(cbuf)) is not None
        and not lb.startswith("batched")
      ):
        Compiled.profile_events += [
          ProfileRangeEvent(self.device, lb, st, en, is_copy=lb.startswith("COPY"))
        ]
...
def metal_src_to_library(device: MetalDevice, src: str) -> objc_instance:
  ...
  library = msg("newLibraryWithSource:options:error:", objc_instance)(
    device.sysdevice,
    to_ns_str(src),
    options,
    ctypes.byref(compileError := objc_instance()),
  )
...
class MetalCompiler(Compiler):
  ...
  def __init__(self):
    self.cgs = ctypes.c_void_p(
      MetalCompiler.support.MTLCodeGenServiceCreate(b"tinygrad")
    )
  ...
  def __reduce__(self):
    return (
      MetalCompiler,
      (),
    )  # force pickle to create new instance for each multiprocessing fork

  def compile(self, src: str) -> bytes:
    ret: Exception | bytes = CompileError(
    ...
    src_padded, params_padded = (
      src.encode() + b"\x00" * (round_up(len(src) + 1, 4) - len(src)),
      params.encode() + b"\x00",
    )
    request = (
      struct.pack("<QQ", len(src_padded), len(params_padded))
      + src_padded
      + params_padded
    )
    ...
    MetalCompiler.support.MTLCodeGenServiceBuildRequest(
      ...
      request,
      len(request),
      ctypes.byref(callback, -0x10),
    )
    if isinstance(ret, Exception):
      raise ret
    assert ret[:4] == b"MTLB" and ret[-4:] == b"ENDT", (
      f"Invalid Metal library. {ret!r}"
    )
    return ret
  ...
  def disassemble(self, lib: bytes):
    with tempfile.NamedTemporaryFile(delete=True) as shader:
      ...
      proc = subprocess.Popen(
        f"cd {pathlib.Path(__file__).parents[2]}/extra/disassemblers/applegpu && python3 compiler_explorer.py {shader.name}",
        stdout=subprocess.PIPE,
        shell=True,
        text=True,
        bufsize=1,
      )
      for line in unwrap(proc.stdout):
        print(line, end="")
      ret = proc.wait()
      if ret:
        print(
          "Disassembler Error: Make sure you have https://github.com/dougallj/applegpu cloned to tinygrad/extra/disassemblers/applegpu"
        )
...
class MetalProgram:
  def __init__(self, dev: MetalDevice, name: str, lib: bytes):
    ...
    if lib[:4] == b"MTLB":
      ...
      self.library = msg("newLibraryWithData:error:", objc_instance)(
        self.dev.sysdevice, data, ctypes.byref(error_lib := objc_instance())
      )
    ...
    self.fxn = msg("newFunctionWithName:", objc_instance)(
      self.library, to_ns_str(name)
    )
    descriptor = msg("new", objc_instance)(
      libobjc.objc_getClass(b"MTLComputePipelineDescriptor")
    )
    ...
    self.pipeline_state = msg(
      ...
      self.dev.sysdevice,
      descriptor,
      MTLPipelineOption.MTLPipelineOptionNone,
      None,
      ctypes.byref(error_pipeline_creation := objc_instance()),
    )
    error_check(error_pipeline_creation)
    # cache these msg calls
    self.max_total_threads: int = cast(
      int, msg("maxTotalThreadsPerThreadgroup", ctypes.c_ulong)(self.pipeline_state)
    )
  ...
  def __call__(
    ...
    *bufs,
    ...
    if prod(local_size) > self.max_total_threads:
      ...
      memory_length = msg("staticThreadgroupMemoryLength", ctypes.c_ulong)(
        self.pipeline_state
      )
      raise RuntimeError(
        f"local size {local_size} bigger than {self.max_total_threads} with exec width {exec_width} memory length {memory_length}"
      )
    ...
    for i, a in enumerate(bufs):
      msg("setBuffer:offset:atIndex:")(encoder, a.buf, a.offset, i)
    for i, a in enumerate(vals, start=len(bufs)):
      msg("setBytes:length:atIndex:")(encoder, bytes(ctypes.c_int(a)), 4, i)
    msg("dispatchThreadgroups:threadsPerThreadgroup:")(
      encoder, to_struct(*global_size), to_struct(*local_size)
    )
    msg("endEncoding")(encoder)
    msg("setLabel:")(
      command_buffer, to_ns_str(self.name)
...
class MetalAllocator(LRUAllocator[MetalDevice]):
  def _alloc(self, size: int, options) -> MetalBuffer:
    if options.external_ptr:
      return MetalBuffer(objc_id(options.external_ptr), size)

    # Buffer is explicitly released in _free() rather than garbage collected via reference count
    ret = msg("newBufferWithLength:options:", objc_id)(
      self.dev.sysdevice,
      ctypes.c_ulong(size),
  ...
  def _transfer(
    ...
    msg("copyFromBuffer:sourceOffset:toBuffer:destinationOffset:size:")(
      encoder,
      src.buf,
      ctypes.c_ulong(src.offset),
      dest.buf,
      ctypes.c_ulong(dest.offset),
      ctypes.c_ulong(sz),
    )
    msg("endEncoding")(encoder)
    if src_dev != dest_dev:
      msg("encodeSignalEvent:value:")(
        src_command_buffer, src_dev.timeline_signal, src_dev.timeline_value
      )
      dest_command_buffer = msg("commandBuffer", objc_instance)(dest_dev.mtl_queue)
      msg("encodeWaitForEvent:value:")(
        dest_command_buffer, src_dev.timeline_signal, src_dev.timeline_value
      )
      msg("commit")(dest_command_buffer)
      dest_dev.mtl_buffers_in_flight.append(dest_command_buffer)
      src_dev.timeline_value += 1
    msg("setLabel:")(
      src_command_buffer, to_ns_str(f"COPY {src_dev.device} -> {dest_dev.device}")
    )
  ...
  def _cp_mv(self, dst, src, prof_desc):
    with cpu_profile(prof_desc, self.dev.device, is_copy=True):
      dst[:] = src

  def _as_buffer(self, src: MetalBuffer) -> memoryview:
    self.dev.synchronize()
    return to_mv(
      cast(int, msg("contents", objc_id)(src.buf).value), src.size + src.offset
    )[src.offset :]
