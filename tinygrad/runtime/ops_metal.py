...
@functools.cache
def to_ns_str(s: str):
  return msg("stringWithUTF8String:", objc_instance)(libobjc.objc_getClass(b"NSString"), s.encode())
...
def cmdbuf_label(cbuf: objc_id) -> str | None:
  return from_ns_str(label) if (label := msg("label", objc_id)(cbuf)).value is not None else None
...
class MetalDevice(Compiled):
  def __init__(self, device: str):
    ...
    super().__init__(
      ...
      MetalGraph if "virtual" not in from_ns_str(msg("name")(self.sysdevice)).lower() else None,
    )
...
def metal_src_to_library(device: MetalDevice, src: str) -> objc_instance:
  ...
  library = msg("newLibraryWithSource:options:error:", objc_instance)(
    device.sysdevice, to_ns_str(src), options, ctypes.byref(compileError := objc_instance())
  )
...
class MetalCompiler(Compiler):
  ...
  def __reduce__(self):
    return (MetalCompiler, ())  # force pickle to create new instance for each multiprocessing fork
  ...
  def disassemble(self, lib: bytes):
    with tempfile.NamedTemporaryFile(delete=True) as shader:
      ...
      ret = proc.wait()
      if ret:
        print("Disassembler Error: Make sure you have https://github.com/dougallj/applegpu cloned to tinygrad/extra/disassemblers/applegpu")
...
class MetalProgram:
  def __init__(self, dev: MetalDevice, name: str, lib: bytes):
    ...
    self.pipeline_state = msg("newComputePipelineStateWithDescriptor:options:reflection:error:", objc_instance)(
      self.dev.sysdevice, descriptor, MTLPipelineOption.MTLPipelineOptionNone, None, ctypes.byref(error_pipeline_creation := objc_instance())
    )
...
class MetalAllocator(LRUAllocator[MetalDevice]):
  ...
  def _transfer(self, dest: MetalBuffer, src: MetalBuffer, sz: int, src_dev: MetalDevice, dest_dev: MetalDevice):
    ...
    msg("copyFromBuffer:sourceOffset:toBuffer:destinationOffset:size:")(
      encoder, src.buf, ctypes.c_ulong(src.offset), dest.buf, ctypes.c_ulong(dest.offset), ctypes.c_ulong(sz)
    )
    msg("endEncoding")(encoder)
    if src_dev != dest_dev:
