...
class NpyAllocator(Allocator["NpyDevice"]):
  def _alloc(self, size: int, options=None) -> np.ndarray:
  ...
  def _copyout(self, dest: memoryview, src: np.ndarray):
    dest[:] = self._as_buffer(src)


class NpyDevice(Compiled):
  def __init__(self, device: str):
    super().__init__(device, NpyAllocator(self), None, None)
