...
class NullRenderer(CStyleLanguage):
  ...
  code_for_op = {
    **CStyleLanguage.code_for_op,
    Ops.THREEFRY: lambda a, b, dtype: f"threefry({a},{b})",
    Ops.MAX: lambda a, b, dtype: f"max({a},{b})",
  }


class NullProgram:
  def __init__(self, device: str, name: str, lib: bytes):
...
class NullAllocator(Allocator["NullDevice"]):
  ...
  def _transfer(self, dest, src, sz: int, src_dev, dest_dev):
    with cpu_profile(f"{src_dev.device} -> {dest_dev.device}", self.dev.device):
      pass

  def _offset(self, buf, offset: int, size: int):
    pass


class NullGraph(MultiGraphRunner):
  def __call__(self, input_rawbuffers, var_vals, wait=False) -> float | None:
    return 1e-1
...
class NullDevice(Compiled):
  def __init__(self, device: str):
    ...
    super().__init__(
      ...
      NullAllocator(self),
      [(renderer, Compiler)],
      functools.partial(NullProgram, device),
      NullGraph,
    )
