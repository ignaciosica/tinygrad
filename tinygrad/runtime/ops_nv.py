...
def make_uvm_type():
  return type(
    ...
    (object,),
    {
      name.replace("UVM_", "").lower(): functools.partial(uvm_ioctl, dt, getattr(nv_gpu, name + "_PARAMS"))
...
class QMD:
  ...
  def __init__(self, dev: NVDevice, addr: int | None = None, **kwargs):
    ...
    if (pref := "NVCEC0_QMDV05_00" if self.ver == 5 else "NVC6C0_QMDV03_00") not in QMD.fields:
      QMD.fields[pref] = {
        **{name[len(pref) + 1 :]: dt for name, dt in nv_gpu.__dict__.items() if name.startswith(pref) and isinstance(dt, tuple)},
        **{
...
class NVCommandQueue(HWQueue[HCQSignal, "NVDevice", "NVProgram", "NVArgsState"]):
  ...
  def __del__(self):
    if self.binded_device is not None:
      self.binded_device.allocator.free(self.hw_page, self.hw_page.size, BufferSpec(cpu_access=True, nolru=True))
...
class NVComputeQueue(NVCommandQueue):
  ...
  def exec(self, prg: NVProgram, args_state: NVArgsState, global_size: tuple[sint, ...], local_size: tuple[sint, ...]):
    ...
    self.bind_sints_to_mem(
      *global_size, mem=qmd_buf.cpu_view(), fmt="I", offset=qmd.field_offset("cta_raster_width" if qmd.ver < 4 else "grid_width")
    )
  ...
  def signal(self, signal: HCQSignal, value: sint = 0):
    ...
    self.nvm(
      0, nv_gpu.NVC56F_SEM_ADDR_LO, *data64_le(signal.value_addr), *data64_le(value), (1 << 0) | (1 << 20) | (1 << 24) | (1 << 25)
...
class NVProgram(HCQProgram):
  def __init__(self, dev: NVDevice, name: str, lib: bytes):
    ...
    if not NAK:
        ...
        if sh.name == f".text.{self.name}":
          ...
          for typ, param, data in self._parse_elf_info(sh):
            if sh.name == f".nv.info.{name}" and param == 0xA:
              ...
              self.lcmem_usage = struct.unpack_from("II", data)[1] + 0x240  # EIATTR_MIN_STACK_SIZE
      ...
      for apply_image_offset, rel_sym_offset, typ, _ in relocs:  # pylint: disable=possibly-used-before-assignment
        ...
        if typ == 2:
          image[apply_image_offset : apply_image_offset + 8] = struct.pack("<Q", self.lib_gpu.va_addr + rel_sym_offset)  # R_CUDA_64
        elif typ == 0x38:
  ...
  def __call__(
    self, *bufs, global_size: tuple[int, int, int] = (1, 1, 1), local_size: tuple[int, int, int] = (1, 1, 1), vals: tuple[int, ...] = (), wait=False
  ):
...
class NVKIface:
  ...
  def __init__(self, dev, device_id):
    if NVKIface.root is None:
      ...
      with contextlib.suppress(RuntimeError):
        uvm.mm_initialize(self.fd_uvm_2, uvmFd=self.fd_uvm.fd)  # this error is okay, CUDA hits it too
  ...
  def setup_usermode(self):
    clsinfo = self.rm_control(
      ...
      nv_gpu.NV0080_CTRL_GPU_GET_CLASSLIST_PARAMS(numClasses=100, classList=mv_address(classlist := memoryview(bytearray(100 * 4)).cast("I"))),
    )
  ...
  def setup_vm(self, vaspace):
    self.rm_control(
      ...
      raw_uuid := nv_gpu.NV2080_CTRL_GPU_GET_GID_INFO_PARAMS(flags=nv_gpu.NV2080_GPU_CMD_GPU_GET_GID_FLAGS_FORMAT_BINARY, length=16),
    )
    ...
    for dev in cast(list[NVDevice], [d for pg in HCQCompiled.peer_groups.values() for d in pg if isinstance(d, NVDevice) and not d.is_nvd()]):
      try:
        uvm.enable_peer_access(self.fd_uvm, gpuUuidA=self.gpu_uuid, gpuUuidB=dev.iface.gpu_uuid)
      except RuntimeError as e:
        raise RuntimeError(f"{e}. Make sure GPUs #{self.gpu_minor} & #{dev.iface.gpu_minor} have P2P enabled.") from e
  ...
  def _gpu_map_to_cpu(self, memory_handle, size, target=None, flags=0, system=False):
    ...
    made = nv_gpu.nv_ioctl_nvos33_parameters_with_fd(
      fd=fd_dev.fd, params=nv_gpu.NVOS33_PARAMETERS(hClient=self.root, hDevice=self.dev.nvdevice, hMemory=memory_handle, length=size, flags=flags)
    )
    nv_iowr(self.fd_ctl, nv_gpu.NV_ESC_RM_MAP_MEMORY, made)
    if made.params.status != 0:
      raise RuntimeError(f"_gpu_map_to_cpu returned {get_error_str(made.params.status)}")
  ...
  def alloc(self, size: int, host=False, uncached=False, cpu_access=False, contiguous=False, map_flags=0, cpu_addr=None, **kwargs) -> HCQBuffer:
    ...
    if host:
      va_addr = cpu_addr or FileIOInterface.anon_mmap(
        va_addr, size, mmap.PROT_READ | mmap.PROT_WRITE, MAP_FIXED | mmap.MAP_SHARED | mmap.MAP_ANONYMOUS, 0
      )
      ...
      attr = (
        ((nv_gpu.NVOS32_ATTR_PHYSICALITY_CONTIGUOUS if contiguous else nv_gpu.NVOS32_ATTR_PHYSICALITY_ALLOW_NONCONTIGUOUS) << 27)
        | (nv_gpu.NVOS32_ATTR_PAGE_SIZE_HUGE if page_size > 0x1000 else 0) << 23
        | ((nv_gpu.NVOS32_ATTR_LOCATION_PCI if uncached else 0) << 25)
      )

      attr2 = (
        ((nv_gpu.NVOS32_ATTR2_GPU_CACHEABLE_NO if uncached else nv_gpu.NVOS32_ATTR2_GPU_CACHEABLE_YES) << 2)
      ...
      if cpu_access:
        va_addr = self._gpu_map_to_cpu(mem_handle, size, target=va_addr, flags=map_flags, system=uncached)
...
class NVDevice(HCQCompiled[HCQSignal]):
  ...
  def __init__(self, device: str = ""):
    ...
    device_params = nv_gpu.NV0080_ALLOC_PARAMETERS(
      deviceId=self.iface.gpu_instance, hClientShare=self.iface.root, vaMode=nv_gpu.NV_DEVICE_ALLOCATION_VAMODE_MULTIPLE_VASPACES
    )
    ...
    compilers: list[CompilerPairT] = [
      (functools.partial(NVRenderer, self.arch), functools.partial(CUDACompiler if MOCKGPU else NVCompiler, self.arch)),
  ...
  def _query_gpu_info(self, *reqs):
    nvrs = [
      getattr(nv_gpu, "NV2080_CTRL_GR_INFO_INDEX_" + r.upper(), getattr(nv_gpu, "NV2080_CTRL_GR_INFO_INDEX_LITTER_" + r.upper(), None)) for r in reqs
    ]
  ...
  def _ensure_has_local_memory(self, required):
    if self.slm_per_thread >= required or ((maxlm := getenv("NV_MAX_LOCAL_MEMORY_PER_THREAD")) > 0 and required >= maxlm):
      return
    ...
    cast(NVComputeQueue, NVComputeQueue().wait(self.timeline_signal, self.timeline_value - 1)).setup(
      local_mem=self.shader_local_mem.va_addr, local_mem_tpc_bytes=bytes_per_tpc
    ).signal(self.timeline_signal, self.next_timeline()).submit(self)

  def invalidate_caches(self):
    if self.is_nvd():
      self.iface.rm_control(self.subdevice, nv_gpu.NV2080_CTRL_CMD_INTERNAL_BUS_FLUSH_WITH_SYSMEMBAR, None)
    else:
      self.iface.rm_control(
        self.subdevice,
  ...
  def on_device_hang(self):
    ...
    if sm_errors.mmuFault.valid:
      mmu = self.iface.rm_control(
        self.debugger, nv_gpu.NV83DE_CTRL_CMD_DEBUG_READ_MMU_FAULT_INFO, nv_gpu.NV83DE_CTRL_DEBUG_READ_MMU_FAULT_INFO_PARAMS()
      )
