...
class QMD:
  ...
  def __init__(self, dev: NVDevice, addr: int | None = None, **kwargs):
    ...
    self.mv, self.pref = (
      (
        memoryview(bytearray(self.sz * 4))
        if addr is None
        else to_mv(addr, self.sz * 4)
      ),
      pref,
    )
    if kwargs:
...
class NVCommandQueue(HWQueue[HCQSignal, "NVDevice", "NVProgram", "NVArgsState"]):
  ...
  def setup(
    ...
    if shared_mem_window:
      self.nvm(
        1,
        nv_gpu.NVC6C0_SET_SHADER_SHARED_MEMORY_WINDOW_A,
        *data64(shared_mem_window),
      )
    if local_mem:
...
class NVProgram(HCQProgram):
      ...
      image, self.cbuf_0 = (
        memoryview(
          bytearray(
            lib[
              ctypes.sizeof(
                info := mesa.struct_nak_shader_info.from_buffer_copy(lib)
              ) :
            ]
          )
        ),
  ...
  def __call__(
    ...
    *bufs,
    ...
    if any(
      cur > mx for cur, mx in zip(global_size, [2147483647, 65535, 65535])
    ) or any(cur > mx for cur, mx in zip(local_size, [1024, 1024, 64])):
...
class NVAllocator(HCQAllocator["NVDevice"]):
  def _alloc(self, size: int, options: BufferSpec) -> HCQBuffer:
    return self.dev.iface.alloc(
      size, cpu_access=options.cpu_access, host=options.host
    )
...
class NVKIface:
  ...
  def alloc(
    ...
    **kwargs,
    ...
    if host:
      ...
      if made.params.status != 0:
        raise RuntimeError(
          f"host alloc returned {get_error_str(made.params.status)}"
        )
  ...
  def map(self, mem: HCQBuffer):
    if mem.owner is not None and mem.owner._is_cpu():
      ...
      mem = mem.mappings[
        next(x for x in mem.mapped_devs if x.device.startswith("NV"))
      ]
...
class PCIIface(PCIIfaceBase):
  ...
  def setup_usermode(self):
    return 0xCE000000, self.pci_dev.map_bar(
      bar=0, fmt="I", off=0xBB0000, size=0x10000
    )
...
class NVDevice(HCQCompiled[HCQSignal]):
  ...
  def __init__(self, device: str = ""):
    ...
    vaspace = self.iface.rm_alloc(
      self.nvdevice, nv_gpu.FERMI_VASPACE_A, vaspace_params
    )
