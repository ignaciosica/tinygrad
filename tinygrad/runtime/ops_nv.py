...
def get_error_str(status):
  return f"{status}: {nv_gpu.nv_status_codes.get(status, 'Unknown error')}"


NV_PFAULT_FAULT_TYPE = {
  dt: name
  for name, dt in nv_gpu.__dict__.items()
...
def uvm_ioctl(cmd, sttyp, fd: FileIOInterface, **kwargs):
  ...
  if ret != 0:
    raise RuntimeError(f"ioctl(uvm) returned {ret}")
  if made.rmStatus != 0:
    raise RuntimeError(f"uvm_ioctl returned {get_error_str(made.rmStatus)}")
  return made
...
def make_uvm_type():
  return type(
    ...
    (object,),
    {
      name.replace("UVM_", "").lower(): functools.partial(
        uvm_ioctl, dt, getattr(nv_gpu, name + "_PARAMS")
      )
      for name, dt in nv_gpu.__dict__.items()
      if name.startswith("UVM_") and nv_gpu.__dict__.get(name + "_PARAMS")
    },
  )
...
class QMD:
  ...
  def __init__(self, dev: NVDevice, addr: int | None = None, **kwargs):
    self.ver, self.sz = (
      (5, 0x60) if dev.iface.compute_class >= nv_gpu.BLACKWELL_COMPUTE_A else (3, 0x40)
    )

    # Init fields from module
    if (
      pref := "NVCEC0_QMDV05_00" if self.ver == 5 else "NVC6C0_QMDV03_00"
    ) not in QMD.fields:
      QMD.fields[pref] = {
        **{
          name[len(pref) + 1 :]: dt
        ...
        **{
          name[len(pref) + 1 :] + f"_{i}": dt(i)
          for name, dt in nv_gpu.__dict__.items()
          for i in range(8)
          if name.startswith(pref) and callable(dt)
        },
      }

    self.mv, self.pref = (
      (
        memoryview(bytearray(self.sz * 4)) if addr is None else to_mv(addr, self.sz * 4)
      ),
      pref,
    )
    if kwargs:
  ...
  def _rw_bits(self, hi: int, lo: int, value: int | None = None):
    ...
    self.mv[lo // 8 : hi // 8 + 1] = int(
      (num & ~mask) | ((value << (lo % 8)) & mask)
    ).to_bytes((hi // 8 - lo // 8 + 1), "little")

  def write(self, **kwargs):
    for k, val in kwargs.items():
      self._rw_bits(*QMD.fields[self.pref][k.upper()], value=val)  # type: ignore [misc]

  def read(self, k, val=0):
    return self._rw_bits(*QMD.fields[self.pref][k.upper()])

  def field_offset(self, k):
    return QMD.fields[self.pref][k.upper()][1] // 8

  def set_constant_buf_addr(self, i, addr):
    if self.ver < 4:
      self.write(**{
        f"constant_buffer_addr_upper_{i}": hi32(addr),
        f"constant_buffer_addr_lower_{i}": lo32(addr),
      })
    else:
      self.write(**{
        f"constant_buffer_addr_upper_shifted6_{i}": hi32(addr >> 6),
        f"constant_buffer_addr_lower_shifted6_{i}": lo32(addr >> 6),
      })
...
class NVCommandQueue(HWQueue[HCQSignal, "NVDevice", "NVProgram", "NVArgsState"]):
  ...
  def __del__(self):
    if self.binded_device is not None:
      self.binded_device.allocator.free(
        self.hw_page, self.hw_page.size, BufferSpec(cpu_access=True, nolru=True)
      )
  ...
  def setup(
    ...
    compute_class=None,
    copy_class=None,
    local_mem_window=None,
    shared_mem_window=None,
    local_mem=None,
    local_mem_tpc_bytes=None,
  ):
    if compute_class:
    ...
    if local_mem_window:
      self.nvm(
        1, nv_gpu.NVC6C0_SET_SHADER_LOCAL_MEMORY_WINDOW_A, *data64(local_mem_window)
      )
    if shared_mem_window:
      self.nvm(
        1, nv_gpu.NVC6C0_SET_SHADER_SHARED_MEMORY_WINDOW_A, *data64(shared_mem_window)
      )
    if local_mem:
      self.nvm(1, nv_gpu.NVC6C0_SET_SHADER_LOCAL_MEMORY_A, *data64(local_mem))
    if local_mem_tpc_bytes:
      self.nvm(
        1,
        nv_gpu.NVC6C0_SET_SHADER_LOCAL_MEMORY_NON_THROTTLED_A,
        *data64(local_mem_tpc_bytes),
        0xFF,
      )
    return self
  ...
  def bind(self, dev: NVDevice):
    ...
    self.hw_page = dev.allocator.alloc(
      len(self._q) * 4, BufferSpec(cpu_access=True, nolru=True)
    )
  ...
  def _submit_to_gpfifo(self, dev: NVDevice, gpfifo: GPFifo):
    if dev == self.binded_device:
      cmdq_addr = self.hw_page.va_addr
    else:
...
class NVComputeQueue(NVCommandQueue):
  def memory_barrier(self):
    self.nvm(
      1, nv_gpu.NVC6C0_INVALIDATE_SHADER_CACHES_NO_WFI, (1 << 12) | (1 << 4) | (1 << 0)
    )
  ...
  def exec(
    ...
    self.bind_sints_to_mem(
      *global_size,
      mem=qmd_buf.cpu_view(),
      fmt="I",
      offset=qmd.field_offset("cta_raster_width" if qmd.ver < 4 else "grid_width"),
    )
    self.bind_sints_to_mem(
      *(local_size[:2]),
      mem=qmd_buf.cpu_view(),
      fmt="H",
      offset=qmd.field_offset("cta_thread_dimension0"),
    )
    self.bind_sints_to_mem(
      local_size[2],
      mem=qmd_buf.cpu_view(),
      fmt="B",
      offset=qmd.field_offset("cta_thread_dimension2"),
    )
    ...
    if self.active_qmd is None:
      ...
      self.active_qmd.write(
        dependent_qmd0_pointer=qmd_buf.va_addr >> 8,
  ...
  def signal(self, signal: HCQSignal, value: sint = 0):
      ...
      for i in range(2):
        if self.active_qmd.read(f"release{i}_enable") == 0:
          ...
          addr_off = self.active_qmd.field_offset(
            f"release{i}_address_lower"
            if self.active_qmd.ver < 4
            else f"release_semaphore{i}_addr_lower"
          )
          self.bind_sints_to_mem(
            signal.value_addr & 0xFFFFFFFF,
            mem=self.active_qmd_buf.cpu_view(),
            fmt="I",
            offset=addr_off,
          )
          self.bind_sints_to_mem(
          ...
          val_off = self.active_qmd.field_offset(
            f"release{i}_payload_lower"
            if self.active_qmd.ver < 4
            else f"release_semaphore{i}_payload_lower"
          )
          self.bind_sints_to_mem(
            value & 0xFFFFFFFF,
            mem=self.active_qmd_buf.cpu_view(),
          ...
          self.bind_sints_to_mem(
            value >> 32, mem=self.active_qmd_buf.cpu_view(), fmt="I", offset=val_off + 4
          )
          return self
    ...
    self.nvm(
      ...
      *data64_le(value),
      (1 << 0) | (1 << 20) | (1 << 24) | (1 << 25),
    )  # RELEASE | RELEASE_WFI | PAYLOAD_SIZE_64BIT | RELEASE_TIMESTAMP
    self.nvm(0, nv_gpu.NVC56F_NON_STALL_INTERRUPT, 0x0)
    self.active_qmd = None
    return self

  def _submit(self, dev: NVDevice):
...
class NVCopyQueue(NVCommandQueue):
  def copy(self, dest: sint, src: sint, copy_size: int):
    for off in range(0, copy_size, step := (1 << 31)):
      self.nvm(
        4, nv_gpu.NVC6B5_OFFSET_IN_UPPER, *data64(src + off), *data64(dest + off)
      )
      self.nvm(4, nv_gpu.NVC6B5_LINE_LENGTH_IN, min(copy_size - off, step))
      self.nvm(
        4, nv_gpu.NVC6B5_LAUNCH_DMA, 0x182
      )  # TRANSFER_TYPE_NON_PIPELINED | DST_MEMORY_LAYOUT_PITCH | SRC_MEMORY_LAYOUT_PITCH
    return self
...
class NVProgram(HCQProgram):
  def __init__(self, dev: NVDevice, name: str, lib: bytes):
    ...
    self.constbufs: dict[int, tuple[int, int]] = {
      0: (0, 0x160)
    }  # dict[constbuf index, tuple[va_addr, size]]
    ...
    if NAK := isinstance(dev.compiler, NAKCompiler):
      image, self.cbuf_0 = (
        memoryview(
          bytearray(
            lib[
              ctypes.sizeof(info := mesa.struct_nak_shader_info.from_buffer_copy(lib)) :
            ]
          )
        ),
        [],
      )
      self.regs_usage, self.shmem_usage, self.lcmem_usage = (
        info.num_gprs,
        round_up(info.cs.smem_size, 128),
        round_up(info.slm_size, 16),
      )
    elif MOCKGPU:
      image, sections, relocs = (
        memoryview(bytearray(lib) + b"\x00" * (4 - len(lib) % 4)).cast("I"),
        [],
        [],
      )  # type: ignore
    else:
    ...
    self.lib_gpu = self.dev.allocator.alloc(
      round_up((prog_sz := image.nbytes), 0x1000) + 0x1000,
      buf_spec := BufferSpec(nolru=True),
    )
    prog_addr = self.lib_gpu.va_addr
    if not NAK:
      # For MOCKGPU, the lib is PTX code, so some values are emulated.
      self.regs_usage, self.shmem_usage, self.lcmem_usage, cbuf0_size = (
        0,
        0x400,
        0x240,
        0 if not MOCKGPU else 0x160,
      )
      for sh in sections:  # pylint: disable=possibly-used-before-assignment
        ...
        if sh.name == f".text.{self.name}":
          prog_addr, prog_sz = (
            self.lib_gpu.va_addr + sh.header.sh_addr,
            sh.header.sh_size,
          )
        elif m := re.match(r"\.nv\.constant(\d+)", sh.name):
          self.constbufs[int(m.group(1))] = (
            self.lib_gpu.va_addr + sh.header.sh_addr,
            sh.header.sh_size,
          )
          ...
          for typ, param, data in self._parse_elf_info(sh):
            if sh.name == f".nv.info.{name}" and param == 0xA:
              cbuf0_size = struct.unpack_from("IH", data)[1]  # EIATTR_PARAM_CBANK
            elif sh.name == ".nv.info" and param == 0x12:
              self.lcmem_usage = (
                struct.unpack_from("II", data)[1] + 0x240
              )  # EIATTR_MIN_STACK_SIZE
            elif sh.name == ".nv.info" and param == 0x2F:
              self.regs_usage = struct.unpack_from("II", data)[1]  # EIATTR_REGCOUNT
      ...
      for apply_image_offset, rel_sym_offset, typ, _ in relocs:  # pylint: disable=possibly-used-before-assignment
        ...
        if typ == 2:
          image[apply_image_offset : apply_image_offset + 8] = struct.pack(
            "<Q", self.lib_gpu.va_addr + rel_sym_offset
          )  # R_CUDA_64
        elif typ == 0x38:
          image[apply_image_offset + 4 : apply_image_offset + 8] = struct.pack(
            "<I", (self.lib_gpu.va_addr + rel_sym_offset) & 0xFFFFFFFF
          )
        elif typ == 0x39:
          image[apply_image_offset + 4 : apply_image_offset + 8] = struct.pack(
            "<I", (self.lib_gpu.va_addr + rel_sym_offset) >> 32
          )
        else:
    ...
    if dev.iface.compute_class >= nv_gpu.BLACKWELL_COMPUTE_A:
      if not NAK:
        self.cbuf_0[188:192], self.cbuf_0[223] = (
          [
            *data64_le(self.dev.shared_mem_window),
            *data64_le(self.dev.local_mem_window),
          ],
          0xFFFDC0,
        )
      qmd = {
        ...
        "shader_local_memory_high_size_shifted4": self.lcmem_usage >> 4
        if NAK
        else self.dev.slm_per_thread >> 4,
      }
    else:
      if not NAK:
        self.cbuf_0[6:12] = [
      ...
      qmd = {
        ...
        **(
          {"shader_local_memory_low_size": self.lcmem_usage}
          if NAK
          else {"shader_local_memory_high_size": self.dev.slm_per_thread}
        ),
      }
    ...
    smem_cfg = (
      min(
        shmem_conf * 1024
        for shmem_conf in [32, 64, 100]
        if shmem_conf * 1024 >= self.shmem_usage
      )
      // 4096
      + 1
    )
    ...
    for i, (addr, sz) in self.constbufs.items():
      ...
      self.qmd.write(**{
        f"constant_buffer_size_shifted4_{i}": sz,
    ...
    self.max_threads = (
      ((65536 // round_up(max(1, self.regs_usage) * 32, 256)) // 4) * 4 * 32
    )
    ...
    super().__init__(
      ...
      self.dev,
      self.name,
      kernargs_alloc_size=round_up(self.constbufs[0][1], 1 << 8) + (8 << 8),
    )
  ...
  def _parse_elf_info(self, sh, start_off=0):
    while start_off < sh.header.sh_size:
      ...
      yield (
        typ,
        param,
        sh.content[start_off + 4 : start_off + sz + 4] if typ == 0x4 else sz,
      )
  ...
  def __call__(
    ...
    *bufs,
    ...
    if (
      prod(local_size) > 1024
      or self.max_threads < prod(local_size)
      or self.lcmem_usage > cast(NVDevice, self.dev).slm_per_thread
    ):
      raise RuntimeError(
        f"Too many resources requested for launch, {prod(local_size)=}, {self.max_threads=}"
      )
    if any(cur > mx for cur, mx in zip(global_size, [2147483647, 65535, 65535])) or any(
      cur > mx for cur, mx in zip(local_size, [1024, 1024, 64])
    ):
...
class NVKIface:
  ...
  # VA space is 48bits.
  low_uvm_vaddr_allocator: BumpAllocator = BumpAllocator(
    size=0x1000000000, base=0x8000000000 if OSX else 0x1000000000, wrap=False
  )
  uvm_vaddr_allocator: BumpAllocator = BumpAllocator(
    size=(1 << 48) - 1,
    base=low_uvm_vaddr_allocator.base + low_uvm_vaddr_allocator.size,
    wrap=False,
  )
  ...
  def __init__(self, dev, device_id):
    if NVKIface.root is None:
      ...
      with contextlib.suppress(RuntimeError):
        uvm.mm_initialize(
          self.fd_uvm_2, uvmFd=self.fd_uvm.fd
        )  # this error is okay, CUDA hits it too

      nv_iowr(
        NVKIface.fd_ctl,
        nv_gpu.NV_ESC_CARD_INFO,
        gpus_info := (nv_gpu.nv_ioctl_card_info_t * 64)(),
      )
      NVKIface.gpus_info = hcq_filter_visible_devices(gpus_info)

    self.dev, self.device_id = dev, device_id
    if (
      self.device_id >= len(NVKIface.gpus_info)
      or not NVKIface.gpus_info[self.device_id].valid
    ):
      raise RuntimeError(
    ...
    self.gpu_info = self.rm_control(
      self.root,
      nv_gpu.NV0000_CTRL_CMD_GPU_GET_ID_INFO_V2,
      nv_gpu.NV0000_CTRL_GPU_GET_ID_INFO_V2_PARAMS(
        gpuId=NVKIface.gpus_info[self.device_id].gpu_id
      ),
    )
  ...
  def rm_alloc(self, parent, clss, params=None, root=None) -> int:
    nv_iowr(
      self.fd_ctl,
      nv_gpu.NV_ESC_RM_ALLOC,
      made := nv_gpu.NVOS21_PARAMETERS(
        hRoot=root if root is not None else self.root,
        hObjectParent=parent,
        hClass=clss,
        pAllocParms=ctypes.cast(ctypes.byref(params), ctypes.c_void_p)
        if params is not None
        else None,
      ),
    )
    if made.status == nv_gpu.NV_ERR_NO_MEMORY:
      raise MemoryError(f"rm_alloc returned {get_error_str(made.status)}")
    if made.status != 0:
  ...
  def rm_control(self, obj, cmd, params=None):
    nv_iowr(
      self.fd_ctl,
      nv_gpu.NV_ESC_RM_CONTROL,
      made := nv_gpu.NVOS54_PARAMETERS(
        hClient=self.root,
        hObject=obj,
        cmd=cmd,
        paramsSize=ctypes.sizeof(params),
        params=ctypes.cast(ctypes.byref(params), ctypes.c_void_p)
        if params is not None
        else None,
      ),
    )
    if made.status != 0:
      raise RuntimeError(f"rm_control returned {get_error_str(made.status)}")
    return params

  def setup_usermode(self):
    clsinfo = self.rm_control(
      self.dev.nvdevice,
      nv_gpu.NV0080_CTRL_CMD_GPU_GET_CLASSLIST,
      nv_gpu.NV0080_CTRL_GPU_GET_CLASSLIST_PARAMS(
        numClasses=100,
    ...
    self.usermode_class: int = next(
      c
      for c in [nv_gpu.HOPPER_USERMODE_A, nv_gpu.TURING_USERMODE_A]
      if c in self.nvclasses
    )
    self.gpfifo_class: int = next(
      c
      for c in [nv_gpu.BLACKWELL_CHANNEL_GPFIFO_A, nv_gpu.AMPERE_CHANNEL_GPFIFO_A]
      if c in self.nvclasses
    )
    self.compute_class: int = next(
      c
      for c in [
        nv_gpu.BLACKWELL_COMPUTE_B,
        nv_gpu.ADA_COMPUTE_A,
        nv_gpu.AMPERE_COMPUTE_B,
      ]
      if c in self.nvclasses
    )
    self.dma_class: int = next(
      c
      for c in [nv_gpu.BLACKWELL_DMA_COPY_B, nv_gpu.AMPERE_DMA_COPY_B]
      if c in self.nvclasses
    )

    usermode = self.rm_alloc(self.dev.subdevice, self.usermode_class)
    return usermode, MMIOInterface(
      self._gpu_map_to_cpu(usermode, mmio_sz := 0x10000, flags=2), mmio_sz, fmt="I"
    )

  def setup_vm(self, vaspace):
    self.rm_control(
      self.dev.subdevice,
      nv_gpu.NV2080_CTRL_CMD_GPU_GET_GID_INFO,
      raw_uuid := nv_gpu.NV2080_CTRL_GPU_GET_GID_INFO_PARAMS(
        flags=nv_gpu.NV2080_GPU_CMD_GPU_GET_GID_FLAGS_FORMAT_BINARY, length=16
      ),
    )
    self.gpu_uuid = nv_gpu.struct_nv_uuid(
      uuid=(ctypes.c_ubyte * 16)(*[raw_uuid.data[i] for i in range(16)])
    )
    ...
    for dev in cast(
      ...
      [
        ...
        for d in pg
        if isinstance(d, NVDevice) and not d.is_nvd()
      ],
    ):
      try:
        uvm.enable_peer_access(
          self.fd_uvm, gpuUuidA=self.gpu_uuid, gpuUuidB=dev.iface.gpu_uuid
        )
      except RuntimeError as e:
        raise RuntimeError(
          f"{e}. Make sure GPUs #{self.gpu_minor} & #{dev.iface.gpu_minor} have P2P enabled."
        ) from e
  ...
  def setup_gpfifo_vm(self, gpfifo):
    uvm.register_channel(
      ...
      hClient=self.root,
      hChannel=gpfifo,
      base=self._alloc_gpu_vaddr(0x4000000, force_low=True),
      length=0x4000000,
    )

  def _new_gpu_fd(self):
    fd_dev = FileIOInterface(
      f"/dev/nvidia{NVKIface.gpus_info[self.device_id].minor_number}",
      os.O_RDWR | os.O_CLOEXEC,
    )
    nv_iowr(
      fd_dev,
      nv_gpu.NV_ESC_REGISTER_FD,
      nv_gpu.nv_ioctl_register_fd_t(ctl_fd=self.fd_ctl.fd),
    )
    return fd_dev
  ...
  def _gpu_map_to_cpu(self, memory_handle, size, target=None, flags=0, system=False):
    fd_dev = (
      self._new_gpu_fd()
      if not system
      else FileIOInterface("/dev/nvidiactl", os.O_RDWR | os.O_CLOEXEC)
    )
    made = nv_gpu.nv_ioctl_nvos33_parameters_with_fd(
      fd=fd_dev.fd,
      params=nv_gpu.NVOS33_PARAMETERS(
        hClient=self.root,
        hDevice=self.dev.nvdevice,
    ...
    if made.params.status != 0:
      raise RuntimeError(
        f"_gpu_map_to_cpu returned {get_error_str(made.params.status)}"
      )
    return fd_dev.mmap(
      target,
      size,
      mmap.PROT_READ | mmap.PROT_WRITE,
      mmap.MAP_SHARED | (MAP_FIXED if target is not None else 0),
      0,
    )
  ...
  def alloc(
    ...
    **kwargs,
    ...
    page_size = (
      (4 << (12 if OSX else 10))
      if uncached or host
      else ((2 << 20) if size >= (8 << 20) else (4 << (12 if OSX else 10)))
    )
    size = round_up(size, page_size)
    va_addr = self._alloc_gpu_vaddr(size, alignment=page_size, force_low=cpu_access)

    if host:
      va_addr = cpu_addr or FileIOInterface.anon_mmap(
        va_addr,
        size,
        mmap.PROT_READ | mmap.PROT_WRITE,
      ...
      flags = (
        (nv_gpu.NVOS02_FLAGS_PHYSICALITY_NONCONTIGUOUS << 4)
        | (nv_gpu.NVOS02_FLAGS_COHERENCY_CACHED << 12)
        | (nv_gpu.NVOS02_FLAGS_MAPPING_NO_MAP << 30)
      )

      NVKIface.host_object_enumerator += 1
      made = nv_gpu.nv_ioctl_nvos02_parameters_with_fd(
        params=nv_gpu.NVOS02_PARAMETERS(
          hRoot=self.root,
          hObjectParent=self.dev.nvdevice,
          flags=flags,
          hObjectNew=NVKIface.host_object_enumerator,
          hClass=nv_gpu.NV01_MEMORY_SYSTEM_OS_DESCRIPTOR,
          pMemory=va_addr,
          limit=size - 1,
        ),
        fd=-1,
      ...
      if made.params.status != 0:
        raise RuntimeError(f"host alloc returned {get_error_str(made.params.status)}")
      mem_handle = made.params.hObjectNew
    else:
      attr = (
        (
          (
            nv_gpu.NVOS32_ATTR_PHYSICALITY_CONTIGUOUS
            if contiguous
            else nv_gpu.NVOS32_ATTR_PHYSICALITY_ALLOW_NONCONTIGUOUS
          )
          << 27
        )
        | (nv_gpu.NVOS32_ATTR_PAGE_SIZE_HUGE if page_size > 0x1000 else 0) << 23
        | ((nv_gpu.NVOS32_ATTR_LOCATION_PCI if uncached else 0) << 25)
      )

      attr2 = (
        (
          (
            nv_gpu.NVOS32_ATTR2_GPU_CACHEABLE_NO
            if uncached
            else nv_gpu.NVOS32_ATTR2_GPU_CACHEABLE_YES
          )
          << 2
        )
        | ((nv_gpu.NVOS32_ATTR2_PAGE_SIZE_HUGE_2MB if page_size > 0x1000 else 0) << 20)
        | nv_gpu.NVOS32_ATTR2_ZBC_PREFER_NO_ZBC
      )

      fl = (
        nv_gpu.NVOS32_ALLOC_FLAGS_MAP_NOT_REQUIRED
        | nv_gpu.NVOS32_ALLOC_FLAGS_MEMORY_HANDLE_PROVIDED
        | nv_gpu.NVOS32_ALLOC_FLAGS_ALIGNMENT_FORCE
        | nv_gpu.NVOS32_ALLOC_FLAGS_IGNORE_BANK_PLACEMENT
      ...
      alloc_params = nv_gpu.NV_MEMORY_ALLOCATION_PARAMS(
        owner=self.root,
        alignment=page_size,
        offset=0,
        limit=size - 1,
        format=6,
        size=size,
        type=nv_gpu.NVOS32_TYPE_NOTIFIER if uncached else nv_gpu.NVOS32_TYPE_IMAGE,
        attr=attr,
        attr2=attr2,
        flags=fl,
      )
      ...
      if cpu_access:
        va_addr = self._gpu_map_to_cpu(
          mem_handle, size, target=va_addr, flags=map_flags, system=uncached
        )

    return self._gpu_uvm_map(
      va_addr, size, mem_handle, has_cpu_mapping=cpu_access or host
    )

  def free(self, mem: HCQBuffer):
    if (
      mem.meta.hMemory > NVKIface.host_object_enumerator
    ):  # not a host object, clear phys mem.
      made = nv_gpu.NVOS00_PARAMETERS(
        hRoot=self.root, hObjectParent=self.dev.nvdevice, hObjectOld=mem.meta.hMemory
      )
      nv_iowr(self.fd_ctl, nv_gpu.NV_ESC_RM_FREE, made)
      if made.status != 0:
        raise RuntimeError(f"_gpu_free returned {get_error_str(made.status)}")

    uvm.free(self.fd_uvm, base=cast(int, mem.va_addr), length=mem.size)
    if mem.meta.has_cpu_mapping:
      FileIOInterface.munmap(cast(int, mem.va_addr), mem.size)

  def _gpu_uvm_map(
    self, va_base, size, mem_handle, create_range=True, has_cpu_mapping=False
  ) -> HCQBuffer:
    if create_range:
      uvm.create_external_range(self.fd_uvm, base=va_base, length=size)
    attrs = (nv_gpu.struct_c__SA_UvmGpuMappingAttributes * 256)(
      nv_gpu.struct_c__SA_UvmGpuMappingAttributes(
        gpuUuid=self.gpu_uuid, gpuMappingType=1
      )
    )
    ...
    return HCQBuffer(
      va_base,
      size,
      meta=uvm.map_external_allocation(
        self.fd_uvm,
        base=va_base,
        length=size,
        rmCtrlFd=self.fd_ctl.fd,
        ...
        perGpuAttributes=attrs,
        mapped_gpu_ids=[self.gpu_uuid],
  ...
  def _alloc_gpu_vaddr(self, size, alignment=(4 << 10), force_low=False):
    return (
      NVKIface.low_uvm_vaddr_allocator.alloc(size, alignment)
      if force_low
      else NVKIface.uvm_vaddr_allocator.alloc(size, alignment)
    )
...
class PCIIface(PCIIfaceBase):
  ...
  def __init__(self, dev, dev_id):
    super().__init__(
      ...
      dev_id,
      vendor=0x10DE,
      devices=[0x2204, 0x2684, 0x2B85],
      bars=[0, 1],
      vram_bar=1,
      va_start=NVMemoryManager.va_allocator.base,
      va_size=NVMemoryManager.va_allocator.size,
    )
    if not OSX:
    ...
    self.gpfifo_class, self.compute_class, self.dma_class = (
      (gsp := self.dev_impl.gsp).gpfifo_class,
      gsp.compute_class,
      gsp.dma_class,
    )
  ...
  def alloc(
    ...
    **kwargs,
    ...
    page_size = (
      (2 << 20) if size >= (8 << 20) and not uncached and not host else (4 << 10)
    )
    return super().alloc(
  ...
  def rm_control(self, obj, cmd, params=None):
    return self.dev_impl.gsp.rpc_rm_control(obj, cmd, params, self.root)
...
class NVDevice(HCQCompiled[HCQSignal]):
  ...
  def __init__(self, device: str = ""):
    ...
    device_params = nv_gpu.NV0080_ALLOC_PARAMETERS(
      deviceId=self.iface.gpu_instance,
      hClientShare=self.iface.root,
      vaMode=nv_gpu.NV_DEVICE_ALLOCATION_VAMODE_MULTIPLE_VASPACES,
    )
    self.nvdevice = self.iface.rm_alloc(
      self.iface.root, nv_gpu.NV01_DEVICE_0, device_params
    )
    self.subdevice = self.iface.rm_alloc(
      self.nvdevice, nv_gpu.NV20_SUBDEVICE_0, nv_gpu.NV2080_ALLOC_PARAMETERS()
    )
    ...
    channel_group = self.iface.rm_alloc(
      self.nvdevice, nv_gpu.KEPLER_CHANNEL_GROUP_A, channel_params
    )

    gpfifo_area = self.iface.alloc(
      0x200000, contiguous=True, cpu_access=True, force_devmem=True, map_flags=0x10D0000
    )

    ctxshare_params = nv_gpu.NV_CTXSHARE_ALLOCATION_PARAMETERS(
      hVASpace=vaspace, flags=nv_gpu.NV_CTXSHARE_ALLOCATION_FLAGS_SUBCONTEXT_ASYNC
    )
    ctxshare = self.iface.rm_alloc(
      channel_group, nv_gpu.FERMI_CONTEXT_SHARE_A, ctxshare_params
    )

    self.compute_gpfifo = self._new_gpu_fifo(
      gpfifo_area, ctxshare, channel_group, offset=0, entries=0x10000, compute=True
    )
    self.dma_gpfifo = self._new_gpu_fifo(
      gpfifo_area,
      ctxshare,
      channel_group,
      offset=0x100000,
    ...
    self.iface.rm_control(
      channel_group,
      nv_gpu.NVA06C_CTRL_CMD_GPFIFO_SCHEDULE,
      nv_gpu.NVA06C_CTRL_GPFIFO_SCHEDULE_PARAMS(bEnable=1),
    )

    self.cmdq_page: HCQBuffer = self.iface.alloc(0x200000, cpu_access=True)
    self.cmdq_allocator = BumpAllocator(
      size=self.cmdq_page.size, base=cast(int, self.cmdq_page.va_addr), wrap=True
    )
    self.cmdq = self.cmdq_page.cpu_view().view(fmt="I")

    (
      self.num_gpcs,
      self.num_tpc_per_gpc,
      self.num_sm_per_tpc,
    ...
    self.arch: str = (
      ...
      else f"sm_{(self.sm_version >> 8) & 0xFF}{(val >> 4) if (val := self.sm_version & 0xFF) > 0xF else val}"
    )
    self.sass_version = ((self.sm_version & 0xF00) >> 4) | (self.sm_version & 0xF)

    compilers: list[CompilerPairT] = [
      (
        functools.partial(NVRenderer, self.arch),
        functools.partial(CUDACompiler if MOCKGPU else NVCompiler, self.arch),
      ),
      (
        functools.partial(PTXRenderer, self.arch, device="NV"),
        functools.partial(PTXCompiler if MOCKGPU else NVPTXCompiler, self.arch),
      ),
      (
        functools.partial(NAKRenderer, dev=self),
        functools.partial(NAKCompiler, self.arch, self.max_warps_per_sm),
      ),
    ]
    super().__init__(
      device,
  ...
  def _new_gpu_fifo(
    self, gpfifo_area, ctxshare, channel_group, offset=0, entries=0x400, compute=False
  ) -> GPFifo:
    ...
    params = nv_gpu.NV_CHANNELGPFIFO_ALLOCATION_PARAMETERS(
      ...
      hObjectBuffer=gpfifo_area.meta.hMemory,
    ...
    if compute:
      ...
      debugger_params = nv_gpu.NV83DE_ALLOC_PARAMETERS(
        hAppClient=self.iface.root, hClass3dObject=self.debug_compute_obj
      )
      self.debugger = self.iface.rm_alloc(
        self.nvdevice, nv_gpu.GT200_DEBUGGER, debugger_params
      )
    else:
      self.iface.rm_alloc(gpfifo, self.iface.dma_class)

    ws_token_params = self.iface.rm_control(
      gpfifo,
      nv_gpu.NVC36F_CTRL_CMD_GPFIFO_GET_WORK_SUBMIT_TOKEN,
    ...
    return GPFifo(
      ...
      token=ws_token_params.workSubmitToken,
      controls=nv_gpu.AmpereAControlGPFifo.from_address(
        gpfifo_area.cpu_view().addr + offset + entries * 8
      ),
    )
  ...
  def _query_gpu_info(self, *reqs):
    nvrs = [
      getattr(
        nv_gpu,
        "NV2080_CTRL_GR_INFO_INDEX_" + r.upper(),
        getattr(nv_gpu, "NV2080_CTRL_GR_INFO_INDEX_LITTER_" + r.upper(), None),
      )
      for r in reqs
    ]

    if self.is_nvd():
      x = self.iface.rm_control(
        self.subdevice,
        nv_gpu.NV2080_CTRL_CMD_INTERNAL_STATIC_KGR_GET_INFO,
        nv_gpu.NV2080_CTRL_INTERNAL_STATIC_GR_GET_INFO_PARAMS(),
      )
      return [x.engineInfo[0].infoList[nvr].data for nvr in nvrs]

    infos = (nv_gpu.NV2080_CTRL_GR_INFO * len(nvrs))(*[
      nv_gpu.NV2080_CTRL_GR_INFO(index=nvr) for nvr in nvrs
    ])
    self.iface.rm_control(
      self.subdevice,
      nv_gpu.NV2080_CTRL_CMD_GR_GET_INFO,
      nv_gpu.NV2080_CTRL_GR_GET_INFO_PARAMS(
        grInfoListSize=len(infos), grInfoList=ctypes.addressof(infos)
      ),
    )
  ...
  def _setup_gpfifos(self):
    ...
    NVCopyQueue().wait(self.timeline_signal, self.timeline_value - 1).setup(
      copy_class=self.iface.dma_class
    ).signal(self.timeline_signal, self.next_timeline()).submit(self)

    self.synchronize()

  def _ensure_has_local_memory(self, required):
    if self.slm_per_thread >= required or (
      (maxlm := getenv("NV_MAX_LOCAL_MEMORY_PER_THREAD")) > 0 and required >= maxlm
    ):
      return

    self.slm_per_thread, old_slm_per_thread = (
      round_up(required, 32),
      self.slm_per_thread,
    )
    bytes_per_tpc = round_up(
      round_up(self.slm_per_thread * 32, 0x200)
      * self.max_warps_per_sm
      * self.num_sm_per_tpc,
      0x8000,
    )
    self.shader_local_mem, ok = self._realloc(
      self.shader_local_mem,
      round_up(bytes_per_tpc * self.num_tpc_per_gpc * self.num_gpcs, 0x20000),
    )
  ...
  def invalidate_caches(self):
    if self.is_nvd():
      self.iface.rm_control(
        self.subdevice, nv_gpu.NV2080_CTRL_CMD_INTERNAL_BUS_FLUSH_WITH_SYSMEMBAR, None
      )
    else:
      self.iface.rm_control(
        self.subdevice,
        nv_gpu.NV2080_CTRL_CMD_FB_FLUSH_GPU_CACHE,
        nv_gpu.NV2080_CTRL_FB_FLUSH_GPU_CACHE_PARAMS(
          flags=(
            (nv_gpu.NV2080_CTRL_FB_FLUSH_GPU_CACHE_FLAGS_WRITE_BACK_YES << 2)
            | (nv_gpu.NV2080_CTRL_FB_FLUSH_GPU_CACHE_FLAGS_INVALIDATE_YES << 3)
  ...
  def on_device_hang(self):
    ...
    report = []
    sm_errors = self.iface.rm_control(
      self.debugger,
      nv_gpu.NV83DE_CTRL_CMD_DEBUG_READ_ALL_SM_ERROR_STATES,
      nv_gpu.NV83DE_CTRL_DEBUG_READ_ALL_SM_ERROR_STATES_PARAMS(
        hTargetChannel=self.debug_channel, numSMsToRead=100
      ),
    )

    if sm_errors.mmuFault.valid:
      mmu = self.iface.rm_control(
        self.debugger,
        nv_gpu.NV83DE_CTRL_CMD_DEBUG_READ_MMU_FAULT_INFO,
        nv_gpu.NV83DE_CTRL_DEBUG_READ_MMU_FAULT_INFO_PARAMS(),
      )
      for i in range(mmu.count):
        pfinfo = mmu.mmuFaultInfoList[i]
        report += [
          f"MMU fault: 0x{pfinfo.faultAddress:X} | {NV_PFAULT_FAULT_TYPE[pfinfo.faultType]} | {NV_PFAULT_ACCESS_TYPE[pfinfo.accessType]}"
        ]
    else:
      for i, e in enumerate(sm_errors.smErrorStateArray):
        if e.hwwGlobalEsr or e.hwwWarpEsr:
          report += [
            f"SM {i} fault: esr={e.hwwGlobalEsr} warp_esr={e.hwwWarpEsr:#x} warp_pc={e.hwwWarpEsrPc64:#x}"
          ]
