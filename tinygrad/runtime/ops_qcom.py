...
def pkt4_hdr(reg: int, cnt: int):
  return adreno.CP_TYPE4_PKT | cnt & 0x7F | parity(cnt) << 7 | (reg & 0x3FFFF) << 8 | parity(reg) << 27
...
class QCOMComputeQueue(HWQueue):
  ...
  def _cache_flush(self, write_back=True, invalidate=False, sync=True, memsync=False):
    # TODO: 7xx support.
    if write_back:
      self.cmd(adreno.CP_EVENT_WRITE, adreno.CACHE_FLUSH_TS, *data64_le(QCOMDevice.dummy_addr), 0)  # dirty cache write-back.
    if invalidate:
      self.cmd(adreno.CP_EVENT_WRITE, adreno.CACHE_INVALIDATE)  # invalidate cache lines (following reads from RAM).
    if memsync:
  ...
  def _build_gpu_command(self, dev: QCOMDevice, hw_addr=None):
    ...
    submit_req = kgsl.struct_kgsl_gpu_command(
      cmdlist=ctypes.addressof(obj), numcmds=1, context_id=dev.ctx, cmdsize=ctypes.sizeof(kgsl.struct_kgsl_command_object)
    )
  ...
  def exec(self, prg: QCOMProgram, args_state: QCOMArgsState, global_size, local_size):
    ...
    self.cmd(
      ...
      qreg.cp_load_state6_0(state_type=adreno.ST_CONSTANTS, state_src=adreno.SS6_INDIRECT, state_block=adreno.SB6_CS_SHADER, num_unit=1024 // 4),
      *data64_le(args_state.buf.va_addr),
    )
    self.cmd(
      adreno.CP_LOAD_STATE6_FRAG,
      qreg.cp_load_state6_0(
        state_type=adreno.ST_SHADER, state_src=adreno.SS6_INDIRECT, state_block=adreno.SB6_CS_SHADER, num_unit=round_up(prg.image_size, 128) // 128
      ),
      *data64_le(prg.lib_gpu.va_addr),
    )
    ...
    if args_state.prg.samp_cnt > 0:
      self.cmd(
        adreno.CP_LOAD_STATE6_FRAG,
        qreg.cp_load_state6_0(
          state_type=adreno.ST_SHADER, state_src=adreno.SS6_INDIRECT, state_block=adreno.SB6_CS_TEX, num_unit=args_state.prg.samp_cnt
        ),
    ...
    if args_state.prg.tex_cnt > 0:
      self.cmd(
        adreno.CP_LOAD_STATE6_FRAG,
        qreg.cp_load_state6_0(
          state_type=adreno.ST_CONSTANTS, state_src=adreno.SS6_INDIRECT, state_block=adreno.SB6_CS_TEX, num_unit=min(16, args_state.prg.tex_cnt)
        ),
    ...
    if args_state.prg.ibo_cnt > 0:
      self.cmd(
        adreno.CP_LOAD_STATE6_FRAG,
        qreg.cp_load_state6_0(
          state_type=adreno.ST6_IBO, state_src=adreno.SS6_INDIRECT, state_block=adreno.SB6_CS_SHADER, num_unit=args_state.prg.ibo_cnt
        ),
...
class QCOMArgsState(HCQArgsState):
  def __init__(self, buf: HCQBuffer, prg: QCOMProgram, bufs: tuple[HCQBuffer, ...], vals: tuple[int, ...] = ()):
    ...
    for cnst_val, cnst_off, cnst_sz in prg.consts_info:
      to_mv(self.buf.va_addr + cnst_off, cnst_sz)[:] = cnst_val.to_bytes(cnst_sz, byteorder="little")

    if prg.samp_cnt > 0:
      to_mv(self.buf.va_addr + prg.samp_off, len(prg.samplers) * 4).cast("I")[:] = array.array("I", prg.samplers)
...
class QCOMProgram(HCQProgram):
  ...
  def __call__(
    ...
    if any(g * l > mx for g, l, mx in zip(global_size, local_size, [65536, 65536, 65536])) and any(
      l > mx for l, mx in zip(local_size, [1024, 1024, 1024])
    ):
  ...
  def _parse_lib(self):
    ...
    if self.samp_cnt:
      self.samp_cnt += 1
      self.samplers = [
        qreg.a6xx_tex_samp_0(wrap_s=(clamp_mode := adreno.A6XX_TEX_CLAMP_TO_BORDER), wrap_t=clamp_mode, wrap_r=clamp_mode),
  ...
  def _alloc(self, size: int, options: BufferSpec) -> HCQBuffer:
    ...
    if options.image is not None:
      ...
      desc = [
        ...
        qreg.a6xx_tex_const_2(type=adreno.A6XX_TEX_2D, pitch=pitch, pitchalign=pitchalign - 6),
        0,
