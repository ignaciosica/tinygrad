...
class RemoteHandler:
  ...
  async def ib_connect(
    ...
    if dsession.host not in self.ib_conns:
      props = safe_eval(
        ast.parse(conn.q(GetProperties(session=dsession), wait=True), mode="eval").body
      )
      ...
      for c in req._q:
        ...
        match c:
          ...
          case Transfer():
            if c.dsession.host == unwrap(c.session).host:
              ...
              if ib_conn is not None:
                ...
                sbuf.copyout(data := memoryview(bytearray(sbuf.nbytes)))
                await conn.aq(
                  CopyIn(c.dbuffer_num, conn.req.h(data), session=c.dsession), wait=True
                )
          case BatchTransfer():
            conn, ib_conn = (
          ...
          case ProgramExec():
            ...
            extra_args = {
              k: v
              for k, v in [("global_size", c.global_size), ("local_size", c.local_size)]
              if v is not None
            }
          ...
          case GraphExec():
            r = session.graphs[c.graph_num](
              [self.sessions[s].buffers[i] for s, i in c.bufs], c.var_vals, wait=c.wait
            )
            if r is not None:
              ret = str(r).encode()
    else:
...
class RemoteProgram:
  ...
  def __call__(
    ...
    *bufs,
    ...
    ret = self.dev.q(
      ProgramExec(self.name, self.datahash, bufs, vals, global_size, local_size, wait),
      wait=wait,
    )
    if wait:
      return float(ret)
...
class RemoteDevice(Compiled):
  ...
  def __init__(self, device: str):
    ...
    if not renderer[0].startswith("tinygrad.") or not renderer[1].endswith("Renderer"):
