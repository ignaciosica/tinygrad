    ...
    if path == "/batch" and method == "POST":
      ...
      for c in req._q:
        ...
        match c:
          ...
          case Event():
            if c.session == c.event_session:
              ...
              for d in Device._opened_devices:
                Device[d].synchronize()  # wait for device*s* to finish executing previous stuff
          ...
          case CopyIn():
            session.buffers[c.buffer_num].copyin(memoryview(bytearray(req._h[c.datahash])))
          case CopyOut():
            session.buffers[c.buffer_num].copyout(memoryview(ret := bytearray(session.buffers[c.buffer_num].nbytes)))
          case Transfer():
            ...
            def _parse_ji(gi: GraphComputeItem | Transfer):
              match gi:
                case GraphComputeItem():
                  ...
                  return ExecItem(
                    CompiledRunner(ps, precompiled=b"", prg=prg), [self.sessions[gi.session].buffers[buf] for buf in gi.bufs], fixedvars=gi.fixedvars
                  )
                case Transfer():
