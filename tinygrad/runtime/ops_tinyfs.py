...
class TinyFSDevice(Compiled):
  def __init__(self, device: str):
    ...
    self.sock.connect((
      TINYFS_ENDPOINT.rsplit(":", 1)[0],
      int(TINYFS_ENDPOINT.rsplit(":", 1)[1]),
    ))
  ...
  async def connection(self, loc):
    if loc not in self.conn_pools:
      ...
      if loc not in self.conn_pools:
        ...
        conn_tasks = [
          asyncio.open_connection(*self.node_info[loc][-1].rsplit(":", 1))
          for _ in range(nw)
        ]
...
class TinyFSBuffer:
  def __init__(
    self, device: TinyFSDevice, size: int, offset=0, copyout_queue=None, hash_buf=None
  ):
  ...
  def __repr__(self):
    return f"<TinyFSBuffer size={self.size} offset={self.offset}>"
...
class TinyFSAllocator(Allocator[TinyFSDevice]):
  ...
  def _copyin(self, dest: TinyFSBuffer, src: memoryview):
    if DEBUG >= 2:
      print(f"Copying in {dest.size} bytes to TINYFS:{dest.device.op}")
  ...
  def _copyout(self, dest: memoryview, src: TinyFSBuffer):
    if DEBUG >= 2:
      print(f"Copying out {src.size} bytes from TINYFS:{src.device.op}")
    if src.device.op == "LOAD":
      asyncio.run_coroutine_threadsafe(
        self._copyout_async(dest, src), src.device.loop
      ).result()
  ...
  async def _copyout_async(self, dest: memoryview, src: TinyFSBuffer):
    ...
    workers = [
      asyncio.create_task(_worker(i, loc)) for i, loc in enumerate(src.copyout_queue)
    ]
