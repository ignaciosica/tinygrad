...
def _wait(future: webgpu.struct_WGPUFuture):
  assert webgpu.wgpuInstanceWaitAny(instance, 1, webgpu.WGPUFutureWaitInfo(future=future), 2**64 - 1) == webgpu.WGPUWaitStatus_Success, (
    "Future failed"
  )
...
def _run(async_fun, cb_info_type, cb_type, status_enum, res_idx: int | None, msg_idx: int | None, *params):
  ...
  if result[0] != 1:
    raise RuntimeError(f"[{status_enum[result[0]] if status_enum else 'ERROR'}]{result[msg_idx] if msg_idx else ''}")
...
def read_buffer(dev: WGPUDevPtr, buf: WGPUBufPtr) -> memoryview:
  ...
  tmp_buffer = webgpu.wgpuDeviceCreateBuffer(
    dev, webgpu.WGPUBufferDescriptor(size=size, usage=webgpu.WGPUBufferUsage_CopyDst | webgpu.WGPUBufferUsage_MapRead, mappedAtCreation=False)
  )
...
class WebGPUProgram:
  def __init__(self, dev: tuple[WGPUDevPtr, bool], name: str, lib: bytes):
    ...
    shader = webgpu.WGPUShaderModuleWGSLDescriptor(
      code=to_wgpu_str(lib.decode()), chain=webgpu.WGPUChainedStruct(sType=webgpu.WGPUSType_ShaderSourceWGSL)
    )
    ...
    *bufs: WGPUBufPtr,
    ...
    for i in range(1, len(bufs)):
      if ctypes.addressof(bufs[i]) == ctypes.addressof(bufs[0]):
        tmp_bufs[0] = webgpu.wgpuDeviceCreateBuffer(
          self.dev, webgpu.WGPUBufferDescriptor(size=webgpu.wgpuBufferGetSize(bufs[0]), usage=webgpu.wgpuBufferGetUsage(bufs[0]))
        )
        buf_patch = True
    ...
    binding_layouts = [
      webgpu.WGPUBindGroupLayoutEntry(
        binding=0, visibility=webgpu.WGPUShaderStage_Compute, buffer=webgpu.WGPUBufferBindingLayout(type=webgpu.WGPUBufferBindingType_Uniform)
      )
    ]
    binding_layouts += [
      webgpu.WGPUBindGroupLayoutEntry(
        ...
        buffer=webgpu.WGPUBufferBindingLayout(
          type=webgpu.WGPUBufferBindingType_Uniform if i >= len(tmp_bufs) else webgpu.WGPUBufferBindingType_Storage
        ),
      )
    ...
    pipeline_layout_desc = webgpu.WGPUPipelineLayoutDescriptor(
      bindGroupLayoutCount=len(bind_group_layouts), bindGroupLayouts=(webgpu.WGPUBindGroupLayout * len(bind_group_layouts))(*bind_group_layouts)
    )
    ...
    compute_desc = webgpu.WGPUComputePipelineDescriptor(
      layout=pipeline_layout, compute=webgpu.WGPUComputeState(module=self.prg, entryPoint=to_wgpu_str(self.name))
    )
    ...
    if wait:
      ...
      query_buf = webgpu.wgpuDeviceCreateBuffer(
        self.dev, webgpu.WGPUBufferDescriptor(size=16, usage=webgpu.WGPUBufferUsage_QueryResolve | webgpu.WGPUBufferUsage_CopySrc)
      )
      comp_pass_desc.timestampWrites = ctypes.pointer(
        webgpu.WGPUComputePassTimestampWrites(querySet=query_set, beginningOfPassWriteIndex=0, endOfPassWriteIndex=1)
      )
    ...
    if wait:
      webgpu.wgpuCommandEncoderResolveQuerySet(command_encoder, query_set, 0, 2, query_buf, 0)
  ...
  def _alloc(self, size: int, options: BufferSpec) -> WGPUBufPtr:
    ...
    return webgpu.wgpuDeviceCreateBuffer(
      ...
      webgpu.WGPUBufferDescriptor(
        size=round_up(size, 4), usage=webgpu.WGPUBufferUsage_Storage | webgpu.WGPUBufferUsage_CopyDst | webgpu.WGPUBufferUsage_CopySrc
      ),
    )
