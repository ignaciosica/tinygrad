...
class AMRegister(AMDReg):
  ...
  def update(self, inst=0, **kwargs):
    self.write(
      self.read(inst=inst) & ~self.fields_mask(*kwargs.keys()), inst=inst, **kwargs
    )
...
class AMFirmware:
  def __init__(self, adev):
    ...
    blob, sos_hdr = self.load_fw(
      f"psp_{fmt_ver(am.MP0_HWIP)}_sos.bin",
    ...
    for fw_i in range(sos_hdr.psp_fw_bin_count):
      fw_bin_desc = am.struct_psp_fw_bin_desc.from_address(
        ctypes.addressof(fw_bin) + fw_i * ctypes.sizeof(am.struct_psp_fw_bin_desc)
      )
      ucode_start_offset = (
    ...
    # SDMA firmware
    blob, hdr = self.load_fw(
      f"sdma_{fmt_ver(am.SDMA0_HWIP)}.bin",
      versioned_header="struct_sdma_firmware_header",
    )
    ...
    # PFP, ME, MEC firmware
    for fw_name, fw_cnt in (
      [("PFP", 1), ("ME", 1)] if self.adev.ip_ver[am.GC_HWIP] >= (12, 0, 0) else []
    ) + [("MEC", 1)]:
      blob, hdr = self.load_fw(
      ...
      # Stack
      stack_fws = [
        getattr(am, f"GFX_FW_TYPE_RS64_{fw_name}_P{fwnum}_STACK")
        for fwnum in range(fw_cnt)
      ]
      self.descs += [
        self.desc(blob, hdr.data_offset_bytes, hdr.data_size_bytes, *stack_fws)
      ]
    ...
    self.descs += [
      self.desc(blob, imu_i_off, imu_i_sz, am.GFX_FW_TYPE_IMU_I),
      self.desc(blob, imu_i_off + imu_i_sz, imu_d_sz, am.GFX_FW_TYPE_IMU_D),
    ]
    ...
    for mem, fmem in [("IRAM", "iram"), ("DRAM_BOOT", "dram")]:
      off, sz = (
        getattr(hdr2, f"rlc_{fmem}_ucode_offset_bytes"),
        getattr(hdr2, f"rlc_{fmem}_ucode_size_bytes"),
      )
    ...
    if hdr0.header.header_version_minor == 3:
      for mem in ["P", "V"]:
        off, sz = (
          getattr(hdr3, f"rlc{mem.lower()}_ucode_offset_bytes"),
          getattr(hdr3, f"rlc{mem.lower()}_ucode_size_bytes"),
        )
        self.descs += [
          self.desc(blob, off, sz, getattr(am, f"GFX_FW_TYPE_RLC_{mem}"))
        ]
  ...
  def load_fw(self, fname: str, *headers, versioned_header: str | None = None):
    fpath = fetch(
      f"https://gitlab.com/kernel-firmware/linux-firmware/-/raw/a9f26799247aa60fbaa3b64267a18f20b72b5235/amdgpu/{fname}",
      subdir="fw",
    )
    blob = memoryview(bytearray(fpath.read_bytes()))
    if AM_DEBUG >= 1:
      print(
        f"am {self.adev.devfmt}: loading firmware {fname}: {hashlib.sha256(blob).hexdigest()}"
      )
    if versioned_header:
      ...
      headers += (
        getattr(
          am,
          versioned_header
          + f"_v{chdr.header_version_major}_{chdr.header_version_minor}",
        ),
      )
    return tuple([blob] + [hdr.from_address(mv_address(blob)) for hdr in headers])

  def desc(
    self, blob: memoryview, offset: int, size: int, *types: int
...
class AMPageTableEntry:
  def __init__(self, adev, paddr, lv):
    self.adev, self.paddr, self.lv, self.entries = (
      adev,
      paddr,
      lv,
      adev.vram.view(paddr, 0x1000, fmt="Q"),
    )
  ...
  def set_entry(
    ...
    assert paddr & self.adev.gmc.address_space_mask == paddr, (
      f"Invalid physical address {paddr:#x}"
    )
    self.entries[entry_id] = self.adev.gmc.get_pte_flags(
      self.lv, table, frag, uncached, system, snooped, valid
  ...
  def is_page(self, entry_id: int) -> bool:
    return self.lv == am.AMDGPU_VM_PTB or self.adev.gmc.is_pte_huge_page(
...
class AMDev(PCIDevImplBase):
  ...
  def __init__(
    ...
    self.pci_dev, self.devfmt, self.dma_regions = (
      pci_dev,
      pci_dev.pcibus,
      dma_regions,
    )
    self.vram, self.doorbell64, self.mmio = (
      self.pci_dev.map_bar(0),
      self.pci_dev.map_bar(2, fmt="Q"),
      self.pci_dev.map_bar(5, fmt="I"),
    )
    ...
    self.partial_boot = (self.reg("regSCRATCH_REG7").read() == AMDev.Version) and (
      getenv("AM_RESET", 0) != 1
    )
    if self.partial_boot and (
      self.reg("regGCVM_CONTEXT0_CNTL").read() != 0
      or self.reg(self.gmc.pf_status_reg("GC")).read() != 0
    ):
      if DEBUG >= 2:
  ...
  def init_sw(self, smi_dev=False):
    ...
    self.mm = AMMemoryManager(
      ...
      palloc_ranges=[
        (1 << (i + 12), 0x1000) for i in range(9 * (3 - am.AMDGPU_VM_PDB2), -1, -1)
      ],
  ...
  def _read_vram(self, addr, size) -> bytes:
    assert addr % 4 == 0 and size % 4 == 0, (
      f"Invalid address {addr:#x} or size {size:#x}"
    )
    res = []
  ...
  def _run_discovery(self):
    ...
    disc_tbl = (
      self.vram.view(tmr_offset, tmr_size)[:]
      if self.large_bar
      else self._read_vram(tmr_offset, tmr_size)
    )
    self.bhdr = am.struct_binary_header.from_buffer(bytearray(disc_tbl))
    ihdr = am.struct_ip_discovery_header.from_address(
      ctypes.addressof(self.bhdr) + self.bhdr.table_list[am.IP_DISCOVERY].offset
    )
    assert (
      self.bhdr.binary_signature == am.BINARY_SIGNATURE
      and ihdr.signature == am.DISCOVERY_TABLE_SIGNATURE
    ), "discovery signatures mismatch"
    ...
    for num_die in range(ihdr.num_dies):
      dhdr = am.struct_die_header.from_address(
        ctypes.addressof(self.bhdr) + ihdr.die_info[num_die].die_offset
      )

      ip_offset = (
        ctypes.addressof(self.bhdr)
        + ctypes.sizeof(dhdr)
      ...
      for _ in range(dhdr.num_ips):
        ...
        ba = (
          (ctypes.c_uint64 if ihdr.base_addr_64_bit else ctypes.c_uint32)
          * ip.num_base_address
        ).from_address(ip_offset + 8)
    ...
    gc_info = am.struct_gc_info_v1_0.from_address(
      gc_addr := ctypes.addressof(self.bhdr) + self.bhdr.table_list[am.GC].offset
    )
    ...
    for prefix, hwip in mods:
      self.__dict__.update(
        import_asic_regs(
          prefix,
          self.ip_ver[hwip],
          cls=functools.partial(AMRegister, adev=self, bases=self.regs_offset[hwip]),
        )
      )
    self.__dict__.update(
      import_asic_regs(
        ...
        cls=functools.partial(
          AMRegister, adev=self, bases=self.regs_offset[am.MP1_HWIP]
        ),
      )
    )
