...
class AM_GMC(AM_IP):
  ...
  def init_hub(self, ip: Literal["MM", "GC"]):
    ...
    self.adev.wreg_pair(
      f"reg{ip}VM_L2_CONTEXT1_IDENTITY_APERTURE_LOW_ADDR", "_LO32", "_HI32", 0xFFFFFFFFF
    )
    self.adev.wreg_pair(
...
class AM_SMU(AM_IP):
  ...
  def read_table(self, table_t, cmd):
    ...
    return table_t.from_buffer(
      bytearray(self.adev.vram.view(self.driver_table_paddr, ctypes.sizeof(table_t))[:])
    )
...
class AM_GFX(AM_IP):
  ...
  def set_clockgating_state(self):
    ...
    self.adev.regRLC_CGCG_CGLS_CTRL.update(
      cgcg_gfx_idle_threshold=0x36, cgcg_en=1, cgls_rep_compansat_delay=0xF, cgls_en=1
    )
...
class AM_PSP(AM_IP):
  def init_sw(self):
    ...
    msg1_region = next(
      (reg for reg in self.adev.dma_regions or [] if reg[1].nbytes >= (512 << 10)), None
    )
  ...
  def _ring_submit(self, cmd: am.struct_psp_gfx_cmd_resp) -> am.struct_psp_gfx_cmd_resp:
    ...
    self.adev.vram.view(self.cmd_paddr, ctypes.sizeof(cmd))[:] = memoryview(cmd).cast(
      "B"
    )
  ...
  def _load_ip_fw_cmd(self, fw_types: list[int], fw_bytes: memoryview):
    ...
    for fw_type in fw_types:
      ...
      cmd.cmd.cmd_load_ip_fw.fw_phy_addr_hi, cmd.cmd.cmd_load_ip_fw.fw_phy_addr_lo = (
  ...
  def _load_toc_cmd(self, toc_size: int) -> am.struct_psp_gfx_cmd_resp:
    ...
    cmd.cmd.cmd_load_toc.toc_phy_addr_hi, cmd.cmd.cmd_load_toc.toc_phy_addr_lo = data64(
      self.msg1_addr
    )
