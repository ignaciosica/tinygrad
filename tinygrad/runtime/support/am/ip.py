...
class AM_IP:
  ...
  def set_clockgating_state(self):
    pass  # Set clockgating state for this IP
...
class AM_SOC(AM_IP):
  ...
  def set_clockgating_state(self):
    self.adev.regHDP_MEM_POWER_CTRL.update(
  ...
  def doorbell_enable(self, port, awid=0, awaddr_31_28_value=0, offset=0, size=0):
    self.adev.reg(
      f"{'regGDC_S2A0_S2A' if self.adev.ip_ver[am.GC_HWIP] >= (12, 0, 0) else 'regS2A'}_DOORBELL_ENTRY_{port}_CTRL"
    ).update(**{
      f"s2a_doorbell_port{port}_enable": 1,
      f"s2a_doorbell_port{port}_awid": awid,
      f"s2a_doorbell_port{port}_awaddr_31_28_value": awaddr_31_28_value,
      f"s2a_doorbell_port{port}_range_offset": offset,
...
class AM_GMC(AM_IP):
  def init_sw(self):
    ...
    self.pf_status_reg = (
      lambda ip: f"reg{ip}VM_L2_PROTECTION_FAULT_STATUS{'_LO32' if self.adev.ip_ver[am.GC_HWIP] >= (12, 0, 0) else ''}"
    )
  ...
  def flush_hdp(self):
    self.adev.wreg(
      self.adev.reg("regBIF_BX0_REMAP_HDP_MEM_FLUSH_CNTL").read() // 4, 0x0
    )
  ...
  def flush_tlb(self, ip: Literal["MM", "GC"], vmid, flush_type=0):
    ...
    if not self.hub_initted[ip]:
      return

    if ip == "MM":
      wait_cond(
        lambda: self.adev.regMMVM_INVALIDATE_ENG17_SEM.read() & 0x1,
        value=1,
        msg="mm flush_tlb timeout",
      )

    self.adev.reg(f"reg{ip}VM_INVALIDATE_ENG17_REQ").write(
    ...
    wait_cond(
      lambda: self.adev.reg(f"reg{ip}VM_INVALIDATE_ENG17_ACK").read() & (1 << vmid),
      value=(1 << vmid),
    ...
    if ip == "MM":
      ...
      self.adev.regMMVM_L2_BANK_SELECT_RESERVED_CID2.update(
  ...
  def enable_vm_addressing(self, page_table, ip: Literal["MM", "GC"], vmid):
    self.adev.wreg_pair(
      f"reg{ip}VM_CONTEXT{vmid}_PAGE_TABLE_START_ADDR",
      "_LO32",
      "_HI32",
      self.vm_base >> 12,
    )
    self.adev.wreg_pair(
      f"reg{ip}VM_CONTEXT{vmid}_PAGE_TABLE_END_ADDR",
      "_LO32",
      "_HI32",
      self.vm_end >> 12,
    )
    self.adev.wreg_pair(
      f"reg{ip}VM_CONTEXT{vmid}_PAGE_TABLE_BASE_ADDR",
      "_LO32",
      "_HI32",
      page_table.paddr | 1,
    )
    self.adev.reg(f"reg{ip}VM_CONTEXT{vmid}_CNTL").write(
      0x1800000,
      ...
      enable_context=1,
      page_table_depth=(3 - page_table.lv),
    )
  ...
  def init_hub(self, ip: Literal["MM", "GC"]):
    ...
    self.adev.wreg_pair(
      f"reg{ip}MC_VM_SYSTEM_APERTURE_DEFAULT_ADDR",
      "_LSB",
      "_MSB",
      self.memscratch_paddr >> 12,
    )
    self.adev.wreg_pair(
      f"reg{ip}VM_L2_PROTECTION_FAULT_DEFAULT_ADDR",
      "_LO32",
      "_HI32",
      self.dummy_page_paddr >> 12,
    )

    self.adev.reg(f"reg{ip}VM_L2_PROTECTION_FAULT_CNTL2").update(
      active_page_migration_pte_read_retry=1
    )

    # Init TLB and cache
    self.adev.reg(f"reg{ip}MC_VM_MX_L1_TLB_CNTL").update(
      enable_l1_tlb=1,
      system_access_mode=3,
      enable_advanced_driver_model=1,
      system_aperture_unmapped_access=0,
      eco_bits=0,
      mtype=self.adev.soc.module.MTYPE_UC,
    )

    self.adev.reg(f"reg{ip}VM_L2_CNTL").update(
      enable_l2_cache=1,
    ...
    self.adev.reg(f"reg{ip}VM_L2_CNTL2").update(
      invalidate_all_l1_tlbs=1, invalidate_l2_cache=1
    )
    self.adev.reg(f"reg{ip}VM_L2_CNTL3").write(
      bank_select=9,
    ...
    self.adev.wreg_pair(
      f"reg{ip}VM_L2_CONTEXT1_IDENTITY_APERTURE_LOW_ADDR",
      "_LO32",
      "_HI32",
      0xFFFFFFFFF,
    )
    self.adev.wreg_pair(
      f"reg{ip}VM_L2_CONTEXT1_IDENTITY_APERTURE_HIGH_ADDR", "_LO32", "_HI32", 0x0
    )
    self.adev.wreg_pair(
      f"reg{ip}VM_L2_CONTEXT_IDENTITY_PHYSICAL_OFFSET", "_LO32", "_HI32", 0x0
    )

    for eng_i in range(18):
      self.adev.wreg_pair(
        f"reg{ip}VM_INVALIDATE_ENG{eng_i}_ADDR_RANGE", "_LO32", "_HI32", 0x1FFFFFFFFF
      )
    self.hub_initted[ip] = True

  @functools.cache  # pylint: disable=method-cache-max-size-none
  def get_pte_flags(
    self, pte_lv, is_table, frag, uncached, system, snooped, valid, extra=0
  ):
    extra |= (
      (am.AMDGPU_PTE_SYSTEM * system)
      | (am.AMDGPU_PTE_SNOOPED * snooped)
      | (am.AMDGPU_PTE_VALID * valid)
      | am.AMDGPU_PTE_FRAG(frag)
    )
    if not is_table:
      extra |= (
        am.AMDGPU_PTE_WRITEABLE | am.AMDGPU_PTE_READABLE | am.AMDGPU_PTE_EXECUTABLE
      )
    if self.adev.ip_ver[am.GC_HWIP] >= (12, 0, 0):
      extra |= am.AMDGPU_PTE_MTYPE_GFX12(
        0, self.adev.soc.module.MTYPE_UC if uncached else 0
      )
      extra |= (
        am.AMDGPU_PDE_PTE_GFX12
        if not is_table and pte_lv != am.AMDGPU_VM_PTB
        else (am.AMDGPU_PTE_IS_PTE if not is_table else 0)
      )
    else:
      extra |= am.AMDGPU_PTE_MTYPE_NV10(
        0, self.adev.soc.module.MTYPE_UC if uncached else 0
      )
      extra |= (
        am.AMDGPU_PDE_PTE if not is_table and pte_lv != am.AMDGPU_VM_PTB else 0
      )
    return extra
  ...
  def on_interrupt(self):
    for ip in ["MM", "GC"]:
      va = (
        self.adev.reg(f"reg{ip}VM_L2_PROTECTION_FAULT_ADDR_HI32").read() << 32
      ) | self.adev.reg(f"reg{ip}VM_L2_PROTECTION_FAULT_ADDR_LO32").read()
      if self.adev.reg(self.pf_status_reg(ip)).read():
        raise RuntimeError(
          f"{ip}VM_L2_PROTECTION_FAULT_STATUS: {self.adev.reg(self.pf_status_reg(ip)).read_bitfields()} {va << 12:#x}"
        )
...
class AM_SMU(AM_IP):
  ...
  def init_hw(self):
    self._send_msg(
      self.smu_mod.PPSMC_MSG_SetDriverDramAddrHigh,
      hi32(self.adev.paddr2mc(self.driver_table_paddr)),
    )
    self._send_msg(
      self.smu_mod.PPSMC_MSG_SetDriverDramAddrLow,
      lo32(self.adev.paddr2mc(self.driver_table_paddr)),
    )
    self._send_msg(self.smu_mod.PPSMC_MSG_EnableAllSmuFeatures, 0)

  def is_smu_alive(self):
    with contextlib.suppress(RuntimeError):
      self._send_msg(self.smu_mod.PPSMC_MSG_GetSmuVersion, 0, timeout=100)
    return self.adev.mmMP1_SMN_C2PMSG_90.read() != 0

  def mode1_reset(self):
    if DEBUG >= 2:
      print(f"am {self.adev.devfmt}: mode1 reset")
    if self.adev.ip_ver[am.MP0_HWIP] >= (14, 0, 0):
  ...
  def read_table(self, table_t, cmd):
    ...
    return table_t.from_buffer(
      bytearray(
        self.adev.vram.view(self.driver_table_paddr, ctypes.sizeof(table_t))[:]
      )
    )

  def read_metrics(self):
    return self.read_table(
      self.smu_mod.SmuMetricsExternal_t, self.smu_mod.TABLE_SMU_METRICS
    )
  ...
  def set_clocks(self, level):
    if not hasattr(self, "clcks"):
      self.clcks = {}
      for clck in [
        self.smu_mod.PPCLK_GFXCLK,
        self.smu_mod.PPCLK_UCLK,
        self.smu_mod.PPCLK_FCLK,
        self.smu_mod.PPCLK_SOCCLK,
      ]:
        cnt = (
          self._send_msg(
            self.smu_mod.PPSMC_MSG_GetDpmFreqByIndex,
            (clck << 16) | 0xFF,
            read_back_arg=True,
          )
          & 0x7FFFFFFF
    ...
    for clck, vals in self.clcks.items():
      self._send_msg(
        self.smu_mod.PPSMC_MSG_SetSoftMinByFreq, clck << 16 | (vals[level])
      )
      self._send_msg(
        self.smu_mod.PPSMC_MSG_SetSoftMaxByFreq, clck << 16 | (vals[level])
      )

  def _smu_cmn_send_msg(self, msg: int, param=0, debug=False):
    (
      self.adev.mmMP1_SMN_C2PMSG_90 if not debug else self.adev.mmMP1_SMN_C2PMSG_54
    ).write(0)  # resp reg
    (
      self.adev.mmMP1_SMN_C2PMSG_82 if not debug else self.adev.mmMP1_SMN_C2PMSG_53
    ).write(param)
    (
      self.adev.mmMP1_SMN_C2PMSG_66 if not debug else self.adev.mmMP1_SMN_C2PMSG_75
    ).write(msg)

  def _send_msg(
    self, msg: int, param: int, read_back_arg=False, timeout=10000, debug=False
...
class AM_GFX(AM_IP):
  def init_hw(self):
    ...
    while (
      self.adev.regCP_STAT.read() != 0
      and self.adev.regRLC_RLCS_BOOTLOAD_STATUS.read_bitfields()["bootload_complete"]
      != 0
    ):
      pass
    ...
    for i in range(0, 16):
      ...
      self.adev.regSH_MEM_CONFIG.write(
        address_mode=self.adev.soc.module.SH_MEM_ADDRESS_MODE_64,
    ...
    # Enable MEC
    self.adev.regCP_MEC_RS64_CNTL.update(
      mec_invalidate_icache=0, mec_pipe0_reset=0, mec_pipe0_active=1, mec_halt=0
    )
  ...
  def setup_ring(
    ...
    mqd_struct = struct_t(
      header=0xC0310800,
      cp_mqd_base_addr_lo=lo32(mqd.va_addr),
      cp_mqd_base_addr_hi=hi32(mqd.va_addr),
      cp_hqd_persistent_state=self.adev.regCP_HQD_PERSISTENT_STATE.encode(
        preload_size=0x55, preload_req=1
      ),
      cp_hqd_pipe_priority=0x2,
      cp_hqd_queue_priority=0xF,
      cp_hqd_quantum=0x111,
      cp_hqd_pq_base_lo=lo32(ring_addr >> 8),
      cp_hqd_pq_base_hi=hi32(ring_addr >> 8),
      cp_hqd_pq_rptr_report_addr_lo=lo32(rptr_addr),
      cp_hqd_pq_rptr_report_addr_hi=hi32(rptr_addr),
      cp_hqd_pq_wptr_poll_addr_lo=lo32(wptr_addr),
      cp_hqd_pq_wptr_poll_addr_hi=hi32(wptr_addr),
      cp_hqd_pq_doorbell_control=self.adev.regCP_HQD_PQ_DOORBELL_CONTROL.encode(
        doorbell_offset=doorbell * 2, doorbell_en=1
      ),
      cp_hqd_pq_control=self.adev.regCP_HQD_PQ_CONTROL.encode(
        rptr_block_size=5,
        unord_dispatch=0,
        queue_size=(ring_size // 4).bit_length() - 2,
        **(
      ...
      cp_hqd_vmid=0,
      cp_hqd_aql_control=int(aql),
      cp_hqd_eop_base_addr_lo=lo32(eop_addr >> 8),
      cp_hqd_eop_base_addr_hi=hi32(eop_addr >> 8),
      cp_hqd_eop_control=self.adev.regCP_HQD_EOP_CONTROL.encode(
        eop_size=(eop_size // 4).bit_length() - 2
      ),
    )
    for se in range(8):
      setattr(mqd_struct, f"compute_static_thread_mgmt_se{se}", 0xFFFFFFFF)

    # Copy mqd into memory
    self.adev.vram.view(mqd.paddrs[0][0], ctypes.sizeof(mqd_struct))[:] = memoryview(
      mqd_struct
    ).cast("B")
    ...
    mqd_st_mv = to_mv(ctypes.addressof(mqd_struct), ctypes.sizeof(mqd_struct)).cast(
      "I"
    )
    for i, reg in enumerate(
      range(
        self.adev.regCP_MQD_BASE_ADDR.addr[0],
        self.adev.regCP_HQD_PQ_WPTR_HI.addr[0] + 1,
      )
    ):
    ...
    self.adev.reg(f"regCP_ME1_PIPE{pipe}_INT_CNTL").update(
      time_stamp_int_enable=1, generic0_int_enable=1
    )

  def set_clockgating_state(self):
    if hasattr(self.adev, "regMM_ATC_L2_MISC_CG"):
      self.adev.regMM_ATC_L2_MISC_CG.write(enable=1, mem_ls_enable=1)

    self.adev.regRLC_SAFE_MODE.write(message=1, cmd=1)
    wait_cond(
      lambda: self.adev.regRLC_SAFE_MODE.read() & 0x1,
      value=0,
      msg="RLC safe mode timeout",
    )

    self.adev.regRLC_CGCG_CGLS_CTRL.update(
  ...
  def _grbm_select(self, me=0, pipe=0, queue=0, vmid=0):
    self.adev.regGRBM_GFX_CNTL.write(meid=me, pipeid=pipe, vmid=vmid, queueid=queue)
  ...
  def _config_gfx_rs64(self):
    def _config_helper(eng_name, cntl_reg, eng_reg, pipe_cnt, me=0):
      for pipe in range(pipe_cnt):
        ...
        self.adev.wreg_pair(
          f"regCP_{eng_reg}_PRGRM_CNTR_START",
          "",
          "_HI",
          self.adev.fw.ucode_start[eng_name] >> 2,
        )
      self._grbm_select()
      self.adev.reg(f"regCP_{cntl_reg}_CNTL").update(**{
        f"{eng_name.lower()}_pipe{pipe}_reset": 1 for pipe in range(pipe_cnt)
      })
      self.adev.reg(f"regCP_{cntl_reg}_CNTL").update(**{
        f"{eng_name.lower()}_pipe{pipe}_reset": 0 for pipe in range(pipe_cnt)
      })
    ...
    _config_helper(
      eng_name="MEC", cntl_reg="MEC_RS64", eng_reg="MEC_RS64", pipe_cnt=1, me=1
    )
...
class AM_IH(AM_IP):
  def init_sw(self):
    ...
    def _alloc_ring(size):
      return (
        self.adev.mm.palloc(size, zero=False, boot=True),
        self.adev.mm.palloc(0x1000, zero=False, boot=True),
      )

    self.rings = [
      (*_alloc_ring(self.ring_size), "", 0),
      (*_alloc_ring(self.ring_size), "_RING1", 1),
    ]

  def init_hw(self):
    for ring_vm, rwptr_vm, suf, ring_id in self.rings:
      self.adev.wreg_pair(
        "regIH_RB_BASE", suf, f"_HI{suf}", self.adev.paddr2mc(ring_vm) >> 8
      )

      self.adev.reg(f"regIH_RB_CNTL{suf}").write(
        mc_space=4,
        wptr_overflow_clear=1,
        rb_size=(self.ring_size // 4).bit_length(),
        mc_snoop=1,
        mc_ro=0,
        mc_vmid=0,
        **(
      ...
      if ring_id == 0:
        self.adev.wreg_pair(
          "regIH_RB_WPTR_ADDR", "_LO", "_HI", self.adev.paddr2mc(rwptr_vm)
        )

      self.adev.reg(f"regIH_RB_WPTR{suf}").write(0)
      self.adev.reg(f"regIH_RB_RPTR{suf}").write(0)

      self.adev.reg(f"regIH_DOORBELL_RPTR{suf}").write(
        offset=(am.AMDGPU_NAVI10_DOORBELL_IH + ring_id) * 2, enable=1
      )
    ...
    for _, rwptr_vm, suf, ring_id in self.rings:
      self.adev.reg(f"regIH_RB_CNTL{suf}").update(
        rb_enable=1, **({"enable_intr": 1} if ring_id == 0 else {})
      )

    self.adev.soc.doorbell_enable(
      port=1,
      awid=0x0,
      awaddr_31_28_value=0x0,
      offset=am.AMDGPU_NAVI10_DOORBELL_IH * 2,
      size=2,
    )
...
class AM_SDMA(AM_IP):
  def init_sw(self):
    self.sdma_name = "F32" if self.adev.ip_ver[am.SDMA0_HWIP] < (7, 0, 0) else "MCU"

  def init_hw(self):
    for pipe in range(2):
      self.adev.reg(f"regSDMA{pipe}_WATCHDOG_CNTL").update(
        queue_hang_count=100
      )  # 10s, 100ms per unit
      self.adev.reg(f"regSDMA{pipe}_UTCL1_CNTL").update(resp_mode=3, redo_delay=9)

      # rd=noa, wr=bypass
      self.adev.reg(f"regSDMA{pipe}_UTCL1_PAGE").update(
        rd_l2_policy=0x2,
        wr_l2_policy=0x3,
        **({"llc_noalloc": 1} if self.sdma_name == "F32" else {}),
      )
      self.adev.reg(f"regSDMA{pipe}_{self.sdma_name}_CNTL").update(
        halt=0, **{f"{'th1_' if self.sdma_name == 'F32' else ''}reset": 0}
      )
      self.adev.reg(f"regSDMA{pipe}_CNTL").update(
        ctxempty_int_enable=1, trap_enable=1
      )
    self.adev.soc.doorbell_enable(
      port=2,
      awid=0xE,
      awaddr_31_28_value=0x3,
      offset=am.AMDGPU_NAVI10_DOORBELL_sDMA_ENGINE0 * 2,
      size=4,
    )
  ...
  def setup_ring(
    ...
    self.adev.wreg_pair(
      f"regSDMA{pipe}_QUEUE{queue}_RB_BASE", "", "_HI", ring_addr >> 8
    )
    self.adev.wreg_pair(
      f"regSDMA{pipe}_QUEUE{queue}_RB_RPTR_ADDR", "_LO", "_HI", rptr_addr
    )
    self.adev.wreg_pair(
      f"regSDMA{pipe}_QUEUE{queue}_RB_WPTR_POLL_ADDR", "_LO", "_HI", wptr_addr
    )
    self.adev.reg(f"regSDMA{pipe}_QUEUE{queue}_DOORBELL_OFFSET").update(
      offset=doorbell * 2
    )
    ...
    self.adev.reg(f"regSDMA{pipe}_QUEUE{queue}_RB_CNTL").write(
      rb_vmid=0,
      rptr_writeback_enable=1,
      rptr_writeback_timer=4,
      **{f"{self.sdma_name.lower()}_wptr_poll_enable": 1},
      rb_size=(ring_size // 4).bit_length() - 1,
      rb_enable=1,
      rb_priv=1,
    )
...
class AM_PSP(AM_IP):
  def init_sw(self):
    ...
    msg1_region = next(
      (reg for reg in self.adev.dma_regions or [] if reg[1].nbytes >= (512 << 10)),
      None,
    )
    if msg1_region is not None:
      self.msg1_addr, self.msg1_view = (
        self.adev.mm.alloc_vaddr(size=msg1_region[1].nbytes, align=am.PSP_1_MEG),
        msg1_region[1],
      )
      self.adev.mm.map_range(
        self.msg1_addr,
        msg1_region[1].nbytes,
        [(msg1_region[0], msg1_region[1].nbytes)],
        system=True,
        uncached=True,
        boot=True,
      )
    else:
      self.msg1_paddr = self.adev.mm.palloc(
        am.PSP_1_MEG, align=am.PSP_1_MEG, zero=False, boot=True
      )
      self.msg1_addr, self.msg1_view = (
        self.adev.paddr2mc(self.msg1_paddr),
        self.adev.vram.view(self.msg1_paddr, am.PSP_1_MEG, "B"),
      )

    self.cmd_paddr = self.adev.mm.palloc(
      am.PSP_CMD_BUFFER_SIZE, zero=False, boot=True
    )
    self.fence_paddr = self.adev.mm.palloc(
      am.PSP_FENCE_BUFFER_SIZE, zero=True, boot=True
    )
    ...
    if not self.boot_time_tmr:
      self.tmr_paddr = self.adev.mm.palloc(
        self.max_tmr_size, align=am.PSP_TMR_ALIGNMENT, zero=False, boot=True
      )
  ...
  def init_hw(self):
    ...
    if not self.is_sos_alive():
      for fw, compid in sos_components:
        self._bootloader_load_component(fw, compid)
      while not self.is_sos_alive():
        time.sleep(0.01)
    ...
    for psp_desc in self.adev.fw.descs:
      self._load_ip_fw_cmd(*psp_desc)
  ...
  def _wait_for_bootloader(self):
    wait_cond(
      lambda: self.adev.reg(f"{self.reg_pref}_35").read() & 0x80000000,
      value=0x80000000,
      msg="BL not ready",
    )

  def _prep_msg1(self, data: memoryview):
    assert len(data) <= self.msg1_view.nbytes, (
      f"msg1 buffer is too small {len(data):#x} > {self.msg1_view.nbytes:#x}"
    )
    padded_data = pad_bytes(
      bytes(data) + b"\x00" * 4, 16
    )  # HACK: apple's memcpy requires 16-bytes alignment
    self.msg1_view[: len(padded_data)] = padded_data
    self.adev.gmc.flush_hdp()

  def _bootloader_load_component(self, fw: int, compid: int):
    if fw not in self.adev.fw.sos_fw:
      return 0
    ...
    if DEBUG >= 2:
      print(
        f"am {self.adev.devfmt}: loading sos component: {am.psp_fw_type__enumvalues[fw]}"
      )
  ...
  def _ring_create(self):
    ...
    wait_cond(
      lambda: self.adev.reg(f"{self.reg_pref}_64").read() & 0x80000000,
      value=0x80000000,
      msg="sOS not ready",
    )

    self.adev.wreg_pair(
      self.reg_pref, "_69", "_70", self.adev.paddr2mc(self.ring_paddr)
    )
    ...
    wait_cond(
      lambda: self.adev.reg(f"{self.reg_pref}_64").read() & 0x8000FFFF,
      value=0x80000000,
      msg="sOS ring not created",
    )

  def _ring_submit(
    self, cmd: am.struct_psp_gfx_cmd_resp
  ) -> am.struct_psp_gfx_cmd_resp:
    msg = am.struct_psp_gfx_rb_frame(
      fence_value=(prev_wptr := self.adev.reg(f"{self.reg_pref}_67").read()) + 1,
      cmd_buf_addr_lo=lo32(self.adev.paddr2mc(self.cmd_paddr)),
      cmd_buf_addr_hi=hi32(self.adev.paddr2mc(self.cmd_paddr)),
      fence_addr_lo=lo32(self.adev.paddr2mc(self.fence_paddr)),
      fence_addr_hi=hi32(self.adev.paddr2mc(self.fence_paddr)),
    )

    self.adev.vram.view(self.cmd_paddr, ctypes.sizeof(cmd))[:] = memoryview(
      cmd
    ).cast("B")
    self.adev.vram.view(self.ring_paddr + prev_wptr * 4, ctypes.sizeof(msg))[:] = (
      memoryview(msg).cast("B")
    )

    # Move the wptr
    self.adev.reg(f"{self.reg_pref}_67").write(
      prev_wptr + ctypes.sizeof(am.struct_psp_gfx_rb_frame) // 4
    )

    wait_cond(
      lambda: self.adev.vram.view(self.fence_paddr, 4, "I")[0],
      value=msg.fence_value,
      msg="sOS ring not responding",
    )

    resp = type(cmd).from_buffer(
      bytearray(self.adev.vram.view(self.cmd_paddr, ctypes.sizeof(cmd))[:])
    )
    if resp.resp.status != 0:
      raise RuntimeError(f"PSP command failed {resp.cmd_id} {resp.resp.status}")
  ...
  def _load_ip_fw_cmd(self, fw_types: list[int], fw_bytes: memoryview):
    ...
    for fw_type in fw_types:
      if DEBUG >= 2:
        print(
          f"am {self.adev.devfmt}: loading fw: {am.psp_gfx_fw_type__enumvalues[fw_type]}"
        )
  ...
  def _tmr_load_cmd(self) -> am.struct_psp_gfx_cmd_resp:
    ...
    cmd.cmd.cmd_setup_tmr.buf_phy_addr_hi, cmd.cmd.cmd_setup_tmr.buf_phy_addr_lo = (
      data64(self.adev.paddr2mc(self.tmr_paddr))
    )
    (
      cmd.cmd.cmd_setup_tmr.system_phy_addr_hi,
      cmd.cmd.cmd_setup_tmr.system_phy_addr_lo,
    ) = data64(self.tmr_paddr)
  ...
  def _load_toc_cmd(self, toc_size: int) -> am.struct_psp_gfx_cmd_resp:
    ...
    cmd.cmd.cmd_load_toc.toc_phy_addr_hi, cmd.cmd.cmd_load_toc.toc_phy_addr_lo = (
      data64(self.msg1_addr)
    )
  ...
  def _rlc_autoload_cmd(self):
    return self._ring_submit(
      am.struct_psp_gfx_cmd_resp(cmd_id=am.GFX_CMD_ID_AUTOLOAD_RLC)
    )
