...
class AM_GMC(AM_IP):
  ...
  def flush_hdp(self):
    self.adev.wreg(self.adev.reg("regBIF_BX0_REMAP_HDP_MEM_FLUSH_CNTL").read() // 4, 0x0)
  ...
  def flush_tlb(self, ip: Literal["MM", "GC"], vmid, flush_type=0):
    ...
    if ip == "MM":
      wait_cond(lambda: self.adev.regMMVM_INVALIDATE_ENG17_SEM.read() & 0x1, value=1, msg="mm flush_tlb timeout")
  ...
  def init_hub(self, ip: Literal["MM", "GC"]):
    ...
    self.adev.reg(f"reg{ip}VM_L2_CNTL3").write(
      bank_select=9, l2_cache_bigk_fragment_size=6, l2_cache_4k_associativity=1, l2_cache_bigk_associativity=1
    )
    ...
    for eng_i in range(18):
      self.adev.wreg_pair(f"reg{ip}VM_INVALIDATE_ENG{eng_i}_ADDR_RANGE", "_LO32", "_HI32", 0x1FFFFFFFFF)
  ...
  def get_pte_flags(self, pte_lv, is_table, frag, uncached, system, snooped, valid, extra=0):
    ...
    if not is_table:
      extra |= am.AMDGPU_PTE_WRITEABLE | am.AMDGPU_PTE_READABLE | am.AMDGPU_PTE_EXECUTABLE
...
class AM_SMU(AM_IP):
  ...
  def read_metrics(self):
    return self.read_table(self.smu_mod.SmuMetricsExternal_t, self.smu_mod.TABLE_SMU_METRICS)
  ...
  def set_clocks(self, level):
    if not hasattr(self, "clcks"):
      ...
      for clck in [self.smu_mod.PPCLK_GFXCLK, self.smu_mod.PPCLK_UCLK, self.smu_mod.PPCLK_FCLK, self.smu_mod.PPCLK_SOCCLK]:
        ...
        self.clcks[clck] = [
          self._send_msg(self.smu_mod.PPSMC_MSG_GetDpmFreqByIndex, (clck << 16) | i, read_back_arg=True) & 0x7FFFFFFF for i in range(cnt)
        ]
  ...
  def _send_msg(self, msg: int, param: int, read_back_arg=False, timeout=10000, debug=False):  # default timeout is 10 seconds
    ...
    wait_cond(
      (self.adev.mmMP1_SMN_C2PMSG_90 if not debug else self.adev.mmMP1_SMN_C2PMSG_54).read,
      value=1,
      timeout_ms=timeout,
...
class AM_GFX(AM_IP):
  def init_hw(self):
    ...
    while self.adev.regCP_STAT.read() != 0 and self.adev.regRLC_RLCS_BOOTLOAD_STATUS.read_bitfields()["bootload_complete"] != 0:
      pass
...
class AM_IH(AM_IP):
  def init_sw(self):
    ...
    def _alloc_ring(size):
      return (self.adev.mm.palloc(size, zero=False, boot=True), self.adev.mm.palloc(0x1000, zero=False, boot=True))
  ...
  def init_hw(self):
    for ring_vm, rwptr_vm, suf, ring_id in self.rings:
      ...
      self.adev.reg(f"regIH_RB_CNTL{suf}").write(
        ...
        mc_ro=0,
        mc_vmid=0,
        **({"wptr_overflow_enable": 1, "rptr_rearm": 1} if ring_id == 0 else {"rb_full_drain_enable": 1}),
      )

      if ring_id == 0:
        self.adev.wreg_pair("regIH_RB_WPTR_ADDR", "_LO", "_HI", self.adev.paddr2mc(rwptr_vm))
...
class AM_PSP(AM_IP):
  ...
  def _bootloader_load_component(self, fw: int, compid: int):
    ...
    if DEBUG >= 2:
      print(f"am {self.adev.devfmt}: loading sos component: {am.psp_fw_type__enumvalues[fw]}")
  ...
  def _load_ip_fw_cmd(self, fw_types: list[int], fw_bytes: memoryview):
    ...
    for fw_type in fw_types:
      if DEBUG >= 2:
        print(f"am {self.adev.devfmt}: loading fw: {am.psp_gfx_fw_type__enumvalues[fw_type]}")
  ...
  def _rlc_autoload_cmd(self):
    return self._ring_submit(am.struct_psp_gfx_cmd_resp(cmd_id=am.GFX_CMD_ID_AUTOLOAD_RLC))
