...
class ClangJITCompiler(Compiler):
  ...
  def compile(self, src: str) -> bytes:
    ...
    arch = {
      ...
      "AMD64": "-march=native",
      "riscv64": "-march=rv64g",
    }.get(platform.machine(), "-mcpu=native")
    args = [
      arch,
    ...
    obj = subprocess.check_output(
      [getenv("CC", "clang"), "-c", "-x", "c", *args, *arch_args, "-", "-o", "-"],
      input=src.encode("utf-8"),
    )
    return jit_loader(obj)

  def disassemble(self, lib: bytes):
    return capstone_flatdump(lib)


def cerr():
  return ctypes.pointer(ctypes.pointer(ctypes.c_char()))


def expect(x, err, ret=None):
  if x:
    raise RuntimeError(
      llvm.string_cast(err.contents) if not isinstance(err, str) else err
    )
  return ret
...
class LLVMCompiler(Compiler):
  ...
  def __init__(self, processor: str, feats: str):
    for component in ["Target", "TargetInfo", "TargetMC", "AsmParser", "AsmPrinter"]:
      getattr(llvm, f"LLVMInitialize{self.target_arch}{component}")()

    triple = {
      "AArch64": b"aarch64-none-unknown-elf",
      "X86": b"x86_64-none-unknown-elf",
      "AMDGPU": b"amdgcn-amd-amdhsa",
    }[self.target_arch]
    target = expect(
      llvm.LLVMGetTargetFromTriple(
        triple, ctypes.pointer(tgt := llvm.LLVMTargetRef()), err := cerr()
      ),
      err,
      tgt,
    )
    if DEBUG >= 3:
      print(f"LLVM init for {processor!r} with {feats!r}")
    self.target_machine = llvm.LLVMCreateTargetMachine(
      target,
      triple,
      processor.encode(),
      feats.encode(),
    ...
    llvm.LLVMContextSetDiagnosticHandler(
      llvm.LLVMGetGlobalContext(), handle_diag, None
    )
    super().__init__(
      f"compile_llvm_{processor}_{feats}{'_jit' if self.jit else ''}{'_opt' if opt else ''}"
    )
  ...
  def compile(self, src: str) -> bytes:
    ...
    src_buf = llvm.LLVMCreateMemoryBufferWithMemoryRangeCopy(
      ctypes.create_string_buffer(src_bytes := src.encode()), len(src_bytes), b"src"
    )
    mod = expect(
      llvm.LLVMParseIRInContext(
        llvm.LLVMGetGlobalContext(),
        src_buf,
        ctypes.pointer(m := llvm.LLVMModuleRef()),
        err := cerr(),
      ),
      err,
      m,
    ...
    expect(
      llvm.LLVMVerifyModule(mod, llvm.LLVMReturnStatusAction, err := cerr()), err
    )
    expect(
      llvm.LLVMRunPasses(mod, self.passes, self.target_machine, self.pbo),
      "failed to run passes",
    )
    if DEBUG >= 7:
      print(ctypes.string_at(llvm.LLVMPrintModuleToString(mod)).decode())
    obj_buf = expect(
    ...
    if self.diag_msgs:
      raise RuntimeError("llvm diagnostic: " + "\n".join(self.diag_msgs))
    return jit_loader(obj) if self.jit else obj

  def disassemble(self, lib: bytes):
    capstone_flatdump(lib)
...
class CPULLVMCompiler(LLVMCompiler):
  def __init__(self):
    ...
    cpu, feats = (
      ctypes.string_at(llvm.LLVMGetHostCPUName()),
      (b"+reserve-x18," if OSX else b"")
      + ctypes.string_at(llvm.LLVMGetHostCPUFeatures()),
    )
