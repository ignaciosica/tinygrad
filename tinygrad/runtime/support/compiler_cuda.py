...
def cuda_disassemble(lib: bytes, arch: str):
  try:
    ...
    print("Failed to generate SASS", str(e), "Make sure your PATH contains ptxas/nvdisasm binary of compatible version.")
...
class PTXCompiler(Compiler):
  ...
  def compile(self, src: str) -> bytes:
    return (
      src.replace("TARGET", self.arch).replace("VERSION", "8.7" if (ver := int(self.arch[3:])) >= 120 else ("7.8" if ver >= 89 else "7.5")).encode()
    )
