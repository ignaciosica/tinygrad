...
class LVPCompiler(CPULLVMCompiler, NIRCompiler):
  ...
  def compile(self, src) -> bytes:
    ...
    params.consts_ptr = mesa.lp_build_struct_get_ptr2(
      gallivm, params.resources_type, ctypes.cast(llvm.LLVMGetParam(fn, 0), mesa.LLVMValueRef), mesa.LP_JIT_RES_CONSTANTS, b"constants"
    )
    ...
    obj_buf = expect(
      llvm.LLVMTargetMachineEmitToMemoryBuffer(
        self.target_machine, module, llvm.LLVMObjectFile, err := cerr(), ctypes.pointer(buf := llvm.LLVMMemoryBufferRef())
      ),
      err,
      buf,
    )
...
class NAKCompiler(NIRCompiler):
  ...
  def disassemble(self, lib: bytes):
    try:
      ...
      print("Failed to generate SASS", str(e), "Make sure your PATH contains nvdisasm binary of compatible version.")
