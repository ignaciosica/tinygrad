...
def link_sym(sym: str, libs: list[str]) -> int:
  for lib in libs:
    try:
      return unwrap(
        ctypes.cast(
          getattr(ctypes.CDLL(ctypes.util.find_library(lib)), sym), ctypes.c_void_p
        ).value
      )
...
def elf_loader(
  ...
  sections = [
    ElfSection(
      _strtab(sh_strtab, sh.sh_name), sh, blob[sh.sh_offset : sh.sh_offset + sh.sh_size]
    )
  ...
  for sh, trgt_sh_name, c_rels in rel + rela:
    ...
    relocs += [
      (
        ...
        if sym.st_shndx == 0
        else sections[sym.st_shndx].header.sh_addr + sym.st_value,
        rtype,
        raddend,
      )
  ...
  def relocate(instr: int, base: int, ploc: int, tgt: int, r_type: int):
    match r_type:
      ...
      case libc.R_AARCH64_CALL26:
        if -(2**25) <= tgt - ploc - base and tgt - ploc - base <= (2**25 - 1) * 4:
          return instr | getbits(tgt - ploc - base, 2, 27)
        nonlocal image
