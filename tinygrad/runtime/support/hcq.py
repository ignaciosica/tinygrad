...
class MMIOInterface:
  ...
  def __getitem__(self, k):
    return (bytes(self.mv[k]) if self.fmt == "B" else self.mv[k].tolist()) if isinstance(k, slice) else self.mv[k]
...
class FileIOInterface:
  ...
  def _mmap(start, sz, prot, flags, fd, offset):
    ...
    if x == 0xFFFFFFFFFFFFFFFF:
      raise OSError(f"Failed to mmap {sz} bytes at {hex(start)}: {os.strerror(ctypes.get_errno())}")
    return x
...
class CLikeArgsState(HCQArgsState[ProgramType]):
  def __init__(self, buf: HCQBuffer, prg: ProgramType, bufs: tuple[HCQBuffer, ...], vals: tuple[sint, ...] = (), prefix: list[int] | None = None):
    ...
    if prefix is not None:
      self.buf.cpu_view().view(size=len(prefix) * 4, fmt="I")[:] = array.array("I", prefix)
...
class HCQProgram(Generic[HCQDeviceType]):
  def __init__(
    self, args_state_t: Type[HCQArgsState], dev: HCQDeviceType, name: str, kernargs_alloc_size: int, lib: bytes | None = None, base: int | None = None
  ):
...
class HCQCompiled(Compiled, Generic[SignalType]):
  ...
  def _at_profile_finalize(self):
    ...
    if self.hw_copy_queue_t is None:
      ...
      gpu2cpu_copy_time_diff = statistics.median([_sync(self, self.hw_copy_queue_t) for _ in range(40)])
...
class HCQAllocatorBase(LRUAllocator[HCQDeviceType], Generic[HCQDeviceType]):
  ...
  def map(self, buf: HCQBuffer):
    ...
    if buf.owner is None:
      raise RuntimeError(f"map failed: buffer {buf.va_addr} has no owner, it's a virtual buffer")
...
class HCQAllocator(HCQAllocatorBase, Generic[HCQDeviceType]):
  def _copyin(self, dest: HCQBuffer, src: memoryview):
    ...
    with hcq_profile(self.dev, queue_type=self.dev.hw_copy_queue_t, desc=f"TINY -> {self.dev.device}", enabled=PROFILE):
      for i in range(0, src.nbytes, self.b[0].size):
        ...
        self.dev.hw_copy_queue_t().wait(self.dev.timeline_signal, self.dev.timeline_value - 1).copy(
          dest.va_addr + i, self.b[self.b_next].va_addr, lsize
  ...
  def copy_from_disk(self, dest: HCQBuffer, src, size):
    ...
    with hcq_profile(self.dev, queue_type=self.dev.hw_copy_queue_t, desc=f"DISK -> {self.dev.device}", enabled=PROFILE):
      for batch_info, dst_off, src_off, copy_size in src.device.allocator._copyout_sharded(src, size, _get_temp_buf, seg_len=self.b[0].size):
        self.dev.hw_copy_queue_t().wait(self.dev.timeline_signal, self.dev.timeline_value - 1).copy(
          dest.va_addr + dst_off, batch_info[0] + src_off, copy_size
        ).signal(self.dev.timeline_signal, self.dev.next_timeline()).submit(self.dev)
  ...
  def _transfer(self, dest: HCQBuffer, src: HCQBuffer, sz: int, src_dev: HCQDeviceType, dest_dev: HCQDeviceType):
    ...
    with hcq_profile(src_dev, queue_type=src_dev.hw_copy_queue_t, desc=f"{src_dev.device} -> {dest_dev.device}", enabled=PROFILE):
      src_dev.hw_copy_queue_t().wait(src_dev.timeline_signal, src_dev.timeline_value - 1).wait(
        dest_dev.timeline_signal, dest_dev.timeline_value - 1
      ).copy(dest.va_addr, src.va_addr, sz).signal(src_dev.timeline_signal, src_dev.next_timeline()).submit(src_dev)

    if src_dev != dest_dev:
      dest_dev.hw_compute_queue_t().wait(src_dev.timeline_signal, src_dev.timeline_value - 1).wait(
        dest_dev.timeline_signal, dest_dev.timeline_value - 1
