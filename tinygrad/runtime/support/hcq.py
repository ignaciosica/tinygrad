...
try:
  import fcntl  # windows misses that
except ImportError:
...
class MMIOInterface:
  def __init__(self, addr: int, nbytes: int, fmt="B"):
    self.mv, self.addr, self.nbytes, self.fmt = (
      to_mv(addr, nbytes).cast(fmt),
      addr,
      nbytes,
      fmt,
    )
  ...
  def __getitem__(self, k):
    return (
      (bytes(self.mv[k]) if self.fmt == "B" else self.mv[k].tolist())
  ...
  def view(self, offset: int = 0, size: int | None = None, fmt=None) -> MMIOInterface:
    return MMIOInterface(
      self.addr + offset, size or (self.nbytes - offset), fmt=fmt or self.fmt
    )
...
class FileIOInterface:
  ...
  def _mmap(start, sz, prot, flags, fd, offset):
    ...
    if x == 0xFFFFFFFFFFFFFFFF:
      raise OSError(
        f"Failed to mmap {sz} bytes at {hex(start)}: {os.strerror(ctypes.get_errno())}"
      )
    return x

  @staticmethod
  def anon_mmap(start, sz, prot, flags, offset):
    return FileIOInterface._mmap(start, sz, prot, flags, -1, offset)
  ...
  def eventfd(initval, flags=None):
    return FileIOInterface(fd=os.eventfd(initval, flags))  # type: ignore[attr-defined]


if MOCKGPU := getenv("MOCKGPU"):
  from test.mockgpu.mockgpu import MockFileIOInterface as FileIOInterface  # noqa: F401 # pylint: disable=unused-import
...
def hcq_filter_visible_devices(dev):
  return (
    ...
    if (
      ids := [
        int(x) for x in (getenv("HCQ_VISIBLE_DEVICES", "")).split(",") if x.strip()
      ]
    )
    else dev
  )
...
class HWQueue(Generic[SignalType, HCQDeviceType, ProgramType, ArgsStateType]):
  ...
  def bind_sints(
    ...
    *vals: sint,
    mem: MMIOInterface,
    struct_t: Type[ctypes.Structure],
    start_field: str,
    fmt,
    mask: int | None = None,
  ):
    self.bind_sints_to_mem(
      *vals, mem=mem, fmt=fmt, mask=mask, offset=getattr(struct_t, start_field).offset
    )

  def bind_sints_to_mem(
    self, *vals: sint, mem: MMIOInterface, fmt, mask: int | None = None, offset: int = 0
  ):
    mv = mem.view(offset=offset, size=len(vals) * 8, fmt=fmt)
    for i, val in enumerate(vals):
      if isinstance(val, int):
        mv[i] = val if mask is None else ((mv[i] & ~mask) | val)
      else:
        self.mv_sints.append((mv, i, self._new_sym(val), mask))
  ...
  def _apply_var_vals(self, var_vals: dict[str, int]):
    ...
    for off, sym_idx in self.q_sints:
      if self._prev_resolved_syms[sym_idx] == resolved_syms[sym_idx]:
        continue
    ...
    for mv, off, sym_idx, mask in self.mv_sints:
      if self._prev_resolved_syms[sym_idx] == resolved_syms[sym_idx]:
        continue
      mv[off] = (
        resolved_syms[sym_idx]
        if mask is None
        else ((mv[off] & ~mask) | resolved_syms[sym_idx])
      )
  ...
  def _submit(self, dev: HCQDeviceType):
    raise NotImplementedError("need _submit")
...
class HCQSignal(Generic[HCQDeviceType]):
  def __init__(
    ...
    self.base_buf, self.value_addr, self.timestamp_addr, self.owner = (
      base_buf,
      base_buf.va_addr + 0,
      base_buf.va_addr + 8,
      owner,
    ...
    if isinstance(self.base_buf.va_addr, int):
      self.value_mv, self.timestamp_mv = (
        self.base_buf.cpu_view().view(0, 8, "Q"),
        self.base_buf.cpu_view().view(8, 8, "Q"),
      )
      self.value_mv[0] = value

  def __del__(self):
    if isinstance(self.base_buf.va_addr, int) and self.owner is not None:
      HCQCompiled.signal_pool[self.owner.peer_group].append(self.base_buf)

  @property
  def value(self) -> int:
    return self.value_mv[0]

  @value.setter
  def value(self, new_value: int):
    self.value_mv[0] = new_value
  ...
  def wait(self, value: int, timeout: int = getenv("HCQDEV_WAIT_TIMEOUT_MS", 30000)):
    ...
    while (not_passed := (prev_value := self.value) < value) and (
      time_spent := int(time.perf_counter() * 1000) - start_time
    ) < timeout:
      self._sleep(time_spent)
      if self.value != prev_value:
        start_time = int(time.perf_counter() * 1000)  # progress was made, reset timer
    if not_passed and self.value < value:
      raise RuntimeError(
        f"Wait timeout: {timeout} ms! (the signal is not set to {value}, but {self.value})"
      )
...
def hcq_profile(
  ...
  enabled,
  desc,
  queue_type: Callable[[], HWQueue] | None = None,
  queue: HWQueue | None = None,
):
  st, en = (dev.new_signal(), dev.new_signal()) if enabled else (None, None)
  assert queue is not None or queue_type is not None, (
    "Either queue or queue_type must be provided"
  )

  if enabled and queue is not None:
    queue.timestamp(st)
  elif enabled and queue_type is not None:
    queue_type().wait(dev.timeline_signal, dev.timeline_value - 1).timestamp(st).signal(
      dev.timeline_signal, dev.next_timeline()
    ).submit(dev)
  ...
  try:
    ...
    if enabled and queue is not None:
      ...
      queue_type().wait(dev.timeline_signal, dev.timeline_value - 1).timestamp(
        en
      ).signal(dev.timeline_signal, dev.next_timeline()).submit(dev)

    if enabled and PROFILE:
      dev.sig_prof_records.append((
        cast(HCQSignal, st),
        cast(HCQSignal, en),
        desc,
        (queue_type or type(queue)) is dev.hw_copy_queue_t,
      ))
...
class HCQArgsState(Generic[ProgramType]):
  ...
  def bind_sints_to_buf(self, *vals: sint, buf: HCQBuffer, fmt, offset=0):
    self.bind_data.append((vals, buf.cpu_view().view(offset=offset), fmt))
...
class CLikeArgsState(HCQArgsState[ProgramType]):
  def __init__(
    ...
    if prefix is not None:
      self.buf.cpu_view().view(size=len(prefix) * 4, fmt="I")[:] = array.array(
        "I", prefix
      )

    self.bind_sints_to_buf(
      *[b.va_addr for b in bufs], buf=self.buf, fmt="Q", offset=len(prefix or []) * 4
    )
    self.bind_sints_to_buf(
      *vals, buf=self.buf, fmt="I", offset=len(prefix or []) * 4 + len(bufs) * 8
    )
...
class HCQProgram(Generic[HCQDeviceType]):
  def __init__(
    ...
    self.args_state_t, self.dev, self.name, self.kernargs_alloc_size = (
      args_state_t,
      dev,
      name,
      kernargs_alloc_size,
    ...
    if PROFILE:
      Compiled.profile_events += [ProfileProgramEvent(dev.device, name, lib, base)]

  @staticmethod
  def _fini(dev, buf, spec):
    dev.allocator.free(buf, buf.size, spec)
  ...
  def __call__(
    ...
    *bufs: HCQBuffer,
    ...
    q = (
      self.dev.hw_compute_queue_t()
      .wait(self.dev.timeline_signal, self.dev.timeline_value - 1)
      .memory_barrier()
    )

    with hcq_profile(self.dev, queue=q, desc=self.name, enabled=wait or PROFILE) as (
      sig_st,
      sig_en,
    ):
...
class HCQCompiled(Compiled, Generic[SignalType]):
  ...
  signal_pages: dict[str, list[HCQBuffer]] = collections.defaultdict(
    list
  )  # per peer group
  signal_pool: dict[str, list[HCQBuffer]] = collections.defaultdict(
    list
  )  # per peer group
  ...
  def __init__(
    ...
    # Map signals if any
    for sig_page in HCQCompiled.signal_pages[self.peer_group]:
      cast(HCQAllocator, self.allocator).map(sig_page)

    self.sigalloc_size = sigalloc_size
    self.signal_t, self.hw_compute_queue_t, self.hw_copy_queue_t = (
      signal_t,
      comp_queue_t,
      copy_queue_t,
    )
    ...
    self.timeline_signal, self._shadow_timeline_signal = (
      self.new_signal(value=0, is_timeline=True),
      self.new_signal(value=0, is_timeline=True),
    )
    self.sig_prof_records: list[tuple[HCQSignal, HCQSignal, str, bool]] = []

    self.kernargs_buf: HCQBuffer = self.allocator.alloc(
      kernargs_size, BufferSpec(cpu_access=True)
    )
    self.kernargs_offset_allocator: BumpAllocator = BumpAllocator(
      self.kernargs_buf.size, wrap=True
    )

    self.error_state: Exception | None = (
      None  # Exception if error is unrecoverable and sync will always fail
    )

    if self._is_cpu():
      HCQCompiled.cpu_devices.append(self)
  ...
  def synchronize(self):
    ...
    if not self._is_cpu():
      for dev in HCQCompiled.cpu_devices:
        dev.synchronize()
    ...
    try:
      ...
      self.error_state = e
      if hasattr(self, "on_device_hang"):
        self.on_device_hang()
      else:
        raise e

    if self.timeline_value > (1 << 31):
      self._wrap_timeline_signal()
    if PROFILE:
      Compiled.profile_events += [
        ProfileRangeEvent(self.device, name, st.timestamp, en.timestamp, cp)
        for st, en, name, cp in self.sig_prof_records
      ]
  ...
  def new_signal(self, **kwargs) -> SignalType:
    if not HCQCompiled.signal_pool[pg := self.peer_group]:
      HCQCompiled.signal_pages[pg].append(
        alc := self.allocator.alloc(
          self.sigalloc_size, BufferSpec(host=True, uncached=True, cpu_access=True)
        )
      )
      HCQCompiled.signal_pool[pg] += [
        alc.offset(offset=off, size=16) for off in range(0, alc.size, 16)
      ]
      for dev in HCQCompiled.peer_groups[pg]:
        cast(HCQAllocator, dev.allocator).map(alc)
    return self.signal_t(
      base_buf=HCQCompiled.signal_pool[pg].pop(), owner=self, **kwargs
    )
  ...
  def _at_profile_finalize(self):
    ...
    def _sync(d: HCQCompiled, q_t: Callable[[], HWQueue]):
      q_t().timestamp(d.timeline_signal).signal(
        d.timeline_signal, d.next_timeline()
      ).submit(d)
    ...
    gpu2cpu_compute_time_diff = statistics.median([
      _sync(self, self.hw_compute_queue_t) for _ in range(40)
    ])
    if self.hw_copy_queue_t is None:
      gpu2cpu_copy_time_diff = decimal.Decimal(0)
    else:
      gpu2cpu_copy_time_diff = statistics.median([
        _sync(self, self.hw_copy_queue_t) for _ in range(40)
      ])
    Compiled.profile_events += [
      ProfileDeviceEvent(self.device, gpu2cpu_compute_time_diff, gpu2cpu_copy_time_diff)
    ]

  def _wrap_timeline_signal(self):
    self.timeline_signal, self._shadow_timeline_signal, self.timeline_value = (
      self._shadow_timeline_signal,
      self.timeline_signal,
      1,
    )
    self.timeline_signal.value = 0
    cast(HCQAllocatorBase, self.allocator).b_timeline = [0] * len(
      cast(HCQAllocatorBase, self.allocator).b
    )

  def _realloc(
    self, oldbuf: HCQBuffer | None, new_size: int, options: BufferSpec | None = None
  ) -> tuple[HCQBuffer, bool]:
    if oldbuf is not None:
      self.allocator.free(oldbuf, oldbuf.size, options=options)
    try:
      buf, realloced = self.allocator.alloc(new_size, options=options), True
    except MemoryError:
      buf, realloced = (
        self.allocator.alloc(
          oldbuf.size if oldbuf is not None else new_size, options=options
        ),
        False,
      )
  ...
  def _select_iface(self, *ifaces: Type):
    errs, err_short = "", ""
    if val := getenv(f"{type(self).__name__[:-6].upper()}_IFACE", ""):
      ifaces = tuple(x for x in ifaces if x.__name__.startswith(val.upper()))
    for iface_t in ifaces:
      try:
        return iface_t(self, self.device_id)
  ...
  def _is_cpu(self) -> bool:
    return hasattr(self, "device") and self.device.split(":")[0] == "CPU"

  def finalize(self):
    try:
      self.synchronize()  # Try to finalize device in any case.
    except RuntimeError as e:
      print(f"{self.device} synchronization failed before finalizing: {e}")

    # If the device has an interface, call its device_fini method to clean up resources.
    if hasattr(self, "iface") and hasattr(self.iface, "device_fini"):
...
class HCQBuffer:
  def __init__(
    ...
    self.va_addr, self.size, self.texture_info, self.meta, self._base, self.view = (
      va_addr,
      size,
      texture_info,
      meta,
  ...
  def offset(self, offset: int = 0, size: int | None = None) -> HCQBuffer:
    return HCQBuffer(
      self.va_addr + offset,
      size or (self.size - offset),
      owner=self.owner,
      texture_info=self.texture_info,
      meta=self.meta,
      _base=self._base or self,
      view=(
        self.view.view(offset=offset, size=size) if self.view is not None else None
      ),
    )
  ...
  @property
  def mappings(self):
    return self._mappings if self._base is None else self._base._mappings

  @property
  def mapped_devs(self):
    return self._devs if self._base is None else self._base._devs
...
class HCQAllocatorBase(LRUAllocator[HCQDeviceType], Generic[HCQDeviceType]):
  ...
  def __init__(
    ...
    super().__init__(dev)
    self.b = copy_bufs or [
      self._alloc(batch_size, BufferSpec(host=True)) for _ in range(batch_cnt)
    ]
    self.b_timeline, self.b_next, self.max_copyout_size = (
      [0] * len(self.b),
      0,
      max_copyout_size,
    )
  ...
  def map(self, buf: HCQBuffer):
    if self.dev in buf.mapped_devs:
      return
    if buf.owner is None:
      raise RuntimeError(
        f"map failed: buffer {buf.va_addr} has no owner, it's a virtual buffer"
      )
    if not hasattr(self, "_map"):
      raise NotImplementedError("map failed: no method implemented")
    ...
    if (mb := self._map(buf)) is not None:
      buf.mappings[self.dev] = mb
    buf.mapped_devs.append(self.dev)

  def _offset(self, buf, size: int, offset: int) -> HCQBuffer:
    return buf.offset(offset=offset, size=size)
...
class HCQAllocator(HCQAllocatorBase, Generic[HCQDeviceType]):
  def _copyin(self, dest: HCQBuffer, src: memoryview):
    ...
    with hcq_profile(
      self.dev,
      queue_type=self.dev.hw_copy_queue_t,
      desc=f"TINY -> {self.dev.device}",
      enabled=PROFILE,
    ):
      for i in range(0, src.nbytes, self.b[0].size):
        ...
        self.b[self.b_next].cpu_view().view(size=lsize, fmt="B")[:] = src.cast("B")[
          i : i + lsize
        ]
        self.dev.hw_copy_queue_t().wait(
          self.dev.timeline_signal, self.dev.timeline_value - 1
        ).copy(dest.va_addr + i, self.b[self.b_next].va_addr, lsize).signal(
  ...
  def copy_from_disk(self, dest: HCQBuffer, src, size):
    def _get_temp_buf():
      ...
      if (
        self.b_timeline[(self.b_next + 1) % len(self.b)]
        <= self.dev.timeline_signal.value
      ):
        self.b_timeline[(self.b_next + 1) % len(self.b)], self.b_next = (
          (1 << 64),
          (self.b_next + 1) % len(self.b),
        )
    ...
    with hcq_profile(
      ...
      for (
        ...
        src_off,
        copy_size,
      ) in src.device.allocator._copyout_sharded(
        src, size, _get_temp_buf, seg_len=self.b[0].size
      ):
        self.dev.hw_copy_queue_t().wait(
          self.dev.timeline_signal, self.dev.timeline_value - 1
        ).copy(dest.va_addr + dst_off, batch_info[0] + src_off, copy_size).signal(
          self.dev.timeline_signal, self.dev.next_timeline()
        ).submit(self.dev)
  ...
  def _copyout(self, dest: memoryview, src: HCQBuffer):
    ...
    with hcq_profile(
      self.dev,
      queue_type=self.dev.hw_copy_queue_t,
      desc=f"{self.dev.device} -> TINY",
      enabled=PROFILE,
    ):
      for i in range(
        0, dest.nbytes, cp_size := (self.max_copyout_size or self.b[0].size)
      ):
        self.dev.hw_copy_queue_t().wait(
          self.dev.timeline_signal, self.dev.timeline_value - 1
        ).copy(
          self.b[0].va_addr, src.va_addr + i, lsize := min(cp_size, dest.nbytes - i)
        ).signal(self.dev.timeline_signal, self.dev.next_timeline()).submit(self.dev)
        self.dev.timeline_signal.wait(self.dev.timeline_value - 1)
        dest.cast("B")[i : i + lsize] = (
          self.b[0].cpu_view().view(size=lsize, fmt="B")[:]
        )
  ...
  def _transfer(
    ...
    src: HCQBuffer,
    sz: int,
    src_dev: HCQDeviceType,
    dest_dev: HCQDeviceType,
  ):
    ...
    with hcq_profile(
      src_dev,
      queue_type=src_dev.hw_copy_queue_t,
      desc=f"{src_dev.device} -> {dest_dev.device}",
      enabled=PROFILE,
    ):
      src_dev.hw_copy_queue_t().wait(
        src_dev.timeline_signal, src_dev.timeline_value - 1
      ).wait(dest_dev.timeline_signal, dest_dev.timeline_value - 1).copy(
        dest.va_addr, src.va_addr, sz
      ).signal(src_dev.timeline_signal, src_dev.next_timeline()).submit(src_dev)

    if src_dev != dest_dev:
      dest_dev.hw_compute_queue_t().wait(
        src_dev.timeline_signal, src_dev.timeline_value - 1
      ).wait(dest_dev.timeline_signal, dest_dev.timeline_value - 1).signal(
        dest_dev.timeline_signal, dest_dev.next_timeline()
      ).submit(dest_dev)
