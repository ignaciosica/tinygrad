...
class IBCtx:
  def __init__(self, idx: int):
    ...
    self.port_attr = checkz(
      self.vctx.contents.query_port(self.ctx, DEFAULT_PORT, ctypes.byref(pa := ib.struct_ibv_port_attr()), ctypes.sizeof(pa)), pa
    )
  ...
  def reg(self, buf: Buffer) -> tuple[int, ctypes._Pointer[ib.struct_ibv_mr]]:
    ...
    if buf not in self.mrs:
      if buf.nbytes > self.device_attr.max_mr_size:
        raise RuntimeError(f"Buffer too big: {buf.nbytes:#x} > {self.device_attr.max_mr_size:#x}")
      if len(self.mrs) >= self.device_attr.max_mr:
        raise RuntimeError(f"Out of memory region cap: {len(self.mrs)} >= {self.device_attr.max_mr}")
      ...
      if not mr:
        raise RuntimeError(f"Couldn't register memory region for {buf} {dmaref} (errno={ctypes.get_errno()})")
...
class IBConn:
  ...
  def connect(self, remote_gid: bytes, remote_qp_num: int):
    ...
    checkz(
      ib.ibv_modify_qp(
        self.qp, qpa, ib.IBV_QP_STATE | ib.IBV_QP_TIMEOUT | ib.IBV_QP_RETRY_CNT | ib.IBV_QP_RNR_RETRY | ib.IBV_QP_SQ_PSN | ib.IBV_QP_MAX_QP_RD_ATOMIC
      )
    )
