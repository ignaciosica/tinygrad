...
DEFAULT_PORT, DEFAULT_GID = (
  getenv("DEFAULT_PORT", 1),
  getenv("DEFAULT_GID", 3),
)  # DEFAULT_GID=0 for RXE
IOVA_ALIGN = resource.getpagesize()


def checkz(x, ret=None):
  if x != 0:
    raise RuntimeError(f"{x} != 0 (errno {ctypes.get_errno()})")
  return ret
...
class IBCtx:
  def __init__(self, idx: int):
    ...
    self.vctx = ctypes.cast(
      ctypes.addressof(self.ctx.contents) - ib.struct_verbs_context.context.offset,
      ctypes.POINTER(ib.struct_verbs_context),
    )
    ...
    self.device_attr = checkz(
      ib.ibv_query_device(self.ctx, ctypes.byref(da := ib.struct_ibv_device_attr())), da
    )
    self.port_attr = checkz(
      self.vctx.contents.query_port(
        self.ctx,
        DEFAULT_PORT,
        ctypes.byref(pa := ib.struct_ibv_port_attr()),
        ctypes.sizeof(pa),
      ),
      pa,
    )
    self.gid_attr = checkz(
      ib.ibv_query_gid(
        self.ctx, DEFAULT_PORT, DEFAULT_GID, ctypes.byref(ga := ib.union_ibv_gid())
      ),
      ga,
    )
    ...
    self.mrs: WeakKeyDictionary[
      Buffer, tuple[int, ctypes._Pointer[ib.struct_ibv_mr], weakref.finalize]
    ] = WeakKeyDictionary()
  ...
  def __del__(self):
    ...
    if hasattr(self, "mrs"):
      [fin() for _, _, fin in self.mrs.values()]
    if hasattr(self, "pd"):
  ...
  def rlimit_fix():
    ...
    if DEBUG >= 2:
      print(f"IB: Increased fd limit from {soft} to {hard}")
  ...
  def reg(self, buf: Buffer) -> tuple[int, ctypes._Pointer[ib.struct_ibv_mr]]:
    ...
    if buf not in self.mrs:
      if buf.nbytes > self.device_attr.max_mr_size:
        raise RuntimeError(
          f"Buffer too big: {buf.nbytes:#x} > {self.device_attr.max_mr_size:#x}"
        )
      if len(self.mrs) >= self.device_attr.max_mr:
        raise RuntimeError(
          f"Out of memory region cap: {len(self.mrs)} >= {self.device_attr.max_mr}"
        )
      ...
      match dmaref := buf.as_dmaref():
        case DMACPURef():
          ...
          mr = ib.ibv_reg_mr_iova2(
            self.pd, ctypes.c_void_p(dmaref.addr), dmaref.size, iova, mr_flags
          )
        case DMAFdRef():
          iova = self.alloc_iova(dmaref.size, dmaref.offset % IOVA_ALIGN)
          mr = ib.ibv_reg_dmabuf_mr(
            self.pd, dmaref.offset, dmaref.size, iova, dmaref.fd, mr_flags
          )
        case _:
          raise RuntimeError(f"Unknown type of dma ref: {dmaref}")
      if not mr:
        raise RuntimeError(
          f"Couldn't register memory region for {buf} {dmaref} (errno={ctypes.get_errno()})"
        )
...
class IBConn:
  def __init__(self, ctx: IBCtx):
    ...
    self.wrid_num: Iterator[int] = itertools.count(
      0
    )  # wc_id is uint64, this will never overflow
    ...
    qp_init_attrs_cap = ib.struct_ibv_qp_cap(
      max_send_wr=1024,
      max_recv_wr=64,
      max_send_sge=8,
      max_recv_sge=8,
    ...
    qp_init_attrs = ib.struct_ibv_qp_init_attr(
      send_cq=self.cq, recv_cq=self.cq, cap=qp_init_attrs_cap, qp_type=ib.IBV_QPT_RC
    )  # Reliable Connection
    ...
    qpa = ib.struct_ibv_qp_attr(
      qp_state=ib.IBV_QPS_INIT, port_num=DEFAULT_PORT, qp_access_flags=qp_access_flags
    )
    checkz(
      ib.ibv_modify_qp(
        self.qp,
  ...
  def connect(self, remote_gid: bytes, remote_qp_num: int):
    # RTR
    qp_ah_attr_grh = ib.struct_ibv_global_route(
      hop_limit=1,
      dgid=ib.union_ibv_gid(raw=(ctypes.c_ubyte * 16)(*remote_gid)),
      sgid_index=DEFAULT_GID,
    )
    qp_ah_attr = ib.struct_ibv_ah_attr(
      is_global=1, port_num=DEFAULT_PORT, grh=qp_ah_attr_grh
    )
    qpa = ib.struct_ibv_qp_attr(
      qp_state=ib.IBV_QPS_RTR,
      path_mtu=ib.IBV_MTU_4096,
  ...
  def wait_cq(self, wr_id: int | None = None):
    while (wr_id in self.pending_wrids) if wr_id is not None else self.pending_wrids:
      if self.ctx.ctx.contents.ops.poll_cq(
        self.cq, _num_entries := 1, ctypes.byref(wc := ib.struct_ibv_wc())
      ):
        if wc.status != ib.IBV_WC_SUCCESS:
          raise RuntimeError(
            f"Work Request completed with error: wr_id={wc.wr_id} status={ib.ibv_wc_status__enumvalues.get(wc.status, wc.status)}"
          )
  ...
  def rdma_write(self, sgl: list[SGE]):
    ...
    def _post():
      ...
      if swr is not None:
        ...
        checkz(
          self.ctx.ctx.contents.ops.post_send(
            self.qp,
            swr,
            ctypes.byref(_bad_wr := ctypes.POINTER(ib.struct_ibv_send_wr)()),
          )
        )
        # TODO: async
        self.wait_cq(wr_id)
    ...
    for sg in reversed(sgl):
      ...
      for off in reversed(range(0, sg.size, self.ctx.port_attr.max_msg_sz)):
        ...
        sge = ctypes.pointer(
          ib.struct_ibv_sge(
            addr=sg.src_iova + off,
            length=min(sg.size - off, self.ctx.port_attr.max_msg_sz),
            lkey=sg.src_key,
          )
        )
        # RDMA struct for remote memory
        wr = ib.union_ibv_send_wr_wr(
          rdma=ib.struct_ibv_send_wr_1_rdma(
            remote_addr=sg.dst_iova + off, rkey=sg.dst_key
          )
        )
        ...
        # Create Send Request
        swr = ctypes.pointer(
          ib.struct_ibv_send_wr(
            opcode=ib.IBV_WR_RDMA_WRITE,
            sg_list=sge,
            num_sge=1,
            wr=wr,
            wr_id=wid,
        ...
        if (swr_cnt := swr_cnt + 1) >= self.qp_cap.max_send_wr:
          _post()
    _post()
