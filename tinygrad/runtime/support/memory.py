...
class PageTableTraverseContext:
  ...
  def _try_free_pt(self) -> bool:
    ...
    if (
      ...
      and all(not pt.valid(i) for i in range(self._pt_pte_cnt(self.pt_stack[-1][0].lv)))
    ):
...
class MemoryManager:
  ...
  def __init__(
    ...
    self.pt_t, self.palloc_ranges, self.level_cnt, self.va_bits, self.reserve_ptable = (
      pt_t,
      palloc_ranges,
      len(va_shifts),
      va_bits,
    ...
    self.pa_allocator = TLSFAllocator(
      vram_size - (64 << 20), base=self.boot_allocator.size + self.ptable_allocator.size
    )
    self.root_page_table = pt_t(
      self.dev, self.palloc(0x1000, zero=not self.dev.smi_dev, boot=True), lv=first_lv
    )
  ...
  def _frag_size(self, va, sz, must_cover=True):
    ...
    return (
      (
        min(va_pwr2_div, sz_pwr2_div) if must_cover else min(va_pwr2_div, sz_pwr2_max)
      ).bit_length()
      - 1
      - 12
    )
