...
class TLSFAllocator:
  ...
  def __init__(
    self, size: int, base: int = 0, block_size: int = 16, lv2_cnt: int = 16
  ):
    self.size, self.base, self.block_size, self.l2_cnt = (
      size,
      base,
      block_size,
      lv2_cnt.bit_length(),
    ...
    self.storage: list = [
      collections.defaultdict(list) for _ in range(size.bit_length() + 1)
    ]
    ...
    self.blocks: dict[int, tuple[int, int | None, int | None, bool]] = {
      0: (size, None, None, True)
    }  # size, next, prev, is_free
    if size > 0:
  ...
  @functools.cache  # pylint: disable=method-cache-max-size-none
  def lv2(self, size):
    return (size - (1 << (size.bit_length() - 1))) // (
      1 << max(0, size.bit_length() - self.l2_cnt)
    )

  def _insert_block(self, start: int, size: int, prev: int | None = None):
    if prev is None:
      prev = self.blocks[start][2]
  ...
  def _split_block(self, start: int, size: int, new_size: int):
    ...
    self._remove_block(start, size)._insert_block(start, new_size)._insert_block(
      start + new_size, size - new_size, prev=start
    )
  ...
  def _merge_right(self, start: int):
    ...
    while is_free and nxt in self.blocks:
      if (blk := self.blocks[nxt])[3] is False:
        break
      self._remove_block(start, size)._remove_block(nxt, blk[0])._insert_block(
        start, size := size + blk[0]
      )
  ...
  def _merge_block(self, start: int):
    ...
    while (x := self.blocks[start][2]) is not None and self.blocks[x][3] is True:
      start = x
  ...
  def alloc(self, req_size: int, align: int = 1) -> int:
    ...
    for l1 in range(self.lv1(size), len(self.storage)):
      if self.lv1_entries[l1] == 0:
        continue
      for l2 in range(
        self.lv2(size) if l1 == size.bit_length() else 0, (1 << self.l2_cnt)
      ):
  ...
  def free(self, start: int):
    self._insert_block(
      start - self.base, self.blocks[start - self.base][0]
    )._merge_block(start - self.base)
...
class PageTableTraverseContext:
  def __init__(self, dev, pt, vaddr, create_pts=False, free_pts=False, boot=False):
    self.dev, self.vaddr, self.create_pts, self.free_pts, self.boot = (
      dev,
      vaddr - dev.mm.va_base,
      create_pts,
      free_pts,
      boot,
    )
    self.pt_stack: list[tuple[Any, int, int]] = [
      (pt, self._pt_pte_idx(pt, self.vaddr), self._pt_pte_size(pt))
    ]

  def _pt_pte_cnt(self, lv):
    return self.dev.mm.pte_cnt[lv]

  def _pt_pte_size(self, pt):
    return self.dev.mm.pte_covers[pt.lv]

  def _pt_pte_idx(self, pt, va):
    return (va // self._pt_pte_size(pt)) % self._pt_pte_cnt(pt.lv)
  ...
  def level_down(self):
    ...
    if not pt.valid(pte_idx):
      ...
      pt.set_entry(
        pte_idx,
        self.dev.mm.palloc(0x1000, zero=True, boot=self.boot, ptable=True),
        table=True,
        valid=True,
      )

    assert not pt.is_page(pte_idx), (
      f"Must be table pt={pt.paddr:#x}, {pt.lv=} {pte_idx=} {pt.read_fields(pte_idx)}"
    )
  ...
  def _try_free_pt(self) -> bool:
    ...
    if (
      self.free_pts
      and pt != self.dev.mm.root_page_table
      and all(
        not pt.valid(i) for i in range(self._pt_pte_cnt(self.pt_stack[-1][0].lv))
      )
    ):
  ...
  def level_up(self):
    while self._try_free_pt() or self.pt_stack[-1][1] == self._pt_pte_cnt(
      self.pt_stack[-1][0].lv
    ):
      pt, pt_cnt, _ = self.pt_stack.pop()
      if pt_cnt == self._pt_pte_cnt(pt.lv):
        self.pt_stack[-1] = (
          self.pt_stack[-1][0],
          self.pt_stack[-1][1] + 1,
          self.pt_stack[-1][2],
    ...
    while size > 0:
      ...
      if self.create_pts:
        assert paddr is not None, (
          "paddr must be provided when allocating new page tables"
        )
        while (
          pte_covers > size
          or not pt.supports_huge_page(paddr + off)
          or self.vaddr & (pte_covers - 1) != 0
        ):
          pt, pte_idx, pte_covers = self.level_down()
      else:
        while not pt.is_page(pte_idx):
          pt, pte_idx, pte_covers = self.level_down()
...
class MemoryManager:
  ...
  def __init__(
    ...
    self.dev, self.vram_size, self.va_shifts, self.va_base, lvl_msb = (
      dev,
      vram_size,
      va_shifts,
      va_base,
    ...
    self.pte_covers, self.pte_cnt = (
      [1 << x for x in va_shifts][::-1],
      [1 << (lvl_msb[i + 1] - lvl_msb[i]) for i in range(len(lvl_msb) - 1)][::-1],
    )
    (
      self.pt_t,
      self.palloc_ranges,
      ...
      self.va_bits,
      self.reserve_ptable,
    ) = pt_t, palloc_ranges, len(va_shifts), va_bits, reserve_ptable

    self.boot_allocator = TLSFAllocator(boot_size, base=0)
    self.ptable_allocator = TLSFAllocator(
      round_up(vram_size // 512, 1 << 20) if self.reserve_ptable else 0,
      base=self.boot_allocator.size,
    )
    self.pa_allocator = TLSFAllocator(
      vram_size - (64 << 20),
      base=self.boot_allocator.size + self.ptable_allocator.size,
    )
    self.root_page_table = pt_t(
      self.dev,
      self.palloc(0x1000, zero=not self.dev.smi_dev, boot=True),
      lv=first_lv,
    )
  ...
  def _frag_size(self, va, sz, must_cover=True):
    ...
    """
    va_pwr2_div, sz_pwr2_div, sz_pwr2_max = (
      va & -(va) if va > 0 else (1 << 63),
      sz & -(sz),
      (1 << (sz.bit_length() - 1)),
    )
    return (
      (
        min(va_pwr2_div, sz_pwr2_div)
        if must_cover
  ...
  def page_tables(self, vaddr: int, size: int):
    ctx = PageTableTraverseContext(
      self.dev, self.root_page_table, vaddr, create_pts=True
    )
    for _ in ctx.next(size, paddr=0):
      return [pt for pt, _, _ in ctx.pt_stack]
  ...
  def map_range(
    ...
    assert size == sum(p[1] for p in paddrs), (
      f"Size mismatch {size=} {sum(p[1] for p in paddrs)=}"
    )

    ctx = PageTableTraverseContext(
      self.dev, self.root_page_table, vaddr, create_pts=True, boot=boot
    )
    for paddr, psize in paddrs:
      for off, pt, pte_idx, pte_cnt, pte_covers in ctx.next(psize, paddr=paddr):
        for pte_off in range(pte_cnt):
          assert not pt.valid(pte_idx + pte_off), (
            f"PTE already mapped: {pt.entry(pte_idx + pte_off):#x}"
          )
          pt.set_entry(
            pte_idx + pte_off,
  ...
  def unmap_range(self, vaddr: int, size: int):
    ...
    ctx = PageTableTraverseContext(
      self.dev, self.root_page_table, vaddr, free_pts=True
    )
  ...
  def valloc(
    ...
    if contiguous:
      ...
      while rem_size > 0:
        while self.palloc_ranges[nxt_range][0] > rem_size:
          nxt_range += 1
        ...
        try:
          paddrs += [
            (
              self.palloc(
                try_sz := self.palloc_ranges[nxt_range][0],
                self.palloc_ranges[nxt_range][1],
                zero=False,
              ),
              try_sz,
            )
          ...
          if nxt_range == len(self.palloc_ranges):
            ...
            raise MemoryError(
              f"Failed to allocate memory. (total allocation size={size:#x}, current try={self.palloc_ranges[nxt_range - 1]})"
            )
          continue
  ...
  def palloc(
    ...
    assert self.dev.is_booting == boot, (
    ...
    allocator = (
      ...
      if boot
        ...
        self.ptable_allocator
        if self.reserve_ptable and ptable
        else self.pa_allocator
      )
    )
  ...
  def pfree(self, paddr: int, ptable=False):
    (
      self.ptable_allocator if self.reserve_ptable and ptable else self.pa_allocator
    ).free(paddr)
