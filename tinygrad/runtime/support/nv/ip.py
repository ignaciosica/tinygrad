...
class NVRpcQueue:
  def __init__(self, gsp: NV_GSP, va: int, completion_q_va: int | None = None):
    ...
    if completion_q_va is not None:
      self.rx = nv.msgqRxHeader.from_address(completion_q_va + nv.msgqTxHeader.from_address(completion_q_va).rxHdrOff)
  ...
  def wait_resp(self, cmd: int) -> memoryview:
    while True:
      ...
      if hdr.rpc_result != 0:
        raise RuntimeError(f"RPC call {hdr.function} failed with result {hdr.rpc_result}")
      if hdr.function == cmd:
        return msg
...
class NV_FLCN(NV_IP):
  ...
  def prep_ucode(self):
    ...
    for i in range(bit_header.TokenEntries):
      ...
      if bit.TokenId != nv.BIT_TOKEN_FALCON_DATA or bit.DataVersion != 2 or bit.DataSize < nv.BIT_DATA_FALCON_DATA_V2_SIZE_4:
        continue
    ...
    def __patch(cmd_id, cmd):
      ...
      hdr = nv.FALCON_APPLICATION_INTERFACE_HEADER_V1.from_buffer_copy(
        image[(app_hdr_off := self.desc_v3.IMEMLoadSize + self.desc_v3.InterfaceOffset) :]
      )
  ...
  def execute_hs(self, base, img_sysmem, code_off, data_off, imemPa, imemVa, imemSz, dmemPa, dmemVa, dmemSz, pkc_off, engid, ucodeid, mailbox=None):
    ...
    self.nvdev.NV_PFALCON_FBIF_TRANSCFG.with_base(base)[ctx_dma := 0].update(
      target=self.nvdev.NV_PFALCON_FBIF_TRANSCFG_TARGET_COHERENT_SYSMEM, mem_type=self.nvdev.NV_PFALCON_FBIF_TRANSCFG_MEM_TYPE_PHYSICAL
    )
    ...
    cmd = self.nvdev.NV_PFALCON_FALCON_DMATRFCMD.with_base(base).encode(
      write=0, size=self.nvdev.NV_PFALCON_FALCON_DMATRFCMD_SIZE_256B, ctxdma=ctx_dma, imem=0, sec=0
    )
...
class NV_GSP(NV_IP):
  ...
  def init_rm_args(self, queue_size=0x40000):
    ...
    queue_args = nv.MESSAGE_QUEUE_INIT_ARGUMENTS(
      sharedMemPhysAddr=queues_sysmem[0], pageTableEntryCount=pte_cnt, cmdQueueOffset=pt_size, statQueueOffset=pt_size + queue_size
    )
  ...
  def init_wpr_meta(self):
    ...
    if self.nvdev.fmc_boot:
      m = nv.GspFwWprMeta(
        **common, vgaWorkspaceSize=0x20000, pmuReservedSize=0x1820000, nonWprHeapSize=0x220000, gspFwHeapSize=0x8700000, frtsSize=0x100000
      )
    else:
      m = nv.GspFwWprMeta(
        **common,
  ...
  def init_golden_image(self):
    ...
    for i, pt in enumerate(self.nvdev.mm.page_tables(res_va, size=res_sz)):
      bufs_p.levels[i] = nv_gpu.struct_NV90F1_CTRL_VASPACE_COPY_SERVER_RESERVED_PDES_PARAMS_0(
  ...
  def rpc_set_page_directory(self, device: int, hVASpace: int, pdir_paddr: int, client=None, pasid=0xFFFFFFFF):
    params = nv.struct_NV0080_CTRL_DMA_SET_PAGE_DIRECTORY_PARAMS_v1E_05(
      physAddress=pdir_paddr, numEntries=self.nvdev.mm.pte_cnt[0], flags=0x8, hVASpace=hVASpace, pasid=pasid, subDeviceId=1, chId=0
  ...
  def rpc_set_gsp_system_info(self):
    def bdf_as_int(s):
      return 0x000 if s.startswith("usb") else (int(s[5:7], 16) << 8) | (int(s[8:10], 16) << 3) | int(s[-1], 16)
  ...
  def rpc_set_registry_table(self):
    ...
    for k, v in table.items():
      entries_bytes += bytes(
        nv.PACKED_REGISTRY_ENTRY(nameOffset=hdr_size + entries_size + len(data_bytes), type=nv.REGISTRY_TABLE_ENTRY_TYPE_DWORD, data=v, length=4)
      )
