...
class NVReg:
  ...
  def encode(self, **kwargs) -> int:
    return functools.reduce(
      int.__or__, (value << self.fields[name][0] for name, value in kwargs.items()), 0
    )
...
class NVPageTableEntry:
  ...
  def set_entry(
    ...
    if not table:
      ...
      x = pde.encode(
        is_pte=False,
        **{f"aperture{small}": 1 if valid else 0, f"address{small}{sys}": paddr >> 12},
  ...
  def read_fields(self, entry_id: int) -> dict:
    ...
    return (self.nvdev.dual_pde_t if self._is_dual_pde() else self.nvdev.pde_t).decode(
      self.entry(entry_id)
    )

  def is_page(self, entry_id) -> bool:
    return (
      (self.entry(entry_id) & 1 == 1) if self.lv < self.nvdev.mm.level_cnt - 1 else True
    )
