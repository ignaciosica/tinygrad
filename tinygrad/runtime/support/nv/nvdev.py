...
class NVReg:
  def __init__(self, nvdev, base, off, fields=None):
    self.nvdev, self.base, self.off, self.fields = nvdev, base, off, fields

  def __getitem__(self, idx: int):
    return NVReg(self.nvdev, self.base, self.off(idx), fields=self.fields)

  def add_field(self, name: str, start: int, end: int):
    self.fields[name] = (start, end)

  def with_base(self, base: int):
    return NVReg(self.nvdev, base + self.base, self.off, self.fields)

  def read(self):
    return self.nvdev.rreg(self.base + self.off)

  def read_bitfields(self) -> dict[str, int]:
    return self.decode(self.read())

  def write(self, _ini_val: int = 0, **kwargs):
    self.nvdev.wreg(self.base + self.off, _ini_val | self.encode(**kwargs))

  def update(self, **kwargs):
    self.write(self.read() & ~self.mask(*kwargs.keys()), **kwargs)
  ...
  def mask(self, *names):
    return functools.reduce(
      ...
      (
        (
          ((1 << (self.fields[nm][1] - self.fields[nm][0] + 1)) - 1)
          << self.fields[nm][0]
        )
        for nm in names
      ),
  ...
  def encode(self, **kwargs) -> int:
    return functools.reduce(
      int.__or__,
      (value << self.fields[name][0] for name, value in kwargs.items()),
      0,
    )

  def decode(self, val: int) -> dict:
    return {
      name: getbits(val, start, end) for name, (start, end) in self.fields.items()
    }


class NVPageTableEntry:
  def __init__(self, nvdev, paddr, lv):
    self.nvdev, self.paddr, self.lv, self.entries = (
      nvdev,
      paddr,
      lv,
      nvdev.vram.view(paddr, 0x1000, fmt="Q"),
    )

  def _is_dual_pde(self) -> bool:
    return self.lv == self.nvdev.mm.level_cnt - 2
  ...
  def set_entry(
    ...
    if not table:
      x = self.nvdev.pte_t.encode(
        valid=valid,
        address_sys=paddr >> 12,
        aperture=2 if system else 0,
        kind=6,
      ...
      small, sys = (
        ("_small" if self._is_dual_pde() else ""),
        "" if self.nvdev.mmu_ver == 3 else "_sys",
      )
      x = pde.encode(
        is_pte=False,
        **{
          f"aperture{small}": 1 if valid else 0,
          f"address{small}{sys}": paddr >> 12,
        },
        **({f"pcf{small}": 0b10} if self.nvdev.mmu_ver == 3 else {"no_ats": 1}),
      )

    if self._is_dual_pde():
      self.entries[2 * entry_id], self.entries[2 * entry_id + 1] = (
  ...
  def entry(self, entry_id: int) -> int:
    return (
      (self.entries[2 * entry_id + 1] << 64) | self.entries[2 * entry_id]
      if self._is_dual_pde()
      else self.entries[entry_id]
    )

  def read_fields(self, entry_id: int) -> dict:
    if self.is_page(entry_id):
      return self.nvdev.pte_t.decode(self.entry(entry_id))
    return (
      self.nvdev.dual_pde_t if self._is_dual_pde() else self.nvdev.pde_t
    ).decode(self.entry(entry_id))

  def is_page(self, entry_id) -> bool:
    return (
      (self.entry(entry_id) & 1 == 1)
      if self.lv < self.nvdev.mm.level_cnt - 1
      else True
    )

  def supports_huge_page(self, paddr: int):
    return (
      self.lv >= self.nvdev.mm.level_cnt - 3
      and paddr % self.nvdev.mm.pte_covers[self.lv] == 0
    )

  def valid(self, entry_id):
    if self.is_page(entry_id):
      return self.read_fields(entry_id)["valid"]
    return (
      self.read_fields(entry_id)[
        "aperture_small" if self._is_dual_pde() else "aperture"
      ]
      != 0
    )
  ...
  def address(self, entry_id: int) -> int:
    small, sys = (
      ("_small" if self._is_dual_pde() else ""),
      "_sys"
      if self.nvdev.mmu_ver == 2 or self.lv == self.nvdev.mm.level_cnt - 1
      else "",
    )
...
class NVMemoryManager(MemoryManager):
  va_allocator = TLSFAllocator(
    (1 << 44), base=0x1000000000
  )  # global for all devices.

  def on_range_mapped(self):
    self.dev.NV_VIRTUAL_FUNCTION_PRIV_MMU_INVALIDATE.write(
      (1 << 0) | (1 << 1) | (1 << 6) | (1 << 31)
    )
...
class NVDev(PCIDevImplBase):
  def __init__(self, pci_dev: PCIDevice):
    self.pci_dev, self.devfmt, self.mmio = (
      pci_dev,
      pci_dev.pcibus,
      pci_dev.map_bar(0, fmt="I"),
    )
    ...
    bits, shifts = (
      (56, [12, 21, 29, 38, 47, 56])
      if self.mmu_ver == 3
      else (48, [12, 21, 29, 38, 47])
    )
    self.mm = NVMemoryManager(
      self,
      self.vram_size,
      boot_size=(2 << 20),
      pt_t=NVPageTableEntry,
      va_bits=bits,
      va_shifts=shifts,
      va_base=0,
    ...
    self.flcn: NV_FLCN | NV_FLCN_COT = (
      NV_FLCN_COT(self) if self.fmc_boot else NV_FLCN(self)
    )
  ...
  def _early_init(self):
    ...
    self.chip_name = {0x17: "GA1", 0x19: "AD1", 0x1B: "GB2"}[
      self.chip_details["architecture"]
    ] + f"{self.chip_details['implementation']:02d}"
    self.fw_name = {"GB2": "GB202", "AD1": "AD102", "GA1": "GA102"}[
      self.chip_name[:3]
    ]
    self.mmu_ver, self.fmc_boot = (
      (3, True) if self.chip_details["architecture"] >= 0x1A else (2, False)
    )

    self.include("src/common/inc/swref/published/turing/tu102/dev_fb.h")
    if self.reg("NV_PFB_PRI_MMU_WPR2_ADDR_HI").read() != 0:
      if DEBUG >= 2:
        print(f"nv {self.devfmt}: WPR2 is up. Issuing a full reset.", flush=True)
      self.pci_dev.reset()
      time.sleep(0.5)
    ...
    # MMU Init
    self.reg_names.update(
      mmu_pd_names := [
        f"NV_MMU_VER{self.mmu_ver}_PTE",
        f"NV_MMU_VER{self.mmu_ver}_PDE",
    ...
    self.include(
      f"kernel-open/nvidia-uvm/hwref/{'hopper/gh100' if self.mmu_ver == 3 else 'turing/tu102'}/dev_mmu.h"
    )
    self.pte_t, self.pde_t, self.dual_pde_t = tuple([
      self.__dict__[name] for name in mmu_pd_names
    ])
  ...
  def _alloc_boot_struct(
    self, struct: ctypes.Structure
  ) -> tuple[ctypes.Structure, int]:
    view, paddrs = System.alloc_sysmem(
      sz := ctypes.sizeof(type(struct)), contiguous=True
    )
  ...
  def extract_fw(self, file: str, dname: str) -> bytes:
    ...
    info, sl = (
      text[
        text[: text.index(dnm := f"{file}_{self.fw_name}_{dname}")].rindex(
          "COMPRESSION:"
        ) :
      ][:16],
      text[text.index(dnm) + len(dnm) + 7 :],
    )
    image = bytes.fromhex(
      sl[: sl.find("};")]
      .strip()
    ...
    return (
      gzip.decompress(struct.pack("<4BL2B", 0x1F, 0x8B, 8, 0, 0, 0, 3) + image)
      if "COMPRESSION: YES" in info
      else image
    )
  ...
  def include(self, file: str):
    ...
    regs_off = {
      "NV_PFALCON_FALCON": 0x0,
      "NV_PGSP_FALCON": 0x0,
      "NV_PSEC_FALCON": 0x0,
      "NV_PRISCV_RISCV": 0x1000,
      "NV_PGC6_AON": 0x0,
      "NV_PFSP": 0x0,
      "NV_PGC6_BSI": 0x0,
      "NV_PFALCON_FBIF": 0x600,
      "NV_PFALCON2_FALCON": 0x1000,
      "NV_PBUS": 0x0,
      "NV_PFB": 0x0,
      "NV_PMC": 0x0,
      "NV_PGSP_QUEUE": 0x0,
    ...
    for raw in self._download(file).splitlines():
      ...
      if m := re.match(
        r"#define\s+(\w+)\s+([0-9\+\-\*\(\)]+):([0-9\+\-\*\(\)]+)", raw
      ):  # bitfields
        name, hi, lo = m.groups()

        reg = next((r for r in self.reg_names if name.startswith(r + "_")), None)
        if reg is not None:
          self.__dict__[reg].add_field(
            name[len(reg) + 1 :].lower(), _do_eval(lo), _do_eval(hi)
          )
        else:
          self.reg_offsets[name] = (_do_eval(lo), _do_eval(hi))
        continue
      ...
      if m := re.match(r"#define\s+(\w+)\s*\(\s*(\w+)\s*\)\s*(.+)", raw):  # reg set
        ...
        name, value = m.groups()[0], int(m.groups()[1], 0)  # reg value
      else:
        continue

      reg_pref = next(
        (prefix for prefix in regs_off.keys() if name.startswith(prefix)), None
      )
