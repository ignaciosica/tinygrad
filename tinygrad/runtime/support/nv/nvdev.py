...
class NVPageTableEntry:
  ...
  def is_page(self, entry_id) -> bool:
    return (self.entry(entry_id) & 1 == 1) if self.lv < self.nvdev.mm.level_cnt - 1 else True

  def supports_huge_page(self, paddr: int):
    return self.lv >= self.nvdev.mm.level_cnt - 3 and paddr % self.nvdev.mm.pte_covers[self.lv] == 0
  ...
  def include(self, file: str):
    ...
    for raw in self._download(file).splitlines():
      ...
      if m := re.match(r"#define\s+(\w+)\s+([0-9\+\-\*\(\)]+):([0-9\+\-\*\(\)]+)", raw):  # bitfields
        ...
        if reg is not None:
          self.__dict__[reg].add_field(name[len(reg) + 1 :].lower(), _do_eval(lo), _do_eval(hi))
        else:
