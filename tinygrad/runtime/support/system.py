...
class _System:
  ...
  def atomic_lib(self):
    return ctypes.CDLL(ctypes.util.find_library("atomic")) if sys.platform == "linux" else None
  ...
  def mach_task_self(self):
    return ctypes.cast(self.libsys.mach_task_self_, ctypes.POINTER(ctypes.c_uint)).contents.value
  ...
  def memory_barrier(self):
    lib.atomic_thread_fence(__ATOMIC_SEQ_CST := 5) if (lib := self.libsys if OSX else self.atomic_lib) is not None else None
  ...
  def alloc_sysmem(self, size: int, vaddr: int = 0, contiguous: bool = False, data: bytes | None = None) -> tuple[MMIOInterface, list[int]]:
    if OSX:
      ...
      va = FileIOInterface.anon_mmap(
        vaddr, size, mmap.PROT_READ | mmap.PROT_WRITE, mmap.MAP_SHARED | mmap.MAP_ANONYMOUS | MAP_POPULATE | MAP_LOCKED | flags, 0
      )
  ...
  def pci_setup_usb_bars(self, usb: ASM24Controller, gpu_bus: int, mem_base: int, pref_mem_base: int) -> dict[int, PCIBarInfo]:
    ...
    while bar_off < 24:
      ...
      if (cfg & pci.PCI_BASE_ADDRESS_SPACE) == pci.PCI_BASE_ADDRESS_SPACE_MEMORY:
        ...
        if bar_64:
          usb.pcie_cfg_req(pci.PCI_BASE_ADDRESS_0 + bar_off + 4, bus=gpu_bus, dev=0, fn=0, value=0xFFFFFFFF, size=4)
        ...
        if bar_64:
          usb.pcie_cfg_req(pci.PCI_BASE_ADDRESS_0 + bar_off + 4, bus=gpu_bus, dev=0, fn=0, value=mem_space_addr[bar_mem] >> 32, size=4)
...
class PCIDevice:
  def __init__(self, pcibus: str, bars: list[int], resize_bars: list[int] | None = None):
    ...
    for i in resize_bars or []:
      if FileIOInterface.exists(rpath := f"/sys/bus/pci/devices/{self.pcibus}/resource{i}_resize"):
        try:
          FileIOInterface(rpath, os.O_RDWR).write(str(int(FileIOInterface(rpath, os.O_RDONLY).read(), 16).bit_length() - 1))
        except OSError as e:
    ...
    if getenv("VFIO", 0) and (vfio_fd := System.vfio) is not None:
      ...
      with contextlib.suppress(OSError):
        vfio.VFIO_SET_IOMMU(vfio_fd, vfio.VFIO_NOIOMMU_IOMMU)  # set iommu works only once for the fd.
  ...
  def map_bar(self, bar: int, off: int = 0, addr: int = 0, size: int | None = None, fmt="B") -> MMIOInterface:
    ...
    libc.madvise(
      loc := fd.mmap(addr, sz, mmap.PROT_READ | mmap.PROT_WRITE, mmap.MAP_SHARED | (MAP_FIXED if addr else 0), off), sz, libc.MADV_DONTFORK
    )
