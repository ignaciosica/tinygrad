...
class USB3:
  def __init__(self, vendor: int, dev: int, ep_data_in: int, ep_stat_in: int, ep_data_out: int, ep_cmd_out: int, max_streams: int = 31):
    ...
    if not self.handle:
      raise RuntimeError(f"device {self.vendor:04x}:{self.dev:04x} not found. sudo required?")
    ...
    while running:
      ...
      for tr in cmds:
        if tr.contents.status == libusb.LIBUSB_TRANSFER_COMPLETED:
          ...
          raise RuntimeError(f"EP 0x{tr.contents.endpoint:02X} error: {tr.contents.status}")
...
class ASM24Controller:
  ...
  def pcie_prep_request(self, fmt_type: int, address: int, value: int | None = None, size: int = 4) -> list[WriteOp]:
    if fmt_type == 0x60 and size == 4 and self._is_pci_cacheable(address) and self._pci_cache.get(address) == value:
      return []
  ...
  def pcie_request(self, fmt_type, address, value=None, size=4, cnt=10):
    ...
    if value is None:
      return (struct.unpack(">I", self.read(0xB220, 4))[0] >> (8 * (address & 0x3))) & ((1 << (8 * size)) - 1)
...
class USBMMIOInterface(MMIOInterface):
  ...
  def _access_items(self, index, val=None):
    if isinstance(index, slice):
      return self._acc((index.start or 0) * self.el_sz, ((index.stop or len(self)) - (index.start or 0)) * self.el_sz, val)
