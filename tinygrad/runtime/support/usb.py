...
class USB3:
  def __init__(
    ...
    self.ep_data_in, self.ep_stat_in, self.ep_data_out, self.ep_cmd_out = (
      ep_data_in,
      ep_stat_in,
      ep_data_out,
      ep_cmd_out,
    ...
    self.handle = libusb.libusb_open_device_with_vid_pid(
      self.ctx, self.vendor, self.dev
    )
    if not self.handle:
      raise RuntimeError(
        f"device {self.vendor:04x}:{self.dev:04x} not found. sudo required?"
      )
    ...
    if libusb.libusb_set_configuration(self.handle, 1):
      raise RuntimeError("set_configuration failed")
    if libusb.libusb_claim_interface(self.handle, 0):
      raise RuntimeError("claim_interface failed. sudo required?")
    if libusb.libusb_set_interface_alt_setting(self.handle, 0, 1):
      raise RuntimeError("alt_setting failed")
    ...
    # Allocate streams
    stream_eps = (ctypes.c_uint8 * 3)(
      self.ep_data_out, self.ep_data_in, self.ep_stat_in
    )
    if (
      rc := libusb.libusb_alloc_streams(
        self.handle, self.max_streams * len(stream_eps), stream_eps, len(stream_eps)
      )
    ) < 0:
    ...
    # Init pools
    self.tr = {
      ep: [libusb.libusb_alloc_transfer(0) for _ in range(self.max_streams)]
      for ep in all_eps
    }

    self.buf_cmd = [
      (ctypes.c_uint8 * len(cmd_template))(*cmd_template)
    ...
    self.buf_data_out = [
      (ctypes.c_uint8 * 0x80000)() for _ in range(self.max_streams)
    ]
    self.buf_data_out_mvs = [
      to_mv(ctypes.addressof(self.buf_data_out[i]), 0x80000)
      for i in range(self.max_streams)
    ]

    for slot in range(self.max_streams):
      struct.pack_into(">B", self.buf_cmd[slot], 3, slot + 1)

  def _prep_transfer(self, tr, ep, stream_id, buf, length):
    (
      tr.contents.dev_handle,
      tr.contents.endpoint,
      tr.contents.length,
      tr.contents.buffer,
    ) = self.handle, ep, length, buf
    (
      tr.contents.status,
      tr.contents.flags,
      tr.contents.timeout,
      tr.contents.num_iso_packets,
    ) = 0xFF, 0, 1000, 0
    tr.contents.type = (
      libusb.LIBUSB_TRANSFER_TYPE_BULK_STREAM
      if stream_id is not None
      else libusb.LIBUSB_TRANSFER_TYPE_BULK
    )
    if stream_id is not None:
      libusb.libusb_transfer_set_stream_id(tr, stream_id)
    return tr
    ...
    while running:
      ...
      for tr in cmds:
        if tr.contents.status == libusb.LIBUSB_TRANSFER_COMPLETED:
          running -= 1
        elif tr.contents.status != 0xFF:
          raise RuntimeError(
            f"EP 0x{tr.contents.endpoint:02X} error: {tr.contents.status}"
          )

  def send_batch(
    self,
    cdbs: list[bytes],
    idata: list[int] | None = None,
    odata: list[bytes | None] | None = None,
  ) -> list[bytes | None]:
    ...
    for idx, (cdb, rlen, send_data) in enumerate(zip(cdbs, idata, odata)):
      ...
      tr_window.append(
        self._prep_transfer(
          self.tr[self.ep_cmd_out][slot],
          self.ep_cmd_out,
          None,
          self.buf_cmd[slot],
          len(self.buf_cmd[slot]),
        )
      )
      tr_window.append(
        self._prep_transfer(
          self.tr[self.ep_stat_in][slot],
          self.ep_stat_in,
          stream,
          self.buf_stat[slot],
          64,
      ...
      if rlen:
        if rlen > len(self.buf_data_in[slot]):
          self.buf_data_in[slot] = (ctypes.c_uint8 * round_up(rlen, 0x1000))()
        tr_window.append(
          self._prep_transfer(
            self.tr[self.ep_data_in][slot],
            self.ep_data_in,
            stream,
            self.buf_data_in[slot],
            rlen,
          )
        )
      ...
      if send_data is not None:
        if len(send_data) > len(self.buf_data_out[slot]):
          ...
          self.buf_data_out_mvs[slot] = to_mv(
            ctypes.addressof(self.buf_data_out[slot]), len(send_data)
          )

        self.buf_data_out_mvs[slot][: len(send_data)] = bytes(send_data)
        tr_window.append(
          self._prep_transfer(
            self.tr[self.ep_data_out][slot],
            self.ep_data_out,
            stream,
            self.buf_data_out[slot],
            len(send_data),
          )
        )
      ...
      if (idx + 1 == len(cdbs)) or len(op_window) >= self.max_streams:
        ...
        for idx, slot, rlen in op_window:
          results.append(bytes(self.buf_data_in[slot][:rlen]) if rlen else None)
        tr_window = []
...
class WriteOp:
  addr: int
  data: bytes
  ignore_cache: bool = True  # noqa: E702
...
class ASM24Controller:
  def __init__(self):
    ...
    self.exec_ops([
      ...
      WriteOp(0x5A8, b"\x02"),
      WriteOp(0x5F8, b"\x04"),
      WriteOp(0x7EC, b"\x01\x00\x00\x00"),
      WriteOp(0xC422, b"\x02"),
      WriteOp(0x0, b"\x33"),
    ])
    ...
    for op in ops:
      if isinstance(op, WriteOp):
        for off, value in enumerate(op.data):
          ...
          if not op.ignore_cache and self._cache.get(addr) == value:
            continue
          _add_req(
            struct.pack(">BBBHB", 0xE5, value, addr >> 16, addr & 0xFFFF, 0), 0, None
          )
        ...
        for i in range(op.size):
          self._cache[addr + i] = None
      elif isinstance(op, ScsiWriteOp):
        sectors = round_up(len(op.data), 512) // 512
        _add_req(
          struct.pack(">BBQIBB", 0x8A, 0, op.lba, sectors, 0, 0),
          0,
          op.data + b"\x00" * ((sectors * 512) - len(op.data)),
        )

    return self.usb.send_batch(cdbs, idata, odata)

  def write(self, base_addr: int, data: bytes, ignore_cache: bool = True):
    return self.exec_ops([WriteOp(base_addr, data, ignore_cache)])

  def scsi_write(self, buf: bytes, lba: int = 0):
    if len(buf) > 0x4000:
      buf += b"\x00" * (round_up(len(buf), 0x10000) - len(buf))

    for i in range(0, len(buf), 0x10000):
      self.exec_ops([
        ScsiWriteOp(buf[i : i + 0x10000], lba),
        WriteOp(0x171, b"\xff\xff\xff", ignore_cache=True),
      ])
      self.exec_ops([WriteOp(0xCE6E, b"\x00\x00", ignore_cache=True)])

    if len(buf) > 0x4000:
      for i in range(4):
        self.exec_ops([WriteOp(0xCE40 + i, b"\x00", ignore_cache=True)])

  def read(self, base_addr: int, length: int, stride: int = 0xFF) -> bytes:
    parts = self.exec_ops([
      ReadOp(base_addr + off, min(stride, length - off))
      for off in range(0, length, stride)
    ])
    return b"".join(p or b"" for p in parts)[:length]

  def _is_pci_cacheable(self, addr: int) -> bool:
    return any(x <= addr <= x + sz for x, sz in self._pci_cacheable)

  def pcie_prep_request(
    self, fmt_type: int, address: int, value: int | None = None, size: int = 4
  ) -> list[WriteOp]:
    if (
      fmt_type == 0x60
      and size == 4
      and self._is_pci_cacheable(address)
      and self._pci_cache.get(address) == value
    ):
      return []

    assert fmt_type >> 8 == 0 and size > 0 and size <= 4, (
      f"Invalid fmt_type {fmt_type} or size {size}"
    )
    if DEBUG >= 5:
      print("pcie_request", hex(fmt_type), hex(address), value, size)
    ...
    return (
      [WriteOp(0xB220, struct.pack(">I", value << (8 * offset)), ignore_cache=False)]
      if value is not None
      else []
    ) + [
      WriteOp(0xB218, struct.pack(">I", masked_address), ignore_cache=False),
      WriteOp(0xB21C, struct.pack(">I", address >> 32), ignore_cache=False),
      WriteOp(0xB217, bytes([((1 << size) - 1) << offset]), ignore_cache=False),
      WriteOp(0xB210, bytes([fmt_type]), ignore_cache=False),
      WriteOp(0xB254, b"\x0f", ignore_cache=True),
      WriteOp(0xB296, b"\x04", ignore_cache=True),
    ]
  ...
  def pcie_request(self, fmt_type, address, value=None, size=4, cnt=10):
    ...
    if status or (
      (fmt_type & 0xBE == 0x04)
      and (
        ((value is None) and (not (b284 & 0x01)))
        or ((value is not None) and (b284 & 0x01))
      )
    ):
      status_map = {
        0b001: f"Unsupported Request: invalid address/function (target might not be reachable): {address:#x}",
        0b100: "Completer Abort: abort due to internal error",
        0b010: "Configuration Request Retry Status: configuration space busy",
      }
      raise RuntimeError(
        f"TLP status: {status_map.get(status, 'Reserved (0b{:03b})'.format(status))}"
      )

    if value is None:
      return (
        struct.unpack(">I", self.read(0xB220, 4))[0] >> (8 * (address & 0x3))
      ) & ((1 << (8 * size)) - 1)

  def pcie_cfg_req(self, byte_addr, bus=1, dev=0, fn=0, value=None, size=4):
    assert (
      byte_addr >> 12 == 0 and bus >> 8 == 0 and dev >> 5 == 0 and fn >> 3 == 0
    ), f"Invalid byte_addr {byte_addr}, bus {bus}, dev {dev}, fn {fn}"
  ...
  def pcie_mem_req(self, address, value=None, size=4):
    return self.pcie_request(
      0x60 if value is not None else 0x20, address, value, size
    )

  def pcie_mem_write(self, address, values, size):
    ops = [
      self.pcie_prep_request(0x60, address + i * size, value, size)
      for i, value in enumerate(values)
    ]

    # Send in batches of 4 for OSX and 16 for Linux (benchmarked values)
    for i in range(0, len(ops), bs := (4 if OSX else 16)):
      self.exec_ops(list(itertools.chain.from_iterable(ops[i : i + bs])))


class USBMMIOInterface(MMIOInterface):
  def __init__(self, usb, addr, size, fmt, pcimem=True):  # pylint: disable=super-init-not-called
    self.usb, self.addr, self.nbytes, self.fmt, self.pcimem, self.el_sz = (
      usb,
      addr,
      size,
      fmt,
  ...
  def __setitem__(self, index, val):
    self._access_items(index, val)

  def _access_items(self, index, val=None):
    if isinstance(index, slice):
      return self._acc(
        (index.start or 0) * self.el_sz,
        ((index.stop or len(self)) - (index.start or 0)) * self.el_sz,
        val,
      )
    return (
      self._acc_one(index * self.el_sz, self.el_sz, val)
      if self.pcimem
      else self._acc(index * self.el_sz, self.el_sz, val)
    )
  ...
  def view(self, offset: int = 0, size: int | None = None, fmt=None):
    return USBMMIOInterface(
      self.usb,
      self.addr + offset,
      size or (self.nbytes - offset),
      fmt=fmt or self.fmt,
      pcimem=self.pcimem,
    )

  def _acc_size(self, sz):
    return next(x for x in [("I", 4), ("H", 2), ("B", 1)] if sz % x[1] == 0)

  def _acc_one(self, off, sz, val=None):
    upper = (
      0
      if sz < 8
      else self.usb.pcie_mem_req(
        self.addr + off + 4, val if val is None else (val >> 32), 4
      )
    )
    lower = self.usb.pcie_mem_req(
      self.addr + off, val if val is None else val & 0xFFFFFFFF, min(sz, 4)
    )
    if val is None:
  ...
  def _acc(self, off, sz, data=None):
    if data is None:  # read op
      if not self.pcimem:
        return (
          int.from_bytes(self.usb.read(self.addr + off, sz), "little")
          if sz == self.el_sz
          else self.usb.read(self.addr + off, sz)
        )
      ...
      return bytes(
        array.array(
          acc,
          [
            self._acc_one(off + i * acc_size, acc_size)
            for i in range(sz // acc_size)
          ],
        )
      )
    ...
    if not self.pcimem:
      ...
      return (
        self.usb.scsi_write(bytes(data))
        if self.addr == 0xF000
        else self.usb.write(self.addr + off, bytes(data), ignore_cache=not use_cache)
      )

    _, acc_sz = self._acc_size(len(data) * struct.calcsize(self.fmt))
    self.usb.pcie_mem_write(
      self.addr + off,
      [
        int.from_bytes(data[i : i + acc_sz], "little")
