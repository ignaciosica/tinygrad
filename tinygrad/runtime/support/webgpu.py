...
if OSX:
  if not os.path.exists(
    brew_prefix := subprocess.check_output(["brew", "--prefix", "dawn"])
    .decode()
    .strip()
  ):
    raise FileNotFoundError(
      "dawn library not found. Install it with `brew tap wpmed92/dawn && brew install dawn`"
    )
  WEBGPU_PATH = os.path.join(brew_prefix, "lib", "libwebgpu_dawn.dylib")
elif platform.system() == "Windows":
  if not os.path.exists(
    pydawn_path := os.path.join(sysconfig.get_paths()["purelib"], "pydawn")
  ):
    raise FileNotFoundError(
      "dawn library not found. Install it with `pip install dawn-python`"
    )
