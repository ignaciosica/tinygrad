...
def realize_srcs(ctx: dict[UOp, None], rb: UOp) -> None:
  for s in rb.src:
    if s.base.op not in ALWAYS_CONTIGUOUS:
      ctx[s] = None


def realize_assign(ctx: dict[UOp, None], a: UOp) -> None:
  if a.src[1].op not in ALWAYS_CONTIGUOUS:
    ctx[a.src[1]] = None
  # if it's a kernel, we don't realize it
  if a.src[1].op is not Ops.KERNEL:
    ctx[a] = None
...
pm_generate_realize_map = PatternMatcher([
  ...
  (
    ...
    lambda ctx, s: ctx.update(
      (x.base, None) for x in s.src if x.base.op not in ALWAYS_CONTIGUOUS
    ),
  ),
...
class IndexingContext:
  ...
  range_map: dict[UOp, tuple[tuple[UOp, ...], tuple[UOp, ...]]] = field(
  ...
  def new_range(self, s: sint, axistype: AxisType = AxisType.LOOP) -> UOp:
    if isinstance(s, UOp) and s.op is Ops.RANGE:
      return s
    # if a range has a 1 src, it's the same as UOp.const(dtypes.index, 0)
    return (
      UOp.range(s, next(self.range_idx), axistype)
      if resolve(s != 1)
      else UOp.const(dtypes.index, 0)
    )


def create_bufferize_and_index_based_on_ranges(ctx: IndexingContext, x: UOp):
  if x.op in {Ops.BUFFERIZE, Ops.INDEX}:
    return None
  if x.op is Ops.AFTER and x.src[1].op is Ops.KERNEL:
    return None
  new_srcs = []
  for s in x.src:
    new_src = s
    if s.op in {Ops.BUFFER, Ops.BUFFER_VIEW, Ops.MSTACK, Ops.MSELECT} or (
      s.op is Ops.AFTER and s.src[1].op is Ops.KERNEL
    ):
      ...
      closed_ranges = tuple([
        r for i, r in enumerate(ctx.range_map[s][1]) if i in realized_ranges
      ])
      # None in the device assigns it a number later
      opts = (
        BufferizeOpts(device=s.device)
        if len(ctx.range_map[s][1]) == len(realized_ranges)
        else BufferizeOpts(None, AddrSpace.LOCAL)
      )
      new_src = UOp(
        Ops.BUFFERIZE,
        s.dtype,
        src=(new_src,) + closed_ranges,
        arg=opts,
        tag=s.tag if opts.addrspace == AddrSpace.GLOBAL else None,
      )
      if x in ctx.range_map:
        new_src = new_src.index(*[
          r for i, r in enumerate(ctx.range_map[x][0]) if i in realized_ranges
        ])
...
def convert_pad_to_where_to_keep_behavior_local(ctx: IndexingContext, x: UOp):
  if x not in ctx.range_map:
    return None
  valid: UOp = functools.reduce(
    operator.and_,
    [r.get_valid() for r in ctx.range_map[x][0]],
...
def convert_reduce_axis_to_reduce_with_ranges(ctx: IndexingContext, x: UOp):
  ...
  ret = UOp(
    Ops.REDUCE, x.dtype, src=(x.src[0],) + tuple(new_ranges), arg=x.arg[0], tag=x.tag
  )
  ctx.range_map[ret] = ctx.range_map[x]
  return ret
...
def remove_movement_op_after_rangeify(ctx: IndexingContext, x: UOp):
  if x in ctx.range_map or x.src[0].op is Ops.INDEX:
    return x.src[0]


def add_third_op_to_assign_to_track_shape(ctx: IndexingContext, assign: UOp):
  if assign.src[1].op is Ops.KERNEL:
    return None
  to_mop = graph_rewrite(
    assign.src[0],
    PatternMatcher([
      (UPat(GroupOp.Movement, name="x"), lambda x: x.replace(tag=()))
    ]),
  )
...
pm_apply_rangeify = PatternMatcher([
  ...
  (
    UPat(Ops.ASSIGN, src=(UPat(), UPat()), name="assign"),
    add_third_op_to_assign_to_track_shape,
  ),
  ...
  (
    ...
    lambda ctx, c: c.replace(src=()) if c in ctx.range_map else None,
  ),
])
...
@functools.cache
def apply_movement_op(
  op: Ops, in_shape: tuple[sint, ...], arg: tuple, rngs: tuple[UOp, ...]
) -> tuple[UOp, ...]:
  match op:
    case Ops.SHRINK:
      rngs = tuple(a if ss == 0 else a + ss for a, (ss, _) in zip(rngs, arg))
    case Ops.PERMUTE:
      rngs = tuple(rngs[p] for p in argsort(arg))
    case Ops.FLIP:
      rngs = tuple(((s - 1) - a) if f else a for a, s, f in zip(rngs, in_shape, arg))
    case Ops.EXPAND:
      rngs = tuple(
        a if in_sh == out_sh else a.const_like(0)
        for a, in_sh, out_sh in zip(rngs, in_shape, arg)
      )
    case Ops.PAD:
      ...
      rngs = tuple(
        ...
        if (s == 0 and e == 0)
          ...
          ((r >= s) & (r < (sh + s))), symbolic + pm_simplify_valid, name="pad"
        ).where(r - s, UOp.invalid())
    ...
    case Ops.RESHAPE:
      ...
      rngs = graph_rewrite(
        UOp.sink(*axes_out[::-1]),
        symbolic + pm_simplify_valid + pm_drop_and_clauses,
        name="reshape",
      ).src
    case _:
...
def run_rangeify(tsink: UOp, debug: bool = False) -> tuple[UOp, IndexingContext]:
  ...
  # get ops to realize
  graph_rewrite(
    tsink, pm_generate_realize_map, ctx=rctx.realize_map, name="get realize"
  )

  # get the traversal order
  with cpu_profile("reverse toposort", "TINY"):
    tsink_reverse_toposort = tsink.reverse_toposort(
  ...
  for x in tsink_reverse_toposort:
    if x.op in {Ops.DEVICE, Ops.UNIQUE}:
      continue
    ...
    consumer_rngs = [
      rctx.range_map[c][0] for c in consumer_map[x] if c in rctx.range_map
    ]
    if x in rctx.realize_map:
      ...
      for i, (local_rngs, valids) in enumerate(rngs_valids):
        ...
        if all_all_same or (PCONTIG and all_same(local_rngs)):
          ...
          minimum_valid = functools.reduce(
            operator.or_, valids, UOp.const(dtypes.bool, False)
          )
          _out_rngs.append(
            graph_rewrite(
              minimum_valid.where(local_rngs[0], UOp.invalid()),
              symbolic,
              name="minimum_valid",
            )
          )
    ...
    if len(ending_ranges[x]) and x.op in GroupOp.Elementwise.union({
      Ops.REDUCE_AXIS
    }):
      _realize_axis = rctx.realize_map.get(x, []) or []
      for i, r in enumerate(out_rngs):
        if i in _realize_axis:
          continue
        if not (PCONTIG > 1) or any(
          any(rr.arg > e.arg for e in ending_ranges[x]) for rr in r.ranges
        ):
          _realize_axis.append(i)
      ending_ranges[x] = []
      if len(_realize_axis):
        rctx.realize_map[x] = _realize_axis
        out_rngs = tuple([
          (rctx.new_range(x.shape[i]) if i in _realize_axis else r)
          for i, r in enumerate(out_rngs)
        ])
    ...
    # apply movement ops
    if x.op in GroupOp.Movement:
      rngs = apply_movement_op(x.op, x.src[0].shape, x.marg, rngs)
    ...
    if x.op is Ops.EXPAND and all(
      isinstance(y, int) or y.op is not Ops.RANGE for y in x.shape
    ):
      ending_ranges[x] = list(
        UOp.sink(*[
          ro for ri, ro in zip(rngs, out_rngs) if ri is not ro
        ]).ranges.keys()
      )
    ...
    if x.op is Ops.REDUCE_AXIS:
      rngs = tuple(
        rctx.new_range(s, axistype=AxisType.REDUCE) if i in x.arg[1] else r
    ...
    if debug:
      ...
      disp = []
      for i, (ri, ro) in enumerate(
        zip([r.render() for r in rngs], [r.render() for r in out_rngs])
      ):
        rng = f"{ri}" if ri == ro else f"{ri} -> {ro}"
        if realized_ranges is not None and i in realized_ranges:
          rng = colored(rng, "yellow")
        disp.append("[" + rng + "]")
      print(
        "***" if x in rctx.realize_map else "   ",
        len(consumer_map[x]),
        f"{str(x.op):20s}",
        "".join(disp),
      )
  ...
  tsink = graph_rewrite(
    tsink, pm_apply_rangeify, ctx=rctx, bottom_up=True, name="apply rangeify"
  )
  return tsink, rctx
