...
def handle_allreduce_multirank(buf: UOp, red: UOp) -> UOp | None:
  if not isinstance(buf.device, tuple):
    return None
  ...
  groups = {
    gid: sorted(bufs, key=lambda x: (x.device not in reduce_leaders, x.device))
    for gid, bufs in groups.items()
  }

  # Skip if only one group or if every group has only one buffer
  if len(groups) <= 1 or not any(len(g) > 1 for g in groups.values()):
    return None

  # Reduce inside each group
  inner = [
    UOp(Ops.MSTACK, buf.dtype, tuple(bufs))
    .allreduce(red.arg, (cast(str, bufs[0].device),))
    .mselect(0)
    for bufs in groups.values()
  ]

  # Allreduce across groups
  outer = UOp(Ops.MSTACK, buf.dtype, tuple(inner)).allreduce(
    red.arg, tuple(buf.device for buf in inner)
  )

  # Broadcast back to all devices in the group
  gid2bid = {Device[device].group_id: i for i, device in enumerate(outer.device)}
  return (
    outer.mselect(gid2bid[Device[red.device].group_id]).copy_to_device(red.device)
    if not isinstance(red.device, tuple)
    else UOp(
      Ops.MSTACK,
      buf.dtype,
      tuple(
        outer.mselect(gid2bid[Device[device].group_id]).copy_to_device(device)
        for device in red.device
      ),
    )
  )
...
def handle_allreduce(buf: UOp, red: UOp) -> UOp | None:
  ...
  if DEBUG >= 2:
    print(
      f"{'RING ALLREDUCE' if use_ring else 'NAIVE ALLREDUCE'} {n_lbs}x{numel} | {buf.dtype}"
    )
  ...
  if not use_ring:
    return functools.reduce(
      lambda x, y: x.alu(red.arg, y),
      [
        UOp(Ops.COPY, buf.dtype, (buf.mselect(i), red.src[1]))
        for i in range(len(buf.device))
      ],
    )
  ...
  for i, (s, e) in enumerate(chunks):
    ...
    for step in range(n_lbs - 1):
      ...
      reduced_chunk = reduced_chunk.copy_to_device(
        buf.device[dest], src if isinstance(reduced_chunk.device, tuple) else None
  ...
  for i, c in enumerate(reduced_chunks):
    ...
    copied_chunks.append(
      UOp(Ops.MSTACK, buf.dtype, tuple(cast(list[UOp], this_chunk)))
    )

  # reassemble
  pads = [((s, numel - e),) for s, e in chunks]
  return functools.reduce(
    operator.add, [c.pad(pad) for pad, c in zip(pads, copied_chunks)]
  ).reshape(shape)
...
def mstack_early_shrink(ms: UOp, shrink: UOp):
  ...
  def apply_shrink(s: UOp, i: int) -> UOp:
    new_arg = [
      tuple([
        x.substitute({dvar[0]: dvar[0].const_like(i)})
        if isinstance(x, UOp)
        and (
          dvar := [
            v
            for v in x.vars()
            if v.op is Ops.DEFINE_VAR and v.arg[0] == "_device_num"
          ]
        )
        else x
        for x in ss
...
replace_allreduce = PatternMatcher([
  (
    UPat(Ops.ALLREDUCE, src=(UPat.var("buf"), UPat()), name="red"),
    handle_allreduce_multirank,
  ),
  ...
  (
    UPat(
      Ops.COPY,
      name="c",
      src=(UPat(GroupOp.All - {Ops.CONST}, name="x"), UPat(Ops.DEVICE)),
    ),
    lambda c, x: UOp(
      Ops.MSTACK, c.dtype, tuple(x.copy_to_device(d) for d in c.device)
    )
    if isinstance(c.device, tuple) and isinstance(x.device, str)
    else None,
  ),
  ...
  (
    ...
    lambda c, x: x.mselect(0).copy_to_device(c.device)
    if isinstance(c.device, str) and isinstance(x.device, tuple)
    else None,
  ),
  # MSELECT on MSTACK is replaced with nothing
  (
    UPat(Ops.MSELECT, src=(UPat(Ops.MSTACK, name="mstack"),), name="ms"),
    lambda mstack, ms: mstack.src[ms.arg],
  ),
  # move shrink before MSTACK
  (
    UPat(
      Ops.SHRINK,
      src=(UPat(Ops.MSTACK, name="ms"),),
      allow_any_len=True,
      name="shrink",
    ),
    mstack_early_shrink,
  ...
  (
    UPat(
      Ops.MSELECT,
      src=(
        UPat(GroupOp.Movement, src=(UPat.var("s"),), allow_any_len=True, name="v"),
      ),
      name="ms",
    ),
...
def alu_multi(root: UOp):
  ...
  for mlb in msrcs:
    if mlb.axis == axis:
      ...
      assert mlb.op is Ops.MULTI
      srcs.append(
        mlb.src[0]._unshard(mlb.axis).allreduce(Ops.ADD, mlb.device)._shard(axis)
      )
...
def _shape_to_single_shard(
  axis, shape: tuple[sint, ...], lb: UOp
) -> tuple[sint, ...]:
...
def reshape_multi(root: UOp, multi: UOp):
  ...
  if (new_axis := root.axis) is None:
    return multi.src[0].reshape(arg).multi(new_axis)
  assert prod(multi.shape) == prod(arg), "reshape must maintain prod(shape)"
  assert prod(multi.src[0].shape[multi.axis :]) % prod(arg[new_axis + 1 :]) == 0, (
    f"reshape cannot move items between shards {multi.shape} -> {arg=}"
  )
  new_shape_axis = prod(multi.src[0].shape[multi.axis :]) // prod(
    arg[new_axis + 1 :]
  )
  return (
    multi.src[0]
    .reshape(
      tuple(s if a != new_axis else new_shape_axis for a, s in enumerate(arg))
    )
    .multi(new_axis)
  )
...
def expand_multi(root: UOp, multi: UOp):
  ...
  assert multi.axis is None or root.marg[multi.axis] == multi.shape[multi.axis], (
    f"expand not supported on sharded axis {root.marg=}"
  )
  return (
    multi.src[0]
    .expand(_shape_to_single_shard(multi.axis, root.marg, multi.src[0]))
    .multi(multi.axis)
  )


def pad_multi(root: UOp, multi: UOp):
  assert multi.axis is None or root.marg[multi.axis] == (0, 0), (
    f"padding not supported for {root.marg=}"
  )
...
def shrink_multi(root: UOp, multi: UOp):
  assert (
    multi.axis is None
    or root.marg[multi.axis] == (0, multi.shape[multi.axis])
    or root.marg[multi.axis] in multi.bounds
  ), f"shrinking not supported for {root.marg=}"
  if (
    multi.axis is not None
    and root.marg[multi.axis] in multi.bounds
    and root.marg[multi.axis] != (0, multi.shape[multi.axis])
  ):
    assert all(
      root.marg[i] == (0, s) or i == multi.axis for i, s in enumerate(multi.shape)
    ...
    return multi.src[0].copy_to_device(
      multi.device, arg=multi.bounds.index(root.marg[multi.axis])
    )
  return (
    multi.src[0]
    .shrink(
      tuple(
        (0, multi.src[0].shape[multi.axis]) if a == multi.axis else s
        for a, s in enumerate(root.marg)
      )
    )
    .multi(multi.axis)
  )
...
def flip_multi(root: UOp, multi: UOp):
  assert multi.axis is None or not root.marg[multi.axis], (
...
def assign_multi(dest: UOp, src: UOp):
  if dest.axis != src.axis:
    raise RuntimeError(f"axis must match in assign {dest.axis} != {src.axis}")
...
multi_pm = (
  PatternMatcher([
    ...
    (
      UPat(Ops.RESHAPE, src=(UPat(Ops.MULTI, name="multi"), UPat()), name="root"),
      reshape_multi,
    ),
    (
      UPat(Ops.EXPAND, src=(UPat(Ops.MULTI, name="multi"), UPat()), name="root"),
      expand_multi,
    ),
    (
      UPat(
        Ops.PAD, src=(UPat(Ops.MULTI, name="multi"), UPat(), UPat()), name="root"
      ),
      pad_multi,
    ),
    (
      UPat(
        Ops.SHRINK, src=(UPat(Ops.MULTI, name="multi"), UPat(), UPat()), name="root"
      ),
      shrink_multi,
    ),
    (
    ...
    (
      UPat(
        Ops.ASSIGN, src=(UPat(Ops.MULTI, name="dest"), UPat(Ops.MULTI, name="src"))
      ),
      assign_multi,
    ),
    (
      UPat(
        Ops.COPY,
        src=(UPat(Ops.MULTI, name="multi"), UPat(Ops.DEVICE, name="device")),
      ),
      copy_multi,
    ),
    (
      ...
      lambda multi, device, red: multi.src[0]
      .allreduce(red.arg, device)
      .multi(axis=multi.axis),
    ),
    (
      UPat(
        (Ops.CAST, Ops.BITCAST, Ops.CONTIGUOUS, Ops.DETACH, Ops.CONTIGUOUS_BACKWARD),
...
def get_multi_map(big_sink: UOp) -> dict[UOp, UOp]:
  if getenv("VIZ"):
    graph_rewrite(big_sink, PatternMatcher([]), name="View Multi AST")
  ret = graph_rewrite_map(big_sink, multi_pm, name="multi_pm")
  if getenv("VIZ"):
    graph_rewrite(ret[big_sink], PatternMatcher([]), name="View Post Multi AST")
  return ret
