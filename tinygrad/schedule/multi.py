  ...
  def apply_shrink(s: UOp, i: int) -> UOp:
    new_arg = [
      tuple([
        ...
        and (
          dvar := [
            v for v in x.vars() if v.op is Ops.DEFINE_VAR and v.arg[0] == "_device_num"
          ]
        )
        else x
        for x in ss
...
replace_allreduce = PatternMatcher([
  ...
  (
    ...
    lambda c, x: UOp(Ops.MSTACK, c.dtype, tuple(x.copy_to_device(d) for d in c.device))
  ...
  (
    UPat(
      Ops.SHRINK, src=(UPat(Ops.MSTACK, name="ms"),), allow_any_len=True, name="shrink"
    ),
    mstack_early_shrink,
  ),
  ...
  (
    UPat(
      Ops.MSELECT,
      src=(UPat(GroupOp.Movement, src=(UPat.var("s"),), allow_any_len=True, name="v"),),
      name="ms",
    ),
...
def reshape_multi(root: UOp, multi: UOp):
  ...
  return (
    multi.src[0]
    .reshape(tuple(s if a != new_axis else new_shape_axis for a, s in enumerate(arg)))
    .multi(new_axis)
  )
...
multi_pm = (
  PatternMatcher([
    ...
    (
      UPat(Ops.PAD, src=(UPat(Ops.MULTI, name="multi"), UPat(), UPat()), name="root"),
      pad_multi,
    ),
    (
      UPat(
    ...
    (
      UPat(Ops.ASSIGN, src=(UPat(Ops.MULTI, name="dest"), UPat(Ops.MULTI, name="src"))),
      assign_multi,
    ),
    (
      UPat(
        Ops.COPY, src=(UPat(Ops.MULTI, name="multi"), UPat(Ops.DEVICE, name="device"))
      ),
      copy_multi,
    ),
    (
