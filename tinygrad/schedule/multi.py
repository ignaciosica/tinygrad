...
def handle_allreduce_multirank(buf: UOp, red: UOp) -> UOp | None:
  ...
  return (
    ...
    else UOp(Ops.MSTACK, buf.dtype, tuple(outer.mselect(gid2bid[Device[device].group_id]).copy_to_device(device) for device in red.device))
  )
...
def handle_allreduce(buf: UOp, red: UOp) -> UOp | None:
  ...
  if DEBUG >= 2:
    print(f"{'RING ALLREDUCE' if use_ring else 'NAIVE ALLREDUCE'} {n_lbs}x{numel} | {buf.dtype}")
  ...
  if not use_ring:
    return functools.reduce(lambda x, y: x.alu(red.arg, y), [UOp(Ops.COPY, buf.dtype, (buf.mselect(i), red.src[1])) for i in range(len(buf.device))])
  ...
  for i, (s, e) in enumerate(chunks):
    ...
    for step in range(n_lbs - 1):
      ...
      reduced_chunk = reduced_chunk.copy_to_device(buf.device[dest], src if isinstance(reduced_chunk.device, tuple) else None).alu(
        red.arg, chunk.copy_to_device(buf.device[dest], dest)
      )
  ...
  def apply_shrink(s: UOp, i: int) -> UOp:
    new_arg = [
      tuple([
        ...
        if isinstance(x, UOp) and (dvar := [v for v in x.vars() if v.op is Ops.DEFINE_VAR and v.arg[0] == "_device_num"])
        else x
        for x in ss
      ])
...
replace_allreduce = PatternMatcher([
  ...
  (
    UPat(Ops.COPY, name="c", src=(UPat(GroupOp.All - {Ops.CONST}, name="x"), UPat(Ops.DEVICE))),
    lambda c, x: x.mselect(0).copy_to_device(c.device) if isinstance(c.device, str) and isinstance(x.device, tuple) else None,
  ),
...
multi_pm = (
  PatternMatcher([
    ...
    (
      UPat((Ops.CAST, Ops.BITCAST, Ops.CONTIGUOUS, Ops.DETACH, Ops.CONTIGUOUS_BACKWARD, Ops.FUSE), src=(UPat(Ops.MULTI, name="multi"),), name="root"),
      passthrough_multi,
    ),
  ])
  + replace_allreduce
