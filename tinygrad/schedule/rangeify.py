...
def find_permutes(a: UOp, b: UOp, assign: UOp):
  if not (
    permutes := [
      ...
      if s.op in GroupOp.Movement and s.op not in {Ops.RESHAPE, Ops.EXPAND, Ops.PAD, Ops.SHRINK}
    ]
  ):
    return
  target = a.base
  for p in permutes:
    if any(s is target for s in p.toposort(gate=lambda s: s.op not in ALWAYS_CONTIGUOUS - {Ops.BUFFER})):
...
def split_reduceop(reduce: UOp, x: UOp):
  if prod(reduce.shape) == 0:
    return None
  if not SPLIT_REDUCEOP or not all_int(x.shape) or (prod(x.shape) // prod(reduce.shape)) < getenv("REDUCEOP_SPLIT_THRESHOLD", 32768):
    return None
...
earliest_rewrites = mop_cleanup + PatternMatcher([
  ...
  (
    ...
    if ((t := target.base).op is not Ops.BUFFER and not (t.op is Ops.MSTACK and all(s.op is Ops.BUFFER for s in t.src)))
    else None,
  ),
...
pm_const_buffer_folding = pm_mops + PatternMatcher([
  ...
  (
    ...
    if isinstance(x.dtype, ImageDType) and (resolve(prod(x.dtype.shape) != prod(x.shape)) or x.shape[-1] % 4 != 0)
    else None,
  ),
  ...
  (
    UPat.any((r := UPat(dtype=dtypes.index).cast()).named("src"), r.eq(UPat()).named("src"))
...
pm_remove_bufferize = PatternMatcher([
  ...
  (
    UPat(
      Ops.COPY,
      src=(
        UPat(GroupOp.All - {Ops.CONTIGUOUS, Ops.COPY}).f(Ops.BUFFERIZE, allow_any_len=True, name="b").f(Ops.INDEX, allow_any_len=True, name="x"),
        UPat(),
      ),
      name="copy",
    ),
...
def limit_bufs(ctx: IndexingContext, root: UOp):
  ...
  def gate_input(u: UOp):
    ...
    if is_load := (u.op in {Ops.BUFFERIZE, Ops.AFTER, Ops.BUFFER, Ops.MSELECT, Ops.MSTACK, Ops.DEFINE_VAR}):
      bufs.add(u)
    return not is_load
...
rangeify_codegen = PatternMatcher([
  ...
  (
    ...
    else idx.replace(dtype=dg.dtype, arg=None).load(dtype=dg.dtype.base.scalar().vec(dg.dtype.vcount)),
  ),
...
add_tags = PatternMatcher([
  ...
  (
    UPat(
      GroupOp.All
      - {Ops.BUFFER, Ops.CONST, Ops.DEVICE, Ops.UNIQUE, Ops.DEFINE_VAR, Ops.BIND, Ops.KERNEL, Ops.MSTACK, Ops.MSELECT, Ops.RANGE}.union(
        GroupOp.Movement
      ),
      name="x",
    ),
...
replace_contiguous = PatternMatcher([
  ...
  (
    ...
    lambda ctx, alu: alu.replace(src=new_src) if (new_src := tuple(ctx.get(s, s) for s in alu.src)) != alu.src else None,
  ),
])
...
def get_rangeify_map(sink: UOp) -> dict[UOp, UOp]:
  ...
  tsink = graph_rewrite(
    tsink, symbolic_flat + pm_reduce_simplify + pm_const_buffer_folding, name="symbolic+reduce_collapse"
  ...
  tsink = UOp.sink(*[
    x for x in tsink.backward_slice if x.base.op in {Ops.BUFFERIZE, Ops.MSTACK, Ops.CONST, Ops.BUFFER, Ops.AFTER} and x.tag is not None and len(x.tag)
  ])
  ...
  for u in tsink.toposort():
    ...
    for s in u.src[1].src:
      ...
      if s.op is not Ops.BUFFER or s is u.buf_uop or (a := kernel_assign.get(s)) is None:
        continue
  ...
  if assign_rep:
    tsink = graph_rewrite(tsink, _substitute, ctx=assign_rep, bottom_up=True, name="fix_assign")
