...
def canonicalize_device(device: str | None) -> str:
  return Device.canonicalize(device)
...
def _apply_map_to_tensors(
  applied_map: dict[UOp, UOp], name: str | None = None
) -> None:
  scope_tensors = [
    t
    for tref in tuple(all_tensors)
...
def get_shape(x) -> tuple[int, ...]:
  ...
  if (
    ...
    or not hasattr(x, "__getitem__")
    or isinstance(x, str)
    or (hasattr(x, "shape") and x.shape == ())
  ):
    return ()
  if not all_same(subs := [get_shape(xi) for xi in x]):
    raise ValueError(f"inhomogeneous shape from {x}")
  return (len(subs),) + (subs[0] if subs else ())


def _frompy(x: list | tuple | bytes, dtype: DType) -> UOp:
  if isinstance(x, bytes):
    ret, data = UOp.new_buffer("PYTHON", len(x) // dtype.itemsize, dtype), x
  else:
    ret = UOp.new_buffer("PYTHON", prod(shape := get_shape(x)), dtype).reshape(shape)
    assert dtype.fmt is not None, f"{dtype=} has None fmt"
    truncate_function = truncate[dtype]
    data = struct.pack(
      f"{ret.size}{dtype.fmt}",
      *[truncate_function(dtypes.as_const(xi, dtype)) for xi in fully_flatten(x)],
    )
  # fake realize
  ret.buffer.allocate(
    memoryview(data if Device.DEFAULT != "PYTHON" else bytearray(data))
  )
  return ret


def _get_winograd_matcols(
  mat, dims: int, shp: tuple[sint, ...], device: str | tuple[str, ...], dtype: DType
) -> list[list[Tensor]]:
  return [
    [
      Tensor.cat(
...
def _broadcast_shape(*shapes: tuple[sint, ...]) -> tuple[sint, ...]:
  return tuple(
    ...
    for nth_dim_sizes in zip(*_align_left(*shapes))
  )


def _masked_setitem(
  target: Tensor, values: Tensor, mask: Tensor, axes: tuple[int, ...]
) -> Tensor:
  # reduce such that if mask contains repeated indices the last one remains
  for dim in reversed(axes):
    mask, values = functools.reduce(
      lambda x, y: (x[0] | y[0], y[0].where(y[1], x[1])),
      zip(mask.split(1, dim), values.split(1, dim)),
    )
  # remove extra dims from reduce
  for dim in reversed(axes):
    mask, values = mask.squeeze(dim), values.squeeze(dim)
...
def _flat_to_grouped(padding: Sequence[sint]) -> tuple[tuple[sint, sint], ...]:
...
class Tensor(MathMixin, MovementMixin):
  ...
  def __init__(
    ...
    if device is None and isinstance(data, pathlib.Path):
      device = f"DISK:{data.resolve()}"  # keep it on the disk if device is None
    _dtype: DType | None = to_dtype(dtype) if dtype is not None else None
    _device: str | tuple[str, ...] = (
      tuple(canonicalize_device(x) for x in device)
      if isinstance(device, (tuple, list))
      else canonicalize_device(device)
    )
    del device, dtype
    ...
    if isinstance(data, UOp):
      assert _dtype is None or _dtype == data.dtype, (
      ...
      if data.dtype == dtypes.index:
        data = _index_to_concrete_int(data)
      ...
      data = UOp.const(
        _dtype or dtypes.default_float, 0, _device, (), unique=_force_unique
      )
    elif isinstance(data, get_args(ConstType)):
      data = UOp.const(
        _dtype or dtypes.from_py(data), data, _device, (), unique=_force_unique
      )
    elif isinstance(data, bytes):
      data = _frompy(data, dtypes.uint8 if _dtype is None else _dtype)
      ...
      if _dtype is None:
        if (d := fully_flatten(data)) and all(isinstance(s, bool) for s in d):
          ...
          _dtype = (
            dtypes.default_int if d and all_int(d) else dtypes.default_float
          )  # NOTE: this works because all_int([True, False]) is True
      if _dtype in [dtypes.bfloat16, *dtypes.fp8s]:
        data = Tensor(_frompy(data, dtypes.float32), device=_device).cast(_dtype).uop
      else:
        data = _frompy(data, _dtype)
      ...
      if data.shape == ():
        data = UOp.const(
          _dtype or _from_np_dtype(data.dtype),
          data.item(),
          _device,
          (),
          unique=_force_unique,
        )
      else:
        data = _fromnp(
          data.astype(npdtype)
          if _dtype is not None and (npdtype := _to_np_dtype(_dtype)) is not None
          else data
        )  # type: ignore [name-defined]
    elif isinstance(data, pathlib.Path):
      _dtype = _dtype or dtypes.uint8
      data = UOp.new_buffer(
        f"DISK:{data.resolve()}", data.stat().st_size // _dtype.itemsize, _dtype
      )

    # by this point, it has to be a UOp
    if not isinstance(data, UOp):
      raise RuntimeError(f"can't create Tensor from {data!r} with type {type(data)}")

    # data might be on a different device
    if isinstance(_device, str):
      self.uop: UOp = (
        data if data.device == _device else data.copy_to_device(_device)
      )
    # if device is a tuple, we should have/construct a MultiLazyBuffer
    elif isinstance(data.device, str):
      self.uop = Tensor(data).shard(_device).uop
    else:
      assert data.device == _device, (
        f"MultiLazyBuffer device mismatch, {data.device} != {_device}"
      )
      self.uop = data
  ...
  def _apply_uop(self, fxn: Callable, *x: Tensor, extra_args=(), **kwargs) -> Tensor:
    ...
    if (metadata := _METADATA.get()) is not None:
      all_metadata[new_uop] = (metadata,)
    needs_input_grad = [t.requires_grad for t in (self,) + x]
    return Tensor(
      new_uop,
      device=new_uop.device,
      requires_grad=True
      if any(needs_input_grad)
      else None
      if None in needs_input_grad
      else False,
    )

  def _apply_broadcasted_uop(
    self, fxn: Callable, x: Tensor | ConstType, reverse=False
  ) -> Tensor:
  ...
  def _binop(self, op, x, reverse):
    return self._apply_broadcasted_uop(
      lambda *u: UOp.alu(u[0], op, *u[1:]), x, reverse
    )
  ...
  def custom_kernel(
    self, *lst: Tensor, fxn: Callable, grad_fxn: Callable | None = None
  ) -> list[Tensor]:
    """
    ...
    return [
      Tensor(u)
      for u in UOp.custom_kernel(
        *[t.uop for t in (self,) + lst], fxn=fxn, grad_fxn=grad_fxn
      )
    ]
  ...
  def schedule_with_vars(
    ...
    if (DEBUG >= 1 and len(schedule) > 1) or DEBUG >= 3:
      print(
        f"scheduled {len(schedule)} kernels in {(time.perf_counter() - st) * 1000:.2f} ms"
      )
  ...
  def realize(self, *lst: Tensor, do_update_stats=True) -> Tensor:
    ...
    if len(to_realize := [x for x in (self,) + lst if not x.uop.is_contiguous()]):
      run_schedule(
        *Tensor.schedule_with_vars(*to_realize), do_update_stats=do_update_stats
      )
    return self
  ...
  def replace(self, x: Tensor, allow_shape_mismatch=False) -> Tensor:
    ...
    assert self.shape == x.shape or allow_shape_mismatch, (
  ...
  def assign(self, x) -> Tensor:
    ...
    # broadcast x
    if least_upper_dtype(self.dtype, x.dtype) == self.dtype:
      x = x._broadcast_to(self.shape).cast(self.dtype)
    assert self.shape == x.shape, f"assign shape mismatch {self.shape} != {x.shape}"
    assert self.device == x.device, (
      f"assign device mismatch {self.device} != {x.device}"
    )
  ...
  def data(self) -> memoryview:
    ...
    """
    if 0 in self.shape:
      return memoryview(bytearray(0)).cast(self.dtype.base.fmt)
  ...
  def tolist(self) -> Sequence[ConstType] | ConstType:
    ...
    if self.dtype in (dtypes.half, dtypes.bfloat16, *dtypes.fp8s):
  ...
  def numpy(self) -> "np.ndarray":  # type: ignore [name-defined] # noqa: F821
    ...
    if self.dtype.base in {dtypes.bfloat16, *dtypes.fp8s}:
      return self.float().numpy()
    if 0 in self.shape:
      return np.empty(self.shape, dtype=_to_np_dtype(self.dtype.base))
  ...
  def to_(self, device: str | tuple[str, ...] | None) -> Tensor:
    ...
    """
    real = self.to(device)
    if self.grad is not None and real.grad is not None:
      self.grad.replace(real.grad)
  ...
  def shard(self, devices: tuple[str, ...], axis: int | None = None) -> Tensor:
    ...
    mlb = (
      self.uop.shard(devices, self._resolve_dim(axis))
      if axis is not None
      else self.uop.copy_to_device(devices)
    )
  ...
  def load(self, size: int) -> Tensor:
    ...
    for i in reversed(range(tree_depth + 1)):
      ...
      if i > 0 or tree_depth == 0:
        level_chunks = max(
          1, math.ceil(base_chunks / (Tensor.CHUNK_SIZE // 16) ** (i - 1))
        )
        pad_amt = 16 * level_chunks
      else:
        pad_amt = Tensor.CHUNK_SIZE * level_chunks
      if (tsize := data.shape[0]) < pad_amt:
        data = data.pad((0, pad_amt - tsize))
  ...
  def store(self) -> Tensor:
    ...
    if (tsize := data.shape[0]) % Tensor.CHUNK_SIZE != 0:
      data = data.pad((0, Tensor.CHUNK_SIZE - tsize % Tensor.CHUNK_SIZE))
    size = data.shape[0]
  ...
  def from_uop(y: UOp, **kwargs) -> Tensor:
    ...
    if y.op is Ops.CONST:
      return Tensor(y.arg, **kwargs, requires_grad=False)
    if y.op is Ops.MUL:
      return Tensor.from_uop(y.src[0]) * Tensor.from_uop(y.src[1])
    if y.op is Ops.ADD:
      return Tensor.from_uop(y.src[0]) + Tensor.from_uop(y.src[1])
  ...
  def empty(
    ...
    **kwargs,
    ...
    """
    dtype, shape = (
      to_dtype(dtype) if dtype is not None else dtypes.default_float,
      argfix(*shape),
    )
    if not isinstance(
      size := prod([x.vmax if isinstance(x, UOp) else x for x in shape]), int
    ):
      raise ValueError(f"size must be int {size}")
    # TODO: add test for multidevice tensor
    device = (
      tuple(canonicalize_device(d) for d in device)
      if isinstance(device, tuple)
      else canonicalize_device(device)
    )
    return (
      Tensor(UOp.new_buffer(device, size, dtype), device, dtype, **kwargs)
      .shrink(((0, prod(shape)),))
      .reshape(shape)
    )
  ...
  def _threefry_random_bits(key: Tensor, counts0: Tensor, counts1: Tensor) -> Tensor:
    ...
    x = x._apply_uop(
      UOp.threefry,
      (key[1]._broadcast_to(x.shape).cast(dtypes.uint64) << 32)
      | key[0]._broadcast_to(x.shape).cast(dtypes.uint64),
    )
    counts0, counts1 = (
    ...
    **kwargs,
    ...
    if device not in Tensor._device_seeds:
      Tensor._device_seeds[device] = Tensor(
        [
          int.from_bytes(
            hashlib.sha256(len(Tensor._device_seeds).to_bytes(4, "big")).digest(),
            "big",
          ),
          Tensor._seed,
        ],
    ...
    bits = Tensor._threefry_random_bits(
      Tensor._device_seeds[device], counts0, counts1
    )[:num]
  ...
  def full(shape: tuple[sint, ...], fill_value: ConstType, **kwargs) -> Tensor:
    ...
    """
    return (
      Tensor(fill_value, _force_unique=True, **kwargs)
  ...
  def arange(start, stop=None, step=1, **kwargs) -> Tensor:
    ...
    if start < (dt := to_dtype(dtype)).min or dt.max < (stop - step):
      raise ValueError(
        f"arange [{start}, {stop}) is not representable in dtype {dtype}"
      )
    # NOTE: this matches numpy, torch raises RuntimeError if stop-start and step have different signs
    if (output_len := ceildiv(stop - start, step)) <= 0:
      return Tensor([], dtype=dtype, **kwargs)
    return (
      Tensor.full((output_len,), step, dtype=dtype, **kwargs)._cumalu(0, Ops.ADD)
      + (start - step)
    ).cast(dtype)

  @staticmethod
  def linspace(
    start: int | float, stop: int | float, steps: int, **kwargs
  ) -> Tensor:
    """
    ...
    if (dtype := to_dtype(kwargs.pop("dtype", dtypes.default_float))) == dtypes.bool:
      raise ValueError("linspace with bool dtype is not supported")
    if steps == 1:
      return Tensor([start], dtype=dtype, **kwargs)
    return (
      start + Tensor.arange(steps, **kwargs) * ((stop - start) / (steps - 1))
    ).cast(dtype)
  ...
  def eye(n: int, m: int | None = None, **kwargs) -> Tensor:
    ...
    if n < 0 or (m is not None and m < 0):
      raise ValueError(f"cannot have negative {n=}, {m=}")
    x = Tensor.ones(n, **kwargs).diag()
    return (
      x
      if m is None
      else x.pad((None, (0, m - n)))
      if m > n
      else x.shrink((None, (0, m)))
    )
  ...
  def rand_like(self, **kwargs) -> Tensor:
    ...
    if isinstance(self.device, tuple):
      if kwargs.get("device") is not None:
        raise RuntimeError(
          "cannot specify `device` on `rand_like` of a multi device tensor"
        )
      if self.uop.axis is None:
        return Tensor.rand(*self.shape, dtype=dtype, **kwargs).shard(self.device)
      contiguous = kwargs.pop("contiguous", True)
      sharded_shape = tuple(
        s // len(self.device) if a == self.uop.axis else s
        for a, s in enumerate(self.shape)
      )
      rands = UOp(
        Ops.MSTACK,
        dtype=dtype,
        src=tuple([
          Tensor.rand(
            sharded_shape, device=d, dtype=dtype, contiguous=contiguous, **kwargs
          ).uop
          for d in self.device
        ]),
      )
      return Tensor(
        UOp.multi(rands, axis=self.uop.axis),
        device=self.device,
        dtype=dtype,
        **kwargs,
      )
    return Tensor.rand(
      *self.shape, device=kwargs.pop("device", self.device), dtype=dtype, **kwargs
    )

  # ***** rng hlops *****

  def randn_like(
    self, dtype: DTypeLike | None = None, requires_grad: bool | None = None, **kwargs
  ) -> Tensor:
    """
  ...
  def randn(
    ...
    **kwargs,
    ...
    ```
    """
    return Tensor.empty(*shape, **kwargs).randn_like(
  ...
  def randint(*shape, low=0, high=10, dtype=dtypes.int32, **kwargs) -> Tensor:
    ...
    if not dtypes.is_int(dtype):
      raise TypeError(f"{dtype=} must be int")
    return Tensor.uniform(*shape, low=low, high=high, dtype=dtype, **kwargs)

  @staticmethod
  def normal(
    *shape, mean=0.0, std=1.0, requires_grad: bool | None = None, **kwargs
  ) -> Tensor:
    """
    ...
    ```
    """
    return ((std * Tensor.randn(*shape, **kwargs)) + mean).requires_grad_(
      requires_grad
    )
  ...
  def uniform(
    ...
    **kwargs,
    ...
    """
    return (
      ((high - low) * Tensor.rand(*shape, **kwargs)).cast(
  ...
  def glorot_uniform(*shape, **kwargs) -> Tensor:
    ...
    """
    return Tensor.uniform(*shape, low=-1.0, high=1.0, **kwargs).mul(
      (6 / (argfix(*shape)[0] + prod(argfix(*shape)[1:]))) ** 0.5
    )
  ...
  def multinomial(
    ...
    ```
    """
    assert 1 <= self.ndim <= 2 and num_samples > 0, (
  ...
  def gradient(
    self, *targets: Tensor, gradient: Tensor | None = None, materialize_grads=False
  ) -> list[Tensor]:
    """
    ...
    ```
    """
    assert gradient is not None or self.shape == tuple(), (
      "when no gradient is provided, backward must be called on a scalar tensor"
    )
    if not (
      self.is_floating_point() and all(t.is_floating_point() for t in targets)
    ):
      raise RuntimeError("only float Tensors have gradient")
    if gradient is None:
      gradient = Tensor(
        1.0, dtype=self.dtype, device=self.device, requires_grad=False
      )
  ...
  def backward(self, gradient: Tensor | None = None) -> Tensor:
    ...
    tensors_need_grad: list[Tensor] = [
      t
      for tref in all_tensors
      if (t := tref()) is not None and t.uop in all_uops and t.requires_grad
    ]
    # clear contexts
    for t, g in zip(
      tensors_need_grad,
      self.gradient(*tensors_need_grad, gradient=gradient, materialize_grads=True),
    ):
  ...
  def _mop(self, op: Ops, arg) -> Tensor:
    return self._apply_uop(UOp._mop, extra_args=(op,), arg=arg)
  ...
  def expand(self, shape, *args) -> Tensor:
    ...
    """
    new_shape = tuple(
      from_ if to == -1 or to is None else to
      for from_, to in zip(*(_align_left(self.shape, argfix(shape, *args))))
    )
  ...
  def flip(self, axis, *args) -> Tensor:
    ...
    return self._apply_uop(
      UOp.flip, arg=tuple([i in axis_arg for i in range(len(self.shape))])
    )
  ...
  def shrink(self, arg: tuple[tuple[sint, sint] | None, ...]) -> Tensor:
    ...
    if (
      shrink_arg := [x if x is not None else (0, s) for x, s in zip(arg, self.shape)]
    ) == [(0, s) for s in self.shape]:
      return self
    return self._apply_uop(UOp.shrink, arg=tuple(shrink_arg))

  def pad(
    self,
    padding: Sequence[sint] | Sequence[tuple[sint, sint] | None],
    mode: str = "constant",
    value: float = 0.0,
  ) -> Tensor:
    """
    ...
    if all(isinstance(p, (int, UOp)) for p in padding):
      ...
      pX = _flat_to_grouped(
        tuple(cast(Sequence[sint], padding))
        + (0, 0) * (self.ndim - len(padding) // 2)
      )
    # group padding
    else:
      pX = tuple(
        (0, 0) if p is None else p
        for p in cast(Sequence[tuple[sint, sint] | None], padding)
      )
    if len(pX) != self.ndim:
      raise ValueError(f"padding length is improper, {padding=} {self.ndim=}")
    X, pads = self, tuple((smax(pB, 0), smax(pA, 0)) for pB, pA in pX)
    if mode == "constant":

      def _constant(x: Tensor, px, v) -> Tensor:
        return (
          x._apply_uop(UOp.pad, arg=px)
          if v == 0
          else (
            x._apply_uop(UOp.pad, arg=px)
            + Tensor.ones_like(x)._apply_uop(UOp.pad, arg=px).where(0, v)
          )
        )

      return (
        _constant(X, pX, value)
        if all(resolve(p >= 0) for p in flatten(pX))
        else _constant(
          X.shrink(
            tuple(
              (-smin(pB, 0), smin(pA + s, s)) for (pB, pA), s in zip(pX, X.shape)
            )
          ),
          pads,
          value,
    ...
    if mode == "circular":
      if any(pB > sh or pA > sh for (pB, pA), sh in zip(pX, X.shape)):
        raise ValueError("Padding value causes wrapping around more than once.")
      if any(pB < 0 or pA < 0 for pB, pA in pX):
        raise NotImplementedError(
          "Negative pads with circular pads is not supported"
        )
      orig_shape, X = (
        X.shape,
        X.repeat(tuple(1 + bool(pB) + bool(pA) for pB, pA in pads)),
      )
      return X.shrink(
        tuple(
          (0 if pB == 0 else osh - pB, xsh if pA == 0 else xsh - osh + pA)
          for (pB, pA), osh, xsh in zip(pads, orig_shape, X.shape)
        )
      )
    for d, (pB, pA) in enumerate(pads):
      if mode == "reflect":
        if pB >= (s := X.shape[d]) or pA >= s:
          raise ValueError(
            f"Padding ({pB}, {pA}) should be less than the input size={s} for dim={d}."
          )
        (
          slcB,
          slcA,
          ...
          slice(pB, 0, -1),
          slice(
            s - 2 if s - 2 >= 0 else None,
            s - 2 - pA if s - 2 - pA >= 0 else None,
            -1,
          ),
        )
        xB, xA = (
          X[[slc if i == d else slice(None) for i in range(X.ndim)]]
          if p > 0
          else None
          for slc, p in ((slcB, pB), (slcA, pA))
        )
      if mode == "replicate":
        (
          shrB,
          ...
          tuple((0, 1) if i == d else None for i in range(X.ndim)),
          tuple(
            (X.shape[i] - 1, X.shape[i]) if i == d else None for i in range(X.ndim)
          ),
        )
        xB, xA = (
          X.shrink(shr).expand(tuple(p if i == d else None for i in range(X.ndim)))
          if p > 0
          else None
          for shr, p in ((shrB, pB), (shrA, pA))
        )
      X = Tensor.cat(*(X_ for X_ in (xB, X, xA) if X_ is not None), dim=d)
    return X.shrink(
      tuple((-min(pB, 0), min(pA + s, s)) for (pB, pA), s in zip(pX, X.shape))
    )

  # convenience
  def pad_to(self, shape, *args):
    if len(new_shape := argfix(shape, *args)) != self.ndim:
      raise ValueError(f"dim mismatch, cannot pad {self.shape} to {new_shape}")
    return self.pad(
      tuple([
        None if ns is None else (0, ns - s) for s, ns in zip(self.shape, new_shape)
      ])
    )

  def shrink_to(self, shape, *args):
    return self.shrink(
      tuple([None if ns is None else (0, ns) for ns in argfix(shape, *args)])
    )
  ...
  def _getitem(self, indices, v: Tensor | None = None) -> Tensor:
    ...
    if (isinstance(indices, list) and all_int(indices)) or not isinstance(
      indices, (tuple, list)
    ):
      indices = [indices]
    ...
    if (
      len(ellipsis_idx := [dim for dim, i in enumerate(indices) if i is Ellipsis])
      > 1
    ):
      raise IndexError("indices can only have a single ellipsis")
    # NOTE: None adds a dim later
    num_indices = (
      len(indices) - len(ellipsis_idx) - sum(1 for i in indices if i is None)
    )
    ...
    for index in indices:
      ...
      match index:
        case Tensor():
          ...
          index = (
            (index < 0).where(index + size, index).to(self.device)
          )  # treat negative index values
        case list() | tuple():
          if not dtypes.is_int((ti := Tensor(index)).dtype):
            raise IndexError(f"{index=} contains non-int element")
          index = Tensor(
            [i + size if i < 0 else i for i in fully_flatten(index)],
            self.device,
            requires_grad=False,
          ).reshape(ti.shape)
        case int() | UOp():  # sint
          if index >= size or index < -size:
            raise IndexError(f"{index=} is out of bounds with {size=}")
          # TODO: is this right for (negative) symbolic?
          boundary = (
            [index, index + 1] if index >= 0 else [index + size, index + size + 1]
          )
        case slice():
          if index.step == 0:
            raise ValueError(f"{index=} cannot have 0 as step")
          start, stop = (
          ...
          if all(isinstance(s, int) for s in (start, stop, step)):
            ...
            *boundary, stride = index.indices(cast(SupportsIndex, size))
            ...
            resolve(step == 1, False)
            and all(isinstance(s, sint) for s in (start, stop))
            and resolve((stop - start) > 0, False)
          ):
    ...
    if mops := [i for i in indices_parsed if i["index"] is not None]:
      ...
      if any(st != 1 for st in strides):
        ...
        x = x.reshape(
          tuple(flatten((s // st, st) for s, st in zip(x.shape, strides)))
        )
        x = x.shrink(tuple(flatten(((0, s), (0, 1)) for s in x.shape[::2]))).reshape(
          x.shape[::2]
        )
    ...
    x = x.reshape(
      tuple(
        index["size"]
        for index in indices_parsed
        if not isinstance(index["index"], sint)
      )
    )
    ...
    if tops := [
      (d, i)
      for d, i in enumerate(
        i_ for i_ in indices_parsed if not isinstance(i_["index"], int)
      )
      if isinstance(i["index"], Tensor)
    ]:
      # unload the tensor object into actual tensors
      dims, tensors, masks = (
        [d for d, _ in tops],
        cast(list[Tensor], [i["index"] for _, i in tops]),
        [],
      )
      pre_reduce_shape = (
        x.shape[: dims[0]]
      ...
      for dim, tensor in zip(dims, tensors):
        try:
          i = tensor.reshape(tensor.shape + (1,) * (x.ndim - dims[0])).expand(
            pre_reduce_shape
          )
        except ValueError as e:
          raise IndexError(f"cannot broadcast indices: {e}") from e
        masks.append(
          i._one_hot_along_dim(num_classes=x.shape[dim], dim=(dim - x.ndim))
        )
      ...
      x = (mask.where(x.reshape(reshape_arg), 0)).sum(
        sum_axis := tuple(d + len(big_shape) for d in dims), dtype=x.dtype
      )
      ...
      if (
        permuted := dims[0] != 0
        and len(dims) != 1
        and tuple(dims) != tuple(range(dims[0], dims[-1] + 1))
      ):
        mask, x = (
          y.permute(
            *range(dims[0], dims[0] + len(big_shape)),
            *range(0, dims[0]),
            *range(dims[0] + len(big_shape), y.ndim),
          )
          for y in (mask, x)
        )
      ...
      if v is not None:
        ...
        x = _masked_setitem(
          ...
          mask,
          tuple(
            range((start := dims[0] if not permuted else 0), start + len(big_shape))
          ),
        )
  ...
  def __setitem__(self, indices, v: Tensor | ConstType) -> None:
    ...
    if res.shape == self.shape and res.uop is not self.uop:
      ...
      v = (
        v.cast(res.dtype)
        ._broadcast_to(_broadcast_shape(res.shape, v.shape))
        .contiguous()
      )
  ...
  def gather(self: Tensor, dim: int, index: Tensor) -> Tensor:
    ...
    ```
    """
    assert index.ndim == self.ndim, (
      f"self.ndim must equal index.ndim, {self.ndim=}, {index.ndim=}"
    )
    dim = self._resolve_dim(dim)
    assert all(
      s >= i for d, (s, i) in enumerate(zip(self.shape, index.shape)) if d != dim
    ), "requires self.shape[d] >= index.shape[d] for all d != dim"
    index = index.to(self.device)
    x = (
      self.shrink(
        tuple((0, i) if d != dim else None for d, i in enumerate(index.shape))
      )
      .unsqueeze(-1)
      .transpose(-1, dim)
    )
    return (index.unsqueeze(-1)._one_hot_along_dim(self.shape[dim]).where(x, 0)).sum(
      -1, dtype=self.dtype
    )
  ...
  def cat(self: Tensor, *args: Tensor, dim: int = 0) -> Tensor:
    ...
    for arg in args:
      assert arg.ndim == self.ndim and all(
        ti == ai for i, (ti, ai) in enumerate(zip(self.shape, arg.shape)) if i != dim
      )
    tensors = [self, *args]
    dim_cumsum = list(
      itertools.accumulate([t.shape[dim] for t in tensors], initial=0)
    )
    for i, t in enumerate(tensors):
      tensors[i] = t.pad([
        (dim_cumsum[i], dim_cumsum[-1] - dim_cumsum[i + 1]) if j == dim else None
  ...
  def repeat_interleave(self, repeats: int, dim: int | None = None) -> Tensor:
    ...
    shp = x.shape
    return (
      x.reshape(*shp[: dim + 1], 1, *shp[dim + 1 :])
      .expand(*shp[: dim + 1], repeats, *shp[dim + 1 :])
      .reshape(*shp[:dim], shp[dim] * repeats, *shp[dim + 1 :])
    )
  ...
  def split(self, sizes: int | Sequence[int], dim: int = 0) -> tuple[Tensor, ...]:
    ...
    if isinstance(sizes, int):
      sizes = [
        min(sizes, self.shape[dim] - i)
        for i in range(0, max(1, self.shape[dim]), max(1, sizes))
      ]
    assert sum(sizes) == self.shape[dim], (
      f"expect sizes to sum exactly to {self.shape[dim]}, but got {sum(sizes)}"
    )
    return tuple(
      self[sl]
      for sl in [
        tuple([slice(None)] * dim + [slice(sum(sizes[:i]), sum(sizes[: i + 1]))])
  ...
  def chunk(self, chunks: int, dim: int = 0) -> list[Tensor]:
    ...
    return list(
      self.split(
        ceildiv(self.shape[dim], chunks) if self.shape[dim] else [0] * chunks,
        dim=dim,
      )
    )
  ...
  def unfold(self, dim: int, size: sint, step: int) -> Tensor:
    ...
    """
    if size < 0:
      raise RuntimeError(f"size must be >= 0 but got {size=}")
    if step <= 0:
      raise RuntimeError(f"step must be > 0 but got {step=}")
    if size > self.shape[dim]:
      raise RuntimeError(
        f"maximum size for tensor at dimension {dim} is {self.shape[dim]} but size is {size}"
      )
    dim = self._resolve_dim(dim)
    perm_to_last = tuple(i for i in range(self.ndim) if i != dim) + (dim,)
    return (
      self.permute(perm_to_last)
      ._pool((size,), step)
      .permute(argsort(perm_to_last) + (self.ndim,))
    )

  def meshgrid(
    self: Tensor, *args: Tensor, indexing: Literal["ij", "xy"] = "ij"
  ) -> tuple[Tensor, ...]:
    """
    ...
    if indexing not in ("ij", "xy"):
      raise RuntimeError(f'indexing must be in ("ij", "xy"), got {indexing}')
    if len(tensors := (self, *args)) == 1:
      return tensors
    basis = (
      tuple(range(len(tensors)))
      if indexing == "ij"
      else (1, 0) + tuple(range(2, len(tensors)))
    )
    tensors = tuple(
      t.reshape((-1,) + (1,) * (len(args) - i)) for i, t in zip(basis, tensors)
    )
  ...
  def squeeze(self, dim: int | None = None) -> Tensor:
    ...
    return (
      ...
      else self.reshape(self.shape[:dim] + self.shape[dim + 1 :])
    )
  ...
  def diag(self) -> Tensor:
    ...
    """
    if self.ndim != 1:
      raise ValueError(f"expect input to be 1-D, getting {self.ndim}-D")
    return (
      self.unsqueeze(-1)
      .pad((None, (0, n := self.shape[0])))
      .flatten()
      .shrink(((0, n * n),))
      .reshape(n, n)
    )
  ...
  def diagonal(self) -> Tensor:
    ...
    ```
    """
    if self.ndim != 2 or (n := self.shape[0]) != self.shape[1]:
      raise ValueError(f"only 2-D square tensor is supported, getting {self.shape=}")
    return self.flatten().pad(((0, n))).reshape(n, n + 1)[:, 0]

  def roll(
    self, shifts: int | tuple[int, ...], dims: int | tuple[int, ...] | None = None
  ) -> Tensor:
    """
    ...
    dims, shifts, slices = (
      tuple(self._resolve_dim(d) for d in make_tuple(dims, 1)),
      make_tuple(shifts, 1),
      [slice(None)] * self.ndim,
    )
    if len(dims) != len(shifts):
      raise RuntimeError(f"{len(dims)=} != {len(shifts)=}")
    for dim, shift in zip(dims, shifts):
      slices[dim] = slice(
        delta := self.shape[dim] - shift % self.shape[dim], delta + self.shape[dim]
      )
    return self.repeat(*tuple(2 if i in dims else 1 for i in range(self.ndim)))[
      slices
    ]
  ...
  def rearrange(self, formula: str, **sizes) -> Tensor:
    ...
    def parse_formula(formula: str):
      ...
      lparens, rparens = map(
        lambda x: [i for i, ch in enumerate(tokens) if ch == x], ("(", ")")
      )
      pairs = list(zip(lparens, rparens))
      assert len(lparens) == len(rparens) and sorted(flatten(pairs)) == flatten(
        pairs
      ), "bracket mismatch"
      return [name for name in tokens if name not in ("(", ")")], [
        (s - 2 * i, e - 1 - 2 * i) for i, (s, e) in enumerate(pairs)
      ]

    assert formula.count("->") == 1, 'need exactly one "->" in formula'

    (lhs, unflatten_dims), (rhs, flatten_dims) = map(
      parse_formula, formula.split("->")
    )

    for name in sizes:
      assert name in lhs, f"axis {name} is not used in transform"
    assert sorted(lhs) == sorted(rhs) and len(lhs) == len(set(lhs)), (
      f"name mismatch in {formula}"
    )
    for name in flatten((lhs, rhs)):
      assert name == "..." or (
        name.isidentifier() and "_" not in (name[0], name[-1])
      ), f"invalid axis name {name}"
    assert "..." not in flatten([lhs[s:e] for s, e in unflatten_dims]), (
      f"cannot have collapsed ellipsis (...) in lhs of {formula}"
    )
    ...
    if "..." in lhs:
      ell_len = (
        len(self.shape) - len(lhs) + 1 + sum(e - s - 1 for s, e in unflatten_dims)
      )
    lhs, rhs = map(
      lambda l: l[: (i := l.index("..."))]
      + [f"...{j}" for j in range(ell_len)]
      + l[i + 1 :]
      if "..." in l
      else l,
      (lhs, rhs),
    ...
    unflatten_dims = [
      (
        s + (ell_len - 1 if "...0" in lhs[:s] else 0),
        e + (ell_len - 1 if "...0" in lhs[:e] else 0),
      )
      for s, e in unflatten_dims
    ]
    flatten_dims = [
      (
        s + (ell_len - 1 if "...0" in rhs[:s] else 0),
        e + (ell_len - 1 if "...0" in rhs[:e] else 0),
      )
      for s, e in flatten_dims
    ]

    # apply movement ops in order unflatten -> permute -> flatten/unsqueeze
    t = functools.reduce(
      lambda x, dims: x.unflatten(
        dims[0], tuple(sizes.get(lhs[d], -1) for d in range(*dims))
      ),
      unflatten_dims,
      self,
    )
    for i, name in enumerate(lhs):
      assert (name not in sizes) or sizes[name] == t.shape[i], (
        f"size provided for dimension {name} incorrect"
      )
    t = t.permute([lhs.index(name) for name in rhs])
    return functools.reduce(
      lambda x, dims: x.flatten(dims[0], dims[1] - 1)
      if dims[0] < dims[1]
      else x.unsqueeze(dims[0]),
      reversed(flatten_dims),
      t,
    )
  ...
  def masked_select(self, mask):
    ...
    ```
    """
    if not dtypes.is_bool(mask.dtype):
      raise RuntimeError(f"masked_select expects bool mask tensor, got {mask.dtype}")
  ...
  def _reduce(
    self, op: Ops, axis: int | Sequence[int] | None = None, keepdim=False
  ) -> Tensor:
    axis = tuple(
      self._resolve_dim(x)
      for x in (range(self.ndim) if axis is None else make_tuple(axis, 1))
    )
    if self.ndim == 0:
      axis = ()
    ...
    return (
      ret
      if keepdim
      else ret.reshape(tuple(s for i, s in enumerate(self.shape) if i not in axis))
    )
  ...
  def sum(
    ...
    ```
    """
    ret = self.cast(sum_acc_dtype(self.dtype) if dtype is None else dtype)._reduce(
      Ops.ADD, axis, keepdim
    )
    return (
      ret.cast(self.dtype)
      if dtype is None
      and self.dtype in (dtypes.float16, dtypes.bfloat16, *dtypes.fp8s)
      else ret
    )
  ...
  def prod(
    ...
    ```
    """
    return self.cast(dtype if dtype is not None else self.dtype)._reduce(
      Ops.MUL, axis, keepdim
    )
  ...
  def _inverse(self) -> Tensor:
    return (
      ...
      if dtypes.is_int(self.dtype)
      else self.logical_not()
    )
  ...
  def isclose(
    self, other: Tensor, rtol: float = 1e-05, atol: float = 1e-08, equal_nan=False
  ) -> Tensor:
    """
  ...
  def allclose(
    ...
    """
    return bool(
      self.isclose(other, rtol=rtol, atol=atol, equal_nan=equal_nan).all().item()
    )
  ...
  def var(
    self, axis: int | Sequence[int] | None = None, keepdim=False, correction=1
  ) -> Tensor:
    """
    ...
    n = prod([
      si
      for si, so in zip(self.shape, squares.sum(axis=axis, keepdim=True).shape)
      if resolve(si != so)
    ])
  ...
  def keccak(self, cfg: str | tuple[int, int] = "sha3_256"):
    ...
    def ctensor(l: Sequence[ConstType], dtype: DType = dtypes.uint64):
      ...
      return Tensor.stack(
        *(Tensor(v, dtype=dtype, device=self.device) for v in l)
      ).contiguous()
    ...
    rot_offsets_v0, rot_offsets_v1 = (
      ctensor([0] + [1 << v for v in rot_offsets]),
      ctensor([1] + [1 << (64 - v) for v in rot_offsets]),
    )

    # calculated from π step
    reorder_indexes = ctensor(
      [
        0,
        6,
        12,
    ...
    rate, dsbyte = (
      {"sha3_224": (144, 6), "sha3_256": (136, 6), "shake_128": (168, 31)}[cfg]
      if isinstance(cfg, str)
      else cfg
    )
    data, data_pad = (
      self.bitcast(dtypes.uint8).reshape(prod(self.shape[:-1]), self.shape[-1]),
      rate - (self.shape[-1] * self.dtype.itemsize % rate),
    )
    # pad batches then pad blocks
    data = (
      data.pad((None, (0, data_pad)))
      .reshape(bs := data.shape[0], -1, rate)
      .pad((None, None, (0, 200 - rate)))
    )

    # create pad mask
    lbe = prod(data.shape[1:]) + rate - data_pad - 200
    if data_pad == 1:
      mb = [(lbe, 0), (1, dsbyte ^ 0x80), (200 - rate, 0)]
    else:
      mb = [(lbe, 0), (1, dsbyte), (data_pad - 2, 0), (1, 0x80), (200 - rate, 0)]
    pad_mask = Tensor.cat(
      *(
        Tensor(v, dtype=dtypes.uint8, device=data.device).expand(l)
        for l, v in mb
        if l > 0
      )
    ).unsqueeze(0)
    ...
    data = (
      (data.flatten(1) ^ pad_mask)
      .reshape(*data.shape[:2], 200)
      .bitcast(dtypes.uint64)
    )
    ...
    for k in range(int(data.shape[1])):
      ...
      for i in range(24):  # f1600
        ...
        t1 = (p[:, :, 0] ^ p[:, :, 1] ^ p[:, :, 2] ^ p[:, :, 3] ^ p[:, :, 4]).roll(
          -1, 1
        )  # xor reduce
        state = state ^ (
          t1.roll(2, 1)
          .bitwise_xor((t1 << 1) ^ (t1 >> 63))
          .unsqueeze(2)
          .expand(bs, 5, 5)
          .transpose(2, 1)
          .flatten(1)
        ...
        state = (
          (state * rot_offsets_v0)
          .bitwise_or(state // rot_offsets_v1)
          .reshape(bs, 5, 5)
        )
        # χ and ι step
        state = state.bitwise_xor(
          ~state.roll(shifts=-1, dims=2) & state.roll(shifts=-2, dims=2)
        )
    ...
    return state.bitcast(dtypes.uint8)[:, : (obytes := (200 - rate) // 2)].reshape(
      *self.shape[:-1], obytes
    )
  ...
  def hash(self) -> Tensor:
    ...
    if (tsize := data.shape[0]) % 2**20 != 0:
      data = data.pad((0, 2**20 - tsize % 2**20))
  ...
  def _softmax(
    self, axis, dtype: DTypeLike | None = None
  ...
  def logcumsumexp(self, axis=0) -> Tensor:
    ...
    x_unsqueezed = x.unsqueeze(-2).expand(
      (None,) * (self.ndim - 1) + (last_dim_size, None)
    )
    x_cummax = x.cummax(-1)
    mask = Tensor.ones(
      last_dim_size, last_dim_size, requires_grad=False, device=self.device
    ).tril()
    ret = (
      mask.where(x_unsqueezed - x_cummax.unsqueeze(-1), dtypes.min(self.dtype))
      .exp()
      .sum(-1)
      .log()
      + x_cummax
  ...
  def argmax(self, axis=None, keepdim=False) -> Tensor:
    ...
    idx = m * Tensor.arange(
      self.shape[axis], 0, -1, requires_grad=False, device=self.device
    ).reshape(self.shape[axis], *[1] * (self.ndim - axis - 1))
    return (self.shape[axis] - idx.max(axis=axis, keepdim=keepdim)).cast(
      dtypes.int32
    )
    ...
    *operands: Tensor | Sequence[Tensor],
    ...
    def parse_formula(formula: str, *operands: Tensor):
      if "..." in (formula := formula.replace(" ", "")):
        ell_chars, ell_longest = (
          "".join(c for c in string.ascii_letters if c not in formula),
          0,
        )
        for i, inp in enumerate(
          filter(lambda x: "..." in x, inputs := formula.split("->")[0].split(","))
        ):
          if (
            ell_count := max(operands[i].ndim, 1) - (len(inp) - len("..."))
          ) > ell_longest:
            ell_longest = ell_count
          inputs[i] = inp.replace("...", ell_chars[-ell_count:])
        inputs_str, out_ellipse = ",".join(inputs), ell_chars[-ell_longest:]
        return (
          (inputs_str, formula.split("->")[1].replace("...", out_ellipse))
          if "->" in formula
          else (
            inputs_str,
            out_ellipse
            + "".join(
              sorted(
                c
                for c in inputs_str
                if inputs_str.count(c) == 1 and c.isalpha() and c not in out_ellipse
              )
            ),
          )
        )
      return (
        ...
        if "->" in formula
          ...
          formula,
          "".join(
            c for c in sorted(formula) if formula.count(c) == 1 and c.isalpha()
          ),
        )
      )
    ...
    if len(xs) != len(inputs):
      raise ValueError(
        f"number of inputs doesn't match number of operands in formula, expected {len(inputs)}, got {len(xs)}"
      )

    # map the value of each letter in the formula
    letter_val = sorted(
      merge_dicts([
        dict(zip(letters, tensor.shape)) for letters, tensor in zip(inputs, xs)
      ]).items()
    )
    ...
    for x, (order, letters) in zip(xs, [list(zip(*l)) for l in lhs]):
      ...
      xs_.append(
        x.permute(order)
        .reshape([val if letter in letters else 1 for letter, val in letter_val])
        .expand([val for _, val in letter_val])
      )
    ...
    return (
      functools.reduce(lambda a, b: a * b, xs_)
      .sum(
        axis=[
          axis for axis, (letter, _) in enumerate(letter_val) if letter not in output
        ],
        dtype=dtype,
      )
      .permute(rhs_order)
  ...
  def _pool(
    ...
    assert all(resolve(d * (k - 1) + 1 <= i) for k, d, i in zip(k_, d_, i_)), (
      "kernel size cannot be greater than actual input size"
    )
    o_ = [ceildiv(i - d * (k - 1), s) for i, d, k, s in zip(i_, d_, k_, s_)]
    if any(resolve(k > s) for k, s in zip(k_, s_)) or any(d != 1 for d in d_):
      # input size scaling factor to make sure shrink for stride is possible
      f_ = [
        1 + int(resolve(o * s > (i - d * (k - 1))))
        for o, s, i, d, k in zip(o_, s_, i_, d_, k_)
      ]
      # # repeats such that we don't need padding
      x = self.repeat(
        [1] * len(noop)
        + [ceildiv(k * (i * f + d), i) for k, i, d, f in zip(k_, i_, d_, f_)]
      )
      # handle dilation
      x = x.shrink(
        tuple(noop + [(0, k * (i * f + d)) for k, i, d, f in zip(k_, i_, d_, f_)])
      ).reshape(
        noop + flatten((k, (i * f + d)) for k, i, d, f in zip(k_, i_, d_, f_))
      )
      # handle stride
      x = x.shrink(
        tuple(noop + flatten(((0, k), (0, o * s)) for k, o, s in zip(k_, o_, s_)))
      ).reshape(noop + flatten((k, o, s) for k, o, s in zip(k_, o_, s_)))
      x = x.shrink(
        tuple(noop + flatten(((0, k), (0, o), (0, 1)) for k, o in zip(k_, o_)))
    ...
    x = self.pad(
      tuple(noop + [(0, max(0, o * s - i)) for i, o, s in zip(i_, o_, s_)])
    ).shrink(tuple(noop + [(0, o * s) for o, s in zip(o_, s_)]))
  ...
  def _resolve_pool_pads(
    self, padding: int | Sequence[int], dims: int
  ) -> Sequence[int]:
    if not isinstance(padding, int) and not (
      len(padding) == 2 * dims or len(padding) == dims
    ):
      raise ValueError(
        f"Padding must be an int or a sequence of length {dims} or {2 * dims}, but got {padding=} for {self.shape=} with {dims=}."
      )
    return (
      [padding] * 2 * dims
  ...
  def _apply_ceil_mode(
    ...
    o_ = [
      ceildiv(i + pB + pA - (d * (k - 1) + 1), s) + 1
      for i, d, k, s, (pB, pA) in zip(i_, d_, k_, s_, grouped_pads)
    ]
    for dim, (o, i, s, k, d, (pB, pA)) in enumerate(
      zip(o_, i_, s_, k_, d_, grouped_pads)
    ):
  ...
  def avg_pool2d(
    ...
    def pool(x: Tensor, padding_: Sequence[int]) -> Tensor:
      return x.pad(padding_)._pool(
        k_, stride if stride is not None else k_, dilation
      )

    reg_pads = self._resolve_pool_pads(padding, len(k_))
    ceil_pads = self._apply_ceil_mode(
      reg_pads, k_, stride if stride is not None else k_, dilation
    )
    ...
    return pool(self, ceil_pads).sum(axis) / pool(
      self.pad(reg_pads).ones_like(),
      tuple(cp - rp for cp, rp in zip(ceil_pads, reg_pads)),
    ).sum(axis)
  ...
  def max_pool2d(
    ...
    if ceil_mode:
      pads = self._apply_ceil_mode(
        pads, k_, stride if stride is not None else k_, dilation
      )
    pooled = self.pad(pads, value=dtypes.min(self.dtype))._pool(
      k_, stride if stride is not None else k_, dilation
    )
    ...
    idx = Tensor.arange(
      spatial_sz, 0, -1, requires_grad=False, device=self.device
    ).reshape(spatial_shape)
    m = pooled == pooled.max(axis, keepdim=True)
    idx = m * idx.pad(pads, value=dtypes.min(idx.dtype))._pool(
      k_, stride if stride is not None else k_, dilation
    )
  ...
  def max_unpool2d(
    ...
    if output_size is None:
      k_, d_, s_ = (
        make_tuple(x, len(spatial_shape))
        for x in (
          kernel_size,
          dilation,
      ...
      output_size = tuple(
        (i - 1) * s - (pB + pA) + (d * (k - 1) + 1)
        for i, k, d, s, (pA, pB) in zip(spatial_shape, k_, d_, s_, p_)
      )
    else:
      output_size = output_size[-len(spatial_shape) :]
    ret = (
      indices.reshape(bs, c, 1, -1)
      ._one_hot_along_dim(prod(output_size), 2)
      .where(self.reshape(bs, c, 1, -1), 0)
    ).sum(3)
  ...
  def conv2d(
    ...
    """
    if IMAGE:
      return self.image_conv2d(
        weight, bias, groups, stride, dilation, padding, dtype
      )
    ...
    if not all(x == 3 for x in HW) or stride != 1 or dilation != 1 or not WINO:
      ...
      ret = (
        ...
        .sum([-1 - i for i in range(1 + len(oyx))], keepdim=True, dtype=dtype)
        .reshape(bs, cout, *oyx)
      )
    ...
    winograd_At = [
      ...
      [0, 1, 1, 4, 4, 0],
      [0, 1, -1, 8, -8, 1],
    ]  # applying At in pre-order doubles compile time
    ...
    gfactors = _apply_winograd_matrix(winograd_G, g, len(HW)).reshape(
      *HWI, 1, groups, rcout, cin, *([1] * len(tyx))
    )
    # (HWI, bs, cin_, tyx) -> (HWI, bs, groups, 1 ,cin, *tyx)
    dfactors = _apply_winograd_matrix(winograd_Bt, d, len(HW)).reshape(
      *HWI, bs, groups, 1, cin, *tyx
    )

    # matmul; sum across cin: (HWI, bs, groups, rcout, *tyx); then HWI -> HWO: (HWO, bs, groups, rcout, *tyx)
    ret = _apply_winograd_matrix(
      winograd_At, (gfactors * dfactors).sum(axis=-1 - len(HW), dtype=dtype), len(HW)
    )
  ...
  def conv_transpose2d(
    ...
    x, w = (
      self,
      weight.unflatten(0, (groups, -1))
      .transpose(1, 2)
      .flip(*range(3, len(weight.shape) + 1)),
    )
    HW = weight.shape[2:]
    padding = _flat_to_grouped(self._resolve_pool_pads(padding, len(HW)))
    stride, dilation, output_padding = [
      make_tuple(x, len(HW)) for x in (stride, dilation, output_padding)
    ]
    ...
    padding = flatten(
      (
        ((k - 1) * d - pB, (k - 1) * d - pA + op)
        for k, d, (pB, pA), op in reversed(
          list(zip(HW, dilation, padding, output_padding))
        )
      )
    )
    return x.conv2d(
  ...
  def dot(self, w: Tensor, dtype: DTypeLike | None = None) -> Tensor:
    ...
    w = w.reshape(
      *w.shape[0:-2], *[1] * min(dx - 1, dw - 1, 1), *w.shape[axis_w:]
    ).transpose(-1, axis_w)
    return (
      (x * w)
      .sum(-1, dtype=dtype)
  ...
  def matmul(
    self, x: Tensor, reverse=False, dtype: DTypeLike | None = None
  ) -> Tensor:
    """
  ...
  def _cumalu(self, axis: int, op: Ops, _include_initial=False) -> Tensor:
    ...
    return {
      Ops.ADD: pooled.sum(-1),
      Ops.MAX: pooled.max(-1),
      Ops.MUL: pooled.prod(-1),
    }[op].transpose(axis, -1)
  ...
  def _split_cumalu(self, axis: int, op: Ops) -> Tensor:
    ...
    ret = (
      self.transpose(axis, -1)
      .pad((round_up(s, SPLIT) - s, 0), value=identity_element(op, self.dtype))
      .unflatten(-1, (-1, SPLIT))
      ._cumalu(-1, op)
    )
    ...
    def fix(x: Tensor) -> Tensor:
      return x.flatten(start_dim=-2)[..., -s:].transpose(axis, -1)

    reduce_fxns: dict[Ops, Callable[[Tensor, Tensor], Tensor]] = {
      Ops.ADD: Tensor.__add__,
      Ops.MAX: Tensor.maximum,
      Ops.MUL: Tensor.__mul__,
    }
  ...
  def _tri(r: sint, c: sint, diagonal: int = 0, **kwargs) -> Tensor:
    ...
    if r == 0 or c == 0 or diagonal >= c:
      return Tensor.zeros(r, c, **kwargs)
    if r + diagonal <= 0:
      return Tensor.ones(r, c, **kwargs)
    s = r + c - 1
    # build a (s, s) upper triangle
    t = (
      Tensor.ones(s, s, **kwargs)
      .pad((None, (0, s)))
      .flatten()
      .shrink(((0, s * (2 * s - 1)),))
      .reshape(s, -1)
    ...
    return (
      t[:r, -diagonal : c - diagonal]
      if diagonal <= 0
      else t[diagonal : r + diagonal, :c]
    )
  ...
  def tril(self, diagonal: int = 0) -> Tensor:
    ...
    return Tensor._tri(
      ...
      device=self.device,
      dtype=dtypes.bool,
    ).where(self.zeros_like(), self)

  def interpolate(
    self, size: tuple[int, ...], mode: str = "linear", align_corners: bool = False
  ) -> Tensor:
    """
    ...
    assert (
      ...
      and all_int(size)
      and 0 < len(size) <= self.ndim
    ), f"invalid {size=}"
    assert mode in ("linear", "nearest", "nearest-exact"), (
      "only supports linear, nearest or nearest-exact interpolate"
    )
    assert not (align_corners and mode != "linear"), (
      "align_corners option can only be set with the interpolating mode linear"
    )
    x, expand = self, list(self.shape)
    for i in range(-1, -len(size) - 1, -1):
      scale = (self.shape[i] - int(align_corners)) / (size[i] - int(align_corners))
      arr, reshape = (
        Tensor.arange(size[i], dtype=dtypes.float32, device=self.device),
        [1] * self.ndim,
      )
      reshape[i] = expand[i] = size[i]
      if mode == "linear":
        index = (scale * arr if align_corners else (scale * (arr + 0.5)) - 0.5).clip(
          0, self.shape[i] - 1
        )
        low, high, perc = [
          y.reshape(reshape).expand(expand)
          for y in (index.floor().int(), index.ceil().int(), index - index.floor())
        ]
        x = x.gather(i, low).lerp(x.gather(i, high), perc)
      else:
        index = (
          (scale * (arr + 0.5) if mode == "nearest-exact" else scale * arr)
          .cast(dtypes.int32)
          .reshape(reshape)
          .expand(expand)
        )
  ...
  def _pre_scatter(
    self, dim: int, index: Tensor, src: Tensor
  ) -> tuple[Tensor, Tensor]:
    index, dim = index.to(self.device), self._resolve_dim(dim)
    assert index.ndim == self.ndim == src.ndim, (
      f"self.ndim, index.ndim and src.ndim must all equal, {self.ndim=} {index.ndim=} {src.ndim=}"
    )
    assert all(
      (d == dim or self_ >= index_) and src_ >= index_
      for d, (self_, index_, src_) in enumerate(
        zip(self.shape, index.shape, src.shape)
      )
    ), (
      f"All dimensions of {index.shape=} should be <= to all dimensions of {src.shape=} and all dimensions except dimension {dim} of {self.shape=}"
    )
    if self.dtype != src.dtype:
      raise RuntimeError(f"expect {self.dtype=} to be equal to {src.dtype=}")
    ...
    src, mask = (
      x.pad(
        tuple(
          (0, self.shape[i] - x.shape[i]) if i != dim else None
  ...
  def scatter(
    ...
    if reduce not in {None, "add", "multiply"}:
      raise TypeError(f"{reduce=} must be one of None, 'multiply', or 'add'")
    if reduce and isinstance(src, Tensor):
      raise TypeError(
        "Tensor src is not supported with reduce arg. see scatter_reduce"
      )
    if not isinstance(src, Tensor):
      src = index.full_like(src, device=self.device, dtype=self.dtype)
    if reduce == "add":
      return self.scatter_reduce(dim, index, src, "sum", include_self=True)
    if reduce == "multiply":
      return self.scatter_reduce(dim, index, src, "prod", include_self=True)
  ...
  def scatter_reduce(
    ...
    def _inv_mask(a: Tensor | ConstType, b: Tensor | ConstType) -> Tensor:
      return mask.any(-1).logical_not().where(a, b)

    if reduce == "sum":
      return (
        mask.where(src, 0).sum(-1).add(self if include_self else _inv_mask(self, 0))
      )
    if reduce == "prod":
      return (
        mask.where(src, 1).prod(-1).mul(self if include_self else _inv_mask(self, 1))
      )
    if reduce == "amax":
      return (
        mask.where(src, m := dtypes.min(src.dtype))
        .max(-1)
        .maximum(self if include_self else _inv_mask(self, m))
      )
    if reduce == "amin":
      return (
        mask.where(src, m := dtypes.max(src.dtype))
        .min(-1)
        .minimum(self if include_self else _inv_mask(self, m))
      )
    if reduce == "mean":
      ...
      return (
        mask.where(src, 0)
        .sum(-1)
        .add(self if include_self else _inv_mask(self, 0))
        .div(count)
      )
    raise RuntimeError(
      f"{reduce=} must be one of 'sum', 'prod', 'mean', 'amax', 'amin'"
    )
  ...
  def sort(self, dim: int = -1, descending: bool = False) -> tuple[Tensor, Tensor]:
    ...
    if (orig_len := x.shape[dim]) <= 1:
      return x, x.zeros_like(dtype=dtypes.default_int)
    # pad to power of 2
    n_stages = (orig_len - 1).bit_length()
    pads = tuple(
      (0, 2**n_stages - orig_len) if i == dim else None for i in range(x.ndim)
    )
    x = x.pad(
      pads, value=dtypes.min(x.dtype) if descending else dtypes.max(x.dtype)
    ).unflatten(dim, (2,) * n_stages)
    ...
    for stage in range(1, n_stages + 1):
      ...
      for substage in range(stage - 1, -1, -1):
        ...
        x = (
          x_larger.cat(x_smaller, dim=partner_dim)
          if descending
          else x_smaller.cat(x_larger, dim=partner_dim)
        ).contiguous()
      if stage != n_stages:
    ...
    mask = (
      Tensor.ones(orig_len, orig_len, dtype=dtypes.bool, device=self.device)
      .tril()
      .reshape((None, None) + (1,) * (self.ndim - dim - 1))
    )

    def compute_counts(t: Tensor):
      return (mask & (t.unsqueeze(dim) == t.unsqueeze(dim + 1))).sum(dim + 1)

    count_orig, count_sorted = compute_counts(self), compute_counts(x)
    cond = (self.unsqueeze(dim + 1) == x.unsqueeze(dim)) & (
      count_orig.unsqueeze(dim + 1) == count_sorted.unsqueeze(dim)
    )
    idx = Tensor.arange(orig_len, device=self.device).reshape(
  ...
  def topk(
    self, k: int, dim: int = -1, largest: bool = True, sorted_: bool = True
  ) -> tuple[Tensor, Tensor]:
    """
    ...
    if k > self.shape[dim := self._resolve_dim(dim)]:
      raise ValueError(f"selected index {k=} is out of range")
  ...
  def exp(self) -> Tensor:
    ...
    if self.is_floating_point():
      return (
        self.cast(least_upper_dtype(self.dtype, dtypes.float32))
        .mul(1 / math.log(2))
        .exp2()
        .cast(self.dtype)
      )
  ...
  def cos(self) -> Tensor:
    ...
    if self.is_floating_point():
      return (
        ((math.pi / 2) - self.cast(least_upper_dtype(self.dtype, dtypes.float32)))
        .sin()
        .cast(self.dtype)
      )
  ...
  def round(self: Tensor) -> Tensor:
    ...
    ```
    """
    return ((self > 0) == ((b := self.trunc() / 2.0).trunc() == b)).where(
      (self - 0.5).ceil(), (self + 0.5).floor()
    )

  def isinf(
    self: Tensor, detect_positive: bool = True, detect_negative: bool = True
  ) -> Tensor:
    """
    ...
    ```
    """
    return (self == float("inf")) * detect_positive + (
      self == float("-inf")
    ) * detect_negative
  ...
  def lerp(self, end: Tensor, weight: Tensor | float) -> Tensor:
    ...
    if self.dtype == dtypes.uint8 and isinstance(weight, Tensor):
      ...
      return (
        self
        + (
          ((end - self).cast(dtypes.int8) * w_i + (1 << W_PREC - 1)).cast(
            dtypes.uint16
          )
          >> W_PREC
        )
  ...
  def sign(self) -> Tensor:
    ...
    return (
      self.ne(0).where(
        (self < 0).where(self.full_like(-1), self.full_like(1)), self.full_like(0)
      )
      + self * 0
    )
  ...
  def erf(self) -> Tensor:
    ...
    return self.sign() * (
      1.0
      - t
      * polyN(t, [1.061405429, -1.453152027, 1.421413741, -0.284496736, 0.254829592])
      * (-self.square()).exp()
    )
  ...
  def gelu(self) -> Tensor:
    ...
    return (
      0.5
      * self
      * (1 + (math.sqrt(2 / math.pi) * (self + 0.044715 * self**3)).tanh())
    )
  ...
  def _broadcast_to(self, new_shape: tuple[sint, ...]) -> Tensor:
    ...
    if self.ndim > len(new_shape):
      raise ValueError(
        f"cannot broadcast tensor to fewer dimensions. shape={self.shape} to {new_shape=}"
      )
    # first unsqueeze left with 1s https://data-apis.org/array-api/latest/API_specification/broadcasting.html
    shape, _ = _align_left(self.shape, new_shape)
    # for each dimension, check either dim is 1, or it does not change
    if not all(
      resolve(s == ns) or resolve(s == 1) for s, ns in zip(shape, new_shape)
    ):
      raise ValueError(f"cannot broadcast {self.shape} to {new_shape=}")
    # NOTE: this cast is no-op in forward and uses sum_acc_dtype in the backward sum
    return (
      self.reshape(shape)
      .cast(sum_acc_dtype(self.dtype))
      ._apply_uop(UOp.expand, arg=new_shape)
      .cast(self.dtype)
    )

  def _broadcasted(
    self,
    y: Tensor | ConstType | UOp,
    reverse: bool = False,
    match_dtype: bool = True,
  ) -> tuple[Tensor, Tensor]:
    x: Tensor = self
    if not isinstance(y, Tensor):
      ...
      if (
        isinstance(x.dtype, ImageDType)
        or dtypes.is_float(x.dtype)
        or (dtypes.is_int(x.dtype) and isinstance(y, int))
      ):
        y_dtype = x.dtype
      elif not isinstance(y, UOp):
        y_dtype = dtypes.from_py(y)
      if isinstance(y, UOp):
        y = Tensor.from_uop(y, device=x.device)
      else:
        y = Tensor(
          dtypes.as_const(y, y_dtype), x.device, y_dtype, requires_grad=False
        )
    ...
    # broadcast
    return x._broadcast_to(
      out_shape := _broadcast_shape(x.shape, y.shape)
    ), y._broadcast_to(out_shape)
  ...
  def div(
    ...
    d = (
      numerator.cast(least_upper_float(numerator.dtype))
      * denominator.cast(least_upper_float(denominator.dtype)).reciprocal()
    )
    output_dtype = numerator.dtype if dtypes.is_int(numerator.dtype) else d.dtype
    if (
      dtypes.is_int(dt := least_upper_dtype(numerator.dtype, denominator.dtype))
      and rounding_mode is not None
    ):
      numerator, denominator = numerator.cast(dt), denominator.cast(dt)
      if rounding_mode == "trunc":
        return numerator.idiv(denominator)
      if rounding_mode == "floor":
        truncate_div, truncate_mod = (
          numerator.idiv(denominator),
          numerator._apply_broadcasted_uop(UOp.mod, denominator),
        )
        opposite_sign = ((numerator > 0) & (denominator < 0)) | (
          (numerator < 0) & (denominator > 0)
        )
        return (opposite_sign & (truncate_mod != 0)).where(
          truncate_div - 1, truncate_div
        )
    if rounding_mode == "trunc":
      return d.trunc().cast(output_dtype)
    if rounding_mode == "floor":
      return d.floor().cast(output_dtype)
  ...
  def bitwise_not(self) -> Tensor:
    ...
    ```
    """
    if self.dtype != dtypes.bool and not dtypes.is_int(self.dtype):
  ...
  def lshift(self, x: Tensor | int, reverse=False) -> Tensor:
    ...
    """
    assert (
      dtypes.is_unsigned(self.dtype)
      and isinstance(x, int)
      and x >= 0
      and not reverse
    ), f"not supported {self.dtype=} {x=}"
  ...
  def pow(self, x: Tensor | ConstType, reverse=False) -> Tensor:
    ...
    # TODO: int pow
    if not base.is_floating_point() and not (isinstance(x, int) and x >= 0):
      raise RuntimeError("base needs to be float")

    ret = base._apply_uop(UOp.pow, exponent)
    # NOTE: pow(int, float) -> int
    return (
      ret.round().cast(self.dtype)
      if not reverse
      and not dtypes.is_float(self.dtype)
      and dtypes.is_float(exponent.dtype)
      else ret
    )
  ...
  def __rfloordiv__(self, x):
    return self.div(x, rounding_mode="floor", reverse=True)
  ...
  def __ifloordiv__(self, x) -> Tensor:
    return self.assign(self.__floordiv__(x))
  ...
  def __iadd__(self, x) -> Tensor:
    return self.assign(self.add(x))  # type: ignore[misc]

  def __isub__(self, x) -> Tensor:
    return self.assign(self.sub(x))  # type: ignore[misc]

  def __imul__(self, x) -> Tensor:
    return self.assign(self.mul(x))  # type: ignore[misc]

  def __itruediv__(self, x) -> Tensor:
    return self.assign(self.div(x))  # type: ignore[misc]

  def __iand__(self, x) -> Tensor:
    return self.assign(self.bitwise_and(x))  # type: ignore[misc]

  def __ior__(self, x) -> Tensor:
    return self.assign(self.bitwise_or(x))  # type: ignore[misc]

  def __ixor__(self, x) -> Tensor:
    return self.assign(self.bitwise_xor(x))  # type: ignore[misc]

  def __ilshift__(self, x) -> Tensor:
    return self.assign(self.lshift(x))  # type: ignore[misc]

  def __irshift__(self, x) -> Tensor:
    return self.assign(self.rshift(x))  # type: ignore[misc]

  def __lt__(self, x) -> Tensor:
    return self._apply_broadcasted_uop(UOp.__lt__, x, False)

  def __gt__(self, x) -> Tensor:
    return self._apply_broadcasted_uop(UOp.__lt__, x, True)
  ...
  def linear(
    self, weight: Tensor, bias: Tensor | None = None, dtype: DTypeLike | None = None
  ) -> Tensor:
    """
    ...
    """
    if dtype is not None:
      return self.cast(dtype).linear(
        weight.cast(dtype), bias.cast(dtype) if bias is not None else bias
      )
  ...
  def dropout(self, p=0.5) -> Tensor:
    ...
    return (
      Tensor.rand_like(
        self, requires_grad=False, dtype=dtypes.default_float, contiguous=False
      )
      >= p
    ).contiguous().where(self, 0) / (1.0 - p)

  # helper function commonly used for indexing
  def _one_hot_along_dim(self: Tensor, num_classes: sint, dim: int = -1) -> Tensor:
    if not dtypes.is_int(self.dtype):
      raise RuntimeError(
    ...
    dt = (
      dtypes.int64
      if sint_to_uop(num_classes).overflows(dtypes.int32)
      else dtypes.int32
    )
    return self == Tensor.arange(
      num_classes, dtype=dt, device=self.device, requires_grad=False
    ).reshape((num_classes,) + (1,) * offset)
  ...
  def one_hot(self, num_classes: int = -1) -> Tensor:
    ...
    """
    if not dtypes.is_int(self.dtype):
      raise RuntimeError(f"expect integer dtype, getting {self.dtype=}")
    if num_classes == -1:
      num_classes = (self.max() + 1).item()
  ...
  def scaled_dot_product_attention(
    ...
    qk = q.matmul(
      key.transpose(-2, -1),
      dtype=least_upper_dtype(q.dtype, key.dtype, dtypes.float32),
    ) / math.sqrt(q.shape[-1])
    # handle attention mask
    if is_causal:
      if attn_mask is not None:
        raise RuntimeError("cannot set attn_mask when is_causal=True")
      attn_mask = qk.ones_like(
        requires_grad=False, device=self.device, dtype=dtypes.bool
      ).tril()
    if attn_mask is not None:
      if attn_mask.dtype == dtypes.bool:
        attn_mask = attn_mask.where(0, -float("inf"))
      qk = qk + attn_mask
    return qk.cast(self.dtype).softmax(-1).dropout(dropout_p) @ value

  def _do_reduction(self, reduction: ReductionStr = "mean") -> Tensor:
    if reduction not in get_args(ReductionStr):
      raise ValueError(f"{reduction=} must be one of {get_args(ReductionStr)}")
    reductions: dict[str, Callable[[Tensor], Tensor]] = {
      "mean": Tensor.mean,
      "sum": Tensor.sum,
      "none": lambda x: x,
    }
  ...
  def binary_crossentropy(
    self, Y: Tensor, reduction: ReductionStr = "mean"
  ) -> Tensor:
    """
  ...
  def binary_crossentropy_logits(
    self,
    Y: Tensor,
  ...
  def sparse_categorical_crossentropy(
    ...
    assert reduction in get_args(ReductionStr), (
      f"reduction must be one of {get_args(ReductionStr)}"
    )
  ...
  def cross_entropy(
    self, Y: Tensor, reduction: ReductionStr = "mean", label_smoothing: float = 0.0
  ) -> Tensor:
    """
    ...
    if self.shape != Y.shape:
      if self.max(classes_dim).shape != Y.shape:
        raise RuntimeError(f"shape mismatch: {self.shape=}, {Y.shape=}")
      Y = Y.unsqueeze(classes_dim)._one_hot_along_dim(
        num_classes=self.shape[classes_dim], dim=classes_dim
      )
    Y = (1 - label_smoothing) * Y + label_smoothing / int(Y.shape[classes_dim])
    return (
      -self.log_softmax(classes_dim).mul(Y).sum(classes_dim)._do_reduction(reduction)
    )
  ...
  def nll_loss(
    ...
    """
    weight = (
      Tensor.ones_like(Y, requires_grad=False) if weight is None else weight[Y]
    )
    masked_weight = weight if ignore_index is None else weight * (Y != ignore_index)
    nll = -self.gather(1, Y.unsqueeze(1)).squeeze(1) * masked_weight
    return (
      nll.sum() / masked_weight.sum()
      if reduction == "mean"
      else nll._do_reduction(reduction)
    )

  def newton_schulz(
    self, steps: int, params: tuple[int, ...], eps: float = 1.0e-7
  ) -> Tensor:
    """
    ...
    if self.shape[-2] > self.shape[-1]:
      return (
        self.transpose(-2, -1).newton_schulz(steps, params, eps).transpose(-2, -1)
      )
    G = self / (self.square().sum(axis=(-2, -1), keepdim=True).sqrt() + eps)
    for _ in range(steps):
      G = sum(
        p * functools.reduce(lambda x, y: (y @ y.transpose(-2, -1)) @ x, [G] * i, G)
  ...
  def qr(self) -> tuple[Tensor, Tensor]:
    ...
    R = self.clone()
    Q = (
      Tensor.eye(m, dtype=self.dtype)
      .reshape((1,) * len(b_shape) + (m, m))
      .expand(b_shape + (m, m))
      .contiguous()
    )
    for i in range(min(m, n)):
      x = (
        R[..., i:m, i].contiguous()
      )  # TODO: without contigous this can silently be wrong, should at least assert
      s = -x[..., 0].sign()
      ...
      R[..., i:m, :] = R[..., i:m, :] - (w * tau) @ (
        w.transpose(-2, -1) @ R[..., i:m, :]
      )
      Q[..., :, i:m] = Q[..., :, i:m] - (Q[..., :, i:m] @ w) @ (tau * w).transpose(
        -2, -1
      )
    return Q, R
  ...
  def svd(self, full_matrices=True) -> tuple[Tensor, Tensor, Tensor]:
    ...
    V = (
      Tensor.eye(num, dtype=self.dtype)
      .reshape((1,) * len(b_shape) + (num, num))
      .expand(b_shape + (num, num))
      .contiguous()
    )
    # prepare round robin pairing
    permute, inverse_permute = (
      Tensor.arange(0, num, dtype=dtypes.int),
      Tensor.zeros(num, dtype=dtypes.int).contiguous(),
    )
    ...
    def one_round_jacobi(U, V, permute, inverse_permute):
      ...
      V_permuted, runoff_V = (
        (V[..., permute].split(num - 1, -1))
        if num % 2 == 1
        else (V[..., permute], None)
      )
      V_left, V_right = V_permuted.split(num // 2, -1)
      U_permuted, runoff_U = (
        (U[..., permute].split(num - 1, -1))
        if num % 2 == 1
        else (U[..., permute], None)
      )
      ...
      U = U_left.cat(
        U_right.cat(runoff_U, dim=-1) if num % 2 == 1 else U_right, dim=-1
      )[..., inverse_permute]
      V_left, V_right = c * V_left - s * V_right, s * V_left + c * V_right
      V = V_left.cat(
        V_right.cat(runoff_V, dim=-1) if num % 2 == 1 else V_right, dim=-1
      )[..., inverse_permute]
      # prepare the next round robin pairings
      if num % 2 == 1:
        permute = (permute - 1) % num
      else:
        permute = permute[0].reshape(1).cat(((permute[1:num] - 2) % (num - 1)) + 1)
      inverse_permute = inverse_permute.scatter(
        0, permute, Tensor.arange(num, dtype=dtypes.int32)
      )
      return U, V, permute, inverse_permute

    max_iterations, iterations_per_round = (
      1,
      int(num * math.log2(num) * 2 + 2),
    )  # sorta heuristic, most use num*log2(num)
    for _ in range(max_iterations * iterations_per_round):
      U, V, permute, inverse_permute = one_round_jacobi(
        U, V, permute, inverse_permute
      )
    # extract singular values and sort. construct U from Q
    S, indices = U.square().sum(-2).sqrt().sort(dim=-1, descending=True)
    new_indices = (
      Tensor.arange(num)
      .reshape((1,) * (self.ndim - 1) + (num,))
      .expand(b_shape + (num, num))
      .contiguous()
    )
    new_indices[..., :num] = indices.reshape(b_shape + (1, num)).expand(
      b_shape + (num, num)
    )
    U, V = (
      U.gather(-1, new_indices[..., 0:num, 0:num]) / S.unsqueeze(-2),
      V.gather(-1, new_indices[..., 0:num, 0:num]).realize(),
    )

    padded_u = (
      Tensor.eye(q_num, dtype=U.dtype)
      .reshape((1,) * len(b_shape) + (q_num, q_num))
      .expand(b_shape + (q_num, q_num))
      .contiguous()
    )
  ...
  def cast(self, dtype: DTypeLike) -> Tensor:
    ...
    ```
    """
    if (dt := to_dtype(dtype)) in {dtypes.uint8, dtypes.uint16} and dtypes.is_float(
      self.dtype
    ):
      # NOTE: values within the int32 range and outside the unsigned dtype range will cause values to wrap around
      return self._apply_uop(UOp.cast, dtype=dtypes.int32)._apply_uop(
        UOp.cast, dtype=dt
      )
  ...
  def bitcast(self, dtype: DTypeLike) -> Tensor:
    ...
    dt = to_dtype(dtype)
    if (ns := dt.itemsize) != (os := self.dtype.itemsize) and (
      self.shape[-1] * os
    ) % ns != 0:
      raise RuntimeError("unsupported size in bitcast")
    if (
      not isinstance(self.device, str) or not self.device.startswith("DISK")
    ) and ns != os:
      new_uint, old_uint = to_dtype(f"uint{8 * ns}"), to_dtype(f"uint{8 * os}")
      tmp = self.bitcast(old_uint)
      if ns > os:
        tmp = tmp.reshape(
          self.shape[:-1] + (self.shape[-1] // (rate := ns // os), rate)
        )
        ...
        return (
          functools.reduce(
            Tensor.add,
            (
              tmp.shrink(nones + ((i, i + 1),)).cast(new_uint) << 8 * i * os
              for i in range(rate)
            ),
          )
          .squeeze(-1)
      ...
      return (
        Tensor.stack(*(tmp >> 8 * i * ns for i in range(os // ns)), dim=-1)
        .flatten(-2)
        .cast(new_uint)
        .bitcast(dtype)
      )
  ...
  def double(self) -> Tensor:
    return self.cast(dtypes.double)

  def long(self) -> Tensor:
    return self.cast(dtypes.long)

  def short(self) -> Tensor:
    return self.cast(dtypes.short)
  ...
  def image_dot(self, w: Tensor, dtype: DTypeLike | None = None) -> Tensor:
    ...
    if not (dx > 0 and dw > 0):
      raise RuntimeError(f"both tensors need to be at least 1D, got {dx}D and {dw}D")
    if x.shape[-1] != w.shape[-min(w.ndim, 2)]:
      raise RuntimeError(f"cannot image_dot {x.shape} and {w.shape}")
    ...
    cx = self.transpose(self.ndim - 1, self.ndim - 2).reshape((
      bs // groups,
      groups * cin,
      -1,
      1,
    ...
    return (
      ...
      .reshape(out_shape_t)
      .transpose(self.ndim - 1, self.ndim - 2)
    )
  ...
  def image_conv2d(
    ...
    if cin % 4 != 0 and not (cin == 1 and groups % 4 == 0):
      ...
      w = w.pad(
        tuple((0, added_input_channels) if i == 2 else None for i in range(w.ndim))
      )
      x = x.pad(
        tuple((0, added_input_channels) if i == 2 else None for i in range(x.ndim))
      )
    ...
    if rcout % 4 != 0 and not (rcout == 1 and groups % 4 == 0):
      ...
      cout = groups * rcout
      w = w.pad(
        tuple((0, added_output_channels) if i == 1 else None for i in range(w.ndim))
      )
    ...
    if IMAGE >= 2:
      x, w = (
        x.cast(base_image_type((bs * iy, ix * groups * cin // 4, 4))),
        w.cast(base_image_type((cout // 4, H * W * cin, 4))),
      )
    ...
    # prepare input
    x = (
      x.permute(0, 3, 4, 5, 1, 2)
      .pad(self._resolve_pool_pads(padding, 2))
      ._pool((H, W), stride, dilation)
    ...
    # prepare weights
    w = w.permute(0, 4, 2, 5, 1, 3).reshape((
      1,
      1,
      1,
      *cout_expand,
    ...
    ret = (
      (x * w)
      .cast(
        base_image_type((bs * oy, ox * cout // 4, 4))
        if IMAGE >= 2
        else dtypes.float32
      )
      .sum((-4, -3, -2, -1), dtype=dtype)
    )
...
def _metadata_wrapper(fn: Callable[P, T]) -> Callable[P, T]:
  def _wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
    ...
    if TRACEMETA >= 2:
      ...
      if caller_module.startswith("tinygrad") and caller_func == "<lambda>":
        caller_frame = sys._getframe(frame := frame + 2)
      caller_module = caller_frame.f_globals.get("__name__", None)
      if caller_module is None:
        return fn(*args, **kwargs)
      caller_func = caller_frame.f_code.co_name
