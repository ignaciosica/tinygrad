...
def _apply_map_to_tensors(applied_map: dict[UOp, UOp], name: str | None = None) -> None:
  scope_tensors = [
    t for tref in tuple(all_tensors) if (t := tref()) is not None and (t.uop in applied_map or len(applied_map.keys() & t.uop.backward_slice.keys()))
  ]
...
def get_shape(x) -> tuple[int, ...]:
  ...
  if not hasattr(x, "__len__") or not hasattr(x, "__getitem__") or isinstance(x, str) or (hasattr(x, "shape") and x.shape == ()):
    return ()
...
def _get_winograd_matcols(mat, dims: int, shp: tuple[sint, ...], device: str | tuple[str, ...], dtype: DType) -> list[list[Tensor]]:
  return [
    [
      Tensor.cat(*[Tensor.full(shp[:dim] + (1,) + shp[dim + 1 :], float(m[k]), device=device, dtype=dtype) for m in mat], dim=dim)
...
class Tensor(MathTrait):
  ...
  def __init__(
    ...
    if isinstance(data, UOp):
      ...
      data = UOp.const(_dtype or dtypes.default_float, 0, _device, (), unique=_force_unique)
    elif isinstance(data, get_args(ConstType)):
      data = UOp.const(_dtype or dtypes.from_py(data), data, _device, (), unique=_force_unique)
      ...
      if _dtype is None:
        if (d := fully_flatten(data)) and all(isinstance(s, bool) for s in d):
          ...
          _dtype = dtypes.default_int if d and all_int(d) else dtypes.default_float  # NOTE: this works because all_int([True, False]) is True
      ...
      if data.shape == ():
        data = UOp.const(_dtype or _from_np_dtype(data.dtype), data.item(), _device, (), unique=_force_unique)
      else:
  ...
  def rand(*shape, device: str | None = None, dtype: DTypeLike | None = None, contiguous: bool = True, **kwargs) -> Tensor:
    ...
    # increment rng counter for devices
      ...
      Tensor._device_rng_counters[device].assign(Tensor._device_rng_counters[device] + num)
  ...
  def rand_like(self, **kwargs) -> Tensor:
    ...
    if isinstance(self.device, tuple):
      if kwargs.get("device") is not None:
        raise RuntimeError("cannot specify `device` on `rand_like` of a multi device tensor")
  ...
  def backward(self, gradient: Tensor | None = None) -> Tensor:
    ...
    tensors_need_grad: list[Tensor] = [t for tref in all_tensors if (t := tref()) is not None and t.uop in all_uops and t.requires_grad]
    # clear contexts
  ...
  def shrink(self, arg: tuple[tuple[sint, sint] | None, ...]) -> Tensor:
    ...
    if (shrink_arg := [x if x is not None else (0, s) for x, s in zip(arg, self.shape)]) == [(0, s) for s in self.shape]:
      return self
  ...
  def pad(self, padding: Sequence[sint] | Sequence[tuple[sint, sint] | None], mode: str = "constant", value: float = 0.0) -> Tensor:
    ...
    # group padding
      ...
      pX = tuple((0, 0) if p is None else p for p in cast(Sequence[tuple[sint, sint] | None], padding))
    if len(pX) != self.ndim:
    ...
    if mode == "constant":
      ...
      def _constant(x: Tensor, px, v) -> Tensor:
        return (
          x._apply_uop(UOp.pad, arg=px) if v == 0 else (x._apply_uop(UOp.pad, arg=px) + Tensor.ones_like(x)._apply_uop(UOp.pad, arg=px).where(0, v))
        )

      return (
        _constant(X, pX, value)
        if all(resolve(p >= 0) for p in flatten(pX))
        else _constant(X.shrink(tuple((-smin(pB, 0), smin(pA + s, s)) for (pB, pA), s in zip(pX, X.shape))), pads, value)
      )
    ...
    if mode == "circular":
      ...
      return X.shrink(
        tuple((0 if pB == 0 else osh - pB, xsh if pA == 0 else xsh - osh + pA) for (pB, pA), osh, xsh in zip(pads, orig_shape, X.shape))
      )
    for d, (pB, pA) in enumerate(pads):
      if mode == "reflect":
        if pB >= (s := X.shape[d]) or pA >= s:
          raise ValueError(f"Padding ({pB}, {pA}) should be less than the input size={s} for dim={d}.")
        (
          slcB,
          slcA,
        ) = slice(pB, 0, -1), slice(s - 2 if s - 2 >= 0 else None, s - 2 - pA if s - 2 - pA >= 0 else None, -1)
        xB, xA = (X[[slc if i == d else slice(None) for i in range(X.ndim)]] if p > 0 else None for slc, p in ((slcB, pB), (slcA, pA)))
      if mode == "replicate":
        (
          shrB,
          shrA,
        ) = tuple((0, 1) if i == d else None for i in range(X.ndim)), tuple((X.shape[i] - 1, X.shape[i]) if i == d else None for i in range(X.ndim))
  ...
  def _getitem(self, indices, v: Tensor | None = None) -> Tensor:
    ...
    if tops := [(d, i) for d, i in enumerate(i_ for i_ in indices_parsed if not isinstance(i_["index"], int)) if isinstance(i["index"], Tensor)]:
      ...
      for dim, tensor in zip(dims, tensors):
        try:
          i = tensor.reshape(tensor.shape + (1,) * (x.ndim - dims[0])).expand(pre_reduce_shape)
      ...
      if permuted := dims[0] != 0 and len(dims) != 1 and tuple(dims) != tuple(range(dims[0], dims[-1] + 1)):
        mask, x = (
          y.permute(*range(dims[0], dims[0] + len(big_shape)), *range(0, dims[0]), *range(dims[0] + len(big_shape), y.ndim)) for y in (mask, x)
        )
  ...
  def gather(self: Tensor, dim: int, index: Tensor) -> Tensor:
    ...
    assert all(s >= i for d, (s, i) in enumerate(zip(self.shape, index.shape)) if d != dim), (
  ...
  def split(self, sizes: int | Sequence[int], dim: int = 0) -> tuple[Tensor, ...]:
    ...
    if isinstance(sizes, int):
      sizes = [min(sizes, self.shape[dim] - i) for i in range(0, max(1, self.shape[dim]), max(1, sizes))]
  ...
  def unfold(self, dim: int, size: sint, step: int) -> Tensor:
    ...
    if size > self.shape[dim]:
      raise RuntimeError(f"maximum size for tensor at dimension {dim} is {self.shape[dim]} but size is {size}")
    dim = self._resolve_dim(dim)
  ...
  def roll(self, shifts: int | tuple[int, ...], dims: int | tuple[int, ...] | None = None) -> Tensor:
    ...
    for dim, shift in zip(dims, shifts):
      slices[dim] = slice(delta := self.shape[dim] - shift % self.shape[dim], delta + self.shape[dim])
  ...
  def mean(self, axis: int | Sequence[int] | None = None, keepdim=False) -> Tensor:
    ...
    return numerator.div(prod([cast(int, si) for si, so in zip(self.shape, self.sum(axis=axis, keepdim=True).shape) if resolve(si != so)])).cast(
      output_dtype
    )
  ...
  def einsum(formula: str, *operands: Tensor | Sequence[Tensor], dtype: DTypeLike | None = None) -> Tensor:
    ...
    def parse_formula(formula: str, *operands: Tensor):
      if "..." in (formula := formula.replace(" ", "")):
        ...
        for i, inp in enumerate(filter(lambda x: "..." in x, inputs := formula.split("->")[0].split(","))):
          if (ell_count := max(operands[i].ndim, 1) - (len(inp) - len("..."))) > ell_longest:
        ...
        return (
          ...
          if "->" in formula
          else (inputs_str, out_ellipse + "".join(sorted(c for c in inputs_str if inputs_str.count(c) == 1 and c.isalpha() and c not in out_ellipse)))
        )
    ...
    if len(xs) != len(inputs):
      raise ValueError(f"number of inputs doesn't match number of operands in formula, expected {len(inputs)}, got {len(xs)}")
    ...
    return (
      ...
      .sum(axis=[axis for axis, (letter, _) in enumerate(letter_val) if letter not in output], dtype=dtype)
      .permute(rhs_order)
    )
  ...
  def _pool(self, k_: tuple[sint, ...], stride: int | tuple[int, ...] = 1, dilation: int | tuple[int, ...] = 1) -> Tensor:
    ...
    if any(resolve(k > s) for k, s in zip(k_, s_)) or any(d != 1 for d in d_):
      ...
      # handle dilation
      x = x.shrink(tuple(noop + [(0, k * (i * f + d)) for k, i, d, f in zip(k_, i_, d_, f_)])).reshape(
        noop + flatten((k, (i * f + d)) for k, i, d, f in zip(k_, i_, d_, f_))
      )
      # handle stride
      x = x.shrink(tuple(noop + flatten(((0, k), (0, o * s)) for k, o, s in zip(k_, o_, s_)))).reshape(
        noop + flatten((k, o, s) for k, o, s in zip(k_, o_, s_))
      )
  ...
  def _resolve_pool_pads(self, padding: int | Sequence[int], dims: int) -> Sequence[int]:
    ...
    return (
      [padding] * 2 * dims if isinstance(padding, int) else (padding if len(padding) == 2 * dims else [p for p in padding for _ in range(2)][::-1])
    )
  ...
  def avg_pool2d(
    self, kernel_size: tuple[int, ...] = (2, 2), stride=None, dilation=1, padding: int | tuple[int, ...] = 0, ceil_mode=False, count_include_pad=True
  ) -> Tensor:
    """
  ...
  def max_pool2d(
    self, kernel_size: tuple[int, ...] = (2, 2), stride=None, dilation=1, padding: int | tuple[int, ...] = 0, ceil_mode=False, return_indices=False
    ...
    if ceil_mode:
      pads = self._apply_ceil_mode(pads, k_, stride if stride is not None else k_, dilation)
  ...
  def max_unpool2d(
    self, indices: Tensor, kernel_size: tuple[int, ...] = (2, 2), stride=None, dilation=1, padding: int | tuple[int, ...] = 0, output_size=None
  ):
    """
  ...
  def conv2d(
    ...
    # (bs, cin_, tyx, HWI)
    pads = [
      [padding_[i * 2], padding_[i * 2 + 1] + (-(dim + sum(padding_[i * 2 : (i + 1) * 2]) - 2) % 4)] for i, dim in enumerate(self.shape[-len(HW) :])
    ]
  ...
  def conv_transpose2d(self, weight: Tensor, bias: Tensor | None = None, groups=1, stride=1, dilation=1, padding=0, output_padding=0) -> Tensor:
    ...
    padding = flatten(
      (((k - 1) * d - pB, (k - 1) * d - pA + op) for k, d, (pB, pA), op in reversed(list(zip(HW, dilation, padding, output_padding))))
    )
  ...
  def scatter_reduce(
    self, dim: int, index: Tensor, src: Tensor, reduce: Literal["sum", "prod", "mean", "amax", "amin"], include_self: bool = True
  ) -> Tensor:
    """
    Scatters `src` values along an axis specified by `dim`.
    ...
    if reduce == "sum":
      return mask.where(src, 0).sum(-1).add(self if include_self else _inv_mask(self, 0))
    if reduce == "prod":
      return mask.where(src, 1).prod(-1).mul(self if include_self else _inv_mask(self, 1))
    if reduce == "amax":
      return mask.where(src, m := dtypes.min(src.dtype)).max(-1).maximum(self if include_self else _inv_mask(self, m))
    if reduce == "amin":
      return mask.where(src, m := dtypes.max(src.dtype)).min(-1).minimum(self if include_self else _inv_mask(self, m))
    if reduce == "mean":
  ...
  def _broadcasted(self, y: Tensor | ConstType | UOp, reverse: bool = False, match_dtype: bool = True) -> tuple[Tensor, Tensor]:
    ...
    if not isinstance(y, Tensor):
      ...
      if isinstance(x.dtype, ImageDType) or dtypes.is_float(x.dtype) or (dtypes.is_int(x.dtype) and isinstance(y, int)):
        y_dtype = x.dtype
  ...
  def linear(self, weight: Tensor, bias: Tensor | None = None, dtype: DTypeLike | None = None) -> Tensor:
    ...
    """
    if dtype is not None:
      return self.cast(dtype).linear(weight.cast(dtype), bias.cast(dtype) if bias is not None else bias)
  ...
  def _one_hot_along_dim(self: Tensor, num_classes: sint, dim: int = -1) -> Tensor:
    if not dtypes.is_int(self.dtype):
      raise RuntimeError(f"_one_hot_along_dim expects int index tensor, getting {self.dtype}")
  ...
  def scaled_dot_product_attention(
    self, key: Tensor, value: Tensor, attn_mask: Tensor | None = None, dropout_p: float = 0.0, is_causal: bool = False, enable_gqa: bool = False
  ) -> Tensor:
    """
    ...
    if FUSE_ATTENTION:
      q, key, value = self.contiguous(), key.contiguous(), value.contiguous()
    else:
      q = self
  ...
  def newton_schulz(self, steps: int, params: tuple[int, ...], eps: float = 1.0e-7) -> Tensor:
    ...
    for _ in range(steps):
      G = sum(p * functools.reduce(lambda x, y: (y @ y.transpose(-2, -1)) @ x, [G] * i, G) for i, p in enumerate(params))
    return G
  ...
  def bitcast(self, dtype: DTypeLike) -> Tensor:
    ...
    dt = to_dtype(dtype)
    if (ns := dt.itemsize) != (os := self.dtype.itemsize) and (self.shape[-1] * os) % ns != 0:
    ...
    if (not isinstance(self.device, str) or not self.device.startswith("DISK")) and ns != os:
      ...
      if ns > os:
        ...
        return (
          functools.reduce(Tensor.add, (tmp.shrink(nones + ((i, i + 1),)).cast(new_uint) << 8 * i * os for i in range(rate)))
          .squeeze(-1)
          .bitcast(dtype)
        )
  ...
  def image_conv2d(self, weight: Tensor, bias: Tensor | None = None, groups=1, stride=1, dilation=1, padding=0, dtype=None) -> Tensor:
    ...
    # prepare input
    x = (
      x.permute(0, 3, 4, 5, 1, 2).pad(self._resolve_pool_pads(padding, 2))._pool((H, W), stride, dilation)
...
if TRACEMETA >= 1:
  for name, fn in inspect.getmembers(Tensor, inspect.isfunction):
    if name in ["__class__", "__init__", "__new__", "__repr__", "backward", "sequential", "gradient"]:
      continue
