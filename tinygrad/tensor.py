...
def _apply_map_to_tensors(applied_map: dict[UOp, UOp], name: str | None = None) -> None:
  scope_tensors = [
    ...
    and (t.uop in applied_map or len(applied_map.keys() & t.uop.backward_slice.keys()))
  ]
  ...
  return [
    [
      Tensor.cat(
        *[
          Tensor.full(
            shp[:dim] + (1,) + shp[dim + 1 :], float(m[k]), device=device, dtype=dtype
          )
          for m in mat
        ],
        dim=dim,
...
class Tensor(MathMixin, MovementMixin):
  ...
  def __init__(
    self,
    data: ConstType | bytes | list | tuple | UOp | "np.ndarray" | pathlib.Path | None,  # type: ignore [name-defined] # noqa: F821
  ...
  def _binop(self, op, x, reverse):
    return self._apply_broadcasted_uop(lambda *u: UOp.alu(u[0], op, *u[1:]), x, reverse)
    ...
    **kwargs,
    ...
    if device not in Tensor._device_seeds:
      Tensor._device_seeds[device] = Tensor(
        [
          int.from_bytes(
            hashlib.sha256(len(Tensor._device_seeds).to_bytes(4, "big")).digest(), "big"
          ),
          Tensor._seed,
        ],
        device=device,
  ...
  def rand_like(self, **kwargs) -> Tensor:
    ...
    if isinstance(self.device, tuple):
      ...
      return Tensor(
        UOp.multi(rands, axis=self.uop.axis), device=self.device, dtype=dtype, **kwargs
      )
    return Tensor.rand(
  ...
  @staticmethod
  def randn(
    *shape, dtype: DTypeLike | None = None, requires_grad: bool | None = None, **kwargs
  ) -> Tensor:
    """
  ...
  def uniform(
    ...
    **kwargs,
    ...
    """
    return (
      ((high - low) * Tensor.rand(*shape, **kwargs)).cast(dtype or dtypes.default_float)
      + low
    ).requires_grad_(requires_grad)
  ...
  def kaiming_uniform(*shape, a: float = 0.01, **kwargs) -> Tensor:
    ...
    """
    bound = (
      math.sqrt(3.0) * math.sqrt(2.0 / (1 + a**2)) / math.sqrt(prod(argfix(*shape)[1:]))
    )
  ...
  def gradient(
    ...
    if not (self.is_floating_point() and all(t.is_floating_point() for t in targets)):
  ...
  def pad(
    ...
    if all(isinstance(p, (int, UOp)) for p in padding):
      ...
      pX = _flat_to_grouped(
        tuple(cast(Sequence[sint], padding)) + (0, 0) * (self.ndim - len(padding) // 2)
      )
    # group padding
    else:
      pX = tuple(
    ...
    if mode == "constant":
      ...
      return (
        ...
        if all(resolve(p >= 0) for p in flatten(pX))
          ...
          X.shrink(
            tuple((-smin(pB, 0), smin(pA + s, s)) for (pB, pA), s in zip(pX, X.shape))
          ),
          pads,
          value,
        )
    ...
    if mode == "circular":
      ...
      if any(pB < 0 or pA < 0 for pB, pA in pX):
        raise NotImplementedError("Negative pads with circular pads is not supported")
      orig_shape, X = (
        X.shape,
    ...
    for d, (pB, pA) in enumerate(pads):
      if mode == "reflect":
        ...
        (
          ...
          slice(pB, 0, -1),
          slice(
            s - 2 if s - 2 >= 0 else None, s - 2 - pA if s - 2 - pA >= 0 else None, -1
          ),
        )
        xB, xA = (
          X[[slc if i == d else slice(None) for i in range(X.ndim)]] if p > 0 else None
  ...
  def _getitem(self, indices, v: Tensor | None = None) -> Tensor:
    ...
    if len(ellipsis_idx := [dim for dim, i in enumerate(indices) if i is Ellipsis]) > 1:
  ...
  def chunk(self, chunks: int, dim: int = 0) -> list[Tensor]:
    ...
    return list(
      self.split(
        ceildiv(self.shape[dim], chunks) if self.shape[dim] else [0] * chunks, dim=dim
      )
    )
  ...
  def sum(
    ...
    return (
      ret.cast(self.dtype)
      if dtype is None and self.dtype in (dtypes.float16, dtypes.bfloat16, *dtypes.fp8s)
      else ret
    )
  ...
  def keccak(self, cfg: str | tuple[int, int] = "sha3_256"):
    ...
    data = (
      (data.flatten(1) ^ pad_mask).reshape(*data.shape[:2], 200).bitcast(dtypes.uint64)
    )
    ...
    for k in range(int(data.shape[1])):
      ...
      for i in range(24):  # f1600
        ...
        state = (
          (state * rot_offsets_v0).bitwise_or(state // rot_offsets_v1).reshape(bs, 5, 5)
        )
        # χ and ι step
  ...
  @staticmethod
  def einsum(
    formula: str, *operands: Tensor | Sequence[Tensor], dtype: DTypeLike | None = None
  ) -> Tensor:
    """
  ...
  def avg_pool2d(
    ...
    def pool(x: Tensor, padding_: Sequence[int]) -> Tensor:
      return x.pad(padding_)._pool(k_, stride if stride is not None else k_, dilation)
  ...
  def max_unpool2d(
    ...
    if output_size is None:
      k_, d_, s_ = (
        ...
        for x in (kernel_size, dilation, stride if stride is not None else kernel_size)
      )
  ...
  def conv2d(
    ...
    """
    if IMAGE:
      return self.image_conv2d(weight, bias, groups, stride, dilation, padding, dtype)
  ...
  def conv_transpose2d(
    ...
    return x.conv2d(
      w.flatten(end_dim=1), groups=groups, bias=bias, dilation=dilation, padding=padding
    )
  ...
  def _cumalu(self, axis: int, op: Ops, _include_initial=False) -> Tensor:
    ...
    return {Ops.ADD: pooled.sum(-1), Ops.MAX: pooled.max(-1), Ops.MUL: pooled.prod(-1)}[
      op
    ].transpose(axis, -1)
  ...
  def interpolate(
    ...
    """
    assert (
      isinstance(size, (tuple, list)) and all_int(size) and 0 < len(size) <= self.ndim
    ), f"invalid {size=}"
  ...
  def scatter(
    ...
    if reduce and isinstance(src, Tensor):
      raise TypeError("Tensor src is not supported with reduce arg. see scatter_reduce")
  ...
  def lerp(self, end: Tensor, weight: Tensor | float) -> Tensor:
    ...
    if self.dtype == dtypes.uint8 and isinstance(weight, Tensor):
      ...
      return (
        self
        + (
          ((end - self).cast(dtypes.int8) * w_i + (1 << W_PREC - 1)).cast(dtypes.uint16)
          >> W_PREC
        )
      ).cast(dtypes.uint8)
  ...
  def gelu(self) -> Tensor:
    ...
    """
    return (
      0.5 * self * (1 + (math.sqrt(2 / math.pi) * (self + 0.044715 * self**3)).tanh())
    )
  ...
  def _broadcasted(
    self, y: Tensor | ConstType | UOp, reverse: bool = False, match_dtype: bool = True
  ) -> tuple[Tensor, Tensor]:
    x: Tensor = self
    if not isinstance(y, Tensor):
      ...
      if isinstance(y, UOp):
        ...
        y = Tensor(dtypes.as_const(y, y_dtype), x.device, y_dtype, requires_grad=False)
  ...
  def lshift(self, x: Tensor | int, reverse=False) -> Tensor:
    ...
    """
    assert (
      dtypes.is_unsigned(self.dtype) and isinstance(x, int) and x >= 0 and not reverse
  ...
  def _one_hot_along_dim(self: Tensor, num_classes: sint, dim: int = -1) -> Tensor:
    ...
    dt = (
      dtypes.int64 if sint_to_uop(num_classes).overflows(dtypes.int32) else dtypes.int32
    )
  ...
  def scaled_dot_product_attention(
    ...
    q = self
    qk = q.matmul(
      key.transpose(-2, -1), dtype=least_upper_dtype(q.dtype, key.dtype, dtypes.float32)
    ) / math.sqrt(q.shape[-1])
  ...
  def binary_crossentropy_logits(
    self, Y: Tensor, reduction: ReductionStr = "mean", pos_weight: Tensor | None = None
  ) -> Tensor:
    """
    ...
    return (
      -((1 if pos_weight is None else pos_weight) * Y * log_p + (1 - Y) * log_1_minus_p)
  ...
  def image_conv2d(
    ...
    ret = (
      (x * w)
      .cast(
        base_image_type((bs * oy, ox * cout // 4, 4)) if IMAGE >= 2 else dtypes.float32
      )
