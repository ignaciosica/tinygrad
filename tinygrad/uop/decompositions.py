...
def ilogb2k(d: UOp) -> UOp:
  ...
  # -1 <= ilog2bk(d) <= 128
  return (shr(dint, mantissa_bits(d.dtype)) & exponent_mask(d.dtype)) - exponent_bias(
    d.dtype
  )
...
def payne_hanek_reduction(d: UOp) -> tuple[UOp, UOp]:
  ...
  def _take(an: UOp, offset: int, count: int = 0) -> UOp:
    ...
    if count + offset < len(two_over_pi_f) - 1:
      an = i.ne(count).where(
        _take(an, offset, count=count + 1), an.const_like(two_over_pi_f[count + offset])
      )
    return an
  ...
  if dtypes.is_int(
    ...
    if m * vmin >= dtypes.min(next_dtype) and m * vmax <= dtypes.max(next_dtype):
      return (
        ...
        if is_unsigned
        else ((x.cast(next_dtype) * m) >> s).cast(x.dtype) + (x < 0).where(x.ufix(1), 0)
      )
  return None
...
def get_late_rewrite_patterns(ops: tuple[Ops, ...], force_transcendental=False):
    ...
    if op not in ops or force_transcendental:
      pat += [
        ...
        (
          UPat(
            op,
            dtype=tuple(dt for dt in dtypes.floats if dt not in TRANSCENDENTAL_DTYPES),
            src=(UPat.var("d"),),
            name="x",
          ),
  ...
  if Ops.CMPLT in ops:
    ...
    pat += [
      ...
      (
        UPat.var("x", dtypes.sints) * -1 < UPat.var("y", dtypes.sints) * UPat.cvar("c"),
        lambda x, y, c: y * (-c) < x,
      ),
  ...
  if Ops.CMPEQ in ops:
    pat += [
      (UPat.var("x").ne(UPat.var("y")).logical_not(), lambda x, y: x.alu(Ops.CMPEQ, y))
    ]
