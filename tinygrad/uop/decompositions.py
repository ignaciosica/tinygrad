...
def _lazy_map_numbers(x: UOp, inf: UOp, _inf: UOp, nan: UOp, ratio: UOp):
  ...
  return x.ne(math.inf).where(
    x.ne(x).where(nan, x.ne(-math.inf).where(ratio, _inf)), inf
  )
...
def rintk(d: UOp) -> UOp:
  ...
  }[d.dtype.scalar()].vec(d.dtype.vcount)
...
def pow2if(q: UOp, float_dtype: DType):
  ...
  out_dtype = {
    ...
    dtypes.int16: float_dtype.scalar(),
  }[q.dtype.scalar()].vec(q.dtype.vcount)
  return shl(q + exponent_bias(out_dtype), mantissa_bits(out_dtype)).bitcast(
    out_dtype
  )
...
def ilogb2k(d: UOp) -> UOp:
  ...
  # -1 <= ilog2bk(d) <= 128
  return (
    shr(dint, mantissa_bits(d.dtype)) & exponent_mask(d.dtype)
...
def ldexp3k(d: UOp, e: UOp) -> UOp:
  ...
  assert (
    d.dtype.scalar() in TRANSCENDENTAL_DTYPES
    and e.dtype.scalar() in TRANSCENDENTAL_DTYPES
  )
  dtype = {
    dtypes.float64: dtypes.int64,
    dtypes.float32: dtypes.int32,
    dtypes.float16: dtypes.int16,
  }[d.dtype.scalar()].vec(d.dtype.count)
  m1 = d.bitcast(dtype)
...
def ldexp2k(d: UOp, e: UOp) -> UOp:
  ...
  assert d.dtype.scalar() in TRANSCENDENTAL_DTYPES and e.dtype.scalar() in (
    dtypes.int16,
    dtypes.int32,
    dtypes.int64,
  )
...
def frexp(v: UOp) -> tuple[UOp, UOp]:
  ...
  m1 = {
    dtypes.float64: 0x000FFFFFFFFFFFFF,
  ...
  m2 = {
    ...
    dtypes.float16: 0x3800,
  }[v.dtype.scalar()]
  bits = v.bitcast(
    {
      dtypes.float64: dtypes.uint64,
      dtypes.float32: dtypes.uint32,
      dtypes.float16: dtypes.uint16,
    }[v.dtype.scalar()].vec(v.dtype.count)
  )
...
def payne_hanek_reduction(d: UOp) -> tuple[UOp, UOp]:
  ...
  intermediate_dtype = (
    dtypes.float32.vec(d.dtype.count)
    if d.dtype.base.scalar() == dtypes.float16
    else d.dtype
  )
  ...
  def _take(an: UOp, offset: int, count: int = 0) -> UOp:
    ...
    if count + offset < len(two_over_pi_f) - 1:
      an = i.ne(count).where(
        _take(an, offset, count=count + 1),
        an.const_like(two_over_pi_f[count + offset]),
      )
    return an

  def _shl_lazy(x: UOp, y: UOp):
    return (x.cast(dtypes.uint64) * pow2if(y, d.dtype).cast(dtypes.uint64)).cast(
      dtypes.uint32
    )
...
def cody_waite_reduction(d: UOp) -> tuple[UOp, UOp]:
  ...
  def _reduce_d(x: UOp, q: UOp):
    ...
    if x.dtype.scalar() == dtypes.float64:
      ...
      d = _reduce_d(x.cast(dtypes.float32), q.cast(dtypes.float32)).cast(
        dtypes.float16
      )
    else:
  ...
  quadrant = (
    rintk(d * m_1_pi - qdh)
    if d.dtype.base.scalar() == dtypes.float64
    else rintk(d * m_1_pi)
  )
...
def xsin(d: UOp, fast: bool = False, switch_over: float = 30.0) -> UOp:
  ...
  x = _lazy_map_numbers(
    d, d.const_like(0.0), d.const_like(0.0), d.const_like(0.0), d
  )
  # x_sign = sign(x)
  x_sign = x.ne(0).where(
    (x < 0).where(x.const_like(-1), x.const_like(1)), x.const_like(0)
  )
  x_abs = x * x_sign
  ...
  if fast:
    ...
    result = (x_abs < switch_over).where(
      sin_poly_small(r_small, q_small), sin_poly_large(r, q)
    )
  # adjusts the sign for abs(x)
  result = result * x_sign
  # sin(Inf) = NaN, sin(-Inf) = NaN, sin(NaN) = NaN
  return _lazy_map_numbers(
    d, d.const_like(math.nan), d.const_like(math.nan), d.const_like(math.nan), result
  )
...
def xexp2(d: UOp) -> UOp:
  ...
  u = ldexp2k(u, q)  # u*2^q
  upper, lower = {
    dtypes.float64: (1024, -2000),
    dtypes.float32: (128, -150),
    dtypes.float16: (23, -22),
  }[d.dtype.scalar()]
...
def xlog2(d: UOp) -> UOp:
  ...
  log2_zero = {dtypes.float64: -1087, dtypes.float32: -191, dtypes.float16: -79}[
    d.dtype.scalar()
  ]
...
def xpow(base: UOp, exponent: UOp) -> UOp:
  ...
  adj = non_int.where(
    ...
    .mod(2)
    .cast(dtypes.bool)
    .where(ret.const_like(-1), ret.const_like(1)),
  )
  # fix 0 ** 0 = 1
  return (base.eq(0) & exponent.eq(0)).where(
    ret.const_like(1), ret * (base < 0).where(adj, ret.const_like(1))
  )
...
def fast_idiv(device: str, x: UOp, d: int, dont_cast=False) -> UOp | None:
  ...
  if m * vmin >= dtypes.min(x.dtype) and m * vmax <= dtypes.max(x.dtype):
    return (
      ((x * m) >> s) if is_unsigned else ((x * m) >> s) + (x < 0).where(x.ufix(1), 0)
    )
  ...
  if (largest_factor_of_two_in_d := (d & -d)) > 1:
    if (
      ret := fast_idiv(
        device,
        x // largest_factor_of_two_in_d,
        d // largest_factor_of_two_in_d,
        dont_cast=True,
      )
    ) is not None:
      return ret
  ...
  if dtypes.is_int(
    next_dtype := promo_lattice[x.dtype.scalar()][-1]
  ) and is_dtype_supported(next_dtype, None if device == "" else device):
    if m * vmin >= dtypes.min(next_dtype) and m * vmax <= dtypes.max(next_dtype):
      return (
        ((x.cast(next_dtype) * m) >> s).cast(x.dtype)
        if is_unsigned
        else ((x.cast(next_dtype) * m) >> s).cast(x.dtype)
        + (x < 0).where(x.ufix(1), 0)
      )
  return None
...
def threefry2x32(x: UOp, key: UOp):
  ...
  x0, x1 = (
    ...
    ((x // 2**32) & 0xFFFFFFFF).cast(dtypes.uint32),
  )
  key0, key1 = (
    (key & 0xFFFFFFFF).cast(dtypes.uint32),
    ((key // 2**32) & 0xFFFFFFFF).cast(dtypes.uint32),
  )
...
def get_late_rewrite_patterns(ops: tuple[Ops, ...], force_transcendental=False):
      ...
      pat += [
        ...
        (
          UPat(
            op,
            dtype=tuple(
              dt for dt in dtypes.floats if dt not in TRANSCENDENTAL_DTYPES
            ),
            src=(UPat.var("d"),),
            name="x",
          ),
  ...
  if Ops.THREEFRY not in ops:
    pat.append((
      UPat(Ops.THREEFRY, dtype=dtypes.uint64, src=(UPat.var("x"), UPat.var("key"))),
      threefry2x32,
    ))
  ...
  if Ops.SQRT not in ops:
    pat.append((
      UPat(Ops.SQRT, src=UPat.var("d")),
      lambda d: xpow(d, d.const_like(0.5)),
    ))
  ...
  if Ops.OR in ops:
    pat += [
      (
        UPat.var("x", dtypes.bool).logical_not()
        & UPat.var("y", dtypes.bool).logical_not(),
  ...
  if Ops.SHL in ops:
    pat += [
      (
        UPat.var("x", dtypes.ints) * UPat.cvar("c"),
        lambda c, x: x << v if (v := powers_of_two.get(c.arg, 0)) else None,
      )
    ]
  if Ops.SHR in ops:
    ...
    pat += [
      (
        UPat.var("x", dtypes.uints) // UPat.cvar("c"),
    ...
    pat += [
      (
        ...
        lambda x, c: (
          x
          + (l.const_like(l.vmin) if (l := (x < 0)).vmin == l.vmax else l).where(
            c - 1, 0
          )
        )
        >> v
    ...
    if not DISABLE_FAST_IDIV:
      pat += [
        (
          UPat.var("x", dtypes.ints) // UPat.cvar("d", vec=False),
          lambda ctx, x, d: fast_idiv(ctx, x, d.arg),
        )
      ]
      pat += [
        (UPat.var("x", dtypes.ints) % UPat.var("d"), lambda x, d: x - d * (x // d))
      ]
  if Ops.NEG in ops:
    pat += [(UPat.var("x") * -1, lambda x: x.alu(Ops.NEG))]
    if Ops.SUB in ops:
      pat += [
        (UPat.var("x") + UPat.var("y").alu(Ops.NEG), lambda x, y: x.alu(Ops.SUB, y))
      ]
  if Ops.CMPLT in ops:
    # These are late rewrites because simplex expects equalities to be a certain format
    pat += [
      (
        (UPat.var("x", dtypes.sints) < UPat.cvar("c", dtypes.sints)).logical_not(),
        lambda x, c: c - 1 < x,
      ),
      (
        (UPat.cvar("c", dtypes.sints) < UPat.var("x", dtypes.sints)).logical_not(),
        lambda x, c: x < c + 1,
      ),
      (
        UPat.var("x", dtypes.sints) * -1
        < UPat.var("y", dtypes.sints) * UPat.cvar("c"),
        lambda x, y, c: y * (-c) < x,
      ),
      (UPat.var("x", dtypes.sints) * -1 < UPat.cvar("c"), lambda x, c: -c < x),
      (
        (UPat.cvar("c1", vec=False) < UPat.var("x", dtypes.sints))
        & (UPat.var("x", dtypes.sints) < UPat.cvar("c2", vec=False)),
  ...
  if Ops.CMPEQ in ops:
    pat += [
      (
        UPat.var("x").ne(UPat.var("y")).logical_not(),
        lambda x, y: x.alu(Ops.CMPEQ, y),
      )
    ]
  ...
  if Ops.FDIV in ops:
    ...
    pat += [
      (
        UPat.var("a", dtypes.floats)
        * UPat.const(dtypes.floats, 1).alu(Ops.FDIV, UPat.var("b")),
        lambda a, b: a.alu(Ops.FDIV, b),
      )
    ]
