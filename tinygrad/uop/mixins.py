...
class MathMixin:
  ...
  def sub(self, x: Self | ConstType, reverse: bool = False):
    return (
      self.ufix(x).alu(Ops.ADD, -self)
      if reverse
      else self.alu(Ops.ADD, self.ufix(-x))
    )

  def div(self, x: Self | ConstType, reverse: bool = False):
    return (
      (self.ufix(x) * self.alu(Ops.RECIPROCAL))
      if reverse
      else (self * self.ufix(x).alu(Ops.RECIPROCAL))
    )
  ...
  def __ne__(self, x: Self | ConstType):
...
class MovementMixin:
  ...
  def reshape(self, shape, *args) -> Self:
    ...
    new_shape = tuple([
      s if s is not None else self.shape[i]
      for i, s in enumerate(argfix(shape, *args))
    ])
    # resolve -1
    if (c := new_shape.count(-1)) > 1:
      raise RuntimeError(
        f"only one dimension can be inferred using -1, getting {new_shape}"
      )
    if c:
      new_shape = tuple([
        -prod(self.shape) // prod(new_shape) if s == -1 else s for s in new_shape
      ])
    if prod(self.shape) != prod(new_shape):
      raise ValueError(
        f"size mismatch, can't reshape ({self.shape}) -> ({new_shape})"
      )
  ...
  def flatten(self, start_dim=0, end_dim=-1) -> Self:
    ...
    return self.reshape(
      ...
      + (prod(self.shape[start_dim : end_dim + 1]),)
