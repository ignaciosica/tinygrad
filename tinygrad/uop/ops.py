...
class KernelInfo:
  ...
  def get_offset(self, axis_name: str) -> int:
    keys = ("global_dims", "local_dims", "reduce_dims", "upcasted")
    counts = (self.global_dims, self.local_dims, self.reduce_dims, self.upcasted)
    return sum(count for count in counts[: keys.index(axis_name)])
