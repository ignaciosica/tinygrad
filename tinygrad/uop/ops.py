...
class UOpMetaClass(type):
  ...
  def __call__(
    ...
    if (wret := UOpMetaClass.ucache.get(key := (op, dtype, src, arg, tag), None)) is not None and (ret := wret()) is not None:
      return ret
...
class UOp(MathTrait, metaclass=UOpMetaClass):
  ...
  def __del__(self):
    if Ops is not None and self.op is Ops.BUFFER and (buffer := buffers.get(self)) is not None:
      buffer.ref(-1)
    try:
    ...
    if self.op in GroupOp.Movement.union({Ops.MULTI, Ops.REDUCE_AXIS, Ops.WMMA}):
      ...
      match self.op:
        ...
        case Ops.PAD:
          ...
          if len(ps) != len(self.marg) or not all(resolve(b >= 0) and resolve(e >= 0) for b, e in self.marg):
        ...
        case Ops.FLIP:
          if len(ps) != len(self.marg) or not all(isinstance(x, bool) for x in self.marg):
            raise ValueError(f"bad flip on {ps}, {self.marg}")
          return ps
        case Ops.MULTI:
          return tuple(s * len(self.device) if a == self.axis else s for a, s in enumerate(ps))
  ...
  def _eval(self, dtype, expected_type: Type[T]) -> T:
    ...
    if vmin != vmax:
      raise ValueError(f"eval failed to be a single number, range is {vmin} to {vmax} in {simple_self.render()}")
  ...
  def load(self, *src: UOp, **kwargs):
    return UOp(Ops.LOAD, dtype=kwargs.pop("dtype", self.dtype.base), src=(self,) + src, **kwargs)
  ...
  def const(
    ...
    if device is not None:
      if unique or not isinstance(unique, bool):
        ret = ret.replace(src=(UOp(Ops.DEVICE, arg=device), UOp.unique(None if unique is True else unique)))
      else:
  ...
  def valid(self, cond):
    return self if cond.op is Ops.WHERE and cond.arg else cond.where(self, UOp.invalid(self.dtype.count))
  ...
  def axis(self) -> int | None:
    ...
    if self.op in GroupOp.ALU:
      return axes[-1] if (axes := dedup([x.axis for x in self.src if x.axis is not None])) else None
    if len(self.src) == 0:
      return None
  ...
  def marg(self):
    match self.op:
      ...
      case Ops.PAD | Ops.SHRINK:
        return tuple((self.src[1].sgep(i), self.src[2].sgep(i)) for i in range(self.src[1].dtype.count))
  ...
  def _mop(self, op: Ops, arg, same_shape_noop: bool = False) -> UOp:
    ...
    for arg in src_args:
      if len(arg) == 0:
        ...
        usrcs.append(UOp(Ops.VECTORIZE, dtypes.index.vec(len(arg)), tuple(UOp.const(dtypes.index, x) if isinstance(x, int) else x for x in arg)))
    if len(usrcs) == 0:
  ...
  def reshape(self, arg: tuple[sint, ...]):
    return self._mop(Ops.RESHAPE, arg, same_shape_noop=True)
  ...
  def permute(self, arg: tuple[int, ...]):
    return self._mop(Ops.PERMUTE, arg, same_shape_noop=False) if arg != tuple(range(len(self.shape))) else self

  def flip(self, arg: tuple[bool, ...]):
    return self._mop(Ops.FLIP, arg, same_shape_noop=False) if any(arg) and len(arg) == len(self.shape) else self
  ...
  def is_increasing(self: UOp) -> bool:
    ...
    if self.op in (Ops.MUL, Ops.IDIV) and self.src[1].op is Ops.CONST and self.src[1].arg >= 0:
  ...
  def divides(self, v: int) -> UOp | None:
    ...
    if self.op is Ops.VCONST:
      return self.const_like(tuple(x // v for x in self.arg)) if all(x % v == 0 for x in self.arg) else None
    if self.op is Ops.ADD:
  ...
  def divide_exact(self, v: UOp) -> UOp | None:
    ...
    if self.op is Ops.ADD:
      return None if (s0 := self.src[0].divide_exact(v)) is None or (s1 := self.src[1].divide_exact(v)) is None else s0 + s1
    if self.op is Ops.MUL:
  ...
  def sum(self: UOp, *uops: UOp) -> UOp:
    return functools.reduce(operator.or_ if self.dtype is dtypes.bool else operator.add, uops, self)

  def prod(self: UOp, *uops: UOp) -> UOp:
    return functools.reduce(operator.and_ if self.dtype is dtypes.bool else operator.mul, uops, self)
  ...
  def _min_max(self) -> tuple[ConstType, ConstType]:
    if self.op in GroupOp.Binary and not dtypes.is_float(self.dtype):
      ...
      if self.op is Ops.AND and dtypes.is_int(self.dtype) and s1_vmin == s1_vmax >= 0 and s0_vmin >= 0:
        return min(0, s0_vmin), min(s0_vmax, s1_vmax)
      if self.op is Ops.MUL:
        return min(vals := (s0_vmin * s1_vmin, s0_vmin * s1_vmax, s0_vmax * s1_vmin, s0_vmax * s1_vmax)), max(vals)
      # SHL/SHR on consts only
      if self.op is Ops.SHL and s1_vmin == s1_vmax and all_int(t := (s0_vmin, s0_vmax, s1_vmin)):
        return t[0] << t[2], t[1] << t[2]
      if self.op is Ops.SHR and s1_vmin == s1_vmax and all_int(t := (s0_vmin, s0_vmax, s1_vmin)):
        return t[0] >> t[2], t[1] >> t[2]
      if self.op is Ops.MOD:
        if s1_vmin > 0:
          return (0, s1_vmax - 1) if s0_vmin >= 0 else (-(s1_vmax - 1), 0) if s0_vmax <= 0 else (-(s1_vmax - 1), s1_vmax - 1)
        if s1_vmax < 0:
          return (0, -s1_vmin - 1) if s0_vmin >= 0 else (-(-s1_vmin - 1), 0) if s0_vmax <= 0 else (-(-s1_vmin - 1), -s1_vmin - 1)
      ...
      if self.op is Ops.CMPNE:
        return ((s0_vmax < s1_vmin) or (s1_vmax < s0_vmin), not (s0_vmin == s0_vmax == s1_vmin == s1_vmax))
...
def print_uops(uops: list[UOp]):
  for i, u in enumerate(uops):
    ...
    print(f"{i:4d} {str(u.op):20s}: {str(u.dtype):40s} {str(formatted_srcs):32s} {u.arg}")
...
class UPat(MathTrait):
  ...
  def __init__(
    ...
    if isinstance(src, list):
      self.src = list(itertools.permutations(src)) if not all_same(src) else [tuple(src)]
  ...
  def named(self, name: str):
    return UPat(self.op, self.dtype, self._in_src, self.arg, name, not self.strict_length, self.custom_early_reject)
  ...
  def or_casted(self, name: str | None = None):
    return UPat.any(self if name is None else self.named(name), UPat(Ops.CAST, name=name, src=(self,)))
  ...
  def match(self: UPat, uop: UOp, store: dict[str, UOp]) -> list[dict[str, UOp]]:
    if (
      ...
      or (self.dtype is not None and uop.dtype not in self.dtype and uop.dtype.scalar() not in self.dtype)
...
class PatternMatcher:
  ...
  def __reduce__(self):
    return PatternMatcher, ([(x, deconstruct_function(fxn) if fxn.__name__ == "<lambda>" else fxn) for x, fxn in self.patterns],)
  ...
  def _decorator(func):
    def __wrapper(*args, **kwargs):
      ...
      if getenv("CAPTURE_PROCESS_REPLAY") and replay:
        ...
        frm = sys._getframe(1)
        while (f_back := frm.f_back) is not None and "unittest" not in f_back.f_code.co_filename:
          frm = f_back
  ...
  def print_match_stats():
    ...
    if getenv("PRINT_MATCH_STATS", TRACK_MATCH_STATS.value):
      ...
      for k, v in sorted(list(match_stats.items()), key=lambda x: x[1][2] + x[1][3]):
        ...
        if v[1] != 0:
          print(f"{v[0]:6d} / {v[1]:7d} -- {v[3] * 1000.0:9.2f} / {(v[2] + v[3]) * 1000.0:9.2f} ms -- {loc_str:20s}", printable(k.location))
        ret = [x + y for x, y in zip(ret, v)]
      print(f"{ret[0]:6d} / {ret[1]:7d} -- {ret[3] * 1000.0:9.2f} / {(ret[2] + ret[3]) * 1000.0:9.2f} ms -- TOTAL")
...
pm_lower_index_dtype = PatternMatcher([
  ...
  (
    ...
    lambda u, x, y: x.cast(dt := least_upper_dtype(select_dtype(u), x.dtype, y.dtype)).alu(u.op, y.cast(dt)).cast(u.dtype),
  ),
  (
  ...
  (
    UPat(Ops.WHERE, dtypes.index, src=(UPat.var("cond"), UPat.var("x").cast(dtypes.index), UPat.var("y").cast(dtypes.index))),
    lambda cond, x, y: cond.where(x.cast(dt := least_upper_dtype(x.dtype, y.dtype)), y.cast(dt)).cast(dtypes.index),
  ),
  ...
  # lower Invalid
  (
    UPat.var("buf").index(UPat.var("cond").where(UPat.var("idx"), UPat(Ops.CONST, arg=Invalid))),
...
def strip_binary_parens(x: UOp, left: str, right: str, code_for_op) -> str:
  ...
  return code_for_op(
    ...
    strip_parens(right) if precedence.get(x.src[1].op, 99) < precedence[x.op] else right,
  )
...
renderer = PatternMatcher([
  ...
  (UPat(Ops.VECTORIZE, name="x"), lambda ctx, x: f"{{{','.join([ctx[y] for y in x.src])}}}" if not all_same(x.src) else f"{{{ctx[x.src[0]]}, ...}}"),
...
def srcs(ctx, src):
  return f"({ctx[src[0]]},)" if len(src) == 1 else f"({', '.join([ctx[x] for x in src])})"
...
pm_pyrender_extra = PatternMatcher([
  ...
  (
    UPat(Ops.BUFFER, src=(UPat(Ops.UNIQUE, name="u"), UPat(Ops.DEVICE, name="d")), name="x"),
  ...
  (
    UPat(set(syms.keys()) - {Ops.SUB}, src=(UPat(name="y"), UPat(Ops.CONST, name="z")), name="x"),
    lambda ctx, x, y, z: strip_binary_parens(x, ctx[y], str(z.arg), lambda a, b: f"({a}{syms[x.op]}{b})"),
  ),
  (
    UPat(set(syms.keys()) - {Ops.SUB}, name="x"),
    lambda ctx, x: strip_binary_parens(x, ctx[x.src[0]], ctx[x.src[1]], lambda a, b: f"({a}{syms[x.op]}{b})"),
  ),
  ...
  (
    ...
    + ", ".join([ctx[y] for y in x.src[1:]] + ([f"arg={repr(x.arg)}"] if x.arg is not None else []))
    + ")",
  ),
])
...
def pyrender(ast: UOp) -> str:
  ...
  for u in lst:
    ...
    if len(cmap[u]) == 1 and len([x for x in list(cmap[u].keys())[0].src if x is u]) == 1 and u.op not in always_rendered:
      continue
    to_render.add(u)
