...
shared_codegen_spec = PatternMatcher([
  ...
  (
    ...
    lambda x: len(x.src) > 1 and len(x.src) == x.dtype.vcount and all(x.dtype == y.dtype.vec(len(x.src)) for y in x.src),
  ),
...
program_spec = (
  PatternMatcher([
    ...
    (
      ...
      lambda x: all(v is not Invalid for v in x.arg) and len(x.arg) == x.dtype.vcount > 1 and type(x.arg) is type(dtypes.as_const(x.arg, x.dtype)),
    ),
...
full_spec = (
  PatternMatcher([
    ...
    (
      UPat((Ops.ADD, Ops.MUL, Ops.MOD, Ops.IDIV, Ops.MAX, Ops.SPECIAL, Ops.CAST, Ops.RANGE, Ops.VCONST, Ops.VECTORIZE), dtype=dtypes.index),
      lambda: True,
    ),
