...
shared_spec = PatternMatcher([
  ...
  (
    UPat(
      Ops.WHERE, name="w", src=(UPat(dtype=dtypes.bool), UPat.var("x"), UPat.var("y"))
    ),
  ...
  (
    ...
    lambda rng, x: rng.dtype == x.dtype
    and isinstance(rng.arg, tuple)
    and len(rng.arg) >= 2
    and all(isinstance(ra, int) for ra in rng.arg[0:-1])
    and isinstance(rng.arg[-1], AxisType),
  ),
  (
    UPat(Ops.INDEX, src=(UPat(),), allow_any_len=True, name="x"),
    lambda x: all(y.dtype == dtypes.index for y in x.src[1:]) or None,
  ),
])
...
movement_ops = PatternMatcher([
  (
    UPat(
      ...
      name="mv",
      src=(UPat.var("x"), UPat(dtype=dtypes.index)),
    ),
    lambda mv, x: True,
  ),
  (
    UPat(
      (Ops.PAD, Ops.SHRINK),
      name="mv",
      src=(UPat.var("x"), UPat(dtype=dtypes.index), UPat(dtype=dtypes.index)),
    ),
    lambda mv, x: True,
  ),
  (
    UPat((Ops.PERMUTE, Ops.FLIP), name="mv", src=(UPat.var("x"),)),
    lambda mv, x: isinstance(mv.arg, tuple),
  ),
...
_tensor_spec = (
  PatternMatcher([
    ...
    (
      ...
      lambda x: isinstance(x.src[0].device, tuple) and x.arg < len(x.src[0].device),
    ),
    ...
    (
      UPat(
        Ops.CONTIGUOUS, name="root", src=(UPat.var("x"),), allow_any_len=True, arg=None
      ),
      lambda root, x: root.dtype == x.dtype
    ...
    (
      UPat(Ops.COPY, name="copy", src=(UPat.var("x"), UPat(Ops.DEVICE)), arg=None),
    ...
    (
      ...
      lambda x: all(y.dtype == dtypes.index for y in x.src[1:]),
    ),
    # AFTER if things were kernelized
    (
      UPat(Ops.AFTER, src=(UPat((Ops.BUFFER, Ops.AFTER)),), allow_any_len=True),
      lambda: True,
    ),
  ])
  + movement_ops
...
shared_codegen_spec = PatternMatcher([
  ...
  (
    ...
    lambda x: isinstance(x.dtype, PtrDType) and x.dtype.addrspace == AddrSpace.LOCAL,
  ),
  (UPat(Ops.DEFINE_REG, src=(), name="x"), lambda x: isinstance(x.arg, int)),
  # allow AFTER on buffers, GROUP anywhere
  (
    UPat(Ops.AFTER, src=(UPat(GroupOp.Defines | {Ops.AFTER}),), allow_any_len=True),
    lambda: True,
  ),
  ...
  # INDEX
  (
    UPat(GroupOp.Defines | {Ops.AFTER}, name="buf").index(UPat.var("idx")),
    validate_index,
  ),
  # SPECIAL
  (
    UPat(Ops.SPECIAL, src=(UPat.var("x", (dtypes.index, dtypes.int32)),), name="s"),
    lambda s, x: s.dtype == x.dtype and isinstance(s.arg, str),
  ),
...
kernel_spec = (
  PatternMatcher([
    ...
    (
      UPat(
        GroupOp.Elementwise | {Ops.CONST, Ops.RANGE, Ops.DEFINE_VAR}, dtype=dtypes.index
      ),
      lambda: True,
    ),
    # END can end multiple axes here
    (
      UPat(Ops.END, src=(UPat(), UPat()), allow_any_len=True, dtype=dtypes.void),
      lambda: True,
    ),
  ...
  PatternMatcher([
    ...
    (
      UPat(
        Ops.INDEX,
        src=(
          UPat(GroupOp.Defines | {Ops.AFTER}, name="buf"),
          UPat.var("idx"),
          UPat.var("gate", dtype=dtypes.bool),
        ),
      ),
      validate_index,
    ),
    ...
    (
      UPat().index(UPat(), UPat(dtype=dtypes.bool)).or_casted().load(UPat()),
      lambda: True,
    ),
    # END closes ranges
    (UPat(Ops.END, src=(UPat(), UPat(Ops.RANGE)), dtype=dtypes.void), lambda: True),
    ...
    (
      ...
      lambda x: all(v is not Invalid for v in x.arg)
      and len(x.arg) == x.dtype.vcount > 1
      and type(x.arg) is type(dtypes.as_const(x.arg, x.dtype)),
    ),
    ...
    (
      UPat(
        Ops.IF,
        dtype=dtypes.void,
        src=(UPat(dtype=dtypes.bool), UPat((Ops.CAST, Ops.INDEX))),
      ),
      lambda: True,
    ),
...
full_spec = (
  PatternMatcher([
    ...
    (
      ...
      lambda gep: gep.dtype is dtypes.void or gep.dtype.vcount == len(gep.arg),
    ),
    # PTRCAT is like VECTORIZE, but it functions on ptrs
    (
      UPat(Ops.PTRCAT, name="x"),
      lambda x: x.dtype.vcount == sum([y.dtype.base.count for y in x.src]),
    ),
    # CAT is like VECTORIZE, but the srcs can be vectors
    (
      UPat(Ops.CAT, name="x"),
      lambda x: x.dtype.vcount == sum([y.dtype.vcount for y in x.src]),
    ),
    # vectorized index
    ...
    (
      UPat(Ops.CONST, arg=Invalid, name="x"),
      lambda x: x.dtype.scalar() == dtypes.index,
    ),
    ...
    (
      UPat(
        Ops.WHERE,
        dtype=dtypes.index,
        src=(UPat(dtype=dtypes.bool), UPat(), UPat(dtype=dtypes.index)),
      ),
      lambda: True,
    ),
    ...
    (
      UPat(Ops.BIND, (dtypes.int, dtypes.index), (UPat(), UPat()), arg=None),
      lambda: True,
    ),
...
def type_verify(ast: UOp | list[UOp], check_spec: PatternMatcher):
  ...
  if SPEC > 1:
    test_pyrender(lst[-1])  # assume this is the sink
...
def test_pyrender(test_ast: UOp, assert_parents=True):
  ...
  if ast is not test_ast:
    if assert_parents:
      for u in test_ast.toposort():
        test_pyrender(u, assert_parents=False)
    raise RuntimeError(
