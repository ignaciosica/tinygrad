...
symbolic_simple = propagate_invalid + PatternMatcher([
  ...
  (
    UPat.var("x") * 0,
    lambda x: x.const_like(float("nan") if x.op is Ops.CONST and isinstance(x.arg, float) and (math.isnan(x.arg) or math.isinf(x.arg)) else 0),
  ),
  # *** cast/bitcast ***
...
symbolic = (
  ...
  + PatternMatcher([
    ...
    *(
    ...
    (
      UPat(GroupOp.Binary, src=(UPat.var("x", dtypes.long), UPat.var("y", dtypes.long)), name="u"),
      lambda u, x, y: x.cast(dtypes.int).alu(u.op, y.cast(dtypes.int)).cast(u.dtype) if not any(v.overflows(dtypes.int) for v in (u, x, y)) else None,
    ),
...
def drop_and_clauses(cond: UOp, x: UOp, i: UOp) -> UOp | None:
  if not (dropped_clauses := [c for c in cond.split_uop(Ops.AND) if not any(r in x.ranges for r in c.ranges)]):
    return None
