...
class UPatCompileError(Exception):
  pass
...
def _get_clause(self: UPat, base: UOp, depth=0) -> UOp:
  if isinstance(self, UPatAny):
    ...
    return UOp(
      Ops.AND,
      src=(
        UOp(Ops.OR, src=tuple(_get_clause(s, base, depth) for s in self.src[0])),
      ),
    )
  ...
  if self.op is not None:
    if len(self.op) > 1:
      and_clause.append(
        UOp(
          Ops.CUSTOM,
          src=(base, UOp(Ops.BIND, arg=tuple(int(x) for x in self.op))),
          arg="{0}.op in {1}",
        )
      )
    else:
      and_clause.append(
        UOp(Ops.CUSTOM, src=(base,), arg="{0}.op == " + str(self.op[0].value))
      )
  if self.arg is not None:
    if isinstance(self.arg, int):
      and_clause.append(
        UOp(Ops.CUSTOM, src=(base,), arg="{0}.arg == " + str(int(self.arg)))
      )
    else:
      and_clause.append(
        UOp(
          Ops.CUSTOM, src=(base, UOp(Ops.BIND, arg=self.arg)), arg="{0}.arg == {1}"
        )
      )
  if self.strict_length or self.required_len > 0:
    and_clause.append(
      UOp(
        ...
        arg=(
          "len({0}.src)"
          + (" == " if self.strict_length else " >= ")
          + str(self.required_len)
        ),
      )
    )
  if self.name is not None:
    and_clause.append(UOp(Ops.STORE, src=(UOp(Ops.DEFINE_VAR, arg=self.name), base)))
  if self.dtype is not None:
    if len(self.dtype) > 1:
      and_clause.append(
        UOp(
          Ops.CUSTOM,
          src=(base, UOp(Ops.BIND, arg=tuple(self.dtype))),
      ...
      and_clause.append(
        UOp(
          Ops.CUSTOM,
          src=(base, UOp(Ops.BIND, arg=self.dtype[0])),
  ...
  if self.src is not None:
    ...
    if len(self.src) == 1 and isinstance(self.src[0], tuple):
      and_clause += [
        _get_clause(s, base.gep(i), depth) for i, s in enumerate(self.src[0])
      ]
    # repeat match
      ...
      and_clause.append(
        UOp(Ops.RANGE, src=(match, it, base), arg="all([{0} for {1} in {2}.src])")
      )
    # multi match (fork)
      ...
      fork_cond = [
        UOp(
          Ops.AND,
          src=tuple([_get_clause(s, base.gep(i), depth) for i, s in enumerate(ss)]),
        )
        for ss in self.src
      ]
...
def do_process_and(a: UOp) -> UOp | None:
  ...
  # too big to compile
  if len(or_clause) >= 4:
    raise UPatCompileError("too big to compile")
  ...
  if len(or_clause) > 1:
    ...
    or_clause = [
      UOp(
        Ops.OR,
        src=tuple([
          UOp(Ops.AND, src=x) for x in itertools.product(*[x.src for x in or_clause])
        ]),
      )
    ]
    found = True
    ...
    if len(or_clause):
      ...
      or_clause = [
        UOp(
          Ops.OR,
          src=tuple([
            x.replace(src=x.src + tuple(stores)) for x in or_clause[0].src
          ]),
        )
      ]
      found = True
      ...
      # put the stores back
      for k, v in dict_stores.items():
        new_src.append(UOp(Ops.STORE, src=(k, v)))

  # reassemble, if there's any deduping to do, do it
  if len(dretand := dedup(new_src + or_clause)) != len(new_src) + len(or_clause):
    found = True
...
pm_renderer = PatternMatcher(
  [
    ...
    (
      ...
      lambda x: UOp(Ops.CUSTOM, src=x.src, arg="{0} is {1}"),
    ),
    # RANGE can't have OR inside it
    (
      UPat(
        Ops.RANGE,
        src=(UPat(Ops.AND, src=UPat(Ops.NOOP), name="x"), UPat(), UPat()),
        name="r",
      ),
      lambda r, x: r.replace(
        op=Ops.CUSTOM,
        src=(UOp(Ops.NOOP, arg="(" + " and ".join(y.arg for y in x.src) + ")"),)
        + r.src[1:],
      ),
    ),
    (
      UPat(Ops.CUSTOM, src=UPat(Ops.NOOP), name="x"),
      lambda x: UOp(Ops.NOOP, arg=x.arg.format(*[y.arg for y in x.src])),
    ),
    (
      UPat(Ops.GEP, src=UPat(Ops.NOOP, name="x"), name="g"),
      lambda x, g: x.replace(arg=x.arg + f".src[{g.arg[0]}]"),
    ),
  ],
  compiled=False,
)
...
def _final_render(x: UOp, has_ctx: bool, depth=1) -> list[str]:
  ...
  for s in x.src:
    if s.op is Ops.OR:
      ...
      for ss in s.src:
        or_pieces.extend(_final_render(ss, has_ctx, depth + 1))
    ...
    elif s.op is Ops.NOOP:
      and_pieces.append(s.arg)
    else:
  ...
  if len(or_pieces):
    ...
    return [
      f"{'  ' * depth}if {and_clause if len(and_clause) else 'True'}:"
    ] + or_pieces
  # if we don't, this is a final return
  store_clause = ", ".join((["ctx=ctx"] if has_ctx else []) + store_pieces)
  and_clause = " and ".join(
    and_pieces + [f"(_ret:=_fxn({store_clause})) is not None"]
  )
...
def _get_code(self: UPat, has_ctx: bool):
  ...
  try:
    ...
    return None
  return "\n".join(
    [f"# match for {self.location}", "def compiled_match(uop, ctx):"]
    + rendered
    + ["  return None"]
  ), dyn_lookup
