...
def _get_clause(self: UPat, base: UOp, depth=0) -> UOp:
  if isinstance(self, UPatAny):
    ...
    return UOp(
      Ops.AND,
      src=(UOp(Ops.OR, src=tuple(_get_clause(s, base, depth) for s in self.src[0])),),
    )
  ...
  if self.arg is not None:
    if isinstance(self.arg, int):
      ...
      and_clause.append(
        UOp(Ops.CUSTOM, src=(base, UOp(Ops.BIND, arg=self.arg)), arg="{0}.arg == {1}")
      )
  ...
  if len(stores):
    if len(or_clause):
      ...
      or_clause = [
        UOp(
          Ops.OR,
          src=tuple([x.replace(src=x.src + tuple(stores)) for x in or_clause[0].src]),
        )
      ]
      found = True
    else:
