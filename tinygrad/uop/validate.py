...
try:
  ...
  z3_renderer = PatternMatcher([
    ...
    (
      UPat(Ops.CAST, dtypes.ints + (dtypes.index,), src=(UPat(dtype=dtypes.floats),), name="x"),
      lambda x, ctx: create_bounded(f"cast{len(ctx[1])}", x.dtype.min, x.dtype.max, ctx[0]),
    ),
    ...
    (
      UPat(Ops.CAST, dtypes.ints + (dtypes.index,), src=(UPat.var("x", dtypes.ints + (dtypes.index,)),), name="c"),
...
def validate_index(buf: UOp, idx: UOp, gate: UOp | None = None):
  ...
  if not z3_imported:
    raise ImportError('z3 >= 4.12.4 is required for bounds checking, try IGNORE_OOB=0 or "pip install \'z3-solver>=4.12.4"')
  ...
  with cpu_profile("validate index with z3", "TINY"):
    match solver.check((z3_idx < 0) | (sz <= z3_idx)):
      ...
      case z3.unknown:
        print(f"# UNKNOWN RESULT FROM Z3: {solver.reason_unknown()}\nconstraints = {solver}")
