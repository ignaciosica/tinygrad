...
def get_full_rewrite(ctx: TrackedGraphRewrite, i: int = 0) -> Generator[GraphRewriteDetails, None, None]:
  ...
  for u0_num, u1_num, upat_loc, dur in tqdm(ctx.matches):
    ...
    yield {
      ...
      "diff": list(difflib.unified_diff(pystr(u0, i).splitlines(), pystr(u1, i).splitlines())),
...
def flatten_events(profile: list[ProfileEvent]) -> Generator[tuple[Decimal, Decimal, DevEvent], None, None]:
  for e in profile:
    if isinstance(e, ProfileRangeEvent):
      ...
      cpu_ts = []
      for ent in e.ents:
        cpu_ts += [e.sigs[ent.st_id] + (diff := cpu_ts_diff(ent.device, ent.is_copy)), e.sigs[ent.en_id] + diff]
...
def mem_layout(
  dev_events: list[tuple[int, int, float, DevEvent]], start_ts: int, end_ts: int, peaks: list[int], dtype_size: dict[str, int], scache: dict[str, int]
) -> bytes | None:
  peak, mem = 0, 0
  temp: dict[int, int] = {}
  events: list[bytes] = []
  ...
  steps = [
    {
      "name": x[0],
      "depth": 0,
      "data": {"src": json.dumps({k: asdict(v) for k, v in x[1].items()}, indent=2), "lang": "txt", "device": "AMD"},
...
def get_llvm_mca(asm: str, mtriple: str, mcpu: str) -> dict:
  ...
  data = json.loads(
    subprocess.check_output(["llvm-mca", "-skip-unsupported-instructions=parse-failure", "--json", "-"] + target_args, input=asm.encode())
  )
...
class Handler(BaseHTTPRequestHandler):
  def do_GET(self):
    ...
    if (url := urlparse(self.path)).path == "/":
      ...
      try:
        with open(os.path.join(os.path.dirname(__file__), self.path.strip("/")), "rb") as f:
          ret = f.read()
      ...
      if url.path == "/render":
        ...
        try:
          return self.stream_json(get_full_rewrite(trace.rewrites[i := get_int(query, "ctx")][get_int(query, "idx")], i))
